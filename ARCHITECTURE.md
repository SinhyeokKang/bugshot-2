...
**페이지 ID 추출**: Notion API의 `createPage` 응답 `url`은 `https://www.notion.so/<workspace>/<title-slug>-<pageIdHex>` 형태. status fetch에 필요한 pageId는 끝의 32자 hex 또는 8-4-4-4-12 UUID. `src/lib/notion-page-id.ts`의 `extractNotionPageId(url)`이 단일 헬퍼 — IssueCreateModal·DraftDetailDialog·`issueListUtils.resolveNotionPageId` 모두 공유. URL split-pop만 쓰면 slug까지 포함된 garbage가 저장돼 후속 status fetch가 깨지므로 반드시 이 헬퍼 사용.
...
**activeTab 만료 복구** — 패널 바인딩 탭이 cross-document 네비게이션하면 activeTab이 회수돼 캡처가 거부된다. 두 갈래로 처리: (1) **비보존 상태**(`shouldPreserveSession`=false) 네비게이션 시 `tab-bindings.deactivatePanelIfVolatile`가 `status:loading`에 패널을 닫고 `setActivated(false)` — 아이콘 재클릭(`activateTab`)이 activeTab을 재부여하므로 재오픈으로 복구(1.2.0 동작). 닫기 판정은 광역 권한이 부풀리는 `info.url` 가독성이 아니라 status 신호에 묶어 권한 보유 여부와 무관하게 동작. (2) **보존 상태**(drafting 등)는 패널 유지, 캡처 시 권한 만료를 `classifyTabSupport`(진입 가드 — content script의 `location.href`로 "미지원 페이지" vs "권한 만료" 구분, `picker.pageUrl` 메시지)·`isActiveTabPermissionError`(캡처 단계)로 감지해 `onPickerPermissionExpired` → "권한이 만료되었습니다 / BugShot을 다시 실행해 주세요" 다이얼로그([확인]→`window.close()`)로 안내.
