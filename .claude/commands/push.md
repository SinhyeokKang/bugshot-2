...
2. **미커밋 변경이 있으면 중단.** 사용자에게 커밋 여부를 물어본다. 임의로 `git add`/`commit` 하지 않는다.
...
4. **CLAUDE.md 신선도 검사.** 푸시될 커밋들의 diff(`git diff @{u}..HEAD`)를 훑어 아래 중 하나라도 해당하면 **CLAUDE.md 업데이트 후보**:
   ...
   - `docs/PRD.md` / `docs/design.md` 신규 섹션 추가
...
5. **TASKS.md 동기화.** 현재 대화의 TodoWrite 상태(세션 리마인더로 주입됨)와 `docs/TASKS.md`의 항목을 비교:
   - 완료/진행중/대기 상태가 다르거나 항목이 추가/제거됐으면 **업데이트 후보**
   - 불일치 발견 시 사용자에게 알리고 확인 후 Edit으로 반영
   - `docs/TASKS.md` 상단의 "마지막 갱신" 날짜도 오늘 날짜(절대값)로 갱신
   - `docs/TASKS.md` 한 파일만 add해서 별도 커밋 (메시지 예: `docs(TASKS): 동기화`)
   - 일치하거나 사용자가 스킵을 원하면 건너뜀

6. **푸시 전 최종 확인.** 사용자에게 "푸시해도 되냐" 묻고 OK면:
