...
**제품 이름이 아니라 능력으로 판정한다.** 브라우저를 실제로 조작해 확장 UI를 찍는 게 본체라, 아래 넷을 모두 갖춘 런타임만 촬영까지 간다.

| # | 필요 능력 | 왜 대체 불가인가 |
|---|---|---|
| 1 | BugShot이 **언팩 확장으로 로드된 브라우저** | 찍을 대상 자체 |
| 2 | `chrome-extension://` 페이지를 탭으로 열기 | 사이드패널을 `?tabId=`로 여는 유일한 경로 |
| 3 | **저수준 CDP** — `Emulation.setDeviceMetricsOverride` | 폭·DPR 고정 수단. 고수준 `setViewportSize`류는 없거나 안 먹는다 |
| 4 | **`chrome.tabs`·`chrome.windows` 특권 API** | 패널을 별도 창으로 빼고 대상 탭을 활성으로 유지해야 `captureVisibleTab`이 통과한다. 이게 없으면 before/after 스냅샷이 통째로 실패한다 |

넷 중 하나라도 없으면 **1~2단계(stale 탐지 + 리포트)까지만** 수행하고 촬영 없이 종료한다. 탐지는 git 메타데이터만 보므로 어느 런타임에서든 돈다 — 덕분에 CLI에서 "지금 몇 장이 낡았나"를 싸게 물어볼 수 있다.

**현재 상태**: Aside 세션만 넷을 모두 만족한다. Claude Code·Codex는 1·4가 없다. **Orca 같은 ADE는 에이전트 CLI를 얹는 셸이라 런타임은 그 안의 Claude Code·Codex이고**, 내장 브라우저 페인이 있어도 1·4를 노출하지 않으면 마찬가지로 탐지 전용이다. 새 런타임이 넷을 만족하게 되면 이 표만 다시 보면 되고 스킬 본문은 안 고쳐도 된다.
