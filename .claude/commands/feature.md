...
### 0. 과거 드랍 사유 소환 (`docs/features/DROPPED.md`)

**아이디어를 받자마자, 질문 라운드보다 먼저** `docs/features/DROPPED.md`를 기능 키워드로 grep한다. 이 파일은 기획까지 갔다가 **안 하기로 한** 것들의 사유 기록이고, 항목마다 *왜 안 하는지*와 *무엇이 바뀌면 다시 볼 만한지*가 적혀 있다. **쓰기만 하고 안 읽으면 죽은 로그다** — `docs/POSTMORTEM.md` 소환 회로와 같은 이유로 여기서 루프를 닫는다.

걸리는 항목이 있으면 **문서를 쓰기 전에 사용자에게 알린다**:

- **같은 기능이 이미 드랍됐다** → 그 사유와 "다시 볼 조건"을 그대로 인용하고, 그 조건이 지금 충족됐는지 묻는다. 충족됐으면 진행하되 **PRD 배경에 "무엇이 바뀌어 다시 하는가"를 명시**한다. 안 됐으면 진행 여부를 사용자가 정한다.
- **드랍된 기획의 일부만 되살리는 것이다** → 그 사실을 밝히고 스코프를 그 조각으로 좁힌다(예: `css-cascade-fidelity`의 G2만).
- **드랍 사유와 같은 함정을 밟는다** → 판정 기준 4개로 대조해 보고한다: **브라우저·기존 도구가 이미 하는가 / 페이지에 무언가를 심어야 하는가 / 사정거리가 이름값보다 좁은가 / 검증 수단이 있는가.** 하나라도 걸리면 그 항목을 PRD의 "위험 요소"에 반드시 싣는다.

걸리는 게 없으면 그대로 1단계로 간다. 보고할 필요 없다.
...
**안 하기로 결정된 기획은 그냥 지우지 않는다** — `docs/features/DROPPED.md`에 *왜 안 하는지*와 *무엇이 바뀌면 다시 볼 만한지*를 남기고 디렉터리를 삭제한다. 0단계 grep이 읽는 대상이 이 파일이므로, 쓰지 않으면 소환 회로가 끊긴다.
