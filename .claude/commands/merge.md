...
10. **dev 동기화 안내.** main 머지 후 dev가 main 뒤로 처지지 않게 `/sync` 실행 권장. (유실 여부는 `/sync`가 diff 검증으로 자체 판단.) 안 하면 다음 PR diff가 지저분해질 수 있음. 이어서 배포할 거면 `git checkout main && git pull` 후 `/deploy`로.
