...
- **oauth-proxy는 이 스킬이 배포하지 않는다.** `oauth-proxy/`는 별도 배포 단위이고, 필요할 때 사용자가 직접 `cd oauth-proxy && wrangler deploy`한다. **확장의 요청 형태(예: 새 필드)를 프록시가 필수로 요구하게 바꾸는 변경은 순서가 있다** — 구버전 확장이 아직 남아 있으므로 확장을 먼저 스토어에 퍼뜨리고 프록시를 나중에 올린다(반대로 하면 구버전 사용자의 연동이 죽는다). 배포 전 `ALLOWED_ORIGINS`에 `*`가 없는지 확인하고(worker가 `*`를 거부한다), 배포 후 `POST /token`으로 스모크 테스트한다: 스토어 ID origin → 400, `https://evil.example` origin → 403.
