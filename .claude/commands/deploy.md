...
8. **후속 작업 안내 (1회성 — 아래 조건 충족 시 이 절차를 통째로 삭제).**
   ...
   > - 배포 직전 **`ALLOWED_ORIGINS`에 `*`가 없는지 대시보드에서 확인**한다. A-22가 `*`를 거부하도록 바꿔서, 들어 있으면 배포 즉시 전 origin이 차단된다.
   ...
   > 통과일 기준으로 다시 잡는다 — 전파 시계는 게시일부터 돈다. 상세 체크리스트는
   > `docs/features/audit-remediation/tasks.md` 인계 3번.
   >
   > 이 안내를 본 시점이 타이머의 시작점이다. **여기서 바로 캘린더/리마인더에 날짜를 박아라.**

   **삭제 조건**: oauth-proxy를 배포하고 나면 이 절차 8과 아래 "주의"의 마지막 항목을 지운다. 상세 근거는 `docs/features/audit-remediation/tasks.md` "인계 사항" 3·4번.
