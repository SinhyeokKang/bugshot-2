...
2. dev push (/push)
3. /merge — dev 위에서 버전 bump 커밋 + dev → main squash PR → 머지 후 /sync 자동 실행 (dev = origin/main)
4. main 체크아웃 + git pull → /deploy ← 여기 (tag만 push, 새 분기 없음)
```
...
7. **심사 요청 안내 + Release publish 안내 + dev 동기화 안내.**
   ...
   ### dev 동기화
   `/merge`가 머지 직후 `/sync`를 자동 실행하므로 통상 dev는 이미 origin/main과 동기 상태다 (`/deploy`는 tag만 push하고 main에 새 커밋을 안 만들어 추가 분기도 없음). 따라서 여기서 별도 조치는 대개 불필요. 예외적으로 `/merge`의 sync가 스킵·실패했거나 `/merge` 없이 배포한 경우에만 `/sync`를 수동 실행한다 (`/sync`가 분기 없으면 "이미 동기화됨"으로 자체 종료).

8. **후속 작업 안내 (1회성 — 아래 조건 충족 시 이 절차를 통째로 삭제).**

   > ⚠️ **oauth-proxy 배포는 이번 스토어 배포가 사용자에게 전파된 뒤에 한다.**
   >
   > `oauth-proxy/worker.ts`의 Atlassian 핸들러에 `body.client_id !== env.ATLASSIAN_CLIENT_ID → 400` 가드가 들어가 있다(A-22). 확장은 커밋 `60b85fa`(= 이 안내가 처음 뜨는 배포 버전)부터 `client_id`를 싣지만(`src/background/oauth.ts`), **이미 설치된 구버전은 안 싣는다** — 프록시를 먼저 올리면 그 사용자들의 Jira 연동·토큰 갱신이 전부 400으로 죽는다.
   >
   > 반대 방향은 안전하다(신버전 확장 → 현재 배포된 프록시는 200, 실측 확인). 그래서 **확장을 먼저 퍼뜨리고 프록시를 나중에** 올리는 순서다.
   >
   > - 심사 통과 후 **1-2주** 전파 대기 → 그다음 `cd oauth-proxy && wrangler deploy`.
   > - 배포 직전 **`ALLOWED_ORIGINS`에 `*`가 없는지 대시보드에서 확인**한다. A-22가 `*`를 거부하도록 바꿔서, 들어 있으면 배포 즉시 전 origin이 차단된다. (2026-07-26 비파괴 프로브로 명시 목록임을 확인 — 스토어/dev ID origin은 400, 가짜 origin은 403.)
   > - 배포 후 같은 프로브로 스모크 테스트한다: `POST /token`에 스토어 ID origin → 400, `https://evil.example` origin → 403.
   > - 급하면 Atlassian `client_id` 검사 줄만 빼고 먼저 배포해도 된다 — A-22의 나머지 둘(미설정 시 503, `*` 거부)은 배포 제약이 없다.
   >
   > **현재 목표일: 2026-08-09** (2026-07-26 v1.6.18 배포 기준 +2주). 그날 심사가 아직 안 끝났으면
   > 통과일 기준으로 다시 잡는다 — 전파 시계는 게시일부터 돈다.
   >
   > 이 안내를 본 시점이 타이머의 시작점이다. **여기서 바로 캘린더/리마인더에 날짜를 박아라.**

   **삭제 조건**: oauth-proxy를 배포하고 나면 이 절차 8과 아래 "주의"의 마지막 항목, 개인 메모리 `project_oauth_proxy_deploy_order`를 함께 지운다.
...
- **oauth-proxy는 이 스킬이 배포하지 않는다.** 절차 8의 전파 대기 조건을 충족한 뒤 사용자가 직접 `wrangler deploy`한다 (1회성 항목 — 배포 후 절차 8과 함께 삭제).
