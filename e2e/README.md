...
| `capture-modes-layout.spec.ts` | screen-recording idle 1×2×2×1 — 6개 진입 버튼(`mode-element`/`-element-shot`/`-screenshot`/`-video`/`-screen-record`/`replay-button`) 노출 + `mode-video`·`mode-screen-record`가 같은 `[data-slot=button-group]`(role=group) 형제, `replay-button`은 그 밖(DOM 부모 구조 판정). 실제 getDisplayMedia 녹화는 자동화 불가(수동 잔여) | 2 tests |
...
- **수동 캡처 video 모드**(`mode-video`=탭 녹화) — `getUserMedia`+`chrome.tabCapture` 의존이라 headed 자동화에서 fake media 없이는 불안정. (Replay 경로는 별개로 `replay-action-log.spec.ts`가 커버 — 아래 참고.)
- **화면 전체 녹화**(`mode-screen-record`) — `getDisplayMedia` 네이티브 picker가 자동화에서 안 떠(fake display 없이는 스트림 미획득) 클릭→녹화→drafting을 e2e로 못 탄다. **버튼 노출·1×2×2×1 DOM 구조는 `capture-modes-layout.spec.ts`가 커버**, 실제 녹화·source 라벨(탭/화면)·공유 중지·60초·picker 취소는 수동(tasks.md). getDisplayMedia가 side panel에서 동작하는지의 PoC도 수동 선행.
