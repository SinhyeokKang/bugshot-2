...
| `capture-modes-layout.spec.ts` | recording-mode-setting idle 레이아웃(Row1 `[mode-element][mode-element-shot]` 동일 ButtonGroup·`mode-screenshot` 단독 / Row3 segmented `[mode-record][replay-button][mode-record-settings]`) + 구 `mode-video`/`mode-screen-record` 부재 / 녹화·리플레이 균등 너비(±2px)·⚙ 정방형 / 기본 모드=탭(`svg.lucide-app-window`) / **비활성 replay-button(force click)·⚙ 둘 다 "녹화 설정" 다이얼로그를 연다**(설정 탭 이동 아님 — `tab-debug` active 유지) / 다이얼로그 안 `recording-mode-screen` 선택 → 닫으면 녹화 버튼 아이콘 라이브 전환(`svg.lucide-monitor-play`). serial, afterAll로 설정 탭 `recording-mode-tab` 복원. 실제 getDisplayMedia/tabCapture 녹화는 자동화 불가(수동 잔여) | serial 7 |
...
- **화면 전체 녹화**(화면 녹화 — `recordingMode="screen"`일 때의 `mode-record`) — `getDisplayMedia` 네이티브 picker가 자동화에서 안 떠(fake display 없이는 스트림 미획득) 클릭→녹화→drafting을 e2e로 못 탄다. **버튼 노출·1×2×2 DOM 구조·모드별 아이콘 전환은 `capture-modes-layout.spec.ts`가 커버**, 실제 녹화·source 라벨(탭/화면)·공유 중지·60초·picker 취소는 수동(tasks.md). getDisplayMedia가 side panel에서 동작하는지의 PoC도 수동 선행. ⚙ 오버레이 시각 정합(우측 중앙·클릭 분리)·좁은 폭 라벨 truncate도 수동.
...
- **`aria-disabled="true"` 버튼을 *클릭해야* 할 때는 `click({ force: true })`**: 일부 Playwright 버전은 actionability "enabled" 검사에서 `aria-disabled="true"`를 disabled로 취급해 일반 `click()`이 타임아웃까지 대기한다(`capture-modes-layout`의 비활성 `replay-button`은 onClick이 가드 없이 다이얼로그를 여는 진짜 동작이라 `force: true`로 클릭). 위 `next-step` 케이스처럼 클릭이 no-op이어야 하는 가드 버튼과 구분 — 그쪽은 클릭 전 부재 단언이 목적.
- **공용 컴포넌트 testid 중복은 컨테이너로 스코프**: `RecordingSettingsCard`가 설정 탭(항상 마운트·hidden)과 ⚙ 다이얼로그 양쪽에 렌더돼 `recording-mode-tab`/`-screen` testid가 둘 존재할 수 있다(다이얼로그 열림 시). 다이얼로그 내부 단언은 `panel.getByRole("dialog").getByTestId(...)`로 스코프. id 충돌은 `replayInputId` prop으로 분리(설정 탭 `replay-enabled` 유지 — 리플레이 spec 의존, 다이얼로그 `replay-enabled-dialog`).
