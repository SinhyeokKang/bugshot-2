...
| `capture-modes-layout.spec.ts` | recording-mode-setting idle 1×2×2 — 진입 버튼(`mode-element`/`-element-shot`/`-screenshot`/`-record`/`-record-settings`/`replay-button`) 노출 + 구 `mode-video`/`mode-screen-record` 분리 버튼 부재 / Row3 segmented `[data-slot=button-group]`에 `mode-record`+`replay-button` 동거·⚙(`mode-record-settings`)는 녹화 버튼 부모(relative 래퍼) 오버레이 / 기본 모드=탭(녹화 버튼 `svg.lucide-app-window`) / ⚙ 클릭 → `tab-settings`+`settings-sub-issue` active(stopPropagation 증거) / 설정 `recording-mode-screen` 선택 → 캡처 복귀 시 녹화 버튼 아이콘 라이브 전환(`svg.lucide-monitor-play`). serial, afterAll로 `recording-mode-tab` 복원. 실제 getDisplayMedia/tabCapture 녹화는 자동화 불가(수동 잔여) | serial 5 |
