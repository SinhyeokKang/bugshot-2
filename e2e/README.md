...
- `actions.html` — `#action-btn`(button) / `#action-input`(textbox, `aria-label`) / `#action-nav`(hashchange anchor) — action 레코더 click·input·navigation 입력 생성용(Replay action 로그 spec).
