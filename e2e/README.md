...
| `logs-prearm.spec.ts` | active origin reload 시 로드 초반(head/body) ① network marker fetch ② console.error/warn(조기 후킹 installEwWrap)·log가 pre-arm 버퍼로 잡혀 logClear 경계를 우회 보존(preArm 마커) → 로그에 나타남. recorders-entry 동기 IIFE 전제 회귀 가드 | serial 2 |
