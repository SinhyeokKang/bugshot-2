...
bugshot-2: Chrome MV3 Side Panel 버그 리포팅 확장. 웹 페이지의 버그를 고정 폭 디바이스 뷰포트(390/768/1024)로 재현하면서, 요소 스타일 편집(before/after 비교)·스크린샷(영역/화면/페이지 전체/요소, 어노테이션)·영상 녹화(탭/화면, 30초 리플레이) 중 원하는 방식으로 캡처하고, 콘솔·네트워크·사용자 액션 로그를 자동 수집한다. 이렇게 만든 리포트를 Jira·GitHub·Linear·Notion·GitLab·Asana·ClickUp 이슈로 등록하거나 Slack 채널·DM으로 공유한다.
...
- 디바이스 뷰포트: 페이지에 same-origin iframe 래퍼(`#__bugshot_device_frame__`)를 심어 390/768/1024 폭으로 `location.href`를 재로드하고 원본 body는 스타일시트 1장으로 `display:none`(`content/device-frame.ts`). **성공·차단·handoff 판정은 background 단일 심판**(`background/device-frame-coordinator.ts`의 `decideDeviceSignal` — webNavigation 3종 + `device.frameReady` + 3초 armTimeout. iframe `load`는 about:blank 오판 때문에 기각됐다). 사이드패널은 모듈 스코프 싱글턴(`sidepanel/device-viewport-controller.ts` — 세그먼트 행이 작성 구간에 언마운트되는데 재수립이 필요한 게 그때라 컴포넌트 수명에 못 매단다). 래퍼가 top과 same-origin이라 **로그를 origin으로 못 가르고**, sentinel 타깃을 documentId로 좁히는 `sidepanel/lib/device-sentinel-gate.ts`가 fail-closed 게이트다. 상세·불변식은 [ARCHITECTURE.md](./docs/ARCHITECTURE.md) "디바이스 뷰포트"
...
- permissions: `sidePanel`, `activeTab`, `scripting`, `storage`, `commands`, `contextMenus`, `identity`, `tabCapture`, `webNavigation` (메인 프레임 네비게이션 커밋 직전 로그 꼬리 sync — cross-page 로그 누적 + **디바이스 뷰포트 래퍼 로드 판정** — `onErrorOccurred`·`getFrame` 위조 검증·`getAllFrames` 문서 트리)
...
- `chrome.scripting.executeScript({..., func})`: 직렬화·재평가라 **클로저가 안 살아남는다**(world와 무관 — `files:` 형태 주입은 규칙 무관). 주입 함수는 self-contained, 헬퍼는 nested로 inline. `func` 형태 사용처는 `github-upload.ts:pageBatchUploadFn`·`picker-control.ts:getTopViewport` 둘뿐이고 **리팩터 시 실제 탭 회귀 필수**. `getTopViewport`는 `DEVICE_FRAME_ID`를 인라인 리터럴로 복제한다 — import하면 런타임 `ReferenceError`를 catch가 삼켜 조용히 null이 되고, `device-viewport-meta.test.ts`가 그 복제를 고정한다. 상세: [ARCHITECTURE.md](./docs/ARCHITECTURE.md) 동명 섹션
