...
절대적 제약은 아니지만 **BugShot의 코어 밸류이자 경쟁 우위 축**. 캡처 데이터(스크린샷·영상·console/network/action 로그·CSS diff·리포트 본문)는 BugShot 서버를 거치지 않고 **사용자 브라우저 → 사용자의 이슈 트래커/Slack으로 직행**한다. 서버를 지나는 건 **OAuth 토큰 교환 프록시**(`VITE_OAUTH_PROXY_URL`)와 **스토어 빌드 익명 PostHog 집계**(`src/background/analytics.ts`) 둘뿐 — 캡처 데이터는 절대 안 지나간다. 클라이언트 온리는 원래 효율성 선택(소규모가 무백엔드로 8개 플랫폼 연동을 감당)이지만, "우리가 물리적으로 못 보는" 구조가 부산물로 딸려와 규제·보안 민감 바이어에게 방어 가능한 차별점이 된다. 경쟁사(예: Jam)의 recorder-side 무설치 캡처는 필연적으로 호스팅 저장소·워크스페이스를 요구하는 SaaS 모델 — 그 편의를 좇아 무서버·데이터 직행을 포기하는 건 기능 추가가 아니라 제품 정체성 변경으로 취급한다. 새 기능이 캡처 데이터를 외부 서버로 보내야 하면 이 밸류와 충돌하는지 먼저 따진다.
