...
- `oauth-proxy/` — OAuth 토큰 교환 프록시(Cloudflare Worker) **소스가 이 저장소 안에 있다**. 8라우트, 자체 `package.json`·`wrangler.toml`이라 배포는 `wrangler deploy`(루트 명령어 표 밖). `GITHUB_CLIENT_SECRET`·`ATLASSIAN_*`·`ALLOWED_ORIGINS` 같은 **서버측 secret 축**은 여기에만 있다. `oauth-proxy/__tests__/`는 루트 `vitest.config.ts`의 exclude에 없어 `pnpm test`가 함께 돌린다.
