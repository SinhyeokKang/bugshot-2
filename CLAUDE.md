...
- 분석: PostHog (익명 이슈 제출·연동 집계 — `src/background/analytics.ts`. 코드는 `VITE_POSTHOG_KEY` 한 이름만 읽고 store/비-store 분기는 `vite.config.ts`의 define 치환이 한다 — store 빌드면 `VITE_POSTHOG_KEY_PROD` 값을 그 자리에 주입(GitHub client id도 같은 패턴); 선택된 키가 없으면 no-op)
...
- `oauth-proxy/` — OAuth 토큰 교환 프록시(Cloudflare Worker) **소스가 이 저장소 안에 있다**. 8라우트, 자체 `package.json`·`wrangler.toml`이라 배포는 `wrangler deploy`(루트 명령어 표 밖). `GITHUB_CLIENT_SECRET_{DEV,PROD}`·`ATLASSIAN_*`·`ALLOWED_ORIGINS` 같은 **서버측 secret 축**은 여기에만 있다. `oauth-proxy/__tests__/`는 루트 `vitest.config.ts`의 exclude에 없어 `pnpm test`가 함께 돌린다.
