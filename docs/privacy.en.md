...
| Information | When collected | Purpose |
|---|---|---|
| Environment details (OS, browser name and version, current page URL, **the size of the captured viewport**, capture time, and the CSS selector of the element if you picked one). The size is the browser window by default; when device viewport mode is on, the width you picked is recorded instead | When you enter the issue drafting screen | Filled into the "Environment" section of the issue body automatically — saved with the issue and sent to the platform you submit to |
...
Input forms (`form`, `fieldset`), where personal data tends to gather, are **excluded** from the container candidates — but that means only "a form is never itself picked as the container to expand to." If another candidate sits outside the form, such as a dialog, that dialog can be picked instead, and in that case **the input values inside it (payment or address fields, for example) are included** as long as the 40% and on-screen conditions above are met. The expansion is never applied to elements inside an iframe. The one exception is **device viewport mode** below: the whole page is opened inside a frame the extension creates, so container expansion applies as usual to elements within that frame (it is still never applied to the page's own third-party frames). An expanded image may carry other content from the same container (for example, the other columns of the same row), so please review the before/after images on the drafting screen before submitting.

**Device viewport mode** — When you pick a screen width (390, 768, or 1024px) on the drafting screen, the extension creates a frame (iframe) inside the page you are viewing and **loads the same address again inside it**. This issues **one additional request** to that site, using the cookies and signed-in session your browser already holds for it, exactly as an ordinary navigation would. The original document is merely hidden from view by a single injected stylesheet — it is not terminated and **keeps running**, so behavior the page performs on its own (autosave, periodic communication, and the like) may happen twice. You are warned of this in a confirmation dialog the first time you turn the mode on, and turning the mode on or off (returning to Full) reloads the page (switching from one width to another does not). Because following a link inside the frame does not change the address bar, the "current page address" in the reproduction environment above — and the page address recorded in the logs — use **the address you are actually viewing inside the frame**. For the same reason, navigating to another page inside the frame clears the drafting session in progress, exactly as an ordinary navigation would. The extension does not collect or transmit page content beyond what this policy describes during this process.

If you navigate to a different site while the mode is on, **the extension navigates the whole tab to that address** and re-establishes the mode at the same width. This is because trapping another site inside that frame makes the browser treat it as a separate visit, which would show you a signed-out screen. At that moment — and at the moment you first turn the mode on — the console, network, and action logs collected up to that point are **deleted from your device (both memory and IndexedDB) and collection starts over**.
...
| Platform connect (platform_connect: platform, success/cancel/failure, failure reason category) | On an OAuth connection attempt | Understanding per-platform popularity and connect success/cancel/failure rates, and telling service outages apart from users abandoning the flow |
...
- **chrome.storage.session**: Editing sessions (including the screenshot, element before/after images and video thumbnail you're working on, plus the issue title, body, environment rows you're drafting [including the auto-derived API hosts], the attached file list, integration field values, and the submission result), the address of the tab the side panel was opened on (so reopening the panel reconnects to that tab), the list of tabs the side panel is active on, the timestamp of the last temporary-log cleanup, and the identifiers of the frame created by device viewport mode (frame id and document id, plus the tab's current page address used to tell whether a navigation stays on the same origin — one entry per tab; the width you picked is not stored) (automatically deleted when the browser closes)
...
| User-specified LLM provider (AI styling) | Selected element's tag, CSS selector, class list, current specified styles, design tokens, computed layout styles (display, position, width, margin, etc.), **the captured viewport size** (the width you picked when device viewport mode is on), and the instruction you type | CSS change suggestion |
...
| scripting | Injecting scripts for DOM selection / overlay display, injecting the console/network log recorder scripts, reading the captured viewport size, and running page scripts for GitHub file upload |
...
| webNavigation | Preserving the tail of console/network logs just before navigation, connecting log collection when a newly loaded frame (iframe) is detected, and identifying the device viewport frame, enumerating a tab's frames, and detecting when that frame is blocked from loading (detecting tab/frame navigation) |
...
- **Device viewport**: Creating a frame inside an arbitrary web page, loading the same address again inside it, and continuing element selection, capture, and log collection within that frame
...
- **Style value enrichment**: When an element-picker session starts, the extension reads, in the background and without credentials (`credentials:omit`), the external stylesheets (cross-origin CSS files) the page references to accurately display the hover tooltip and the selected element's "author-specified" styles. Only public http(s) hosts are targeted (loopback, internal networks, and private IPs are blocked), and the CSS received is used only on the device and not transmitted to third parties.
