...
- **chrome.storage.session**: Editing sessions (including the screenshot, element before/after images and video thumbnail you're working on, plus the issue title, body, environment rows you're drafting [including the auto-derived API hosts], the attached file list, integration field values, and the submission result), the address of the tab the side panel was opened on (so reopening the panel reconnects to that tab), the list of tabs the side panel is active on, and the timestamp of the last temporary-log cleanup (automatically deleted when the browser closes)
