...
- 커스텀 raw 색(`text-blue-600` 등)은 semantic 토큰으로 표현 못 하는 **상태/기능 색**에만 쓰고, 가능하면 `dark:` 짝을 함께 둔다. 현재 사용처:
  ...
  - DOM 마크업 팔레트: 태그 `sky-600/400` · 속성명 `amber-600/400` · 값 `red-700/400`(`ActionLogContent.tsx`) / DOM 트리는 태그·class를 같은 색으로 쓰되 `#id`만 **fuchsia-600/400**(`DomTreeDialog.tsx`). 두 화면이 값을 각자 들고 있고, **fuchsia는 이 문서 어디에도 없는 유일 색상환**이라 새로 늘리지 말 것.
