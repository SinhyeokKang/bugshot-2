...
4. **`code-collapse.css`가 그 `pre`의 행간·패딩을 상수로 재현한다** — 코드블럭 접기 높이 `max-height` calc가 위 클론의 `line-height`·`padding`·`border` 값에 **침묵으로 묶여 있다**(구체 값은 여기 안 적는다 — 문서에 복제하는 순간 네 번째 짝이 된다). 행간·패딩을 바꾸면 접힘 클램프가 조용히 어긋난다(짝 2개를 규칙대로 함께 고쳐도 세 번째가 안 따라온다). `sidepanel/lib/__tests__/codeCollapse.test.ts`가 **세 파일을 읽어 대조**해 이걸 red로 잡는다.
