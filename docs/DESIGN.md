...
- **인라인 진행률 바**: shadcn `Progress`를 쓰지 않는다(미설치). 마크업은 `<div class="h-1.5 w-N overflow-hidden rounded-full bg-muted">` + 내부 `<div class="h-full rounded-full bg-foreground transition-all duration-300" style={{width:`${percent}%`}}>`. 폭은 슬롯에 맞춘다 — 패널 본문 `w-40`, 좁은 툴바 슬롯 `w-24`. **`role="progressbar"` + `aria-valuenow/min/max` + `aria-label`을 붙인다**(퍼센트 텍스트에 `aria-live`를 거는 대신 — 프레임마다 갱신되면 낭독이 폭주한다). 현재 3곳(`IssueTab`의 스크롤 캡처·녹화 진행, `ReplayTrimDialog`의 트림 진행) ⚠️ 앞 둘은 `role` 없이 `duration-300`/`duration-500`으로 갈려 있어 정리 대상. `log-viewer`의 `ProgressBar.tsx`는 `role="slider"`인 **인터랙티브 seek 바**라 별개 종이다.
