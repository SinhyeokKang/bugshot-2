...
- **임의값(`text-[…]`)은 스케일에 대응값이 없을 때만.** 12px은 `text-xs`, 14px은 `text-sm`이 있으므로 임의값으로 쓰지 않는다. 실제로 남은 임의값은 10·11px처럼 스케일 밖 값뿐이다.
...
2. **클론 파일 2개** — `tiptap-editor.css`(에디터)와 `doc-section-body.css`(프리뷰)의 code/pre 규칙은 **바이트 동일**하다. 같은 마크다운의 편집 화면/프리뷰라 **항상 함께** 움직인다.
