...
- **전체화면 비-Radix 오버레이 패턴**: 사이드패널 위 풀스크린은 `inset-0 z-50 bg-background` + `flex h-full flex-col`로 만든다. 풀스크린 컴포넌트 오버레이(`AnnotationOverlay`·`ReplayTrimDialog`)는 `fixed`로 패널 뷰포트를 덮고, App root(`h-screen`) 직속 임시 오버레이(App AI 로딩·Suspense fallback)는 `absolute`를 쓴다. Radix Dialog가 아니라 컨테이너 직접 렌더 + `lazy`/`Suspense`. 이 오버레이들은 **사이드패널 z축 안**(z-50, 필요 시 내부 모달 z-[60])이다. **a11y**: Radix가 아니라 포커스 트랩·`inert`가 없으므로 `role="dialog"` + `aria-label`만 붙이고 **`aria-modal`은 붙이지 않는다** — 바깥 트리가 실제로 비활성이 아닌데 AT에게 그렇다고 알리면 거짓 약속이 된다(`ReplayTrimDialog`가 이 규칙을 따른다. ⚠️ `AnnotationOverlay`는 아직 role·label 둘 다 없어 미준수).
