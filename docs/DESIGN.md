...
- 커스텀 raw 색(`text-blue-600` 등)은 semantic 토큰으로 표현 못 하는 **상태/기능 색**에만 쓰고, 가능하면 `dark:` 짝을 함께 둔다. 현재 사용처:
  ...
  - **AI 기능 액센트 색쌍**: 스타일링 AI = **teal** / 초안 AI = **purple**. AI 진입 버튼·배너·로딩 오버레이 틴트를 관통하는 기능 색이다 — `StyleEditorPanel.tsx`(teal), `DraftingPanel.tsx`(purple), `App.tsx`(오버레이 `bg-<c>-500/5`·`via-<c>-400/10`). **배너 명도 쌍 관용: light `bg-<c>-100/80 text-<c>-700` / dark `bg-<c>-950 text-<c>-300`(hover `bg-<c>-900`)**.
...
- **Tailwind preflight가 `pre`·`code`·`kbd`·`samp`에도 `fontFamily.mono`를 깐다** — 클래스 없는 코드블록(Tiptap 등)이 자동으로 Geist가 된다. sans여야 하는 `<pre>`는 `font-sans`를 명시해야 한다(`NetworkLogContent.tsx`의 `FrameBody` — WebSocket 프레임 본문이 그 사례).
- 크기 관용: **`text-xs`·`text-sm`이 지배적**(UI 라벨·필드·보조 텍스트). `text-base`=본문, `text-lg`=섹션/빈 상태 제목, `text-xl`=큰 헤딩.
- **임의값(`text-[…]`)은 스케일에 대응값이 없을 때만.** 12px은 `text-xs`, 14px은 `text-sm`이 있으므로 임의값으로 쓰지 않는다. 실제로 남은 임의값은 10·11·**13**px처럼 스케일 밖 값뿐이다. 그중 **13px이 지배적**이다 — 로그 행·트리와 소형 텍스트 버튼의 관용값(`h-7 px-2.5 text-[13px] font-normal` — `OriginFilterBar`·`NetworkLogContent` 등)이라 12/14 사이 자리를 메운다.

### mono 표면 불변식

**모든 mono 표면은 12px이다.** 행간만 표면 성격에 따라 두 그룹으로 갈린다:

| 그룹 | 표면 | 값 | 출처 |
|---|---|---|---|
| **코드블럭** | CSS 코드 뷰 본문 · Tiptap `pre` · 프리뷰 `pre` | 12 / **18px**(1.5) | `CssCodeMirror.tsx` 인라인 theme · `tiptap-editor.css`·`doc-section-body.css`의 `line-height: 1.5` |
| **리스트·칩** | DOM 트리 · 콘솔 본문/스택 · LogSeekChip 2 · 네트워크 본문 | 12 / **16px** | `text-xs` |

- **`text-[12px]`가 아니라 `text-xs`를 쓴다** — `text-xs`는 12px에 `line-height: 1rem`(16px)을 **동반**한다. 리스트·칩은 한 줄이 한 항목이라 그 행간이 맞고, 코드블럭은 여러 줄을 훑는 면이라 1.5가 필요해 CSS로 따로 준다. **리스트·칩에 `text-[12px]`를 쓰면 행간이 18px로 새고, 코드블럭에 `text-xs`를 쓰면 16px로 조인다.**
- 축이 달라 이 표에서 제외: **인라인 `code`**(문단 행간 상속) · **CM 자동완성 li**(`lineHeight: 1.25rem` — 텍스트 행간이 아니라 li 높이, 편집 패널 `CommandItem`과 맞춘 값).
- 13px에서 내려온 이유: 좁은 패널에서 컸다. 14px로 올리지 않는 이유: mono는 자폭이 sans의 1.2배라 트렁케이션·가로 스크롤이 함께 늘어난다.
- **자간은 건드리지 않는다**(브라우저 기본 0). 12px에서 유의미한 차이를 내려면 `-0.24px` 이상이 필요한데 그 구간은 고정폭 그리드를 흐트러뜨린다.

**⚠ 짝이 둘이다 — 한쪽만 고치면 조용히 갈라진다.** v1.6.0이 "13px 2표면 통일"을 선언하고 Tiptap을 빠뜨린 게 정확히 이 구조 때문이다:

1. **진입 경로 2개** — `.font-mono` 유틸(CSS 뷰·DOM 트리·로그 5곳)과 **Tailwind preflight**(`pre`/`code` — Tiptap·프리뷰). 만나는 지점이 없다. mono 전역 규칙(`globals.css @layer base`의 `font-variant-ligatures: none`)은 **두 경로를 한 셀렉터 리스트로 묶어** 준다.
2. **클론 파일 2개** — `tiptap-editor.css`(에디터)와 `doc-section-body.css`(프리뷰)의 `code` 규칙은 **바이트 동일**하고, `pre`는 에디터 쪽에만 **`white-space: pre` 한 줄이 더 있다**(@tiptap/core가 런타임 주입하는 `.ProseMirror pre { white-space: pre-wrap }`을 특이도로 이기려는 것 — 프리뷰엔 그 주입이 없어 불필요. 의도된 발산이다). 나머지는 같은 마크다운의 편집 화면/프리뷰라 **항상 함께** 움직인다.
3. **log-viewer는 `globals.css`를 안 받는다**(별도 빌드) — `log-viewer/styles.css`에 mono 블록 **손복사본**이 있다. `App.tsx`가 사이드패널 컴포넌트를 import해 mono 표면이 실재하므로 필요하다. `__tests__/tokens.test.ts`가 두 블록의 일치를 강제한다.
4. **`code-collapse.css`가 그 `pre`의 행간·패딩을 상수로 재현한다** — 코드블럭 접기 높이가 `max-height: calc(var(--code-collapse-lines) * 1.5em + 2em + 0.75em)`이라 위 클론의 `line-height: 1.5`·`padding: 1em`에 **침묵으로 묶여 있다**. 행간·패딩을 바꾸면 접힘 클램프가 조용히 어긋난다(짝 2개를 규칙대로 함께 고쳐도 세 번째가 안 따라온다). `sidepanel/lib/__tests__/codeCollapse.test.ts`가 **세 파일을 읽어 대조**해 이걸 red로 잡는다.
...
- `0.875em` → `12px`로 바꾼 대가: 코드블럭·인라인 코드가 부모(`text-sm`) 변화를 **안 따라간다**. 전 표면 12px이 불변식이라 의도한 트레이드오프다.
