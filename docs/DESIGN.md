...
- **인라인 진행률 바**: shadcn `Progress`를 쓰지 않는다(미설치). 마크업은 `<div class="h-1.5 w-N overflow-hidden rounded-full bg-muted">` + 내부 `<div class="h-full rounded-full bg-foreground transition-all duration-300" style={{width:`${percent}%`}}>`. 폭은 슬롯에 맞춘다 — 패널 본문·대기 다이얼로그 `w-40`. **`role="progressbar"` + `aria-valuenow/min/max` + `aria-label`을 붙이고**, 퍼센트 텍스트는 `aria-live="polite"`로 함께 알린다(현재 3곳 모두 그렇게 돼 있다). `polite`라 낭독이 큐에 밀리지 않는다는 전제이고, 실제로 낭독이 폭주하면 `aria-live`를 떼기 전에 갱신 빈도부터 본다. 현재 3곳(`IssueTab`의 스크롤 캡처·녹화 진행, `LoadingDialog`) ⚠️ 앞 둘은 `role` 없이 `duration-300`/`duration-500`으로 갈려 있어 정리 대상 — `LoadingDialog`가 이 규칙을 지키는 기준 구현이다. **좁은 슬롯(액션바 중앙 등)에 진행률을 끼워 넣지 않는다**: 트림 재인코딩이 ≈144px 슬롯에 percent만 넣고 안내 문장을 토스트로 갈랐다가, 토스트가 대기(최대 15초)보다 먼저 사라져 경고가 증발했다. 안내가 필요한 진행은 `LoadingDialog`로 간다. `log-viewer`의 `ProgressBar.tsx`는 `role="slider"`인 **인터랙티브 seek 바**라 별개 종이다.
