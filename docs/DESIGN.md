...
- `tailwind.config.js`의 **`theme.container`**(`center: true`, `padding: "1rem"`)는 설정만 있고 `container` 유틸 사용처가 0이다. 위 컨테이너 쿼리와 **이름만 닮은 별개 축**이니 혼동하지 말 것
...
- **토큰 표는 세 벌이다** — `src/styles/globals.css`(사이드패널), `src/log-viewer/styles.css`(다운로드되는 `logs.html`, 별도 Vite 빌드라 globals를 import 못 해 수작업 복제), `src/content/overlay.ts`(picker 인스펙터 카드 — 페이지에 주입되는 Shadow DOM이라 CSS 변수를 못 받고 `hsl()` 리터럴로 복제). **한쪽만 고치면 사이드패널·첨부 logs.html·페이지 오버레이가 서로 다른 톤으로 갈린다.** `src/styles/__tests__/tokens.test.ts`가 세 표의 일치 + 위 테마별 채도 규칙 + destructive 대비 하한을 고정한다 — 토큰 값을 바꾸면 두 파일을 함께 고쳐야 이 테스트가 통과한다.
  - **세 번째 표의 다크 전환은 OS가 아니라 앱 theme을 따른다.** `@media (prefers-color-scheme: dark)`가 아니라 `.picker-label[data-theme="dark"][data-mode="inspector"]`이고, 그 속성은 사이드패널이 `picker.start`에 실어보낸 값(`resolveDark`로 해석한 최종 다크 여부)을 content script가 세팅한다 — OS 다크 + 앱 라이트 조합에서 페이지 카드만 검게 뜨던 걸 없앤 구조다. `tokens.test.ts`의 `parseOverlayTokens`가 **이 셀렉터 문자열을 앵커로 라이트/다크 영역을 가르므로**(다크 블록이 라이트 블록보다 뒤에 있어야 한다) 셀렉터를 바꾸면 파서도 함께 고친다. **값은 `picker.start` 시점 스냅샷이다** — picker를 띄워둔 채 theme을 바꾸면 카드는 다음 start(재arm·프레임 커밋)까지 옛 톤을 유지한다. 미전달이면 라이트가 기본이라 구버전 content script도 안 깨진다. (같은 "별도 번들이 복제" 함정 계열: log-viewer i18n dict·recorder pre-arm 청크 — docs/POSTMORTEM.md)
...
- ⚠ **글자색의 대비는 토큰 이름이 아니라 "무슨 표면 위냐"가 정한다.** 같은 `--muted-foreground`라도 `--background`(흰색) 위에선 4.75:1로 AA를 넘지만 `--muted`(코드블럭) 위에선 **4.34:1로 미달**한다. 실제로 코드블럭 행 번호가 CSS 코드 뷰의 gutter에서 색만 베껴왔다가 이걸 밟았다(그쪽은 배경이 투명이라 통과한다). **`muted` 표면 위 글자엔 `--muted-foreground`를 반사적으로 쓰지 말 것** — 옅게 깐 `--foreground`(예: `/ 0.6`)가 양 테마 AA를 넘으면서도 본문보다 약하다. 하한은 **그물 둘**이 지킨다(둘 다 토큰 이름이 아니라 값을 계산한다) — `styles/__tests__/tokens.test.ts`가 CSS 선언에서 알파를 읽어 재고(코드블럭 gutter), `styles/__tests__/muted-surface-contrast.test.ts`가 TSX의 Tailwind 클래스 축을 훑어 muted 값 표면(`--muted`·동값인 `--secondary`·`--accent`) 위 글자색을 라이트·다크 양쪽에서 잰다. 계산 자체는 `src/test/cssContrast.ts` 단일 출처이고 **알파 합성은 sRGB에서** 한다 — HSL 성분을 보간하면 두 토큰의 채도가 갈릴 때 관대한 쪽으로 어긋나 AA 미달을 통과로 읽는다(라이트 전용 함정: 다크는 두 토큰이 무채색이라 값이 같다).
...
- 커스텀 raw 색(`text-blue-600` 등)은 semantic 토큰으로 표현 못 하는 **상태/기능 색**에만 쓰고, 가능하면 `dark:` 짝을 함께 둔다. 현재 사용처:
  ...
  - 로그 semantic 색(console 레벨·network 메서드·action 톤): `src/lib/log-colors.ts` **단일 출처**. `TONE_TEXT`(red/amber/blue/green/neutral → `text-<c>-600 dark:text-<c>-400`) + `TONE_BG`(행 배경 틴트 base → `bg-<c>-100 dark:bg-<c>-950/50`, `consoleLevelBgClass` 포함) + **`TONE_BG_HOVER`**(base와 강조 사이 hover 단계 → `hover:bg-<c>-200/70 dark:hover:bg-<c>-950/70`) + **`TONE_BG_STRONG`**(선택된 행·이미 틴트된 행 **안**의 코드블럭처럼 그 면 위에서 다시 떠야 하는 표면 → `bg-<c>-200 dark:bg-<c>-950/70`, `consoleLevelBgStrongClass` 포함) + **`TONE_DOT`**(칠해진 점 — 네트워크 상세 상태 dot → `bg-<c>-500 dark:bg-<c>-400`) + `CONSOLE_LEVEL_TONE`/`NETWORK_METHOD_TONE`.
    - ⚠ **`neutral`은 `TONE_BG`(빈 문자열)와 `TONE_BG_STRONG`(`bg-muted`)이 비대칭이고 의도다.** 전자는 "틴트 없음"이라 면이 없어도 되지만 후자는 면이 반드시 있어야 하는 자리라, 그대로 빈 값을 물려받으면 콘솔 `log`/`debug` 코드블럭 배경이 무음으로 사라진다.
    - ⚠ **다크에서 `TONE_BG_HOVER`와 `TONE_BG_STRONG`이 같은 값(`-950/70`)이라 hover와 선택이 구별되지 않는다.** 라이트는 `-200/70` vs `-200`으로 갈린다. 알려진 미해결 항목이고, 고칠 땐 §2 상단의 "다크 배너 배경에 알파를 얹지 말 것"이 로그 행에는 적용되지 않는다는 점(비교 대상이 페이지 배경이 아니라 이웃 행)을 함께 본다. 사이드패널 로그 탭·다이얼로그(Console/Network/ActionLogContent 아이콘·행 배경)와 log-viewer `markers.ts` 툴팁 + 통합 타임라인 패널(`TimelineRow`·`timeline-merge.timelineFillClass`)이 공유해 라이트/다크 모두 일치(우측 로그 탭 ↔ 좌측 타임라인 면색 sync). 마커 핀 색·content-type 아이콘·syntax highlight는 스코프 밖(각 컴포넌트 로컬)
  ...
  - DOM 마크업 팔레트: 액션 로그 칩(`ActionLogContent.tsx`)은 `TONE_TEXT`로 수렴했다 — 태그 `TONE_TEXT.blue` · 속성명 `TONE_TEXT.amber` · 값 `TONE_TEXT.red`. DOM 트리(`DomTreeDialog.tsx`)는 아직 로컬 리터럴(태그·class `sky-600/400`, `#id`만 **fuchsia-600/400**)이라 **두 화면의 태그 색이 지금 갈려 있다**(sky vs blue) — 다음에 손댈 때 트리 쪽도 `TONE_TEXT`로 수렴시키거나 "별도 축"으로 확정할 것. **fuchsia는 이 문서 어디에도 없는 유일 색상환**이라 새로 늘리지 말 것.
  - 외부 링크: `text-blue-600 underline dark:text-blue-400` (`InlineLink.tsx`)
  - 주석(annotation) 색 팔레트: `src/sidepanel/lib/annotationDefaults.ts` (`ANNOTATION_COLORS` — 캔버스 그리기용 고정 5색 raw hex, `dark:` 짝 없음. 팔레트 값은 `components/annotation/presets.ts`가 여기서 re-export한다 — store가 컴포넌트 그래프를 물지 않으려고 값을 leaf로 내렸다)
  - **AI 기능 액센트 색쌍**: 스타일링 AI = **teal** / 초안 AI = **purple**. AI 진입 버튼·배너·로딩 오버레이의 틴트·ripple·문구·중단 버튼 4개 슬롯을 관통하는 기능 색이다 — `StyleEditorPanel.tsx`(teal), `DraftingPanel.tsx`(purple), `App.tsx`(오버레이 틴트 `bg-<c>-500/5` + radial ripple `<c>-400@0.1`). 재현 자동작성(repro) 오버레이는 draft·styling과 구분해 **amber**로 뗀다(오버레이 색은 표면별 3분기 — `App.tsx`의 `AI_OVERLAY_STYLE` 맵 단일 출처). **배너 명도 쌍 관용: light `bg-<c>-100/80 text-<c>-700` / dark `bg-<c>-950 text-<c>-300`(hover `bg-<c>-900`)**. 배너 안 텍스트는 **액센트색 → 인접 색 그라디언트**(`from-purple-500 to-indigo-500` / `from-teal-600 to-cyan-500`, 다크는 300단)로 칠한다 — 두 번째 정지점은 기능색이 아니라 시각 효과이므로 §2 등재 색 규칙 밖이지만, **임의의 색을 새로 넣지는 않는다**(인접 색상환 하나로 고정).
  ...
  - 경고 배너: **세 번째 명도 쌍** — `bg-amber-100/80 text-amber-800 border-amber-300` / `dark:bg-amber-950 dark:text-amber-200 dark:border-amber-800`(`StyleCssView.tsx`의 미적용 draft 안내). 배경은 위 둘과 같고 글자가 800/200이며 border 축이 하나 더 붙는다.
  ...
  - 단발 상태·기능 색 4종(넷 다 `dark:` 짝 보유): 제출 완료 아이콘 **green**(`SubmitSuccessView.tsx`), Tiptap 드래그오버 **sky**(`TiptapEditor.tsx`), 로그 패널 리사이저 hover **blue**(`NetworkLogContent.tsx` — log-viewer `App.tsx`의 분할 핸들 `via-blue-300`도 같은 계열이나 구현은 별개), 녹화 중 화면 **red**(`IssueTab.tsx`)
...
- 영속: `src/store/settings-ui-store.ts` (Zustand persist). key 리터럴은 스토어가 아니라 `src/lib/session-keys.ts`의 `APP_SETTINGS_PERSIST_KEY`(`bugshot-app-settings`)에 있다 — background가 같은 봉투를 직독하므로 두 벌을 두지 않는다.
...
- `font-sans` 스택: Pretendard Variable → Pretendard → 시스템 한/영 폴백 (`tailwind.config.js`). body에 `font-feature-settings: "rlig" 1, "calt" 1`. **log-viewer(`logs.html`)는 Pretendard `@import`가 없어 시스템 sans로 폴백한다** — `log-viewer/styles.css`가 body 스택을 Pretendard 없이 따로 하드코딩한다(내보낸 파일에 한글 폰트를 base64로 싣지 않으려는 의도된 발산 — Pretendard는 너무 크다).
- `font-mono` 스택: Geist Mono Variable → Tailwind 기본 mono 폴백 (`tailwind.config.js`). **폴백을 지우지 말 것** — 사이드패널은 `globals.css`의 `@fontsource @import`로 전 charset을 받고, 별도 빌드인 log-viewer(`logs.html`)는 `log-viewer/styles.css`의 `@font-face`로 **latin Geist Mono만 임베드**한다(자립 파일이라 뷰어 시스템 폰트 비의존 — variable 1파일 base64 ~40KB/파일). **non-latin(CJK·키릴·베트남 등)은 여전히 시스템 mono로 폴백**하므로 스택의 폴백이 안전망이다. `__tests__/tokens.test.ts`가 폴백 존재를 고정한다.
...
- **content script 오버레이(`overlay.ts`)는 폰트 토큰을 못 받는다** — Shadow DOM `all: initial`이라 `font-sans`가 안 흐르고, `-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif` 스택을 **5벌 하드코딩**한다(banner 12px·selector 11px·inspector 14px·box-label 10px·11px). §2의 토큰 표와 같은 계열의 불가피한 사본이라 CSS 변수로 묶지 않는다(`:host { all: initial }` 안의 변수 선언·참조는 브라우저별 동작 차가 있어 유닛으로 못 고정한다).
- 크기 관용: **`text-xs`·`text-sm`이 지배적**(UI 라벨·필드·보조 텍스트). `text-base`=본문, `text-lg`=섹션/빈 상태 제목, `text-xl`=큰 헤딩.
- **임의값(`text-[…]`)은 스케일에 대응값이 없을 때만.** 12px은 `text-xs`, 14px은 `text-sm`, 20px은 `text-xl`이 있으므로 임의값으로 쓰지 않는다. 의도된 임의값은 10·11·**13**px처럼 스케일 밖 값뿐이다. `IssueTitleOverlay.tsx`의 14px·20px는 아직 남은 규칙 위반이며 새 선례가 아니다. 그중 **11px이 가장 많다**(36곳 — 콤보박스 보조 라벨·connect 폼 힌트). 10px 18곳, 13px 9곳이 뒤를 잇는다. 13px은 소형 텍스트 버튼의 관용값(`h-7 px-2.5 text-[13px] font-normal` — `OriginFilterBar` 등)으로 남아 12/14 사이를 메우지만, 로그 표면 다수가 `text-mono`로 옮겨 가며 비중이 줄었다.
...
| 표면 | 소비 경로 |
|---|---|
| DOM 트리 · 콘솔 본문/스택/인라인 요약/출처 URL · 액션 로그 행·값 칩 · LogSeekChip 2 · 네트워크 본문 · JSON 트리 · WS 프레임 프리뷰/본문 · 마커 툴팁 · log-viewer 통합 타임라인 행(`TimelineRow` action/console/network 3) | Tailwind `fontSize.mono` 토큰 → **`text-mono`** 유틸(과거 `text-xs`) |
...
1. **진입 경로 2개** — `.font-mono` 유틸(CSS 뷰·DOM 트리·로그 15곳: 콘솔 본문·스택·인라인 요약·출처 URL, 액션 행, LogSeekChip 2, 네트워크 raw body, WS 프레임 프리뷰·본문, JSON 트리, 마커 툴팁, log-viewer 통합 타임라인 행 3)과 **Tailwind preflight**(`pre`/`code` — Tiptap·프리뷰). 만나는 지점이 없다. 전자는 `text-mono` 토큰, 후자는 CSS의 `var(--mono-*)`로 **같은 두 변수**에 묶인다. mono 전역 규칙(`globals.css @layer base`의 `font-variant-ligatures: none`)은 두 경로를 한 셀렉터 리스트로 묶어 준다.
...
| 토스트 | `rounded-xl` |
| 칩·스와치 등 작은 인라인 요소 | `rounded-sm` / `rounded-[3px]` / `rounded-[4px]` |
| 원형(스위치 thumb·pill 배지·아바타) | `rounded-full` |

- **`rounded-[3px]`와 `rounded-[4px]`는 통일하지 않는다.** 3px은 `ColorSwatch.tsx`가 content script 오버레이의 `.pl-swatch`(`overlay.ts`, `border-radius:3px`)와 맞춘 **cross-file 앵커**라 값이 곧 계약이고, 4px은 그와 무관한 두 표면(`HighlightedText`의 `<mark>` 하이라이트·`slackFields/ChannelIcon`의 16px 아바타)이 쓴다. 한쪽으로 밀면 앵커의 의미가 희석된다.
...
- **닫기는 z축이 아니라 dismiss 계층을 따른다** — Dialog 안에서 Popover(콤보박스)가 열려 있으면 dim 클릭은 **Popover만** 닫고 다이얼로그는 남긴다(Escape도 같다). Radix는 modal Dialog 자신을 pointer-events 기준선으로 삼아 위에 쌓인 non-modal 레이어와 Dialog 양쪽에 outside 이벤트를 뿌리므로, `DialogContent`가 `onPointerDownOutside`에서 되받아 막는다. 판정은 포커스 위치가 아니라 `PopoverContent`가 심는 `data-dismiss-layer` 마커(`@/lib/dismiss-guard`) — Dialog 아래 깔려 pointer-events가 꺼진 레이어는 세지 않는다. `AlertDialog`(Radix가 이미 outside를 무시)·`Select`(`disableOutsidePointerEvents`)는 마커가 필요 없고, Tooltip은 `data-state`가 `delayed-open`/`instant-open`이라 애초에 안 걸린다. **body-portal 풀스크린 오버레이를 modal Dialog 안에서 띄우는 건 여전히 금지**(docs/POSTMORTEM.md 2026-07-22 — 그건 마커가 없어 이 계약 밖이다).
- 그 위로 강제로 떠야 하는 예외는 `z-[60]` (현재 3곳 — `AnnotationOverlay` 텍스트 편집 입력, `ReplayTrimDialog` 녹화 폐기 AlertDialog, 같은 다이얼로그의 트림 재인코딩 `LoadingDialog`). 새로 만들 땐 `z-50` 기준으로 잡고, 꼭 필요할 때만 `z-[60]`.
- **전체화면 비-Radix 오버레이 패턴**: 사이드패널 위 풀스크린은 `inset-0 z-50 bg-background` + `flex h-full flex-col`로 만든다. 풀스크린 컴포넌트 오버레이(`AnnotationOverlay`·`ReplayTrimDialog`)는 `fixed`로 패널 뷰포트를 덮고, App root(`h-screen`) 직속 임시 오버레이(App AI 로딩·Suspense fallback)는 `absolute`를 쓴다. Radix Dialog가 아니라 컨테이너 직접 렌더 + `lazy`/`Suspense`. 이 오버레이들은 **사이드패널 z축 안**(z-50, 필요 시 내부 모달 z-[60])이다. **a11y**: Radix가 아니라 포커스 트랩·`inert`가 없으므로 `role="dialog"` + `aria-label`만 붙이고 **`aria-modal`은 붙이지 않는다** — 바깥 트리가 실제로 비활성이 아닌데 AT에게 그렇다고 알리면 거짓 약속이 된다(`ReplayTrimDialog`·`AnnotationOverlay` 둘 다 이 규칙을 따른다 — `role="dialog"` + `aria-label`만, `aria-modal` 없음).
...
- **아이콘 전용 버튼**(`size="icon"`)은 텍스트가 없으므로 `aria-label`(또는 `sr-only` 텍스트)을 붙인다 — 안 그러면 스크린리더에서 무명 버튼. **툴바 아이콘 버튼의 hover 툴팁은 native `title`이 아니라 `TooltipIconButton`(Radix Tooltip + `aria-label`, §13)을 기본으로 한다** — 캡처 방식 툴바·어노테이션 툴바(이미지·녹화)가 전부 이걸 쓴다. **`title`-only는 현재 0곳이다**(실측 — `size="icon"` 48곳 중 `aria-label`+`title` 병기 40 / `aria-label`만 8). 마지막 하나였던 `OrderedListEditor`의 삭제 버튼이 `aria-label`을 얻으면서 레거시 목록이 공집합이 됐으니, 새 아이콘 버튼은 `TooltipIconButton`을 쓰거나 직접 만들면 `aria-label`을 반드시 붙여 이 상태를 유지한다. **jsdom 접근명 계산은 `title`로 폴백하므로 `getByRole(..., { name })`만으로는 title-only가 통과한다** — 속성 존재를 따로 단언해야 그물이 된다(`OrderedListEditor.test.tsx` 선례).
...
| 계열 | 자리 | raw인 이유 | 추가 요구 |
|---|---|---|---|
| 입력 내 클리어 | 검색창 우측 `X` (`IssueListTab`·`Action/Console/NetworkLogContent`·log-viewer `TimelinePanel`) | `size="icon"`(h-9 w-9)이 필드 안에 안 들어간다 | `aria-label` 필수 |
| 트리 chevron | `TreeChevronButton.tsx` (JSON·DOM 트리 공용) | 같은 이유(4px 슬롯) | `aria-expanded` 필수, 라벨은 호출부 주입. **새 트리는 이걸 쓴다** |
| sticky 헤더 제목 | `DomTreeDialog`의 선택 요소 제목 (`DialogTrigger` — 누르면 DOM 트리 다이얼로그가 열린다) | `Button variant="ghost"`의 base cva `inline-flex h-9 px-4`가 원본 `block w-full truncate text-2xl`과 충돌해 **`truncate`가 파손된다**(익명 flex item이 되어 하드 클립) — 행 높이 +4px, 텍스트 폭 −32px, hover가 `opacity-70` 전이 대신 면색 블록. override 4종이 원본 className보다 길어진다 | `focus-visible:ring-2 focus-visible:ring-ring` + `title`(잘린 제목 전문) |
| 트리 행 더보기 | `JsonTreeViewer`의 "n개 더" | `Button variant="link"`도 base의 `h-9 px-4`를 물고 와 트리 행 높이(`py-0.5`)를 깨뜨린다 | `w-full text-left` — 원래 block-level `<div>`라 없으면 클릭 영역이 텍스트 폭으로 좁아진다. `aria-expanded`는 **붙이지 않는다**(disclosure가 아니라 다음 청크 추가라 상태 속성이 거짓말이 된다) |
| 문자열 인라인 더보기 | `JsonTreeViewer`의 "모두 보기" | 같은 이유 | **`w-full`을 붙이지 않는다** — 원래 인라인 `<span>`이라 붙이면 클릭 영역이 행 전체로 번진다. 위 행과 계열은 같지만 요구가 반대인 지점 |
...
| 컴포넌트 | 표준화 대상 |
|---|---|
| `PageShell`/`PageScroll`/`PageFooter` (`Section.tsx`) | 탭 페이지 골격 — Shell=`flex min-h-0 flex-1 flex-col`, Scroll=`min-h-0 flex-1 overflow-y-auto`(내부 스크롤 영역 — 자식을 `flex flex-col` 콘텐츠 컬럼으로 감싸고, `contentClassName`으로 `min-h-full`을 주입해 `flex-1` 자식이 뷰포트를 채우게 할 수 있다 — 앱 내 유일 사례가 `StyleEditorPanel`의 CSS 뷰(`styleEditorView === "code"`일 때만 주입)이고, 위 §11 세그먼트 토글의 `flex-1` 채움과 한 쌍으로 동작한다), Footer=`shrink-0 flex flex-col gap-2 border-t bg-muted/50 p-4`(하단 고정 액션 — 자식을 세로로 쌓는다. 가로 배치가 필요하면 자식 쪽에서 감쌀 것). 전 탭 공용. **취소·제출 같은 액션이 없는 순수 툴바 footer는 `bg-muted/50`이 아니라 `bg-background`**(캡처 방식 툴바·녹화 그리기 툴바 — `PageFooter`엔 className override prop이 없어 이 경우는 raw div로 짠다) |
...
| `SingleLazyCombobox.tsx` / `CcMultiCombobox.tsx` | 플랫폼 필드 콤보박스 — Popover+Command 기반 단일 선택(lazy fetch) / 다중 선택(CC 멘션). `CcMultiCombobox`는 **8개 플랫폼 필드 폼 전부**(jira/github/gitlab/linear/notion/asana/clickup/slack `*Fields/`)의 표준이고, `SingleLazyCombobox`는 **5개 플랫폼 12곳**(github 2·gitlab 2·asana 3·clickup 4·slack 1)이 쓴다. 새 플랫폼 필드는 직접 만들지 말고 이 둘을 조합한다. 남은 예외 3계열은 로드 모델이 달라 의도적으로 밖에 있다 — **서버 검색형**(`githubFields/RepoCombobox`·`gitlabFields/ProjectCombobox`·`notionFields/DatabaseCombobox`: 질의마다 서버 재조회라 `load()`가 query를 받아야 한다), **linear 3개**(`linearFields/{Label,Assignee,Project}Combobox`: `loadedTeamId` 마커로 스코프 교체를 판정), **jira**(`FieldCombobox` + `useDebouncedSearch` 자체 계열). `SingleLazyCombobox`는 open 시 1회 로드 + 클라이언트 부분일치 필터 모델이고, 검색어(`query`)를 Popover 바깥 state로 들고 있어 **닫아도 검색어가 남는다**. 선택 항목 상단 고정은 `ccOptions.ts`(`pinSelectedFirst`/`orderSelectedFirst`) 공유 |
...
| `FieldRow.tsx` | 라벨+필드 쌍 — `grid gap-1.5`, 라벨 `text-xs text-muted-foreground`, `required` 시 빨간 별. `htmlFor`로 라벨↔입력을 잇고(connect 폼 10곳), `labelAction`으로 라벨 우측에 보조 링크를 나란히 둔다("토큰 발급" 7곳 — 이게 없으면 그 7곳은 label→children 세로 구조로 표현이 안 된다). **`tabs/connect/` 이행 완료**(2026-08-16) — 8개 폼이 반복하던 42곳 중 **`<label>`을 동반한 34곳이 전부 `FieldRow`**이고, 남은 8곳은 라벨 없는 세로 스택이라 애초에 대상이 아니다(두 수치가 세는 대상이 다르다). 치환하며 컨테이너가 `flex flex-col gap-1.5` → `grid gap-1.5`로 바뀌었는데 단일 컬럼·자식 stretch라 렌더가 같다 |
...
| `LoadingDialog.tsx` | **끊을 수 없는 장기 작업의 대기 화면** — `AlertDialog`(바깥 클릭 무시) + `onEscapeKeyDown` preventDefault로 **닫는 컨트롤이 없다**. `open`이 false가 될 때만 사라진다. 본문 골격은 `IssueTab`의 `EmptyShell`·녹화 중 화면과 같다(원형 아이콘 뱃지 → `text-lg font-semibold` 제목 → 설명 → 진행률 바 → 퍼센트). 제목·설명은 `gap-1` wrapper로 묶어 한 덩어리로 읽히게 하고(컨테이너 `gap-4`를 그대로 물려받으면 둘이 갈라진다), 퍼센트 텍스트는 바 **아래**에 둔다. 다이얼로그 셸만 바뀐 것이라 전체화면 대기와 모달 대기가 같은 얼굴을 유지한다. `description`은 **필수**(왜 기다려야 하는지 없는 대기 화면은 성립하지 않고, Radix가 Description 부재를 접근성 경고로 잡는다), `percent`는 optional(없으면 바 미표시 — 진척을 못 재는 작업은 `icon`에 `Loader2 animate-spin`을 넣는다). 전체화면 오버레이 위에 띄울 땐 `className="z-[60]"`(트림 화면 관용구). 사용처: 리플레이 트림 재인코딩 |
...
| `LogAttachmentCards.tsx` (+ `logCardTypeCounts.ts`) | logs.html 단일 첨부 토글 카드 — 카드 설명 문자열은 `logCardTypeCounts.ts` 단일 출처(`ccOptions.ts`와 같은 등급의 콜로케이션 헬퍼)(Switch + 스위치 영역 hover-suppress — **`IssueRow`와 공유하는 건 이 hover-suppress 로직뿐**이고 카드 골격은 `AttachmentList`와 동일[`Card`+`p-3`, 제목 `text-sm font-medium`]. `IssueRow`는 Card가 아닌 div에 `px-4 py-3`·제목 `text-base`) — 클릭 시 LogPreviewDialog |
| `IssuePreviewView.tsx` | 이슈 본문 프리뷰 — 제목·재현 환경·섹션 + 마크다운 복사, `media`/`logCards` 슬롯. PreviewPanel과 log-viewer Report 탭 공용(두 표면이 같은 본문 **구조**를 그리도록 강제). **언어 정책은 표면별로 갈리고, log-viewer 쪽은 두 층이다** — PreviewPanel은 화면 언어. log-viewer Report 탭은 **본문(`data.report.sections`)이 export 시점 언어로 박제**되지만 섹션 라벨·UI 문구는 `t("logViewer.report.*")`라 **읽는 사람의 브라우저 언어**로 나온다(log-viewer는 `bodyLocale`을 모르고, 사용자 locale 주입을 의도적으로 안 한다). 같은 컴포넌트가 다른 언어를 렌더할 수 있는 게 의도다 |
...
| `ImageActions.tsx` | 이미지 위 우상단 오버레이 컨트롤 — §6 관용구(`bg-background/90 backdrop-blur-sm` + 그룹 `rounded-md shadow-md`)를 `TooltipIconButton`·`ButtonGroup`으로 조합. `StyleChangesTable`이 소비한다. **아래 `blockActions` 행이 "수동 동기화" 대상으로 가리키는 React 원본**이라 아이콘·순서를 바꾸면 vanilla 쌍둥이도 함께 고친다 |
| `LogInsertDialog.tsx` | 본문에 넣을 로그 1건을 고르는 다이얼로그 셸 — `DraftingPanel`·`LogPreviewDialog` 2곳 공용. 탭 전환은 아래 `*LogPreviewDialog` 계열과 같은 `data-[state=inactive]:hidden` 구조(스크롤 위치 보존) |
| `TreeChevronButton.tsx` | JSON 트리·DOM 트리 공용 접기 chevron — raw `<button>` + `h-4 w-4`(§10 예외 항목 참조). 새 트리를 만들면 이걸 쓴다 |
| `IntegrationsCta.tsx` | 연동 0개일 때 뜨는 amber 유도 배너 — `IssueTab`·`PreviewPanel`·`DraftDetailDialog` 3곳 공용(색은 §2 참조) |
...
### 폼 요소 (shadcn)
`Input`/`Textarea`/`Select`/`Checkbox`/`Switch`/`Label` 모두 shadcn 기본 치수(`Input` h-9, `Checkbox` h-4 w-4, `Switch` h-5 w-8 등). 표면은 `Card`(+`CardHeader`/`CardContent`) — 연결 폼 8개·설정 카드·로그 첨부 카드 등 18개+ 파일의 기본 컨테이너다(§6 radius 표의 "카드=`rounded-xl`"가 이것). **라벨+필드는 `FieldRow`로 감싸는 것을 기본**으로 한다:
```tsx
...
- **로딩 스피너**: lucide `Loader2` + `animate-spin`(`h-4 w-4` 또는 `h-3 w-3`). 오버레이는 `absolute inset-0 flex items-center justify-center`. **lazy 청크 대기는 스피너가 아니라 스켈레톤**이다 — `StyleCssView`의 Suspense fallback `min-h-24 flex-1 animate-pulse rounded-md border border-border bg-muted/40`(앱 내 유일한 `animate-pulse`). 화면을 막지 않는 안내는 같은 파일의 `role="status" aria-live="polite"` 인라인 배너를 쓴다 — toast·tooltip·인라인 에러와 다른 종이다.
- **진행 중 잠금**: `disabled` 대신 **`aria-disabled` + 핸들러 early-return 가드**를 쓴다 — shadcn Button base의 `disabled:pointer-events-none` 때문에 `disabled`면 툴팁·hover가 죽고 스피너까지 흐려진다. 스타일은 `aria-disabled:cursor-not-allowed aria-disabled:opacity-50`(스피너를 든 버튼은 `opacity-50`을 뺀다). **hover 중화는 variant 종속이라 이 2종에 안 들어 있다** — base cva의 hover가 살아 있어 잠긴 버튼이 hover에서 진해지므로, 쓰는 variant의 hover를 되돌리는 클래스를 함께 건다(`outline`은 `aria-disabled:hover:bg-background aria-disabled:hover:text-foreground` — `IssueTab`, `default`는 `aria-disabled:hover:bg-primary` — `IssueCreateModal`). **선례를 그대로 복사하면 안 되는 자리다**: outline용 `bg-background`를 default 버튼에 얹으면 hover에서 하얗게 뒤집힌다. 선례는 이제 광범위하다 — statusBadges 7개, connect 폼 8개(`JiraConnectForm` 주석이 "8개 폼 공통 패턴"으로 자칭), `TooltipIconButton`의 `ariaDisabled`, `ReplayButton`, 캡처 방식 툴바(`IssueTab`), `StyleChangesDialog`·`StyleEditorPanel`·`DraftingPanel`·`LlmConnectForm`·`ReplayTrimDialog` 등 24파일 30곳. **하이브리드 분기**도 관용구다 — 진짜 불가 상태만 순수 `disabled`, 진행 중은 `aria-disabled`(`ReplayTrimDialog`의 확정 버튼: `disabled={duration <= 0 && !busy}` + `aria-disabled={busy}`). ⚠️ 잠금 클래스에 단일 출처가 없다 — **명명 상수 3곳**(`IssueTab`·`DraftingPanel`·`annotation/ZoomControl`. 셋의 **값이 같지도 않다** — `ZoomControl`의 `LOCK_CLASS`만 `bg-transparent` 접두가 붙는다) + **인라인 리터럴 9곳** + **`opacity-50` 없는 축약형 21곳**. 그중 8곳은 위 "스피너 든 버튼은 `opacity-50`을 뺀다"의 **정규 구현**이다 — `StyleEditorPanel.tsx`의 `!nextBusy && "aria-disabled:opacity-50"`(조건부)와 statusBadges 7개(갱신 중 `Loader2`를 띄우고 흐림은 내부 `Badge`가 조건부 `opacity-50`으로 맡는다. Slack은 상태 개념이 없어 정적 제출 배지뿐이라 대상 아님). 즉 정확히는 **무조건 누락 13곳 + 정규 예외 8곳**이다. statusBadges는 raw `<button>`이라 base cva hover가 없어 **hover 중화 클래스도 대상이 아니다** — 표준 잠금 관용구를 통째로 복사하면 opacity가 이중으로 얹힌다(`statusBadges/__tests__/lockAffordance.test.ts`가 전수 스캔으로 양방향을 잠근다). **진행 중이 아닌 영속 상태**(설정 off, 미지원 페이지 등)에도 쓸 수 있으나, 그때는 **이유를 툴팁으로 함께 준다** — `aria-disabled`는 `disabled`와 달리 hover가 살아 있어서, 이유가 없으면 "밝아지는데 안 눌린다"가 된다. 선례: `ReplayButton`(설정 off → 툴팁 + 설정 탭으로 이동), 미지원 페이지의 `[이슈 작성]`(툴팁으로 사유 표기).
- **인라인 진행률 바**: shadcn `Progress`를 쓰지 않는다(미설치). 마크업은 `<div class="h-1.5 w-N overflow-hidden rounded-full bg-muted">` + 내부 `<div class="h-full rounded-full bg-foreground transition-all duration-300" style={{width:`${percent}%`}}>`. 폭은 슬롯에 맞춘다 — 패널 본문·대기 다이얼로그 `w-40`. **`role="progressbar"` + `aria-valuenow/min/max` + `aria-label`을 붙인다**(퍼센트 텍스트에 `aria-live`를 겹쳐 거는 대신 — role이 이미 값 변경을 알리므로, 겹치면 프레임마다 낭독이 큐에 쌓인다). 세 곳 전부 이 규칙을 지킨다. 현재 3곳(`IssueTab`의 스크롤 캡처·녹화 진행, `LoadingDialog`) 전부 `role`·`aria-value*`·`aria-label`을 갖췄다. ⚠️ 남은 차이는 전이 시간뿐 — 스크롤 캡처 `duration-300` vs 녹화 `duration-500`으로 갈려 있다. `LoadingDialog`가 이 규칙의 기준 구현이다. **`ReplayTrimDialog`의 선택 길이 readout은 이 계열이 아니다** — 진행률이 아니라 슬라이더 readout이라 `aria-live="polite"`가 붙어 있고 그게 맞다(핸들을 움직일 때 값을 알릴 다른 수단이 없다). **좁은 슬롯(액션바 중앙 등)에 진행률을 끼워 넣지 않는다**: 트림 재인코딩이 ≈144px 슬롯에 percent만 넣고 안내 문장을 토스트로 갈랐다가, 토스트가 대기(최대 15초)보다 먼저 사라져 경고가 증발했다. 안내가 필요한 진행은 `LoadingDialog`로 간다. `log-viewer`의 `ProgressBar.tsx`는 `role="slider"`인 **인터랙티브 seek 바**라 별개 종이다.
- **AI 로딩 오버레이**: `App.tsx`(`absolute inset-0 z-50`, 전체 오버레이 + `backdrop-blur-[2px]`). 표면별 틴트(styling=teal / draft=purple / repro=amber, `bg-<c>-500/5` — `AI_OVERLAY_STYLE` 맵) 위에 **중앙 문구(20px semibold — `text-xl`) + 중앙에서 퍼지는 도넛 물결 ripple**. 문구는 `useAiLoadingStep`이 3s마다 step을 올려 `aiLoadingPhraseKey`로 표면당 5개를 무한 루프(홀드 없음), 전환은 `AiLoadingText`가 이전 문구를 위로 이탈(`animate-out slide-out-to-top`)·새 문구를 아래에서 진입(`animate-in slide-in-from-bottom`, `duration-700`)시킨다. ripple은 `tailwind.config.js`의 `ai-ripple` keyframe(scale 0→1 + opacity, 3s)을 `key={step}` 리마운트로 매 전환마다 replay 하고, 링(도넛)은 arbitrary radial-gradient(`transparent→<c>-400@0.1→transparent`) `h-[240vh] w-[240vh]`가 화면 밖까지 커지며 물결처럼 번지고(`blur-3xl`로 끝단 부드럽게), 문구는 래퍼 `animate-text-breathe`(opacity 0.5↔0.45)로 은은하게 숨쉰다. 정렬은 래퍼 div(`-translate-x-1/2 -translate-y-1/2`)가 맡고 키프레임은 scale만 한다. **중단 버튼**은 `variant="ghost" size="sm"` + `absolute bottom-8 left-1/2 -translate-x-1/2 rounded-full shadow-sm [&_svg]:size-3.5`이고, 색은 틴트와 별개로 `AI_OVERLAY_STYLE`의 `button` 슬롯을 쓴다(표면별 teal/purple/amber).
- **빈 상태(empty state)** 관용형:
  ...
  ```
  콘텐츠 단위 빈 값은 `text-sm text-muted-foreground/70`. **문구는 표면에 따라 갈리고 통일하지 않는다** — 사이드패널 UI 안(프리뷰·`DocSectionBody`·`DraftDetailDialog`)은 `common.empty`(`비어 있음`), 이슈 트래커로 나가는 본문(8개 빌더)은 `md.noValue`(`(없음)`). 독자가 다르다: 전자는 사용자에게 "여기가 비었다"를 알리는 상태 표시고, 후자는 남의 트래커에 실리는 텍스트라 괄호 플레이스홀더 관용을 따른다(`비어 있음`이 단독으로 들어가면 사람이 쓴 내용처럼 읽힌다).
