...
- 커스텀 raw 색(`text-blue-600` 등)은 semantic 토큰으로 표현 못 하는 **상태/기능 색**에만 쓰고, 가능하면 `dark:` 짝을 함께 둔다. 현재 사용처:
  ...
  - 단발 상태·기능 색 4종(넷 다 `dark:` 짝 보유): 제출 완료 아이콘 **green**(`SubmitSuccessView.tsx`), Tiptap 드래그오버 **sky**(`TiptapEditor.tsx`), 로그 패널 리사이저 hover **blue**(`NetworkLogContent.tsx`), 녹화 중 화면 **red**(`IssueTab.tsx`)
...
- **플랫폼 브랜드 = `@icons-pack/react-simple-icons`**, `Si{Name}` + **`color="default"`**(브랜드 색 유지). 크기는 className.
  - **GitHub·Notion만 `dark:invert`**(어두운 단색 마크). 동적 케이스는 플래그 + `cn(…, flag && "dark:invert")` — `SubmitFieldsDialog`의 `PLATFORM_TABS`는 `invertOnDark`, `LlmConnectDialog`는 같은 개념을 `darkInvert`로 부른다(OpenAI·Anthropic·Groq·OpenRouter·Ollama — gemini·mistral·together는 유색 마크라 미적용). ⚠ 한 개념에 두 이름 — **새 코드는 `invertOnDark`로 통일**.
...
- **진행 중 잠금**: `disabled` 대신 **`aria-disabled` + 핸들러 early-return 가드**를 쓴다 — shadcn Button base의 `disabled:pointer-events-none` 때문에 `disabled`면 툴팁·hover가 죽고 스피너까지 흐려진다. 스타일은 `aria-disabled:cursor-not-allowed aria-disabled:opacity-50`(스피너를 든 버튼은 `opacity-50`을 뺀다). 선례는 이제 광범위하다 — statusBadges 8개, connect 폼 8개(`JiraConnectForm` 주석이 "8개 폼 공통 패턴"으로 자칭), `TooltipIconButton`의 `ariaDisabled`, `ReplayButton`, 캡처 방식 툴바(`IssueTab`), `StyleChangesDialog`·`StyleEditorPanel`·`DraftingPanel`·`LlmConnectForm`·`ReplayTrimDialog` 등 23파일 ~30곳. **하이브리드 분기**도 관용구다 — 진짜 불가 상태만 순수 `disabled`, 진행 중은 `aria-disabled`(`ReplayTrimDialog`의 확정 버튼: `disabled={duration <= 0 && !busy}` + `aria-disabled={busy}`). ⚠️ 잠금 클래스에 단일 출처가 없다 — **명명 상수 3곳**(`IssueTab`·`DraftingPanel`·`annotation/ZoomControl`. 셋의 **값이 같지도 않다** — `ZoomControl`의 `LOCK_CLASS`만 `bg-transparent` 접두가 붙는다) + **인라인 리터럴 8곳** + **`opacity-50` 없는 축약형 19곳**. 축약형 19곳 중 `StyleEditorPanel.tsx`의 `!nextBusy && "aria-disabled:opacity-50"` 하나는 위 "스피너 든 버튼은 `opacity-50`을 뺀다"의 **정규 구현**이므로, 정확히는 **무조건 누락 18곳 + 조건부 1곳**이다. statusBadges 8개는 `aria-disabled`만 걸고 잠금 클래스가 없어 시각 어포던스가 빠져 있다. **진행 중이 아닌 영속 상태**(설정 off, 미지원 페이지 등)에도 쓸 수 있으나, 그때는 **이유를 툴팁으로 함께 준다** — `aria-disabled`는 `disabled`와 달리 hover가 살아 있어서, 이유가 없으면 "밝아지는데 안 눌린다"가 된다. 선례: `ReplayButton`(설정 off → 툴팁 + 설정 탭으로 이동), 미지원 페이지의 `[이슈 작성]`(툴팁으로 사유 표기).
- **인라인 진행률 바**: shadcn `Progress`를 쓰지 않는다(미설치). 마크업은 `<div class="h-1.5 w-N overflow-hidden rounded-full bg-muted">` + 내부 `<div class="h-full rounded-full bg-foreground transition-all duration-300" style={{width:`${percent}%`}}>`. 폭은 슬롯에 맞춘다 — 패널 본문·대기 다이얼로그 `w-40`. **`role="progressbar"` + `aria-valuenow/min/max` + `aria-label`을 붙인다**(퍼센트 텍스트에 `aria-live`를 거는 대신 — 프레임마다 갱신되면 낭독이 폭주한다). 현재 3곳(`IssueTab`의 스크롤 캡처·녹화 진행, `LoadingDialog`) ⚠️ 앞 둘은 `role` 없이 `duration-300`/`duration-500`으로 갈려 있어 정리 대상 — `LoadingDialog`가 이 규칙을 지키는 기준 구현이다. **좁은 슬롯(액션바 중앙 등)에 진행률을 끼워 넣지 않는다**: 트림 재인코딩이 ≈144px 슬롯에 percent만 넣고 안내 문장을 토스트로 갈랐다가, 토스트가 대기(최대 15초)보다 먼저 사라져 경고가 증발했다. 안내가 필요한 진행은 `LoadingDialog`로 간다. `log-viewer`의 `ProgressBar.tsx`는 `role="slider"`인 **인터랙티브 seek 바**라 별개 종이다.
