...
- **진행 중 잠금**: `disabled` 대신 **`aria-disabled` + 핸들러 early-return 가드**를 쓴다 — shadcn Button base의 `disabled:pointer-events-none` 때문에 `disabled`면 툴팁·hover가 죽고 스피너까지 흐려진다. 스타일은 `aria-disabled:cursor-not-allowed aria-disabled:opacity-50`(스피너를 든 버튼은 `opacity-50`을 뺀다). 선례: `ReplayButton`·캡처 방식 툴바(`IssueTab`)·`TooltipIconButton`의 `ariaDisabled`. **진행 중이 아닌 영속 상태**(설정 off, 미지원 페이지 등)에도 쓸 수 있으나, 그때는 **이유를 툴팁으로 함께 준다** — `aria-disabled`는 `disabled`와 달리 hover가 살아 있어서, 이유가 없으면 "밝아지는데 안 눌린다"가 된다. 선례: `ReplayButton`(설정 off → 툴팁 + 설정 탭으로 이동), 미지원 페이지의 `[이슈 작성]`(툴팁으로 사유 표기).
