...
- **토큰 표는 네 벌이다** — `src/styles/globals.css`(사이드패널), `src/log-viewer/styles.css`(다운로드되는 `logs.html`, 별도 Vite 빌드라 globals를 import 못 해 수작업 복제), `src/content/overlay.ts`(picker 인스펙터 카드 — 페이지에 주입되는 Shadow DOM이라 CSS 변수를 못 받고 `hsl()` 리터럴로 복제), `src/content/device-frame.ts`(디바이스 뷰포트 래퍼의 좌우 여백 — `--muted` 한 쌍만 복제. 캡처 이미지에 그대로 찍히는 색이라 어노테이션 캔버스와 같은 토큰이어야 한다). **한쪽만 고치면 사이드패널·첨부 logs.html·페이지 오버레이·디바이스 래퍼가 서로 다른 톤으로 갈린다.** `src/styles/__tests__/tokens.test.ts`가 네 표의 일치 + 위 테마별 채도 규칙 + destructive 대비 하한을 고정한다 — 토큰 값을 바꾸면 나머지 세 파일을 함께 고쳐야 이 테스트가 통과한다. (같은 "별도 번들이 복제" 함정 계열: log-viewer i18n dict·recorder pre-arm 청크 — docs/POSTMORTEM.md)
...
- 다이얼로그를 **코드로(프로그램적으로) 열 때**는 `blurActiveElement()`(`src/sidepanel/lib/blurActiveElement.ts`)로 포커스를 먼저 떼야 Radix `aria-hidden` 경고를 피한다 — 새 전역 다이얼로그 추가 시 참고. 이벤트로 여는 다이얼로그는 구독 콜백에서, **스토어 플래그로 여는 다이얼로그는 그 플래그를 보는 effect에서** 부른다(후자 선례: `App.tsx`의 루프 경고, `DeviceViewportBar`의 진입 경고).
...
- **세그먼트 뷰 토글 — 두 번째 사례**(`DeviceViewportBar.tsx`, `grid h-9 w-full grid-cols-4`): 같은 관용구를 쓰되 규칙 셋이 더 붙는다. ① **바깥 `Tabs` 안에 중첩될 땐 자기 `<Tabs>` 래퍼를 스스로 들고 온다** — `TabsList`만 렌더하면 세그먼트 클릭이 바깥 컨텍스트를 잡아 존재하지 않는 값으로 서브탭이 전환되고 화면이 조용히 빈다. ② 세그먼트별 잠금은 `disabled`가 아니라 `aria-disabled`인데 **Radix Tabs는 이걸 동작 가드로 안 보므로** `onValueChange`에서 별도 거부가 필요하다. ③ **사유 툴팁 래퍼는 조건부로 감싸지 않는다** — 사유가 생기고 사라질 때마다 트리 모양이 바뀌어 트리거가 리마운트되며 포커스가 날아간다.
- `collapsing-tabs.tsx` (`CollapsingTabsList`): ResizeObserver/MutationObserver로 폭을 감시해, 트리거가 넘치면 **모든 탭 라벨을 동시에 숨기고 아이콘+배지만** 남긴다. 측정 중엔 `group-data-[measuring]/tabs:inline`로 라벨을 강제 노출해 자연 폭 계산. 사용처: 메인 탭 바·서브탭·다이얼로그 탭 바(제출 필드·트림·로그 삽입/프리뷰), 그리고 **탭이 아닌 세그먼티드 컨트롤**(`DeviceViewportBar` — 라벨이 폭 숫자라 접히면 아이콘만 남고 폭은 `aria-label`이 계속 알린다).
...
| `TooltipIconButton` | 툴바 아이콘 버튼 — `size="icon"` + `h-8 w-8` + `variant="outline"` + Radix 툴팁(Provider 내장 — 툴바가 오버레이·footer로 흩어져 상위 Provider 보장이 없다) + 토글(`data-active`/`aria-pressed`/`bg-muted`) + `ariaDisabled` 잠금 + `disabledReason`(잠금 사유 — **툴팁 본문만** 대체하고 `aria-label`은 `label` 유지. 사유를 label에 섞으면 접근명이 오염된다). 캡처 방식 툴바·어노테이션 툴바(이미지·녹화) 공용 |
...
| `SingleLazyCombobox.tsx` / `CcMultiCombobox.tsx` | 플랫폼 필드 콤보박스 — Popover+Command 기반 단일 선택(lazy fetch) / 다중 선택(CC 멘션). `CcMultiCombobox`는 **8개 플랫폼 필드 폼 전부**(jira/github/gitlab/linear/notion/asana/clickup/slack `*Fields/`)의 표준이지만, `SingleLazyCombobox`는 현재 **clickup·slack 2곳만** 쓴다 — 나머지 6개는 Popover+Command를 손으로 조립한 잔재다. 새 플랫폼 필드는 직접 만들지 말고 이 둘을 조합한다. 선택 항목 상단 고정은 `ccOptions.ts`(`pinSelectedFirst`/`orderSelectedFirst`) 공유 |
...
- **진행 중 잠금**: `disabled` 대신 **`aria-disabled` + 핸들러 early-return 가드**를 쓴다 — shadcn Button base의 `disabled:pointer-events-none` 때문에 `disabled`면 툴팁·hover가 죽고 스피너까지 흐려진다. 스타일은 `aria-disabled:cursor-not-allowed aria-disabled:opacity-50`(스피너를 든 버튼은 `opacity-50`을 뺀다). 선례는 이제 광범위하다 — statusBadges 8개, connect 폼 8개(`JiraConnectForm` 주석이 "8개 폼 공통 패턴"으로 자칭), `TooltipIconButton`의 `ariaDisabled`, `ReplayButton`, 캡처 방식 툴바(`IssueTab`), `StyleChangesDialog`·`StyleEditorPanel`·`DraftingPanel`·`LlmConnectForm`·`ReplayTrimDialog` 등 23파일 ~30곳. **하이브리드 분기**도 관용구다 — 진짜 불가 상태만 순수 `disabled`, 진행 중은 `aria-disabled`(`ReplayTrimDialog`의 확정 버튼: `disabled={duration <= 0 && !busy}` + `aria-disabled={busy}`). 잠금 클래스는 `src/sidepanel/lib/locked-class.ts`의 **`LOCKED_CLASS` 단일 출처**다 — `IssueTab`·`DraftingPanel`·`annotation/ZoomControl`(앞에 `bg-transparent`를 덧댐)·`DeviceViewportBar`가 이걸 쓴다. ⚠️ 다른 클래스와 합쳐 쓰는 인라인 리터럴은 아직 `ReplayTrimDialog`·`StyleChangesDialog`·`StyleEditorPanel`·`IssueTab`(일부)·`DraftingPanel`(섹션 헤더 1곳)·connect 폼 8종·`LlmConnectForm`(뒤 둘은 `opacity-50` 없는 스피너 변형)에 남아 있고, statusBadges 8개는 `aria-disabled`만 걸고 잠금 클래스가 없어 시각 어포던스가 빠져 있다. **진행 중이 아닌 영속 상태**(설정 off, 미지원 페이지 등)에도 쓸 수 있으나, 그때는 **이유를 툴팁으로 함께 준다** — `aria-disabled`는 `disabled`와 달리 hover가 살아 있어서, 이유가 없으면 "밝아지는데 안 눌린다"가 된다. 선례: `ReplayButton`(설정 off → 툴팁 + 설정 탭으로 이동), 미지원 페이지의 `[이슈 작성]`(툴팁으로 사유 표기), 디바이스 뷰포트 세그먼트(창 폭 부족·캡처 중), 모드 ON의 페이지 전체 캡처(`disabledReason` 툴팁).
