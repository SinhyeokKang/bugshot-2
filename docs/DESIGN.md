...
- **진행 중 잠금**: `disabled` 대신 **`aria-disabled` + 핸들러 early-return 가드**를 쓴다 — shadcn Button base의 `disabled:pointer-events-none` 때문에 `disabled`면 툴팁·hover가 죽고 스피너까지 흐려진다. 스타일은 `aria-disabled:cursor-not-allowed aria-disabled:opacity-50`(스피너를 든 버튼은 `opacity-50`을 뺀다). 선례는 이제 광범위하다 — statusBadges 7개, connect 폼 8개(`JiraConnectForm` 주석이 "8개 폼 공통 패턴"으로 자칭), `TooltipIconButton`의 `ariaDisabled`, `ReplayButton`, 캡처 방식 툴바(`IssueTab`), `StyleChangesDialog`·`StyleEditorPanel`·`DraftingPanel`·`LlmConnectForm`·`ReplayTrimDialog` 등 24파일 29곳. **하이브리드 분기**도 관용구다 — 진짜 불가 상태만 순수 `disabled`, 진행 중은 `aria-disabled`(`ReplayTrimDialog`의 확정 버튼: `disabled={duration <= 0 && !busy}` + `aria-disabled={busy}`). ⚠️ 잠금 클래스에 단일 출처가 없다 — **명명 상수 3곳**(`IssueTab`·`DraftingPanel`·`annotation/ZoomControl`. 셋의 **값이 같지도 않다** — `ZoomControl`의 `LOCK_CLASS`만 `bg-transparent` 접두가 붙는다) + **인라인 리터럴 8곳** + **`opacity-50` 없는 축약형 14곳**. 그중 `StyleEditorPanel.tsx`의 `!nextBusy && "aria-disabled:opacity-50"` 하나는 위 "스피너 든 버튼은 `opacity-50`을 뺀다"의 **정규 구현**이므로, 정확히는 **무조건 누락 13곳 + 조건부 1곳**이다. statusBadges 7개(Slack은 상태 개념이 없어 정적 제출 배지뿐이라 대상 아님)는 `aria-disabled`만 걸고 잠금 클래스가 없어 시각 어포던스가 빠져 있다. **진행 중이 아닌 영속 상태**(설정 off, 미지원 페이지 등)에도 쓸 수 있으나, 그때는 **이유를 툴팁으로 함께 준다** — `aria-disabled`는 `disabled`와 달리 hover가 살아 있어서, 이유가 없으면 "밝아지는데 안 눌린다"가 된다. 선례: `ReplayButton`(설정 off → 툴팁 + 설정 탭으로 이동), 미지원 페이지의 `[이슈 작성]`(툴팁으로 사유 표기).
