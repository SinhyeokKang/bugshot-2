...
- 커스텀 raw 색(`text-blue-600` 등)은 semantic 토큰으로 표현 못 하는 **상태/기능 색**에만 쓰고, 가능하면 `dark:` 짝을 함께 둔다. 현재 사용처:
  ...
  - 삽입 로그 코드블럭의 JSON syntax 색: `src/sidepanel/lib/highlightJson.ts`의 `JSON_TOKEN_CLASS`(key `purple-700/400` · string `red-700/400` · number·boolean `blue-700/400` · null `text-muted-foreground italic`). `log-colors.ts`(로그 semantic 색)와 **별개 스코프**지만 같은 원리 — 라이트만 600이 아니라 700. tiptap decoration(`TiptapEditor`)과 markdown-it highlight(`renderMarkdown`)가 공유해 에디터·프리뷰가 안 갈린다. CSS 뷰 하이라이트(`cssHighlightLight/Dark`)는 CodeMirror 전용이라 여전히 로컬.
