...
| `FieldRow.tsx` | 라벨+필드 쌍 — `grid gap-1.5`, 라벨 `text-xs text-muted-foreground`, `required` 시 빨간 별. **⚠ `tabs/connect/`는 아직 안 따른다** — 8개 폼이 동일 마크업 `div.flex flex-col gap-1.5`를 **42곳** 반복하고, **그중 `<label>`을 동반한 라벨+필드 쌍이 34곳**이다(나머지 8곳은 라벨 없는 세로 스택이라 `FieldRow` 대상이 아니다 — 두 수치가 세는 대상이 다르다). 신규 필드는 `FieldRow`를 쓴다 |
