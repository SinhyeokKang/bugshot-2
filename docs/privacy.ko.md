...
LLM API 키는 난독화하여 저장됩니다. 프로바이더는 사용자가 직접 고르며, 프리셋으로 OpenAI·Anthropic·Gemini·Groq·OpenRouter·Together·Ollama의 base URL이 제공됩니다 — **AI 기능을 쓰면 이슈 초안 재료(제목·본문·스타일 변경 내역·주석 이미지·로그 요약 등)가 사용자가 고른 그 프로바이더로 전송됩니다.** 이때 액션 로그 요약에는 마스킹되지 않은 입력·선택 값이 그대로 포함될 수 있습니다(마스킹된 값은 `***`로 전송). AI 기능을 켜지 않으면 전송되지 않습니다.
