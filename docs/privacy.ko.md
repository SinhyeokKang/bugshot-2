...
| 정보 | 수집 시점 | 목적 |
|---|---|---|
| 재현 환경 정보 (OS·브라우저 이름과 버전·창 크기·현재 페이지 주소와 제목) | 이슈 작성 화면 진입 시 | 이슈 본문의 "재현 환경"에 자동 기입 — 저장한 이슈와 제출한 플랫폼에 함께 전달됩니다 |
| DOM 요소 스타일 정보 | 요소 선택 시 | 스타일 비교 및 이슈 본문 생성 |
| 스크린샷 (영역·화면·페이지 전체·요소)·탭 영상 | 캡처/탭 녹화 시 | 사용자가 고른 범위를 캡처해 이슈 첨부 — **페이지 전체 캡처는 보이는 화면을 넘어 페이지를 자동으로 스크롤하며 여러 장을 찍어 하나로 이어 붙이므로, 화면 밖에 있던 내용까지 이미지에 포함됩니다.** **요소 스타일 편집의 편집 전·후 이미지는 고른 요소가 표의 행·목록 항목·대화상자 같은 컨테이너 안에 있으면 그 컨테이너 전체까지 넓혀 찍습니다** |
...
요소 스타일을 편집하면 편집 전·후 이미지가 자동으로 캡처되는데, 이때 선택한 요소만이 아니라 그 요소를 감싼 **가장 가까운 컨테이너**(대화상자·표의 행·목록 항목·`article` 등)까지 넓혀 찍을 수 있습니다. 어떤 항목을 고쳤는지 이미지만 보고 알아볼 수 있게 하기 위함이며, 그 컨테이너가 **현재 화면에 온전히 들어오고 화면 면적의 40% 이하일 때만** 적용됩니다. 따라서 넓혀 찍은 캡처도 항상 보이는 화면 안이고, 페이지 전체 캡처와 달리 화면 밖 내용은 포함되지 않습니다. 개인정보가 모이기 쉬운 입력 폼(`form`·`fieldset`)은 컨테이너 후보에서 제외하며, iframe 내부 요소에는 확장을 적용하지 않습니다. 넓혀 찍은 이미지에는 같은 컨테이너 안의 다른 내용(예: 같은 행의 다른 열 값)이 함께 담길 수 있으므로, 제출 전 작성 화면에서 편집 전·후 이미지를 확인하시기 바랍니다.
...
- **chrome.storage.session**: 편집 세션(작업 중인 스크린샷·요소 before/after 이미지·영상 썸네일과 작성 중인 이슈 제목·본문·연동 입력값 포함), 사이드 패널을 연 탭의 주소(패널 재열기 시 같은 탭을 잇기 위함), 사이드 패널이 활성화된 탭 목록, 임시 로그 정리 시각 (탭별, 브라우저 종료 시 자동 삭제)
...
| 현재 페이지가 참조하는 cross-origin 스타일시트를 호스팅하는 HTTP(S) origin | 스타일시트 URL에 대한 GET 요청(URL의 경로·쿼리와 표준 요청 메타데이터 포함, 쿠키 등 자격 증명은 제외) | 브라우저 CSSOM이 cross-origin 스타일시트를 노출하지 않을 때 CSS 원문을 기기 안에서 읽어 스타일 값·디자인 토큰 이름을 보강. 페이지가 제공한 URL은 정적 SSRF 가드를 거쳐 loopback·사설·link-local 등 공개되지 않은 리터럴 주소 대역을 거부하며 redirect는 따르지 않음 |
...
| 빌드에 설정된 PostHog 호스트 (기본 `us.i.posthog.com`) | 익명 집계 이벤트 (설치·패널 열기·플랫폼 연결/해제·이슈 제출) | 익명 사용량 집계 |
