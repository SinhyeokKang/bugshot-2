...
작성 화면에서 수집된 네트워크·콘솔 로그 중 **한 건을 직접 골라 이슈 본문에 코드블럭으로 삽입**할 수도 있습니다(첨부와 별개 기능이며, 사용자가 버튼을 눌러 고른 로그만 삽입됩니다). 삽입된 로그에는 요청 주소·상태 코드와 함께 **요청·응답 본문 원문**이 담기며, 위에 설명한 수집 시점의 마스킹(민감 헤더·파라미터, `token`·`password`·`secret` 등 민감 키)만 적용되고 그 밖의 본문은 원문 그대로입니다. 첨부 파일(`logs.html`)과 달리 **이슈 본문 평문으로 노출되어 그 이슈를 볼 수 있는 사람 전원이 열람**할 수 있고, AI 초안 작성을 쓰면 본문의 일부로 AI 프로바이더에 전송될 수 있습니다. 삽입 전 상세 화면에서 내용을 확인하시고, 삽입된 코드블럭은 일반 텍스트이므로 제출 전 자유롭게 수정·삭제할 수 있습니다.
