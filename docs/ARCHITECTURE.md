...
**상태 공유**: `replay-context.ts`의 `ReplayProvider`가 `replayEnabled`/`isReady`/`isEncoding`/`bufferedSeconds`/`capture`/`trimming` 6개를 공급 — 훅 인스턴스가 App에만 있어서 하위가 직접 못 읽는 값들이 전부 여기를 탄다. `replayEnabled`는 IssueTab이 캡처 버튼 상태·툴팁에 쓰고(설정 store 직독이 아니라 context 경유 — App이 훅에 넘긴 값과 같은 값을 보장), `trimming`(=`replayTrim != null`)은 IssueTab이 DraftingPanel 마운트를 보류하는 가드 신호다. 트림 대기 상태는 **게이트와 페이로드가 store `replayTrim` 하나**로 합쳐져 있다(비영속 — `frames`가 Blob 배열이라 `chrome.storage` 직렬화 불가, `videoBlob`과 같은 이유로 `EditorSnapshot`에서 제외). 그래서 오버레이 페이로드는 App이 store에서 직접 구독하고, `reset()`의 `...initial`이 페이로드까지 자동 청소한다(그래서 작성 취소 경로는 `reset()`만 부르면 된다 — `resolveTrim` 불요). `resolveTrim`(=`resolveReplayTrim()` 패스스루)은 App이 훅에서 직접 받아 오버레이 확정 시 호출한다(context 미경유).
...
**AI Draft 로그 코드블럭 삽입(logRefs)**: AI가 관련 로그를 **id로만 지목**하면 앱이 store 원본을 `logToCodeBlock` 직렬화로 description 끝에 코드블럭으로 삽입한다(수동 [로그 추가]와 같은 직렬화 — 로그 본문이 모델을 거치지 않아 내용 환각이 구조적으로 불가능). 불변식 4개: ① **후보 단일 출처** — `prompts/logCandidates.ts:selectLogCandidates(fitted.ctx)`가 프롬프트 인쇄(`[n1]`/`[c1]` 태그)·스키마 enum·few-shot 선택·응답 역참조의 공통 파생 원점(support 게이트→network `method+path+status` dedup→`PROMPT_CAPS[style]` 캡→ref 부여 순 — 캡 통과 후 부여라 인쇄 안 된 ref가 실재할 수 없다). `canRequestLogRefs`가 description 활성 게이트도 단일화(스키마·지시·few-shot·삽입부 공유 — 비활성이면 후보 줄만 컨텍스트로 남고 logRefs는 안 요청). ② **id-only 환각 차단** — 원본 로그 id는 프롬프트에 절대 미인쇄, nano는 스키마 `enum`이 구조 강제, BYOK는 스키마가 무력하므로(OpenAI-compat은 스키마 폐기·Anthropic은 텍스트 주입) `renderLogRefs.ts:findCandidate` **역참조가 프로바이더 무관 실질 방어선**(페이지가 `[n2]` 텍스트를 위조해도 후보 집합을 못 만든다 — oneLine이 아니라 역참조가 방어다). ③ **상한 초과 전량 폐기** — 유효 지목 dedup 후 `MAX_LOG_REFS(3)` 초과면 자르지 않고 전부 버린다(나열 신호) + 로컬 `console.warn`뿐(무토스트 — 의도된 침묵). ④ **삽입·보존** — `appendLogBlocks`가 fence 문자열이 아니라 **내부 텍스트**로 기존 블럭과 비교해 스킵(기존 블럭은 Tiptap 왕복을 거쳐 fence 생성 주체가 다르다 — 재생성 누적 중복 방어), 삽입 게이트는 `parsed.sections.description?.trim()` truthy(키 누락·빈 문자열 = AI 실패 신호 차단. 예산 절삭으로 prev가 보존된 경우의 삽입은 의도된 동작), 로그 스냅샷은 await 이전 지역 변수(네비게이션 logClear 방어). `logRefs`는 `AiDraftResponse`의 transient 값 — store에 저장되지 않고 삽입 블럭은 일반 마크다운 텍스트로만 영속.
...
현재 사용처: MAIN world는 `github-upload.ts:pageBatchUploadFn` 하나. 다만 **직렬화 경계는 world와 무관하게 `func:` 전부에 걸린다** — `picker-control.ts:getTopViewport`는 `world` 미지정(ISOLATED)이지만 같은 제약을 받는다(인라인 화살표라 클로저가 없어 현재는 안전). TypeScript·단위 테스트 모두 이 경계를 못 잡으므로 **inject 경로 리팩터 시 실제 탭 수동 회귀 필수**.
...
**자동 메타 위치**: `POST_MEDIA_SECTION_IDS = {"expectedResult","notes"}` — 첫 해당 섹션 직전에 media/styleChanges emit. 둘 다 disabled면 모든 섹션 끝에 emit. **8개 플랫폼 빌더**(GitHub·GitLab은 `buildMarkdownIssueBody` 공용이라 모듈은 7개) + 클립보드 `buildIssueMarkdown` + DraftingPanel + DraftDetailDialog에서 동일 룰. PreviewPanel·log-viewer Report 탭 프리뷰는 순수 헬퍼 `composePreviewLayout`로 이 순서를 단일화(`IssuePreviewView` 공용 컴포넌트).
