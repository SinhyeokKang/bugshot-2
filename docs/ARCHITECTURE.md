...
- **접힘은 순수 표시 상태**: 영속 0(store·chrome.storage·draft 무접촉). 상태는 셸 클로저(`readonly expanded` getter가 단일 출처 — 호출자가 미러링하지 않는다)와 NodeView 인스턴스 수명에만 산다. 커서가 블럭에 진입하면 `expanded`를 **단방향 승격**만 한다(이탈해도 안 되돌림 — 40줄이 15줄로 도로 접히면 본문이 점프).
