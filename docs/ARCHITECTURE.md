...
**스타일 편집 폼/코드 뷰(`styleEditorView`)**: `SelectedPanel`은 sticky 헤더의 shadcn Tabs 토글로 **폼/코드 두 뷰**를 스왑한다(`useSettingsUiStore.styleEditorView`, persist v7, 기본 `"form"` — 영속이라 다음 요소 선택 시 유지). 폼 뷰는 섹션별 컨트롤, 코드 뷰(`styleEditor/StyleCodeEditor.tsx`)는 raw CSS textarea 하나로 `styleEdits.inlineStyle`을 직접 편집한다. 두 뷰는 **같은 `inlineStyle`(`Record<string,string>`) 단일 출처**를 공유 — `styleEditor/inlineCssText.ts`의 `serializeInlineStyle`/`parseInlineStyle`(tolerant: top-level `;`·개행만 분리[괄호·따옴표 내부 `;` 보존], 중복 prop last-wins, prop trim+lowercase[`--*` 커스텀 프로퍼티는 케이스 보존], `!important`·임의 속성을 opaque 문자열로 왕복)이 맵 ↔ CSS 텍스트를 왕복시킨다. 코드 textarea는 `ClassEditor`식 `lastCommittedRef` 패턴으로 **외부 변경**(폼 편집·revert·버퍼 복원)만 재직렬화해 커서 점프를 막고, onChange마다 폼과 동일 경로(`setStyleEdits`+`applyStyles`)로 라이브 반영한다. 뷰 전환은 스타일 편집 영역만 조건부 `hidden`(display:none)으로 스왑하고 비활성 뷰를 **언마운트하지 않아**(collapsible 접힘·미파싱 코드 입력 보존) class·Text·변경사항·AI 배너·푸터를 두 뷰가 공유한다(hidden wrapper가 `:last-child` 스코프를 나눠 Section `last:border-b-0`이 어긋나는 건 `[&>section:last-child]:border-b`로 복원). 코드 뷰가 허용하는 값 끝 `!important`는 content 공통 헬퍼 `applyInlineStyle`(`picker.ts`, `handleApplyStyles`·`handleApplyEditsBySelector` **공유**)이 `/\s*!\s*important\s*$/i`로 분리해 `setProperty(prop, base, "important")` 3-arg로 적용한다(2-arg는 `"red !important"`를 무효값으로 조용히 드롭 — 두 핸들러를 함께 커버해야 패널 재오픈·버퍼 복원 경로에서 유실 안 됨, base가 빈 문자열이면 skip). baseline(`specifiedStyles`/`computedStyles`)은 수집 단계에서 `!important`를 strip하므로 `!important` 값은 diff에서 항상 changed(phantom diff — 허용).
