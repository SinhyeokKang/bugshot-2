...
- **셸 1개, 부착 2경로**(`lib/codeCollapseShell.ts`): wrapper·fade·pill DOM을 만드는 vanilla 팩토리(React·PM 비의존)를 두 부착기가 공유한다 — 에디터는 **ProseMirror NodeView**(`TiptapEditor.tsx`의 `CodeBlockCollapse`가 플러그인 `props.nodeViews.codeBlock`으로 제공 — **코드베이스 최초 NodeView**. StarterKit 재구성·새 의존성 없음), 프리뷰는 `hooks/useCodeCollapse.ts`가 `dangerouslySetInnerHTML` 컨테이너를 렌더 후 `querySelectorAll("pre")`로 순회. **표면 5개**: DraftingPanel·DraftEditDialog(NodeView) / PreviewPanel·DraftDetailDialog `DocSectionBody`·log-viewer Report 탭(훅).
