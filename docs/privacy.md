...
CC(참조) 기능을 사용하면 입력한 검색어로 연동된 플랫폼(Jira·GitHub·Linear·Notion·GitLab·Asana·ClickUp)의 사용자 디렉터리를 조회해 멘션 후보(이름·핸들·아바타)를 표시합니다. 사용자가 선택한 멘션 대상의 식별자는 생성되는 이슈 본문에 포함되어 해당 플랫폼으로 전송됩니다. 검색어와 조회된 후보 목록은 기기에 저장되지 않습니다.
...
| 이슈 제출(issue_submitted: 플랫폼·캡처 모드·제출 결과) | 이슈 제출 시 | 플랫폼별 사용량·캡처 방식 우선순위·제출 성공·실패 비율 파악 |
...
- **chrome.storage.local**: Jira·GitHub·Linear·Notion·GitLab·Asana·ClickUp 연동 설정, 이슈 기록, 앱 설정, LLM 프로바이더 설정 (API 키는 난독화 저장)
...
| OAuth 프록시 서버 | OAuth 인가 코드 | 토큰 교환 (Jira, GitHub, Notion, Asana, ClickUp) |
...
사용자가 "파일 첨부" 기능으로 직접 선택한 로컬 파일은 이슈(작업) 제출 시 위 각 플랫폼(Jira·GitHub·Linear·Notion·GitLab·Asana·ClickUp)에 본문 첨부로 함께 업로드됩니다. 이 기능은 기본적으로 꺼져 있으며, 설정에서 활성화한 경우에만 동작합니다.
...
- **플랫폼 연동 해제**: 각 플랫폼(Jira, GitHub, Linear, Notion, GitLab, Asana, ClickUp) 연동 탭에서 연결을 해제하면 저장된 자격 증명이 삭제됩니다.
...
| identity | OAuth 로그인 (Jira, GitHub, Linear, Notion, GitLab, Asana, ClickUp) |
| tabCapture | 탭 영상 녹화 |
...
- `api.clickup.com` — ClickUp API (OAuth authorize는 외부 팝업이라 호스트 권한 불필요)
- OAuth 프록시 서버 — 토큰 교환
