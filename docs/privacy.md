...
네트워크 로그 수집 시 `authorization`, `cookie` 등 민감 헤더와 `token`, `access_token` 등 민감 쿼리 파라미터는 자동으로 마스킹 처리됩니다. 요청·응답 본문(JSON·폼 데이터)에 포함된 `token`, `password`, `secret` 등 민감 키의 값도 자동으로 마스킹됩니다. 콘솔 로그는 페이지가 출력한 메시지를 원문 그대로 수집하므로(별도 마스킹 없음), 민감 정보를 콘솔에 출력하는 페이지에서는 디버그 캡처 시 유의하시기 바랍니다.
