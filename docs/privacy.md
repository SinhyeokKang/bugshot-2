...
- **chrome.storage.local**: Jira·GitHub·Linear·Notion·GitLab·Asana 연동 설정, 이슈 기록, 앱 설정, LLM 프로바이더 설정 (API 키는 난독화 저장)
...
OAuth 프록시 서버는 토큰 교환만 중계하며, 사용자 데이터를 저장하거나 로깅하지 않습니다. Linear·GitLab·Asana는 PKCE 방식으로 프록시 없이 직접 토큰을 교환합니다.

GitLab self-managed 인스턴스에 PAT로 연결할 경우, 사용자가 입력한 인스턴스 주소(임의 origin)로 직접 통신합니다. 이 접근은 선택적 호스트 권한(`https://*/*`, `http://*/*`)을 통해 연결 시 브라우저의 권한 요청 대화상자로 1회 허가받으며, gitlab.com 연결에는 별도 권한 요청이 없습니다.
...
- **플랫폼 연동 해제**: 각 플랫폼(Jira, GitHub, Linear, Notion, GitLab, Asana) 연동 탭에서 연결을 해제하면 저장된 자격 증명이 삭제됩니다.
...
| identity | OAuth 로그인 (Jira, GitHub, Linear, Notion, GitLab, Asana) |
| tabCapture | 탭 영상 녹화 |
...
- `gitlab.com` — GitLab API 및 OAuth (gitlab.com 연결 시). self-managed 인스턴스는 선택적 호스트 권한으로 런타임 허가
- `app.asana.com` — Asana API 및 OAuth
- OAuth 프록시 서버 — 토큰 교환
...
- **GitLab self-managed**: 사용자가 입력한 GitLab self-managed 인스턴스(임의 origin)에 PAT로 연결할 때 (연결 시 1회 동의, gitlab.com 연결에는 불필요)
