...
이 이벤트들은 위 분류 문자열만 담으며, 이슈 제목·본문·URL·개인 식별 정보는 일절 포함하지 않습니다. 같은 설치를 구분하기 위해 설치 시 무작위 식별자(distinct_id)를 한 번 생성해 기기에 저장하고 이후 이벤트에 함께 보냅니다. 이 식별자는 무작위 값일 뿐 이메일·계정·IP 등 어떤 개인정보와도 연결되지 않습니다. 실제 IP 주소가 저장되지 않도록 이벤트에 IP 값을 `0.0.0.0`으로 실어 보내고, 위치 추정(GeoIP)도 비활성화합니다(`$geoip_disable`). 이 집계에는 별도의 옵트아웃(off) 설정이 없습니다.
