...
### Task 5: buildIssueMarkdown — 클립보드 복사 분기

- **변경 대상**: `src/sidepanel/lib/buildIssueMarkdown.ts`
- **작업 내용**: element 모드 분기(lines 99-112)에서 `ctx.diffs.length === 0`이면 "Media" 섹션 + `t("md.imageAttached")` 출력 (screenshot 모드와 동일). `ctx.diffs.length > 0`이면 기존 Style Changes table 유지.
- **검증**:
...
Task 1 (StyleEditorPanel) → Task 2 (DraftingPanel) → Task 3 (DraftDetailDialog 미디어)
                                                    ...
                                                    Task 4 (buildCtxForSubmit)
                                                       ↓
                              Task 5~9 (빌더 5개) — 병렬 가능
```

Task 1~3은 순차 (UI 흐름을 따라 검증). Task 4는 Task 3 이후. Task 5~9는 서로 독립적이므로 병렬 구현 가능.
