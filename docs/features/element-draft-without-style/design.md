...
- `handleNext`의 `captureElementSnapshot` + `setAfterImage` + `confirmStyles` 로직은 변경 없음. 스타일 변경 없이 호출하면 afterImage는 beforeImage와 시각적으로 동일한 스냅샷이 됨. 이 afterImage는 스타일 변경 없는 경우 사용하지 않지만, 저장 로직의 일관성을 위해 그대로 캡처한다.
...
- `hasStyleBlock`은 `!!issue.snapshot.before || !!issue.snapshot.after || diffs.length > 0`으로 계산됨 (line 216-218). 스타일 변경 없이 저장해도 `snapshot.before === true`이므로 `hasStyleBlock === true`.
...
- **변경**: `diffs.length === 0`일 때 `screenshotImage` 파라미터로 `beforeDataUrl`을 넘기고 `beforeImage`/`afterImage`는 null 처리 — screenshot 모드와 동일한 `screenshot.webp` 파일명으로 생성
...
`ctx.captureMode`는 `"element"`이지만 `ctx.diffs`가 빈 배열이고 images에 `screenshot.webp`가 오는 경우를 처리해야 한다.

#### 6-1. `src/sidepanel/lib/buildIssueMarkdown.ts` (클립보드 복사용)
...
- **현재**: element 모드 else 절에서 항상 Style Changes heading + diff table (lines 101-108)
- **변경**: `ctx.diffs.length === 0`이면 Media heading + IMAGE_PLACEHOLDER (screenshot 모드와 동일)
