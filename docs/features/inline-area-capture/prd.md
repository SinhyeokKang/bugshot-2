...
기존에 screenshot 캡처 모드(IssueTab → area capture → 메인 스크린샷)는 있지만, 이는 이슈의 대표 스크린샷 한 장을 위한 것이고, drafting 중 편집기 안에 인라인 이미지를 추가하는 용도가 아니다. 기존 방식(OS 스크린샷 → 파일 저장 → 첨부 버튼 → 파일 선택)은 최소 4단계 전환이 필요하지만, 인라인 캡처는 클릭 + 드래그 1단계로 줄인다.
...
- 캡처 이미지를 에디터 커서 위치에 삽입. 항상 섹션의 마지막 노드에 추가.
...
2. paragraph 타입 섹션(설명, 재현 단계 등)의 헤더에서 [캡처] 아이콘 버튼(Camera)을 클릭한다. 이 버튼은 기존 [첨부] 아이콘 좌측에 위치한다.
...
- element 모드에서 인라인 캡처 시 picker가 area-select 모드로 전환. 캡처/취소 완료 후 picker가 기존 선택 요소의 오버레이(아웃라인·배지)를 자동 복원하므로 "스타일링으로 돌아가기"가 정상 동작한다.
- video 모드 drafting 중 인라인 캡처: `videoBlob`/`videoThumbnail` 등 녹화 데이터에 영향 없음. `startInlineAreaCapture`는 editor state를 리셋하지 않으므로 안전.

## 성공 기준

1. DraftingPanel의 모든 paragraph 섹션에 캡처 버튼이 노출된다.
2. 캡처 버튼 클릭 → 영역 드래그 → 이미지가 해당 섹션 에디터 끝에 삽입된다.
3. 캡처 이미지가 에디터에 즉시 렌더링되고, 기존 인라인 이미지와 동일하게 이슈 제출 시 플랫폼에 첨부된다.
