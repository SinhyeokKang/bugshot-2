...
- **작업 내용**: 테스트 먼저 작성(red) 후 구현(green). design.md "specificity 계산
  ...
  - 기본: `div`(1), `.a`(1000), `#a`(1_000_000), `div.a#b`, `*`(0), 결합자 무기여
    (`.a > .b` = 2000)
...
- **작업 내용**: `matchedSpecificity`는 `splitSelectorList` import 후 el 스텁
  ...
  → null / 매치 0개 → null. `isRuleLayered`는 `globalThis.CSSLayerBlockRule` 스텁
  클래스로 parentRule 체인·`CSSImportRule.layerName`(`""` 포함) 케이스 고정.
- **검증**:
  - [ ] 신규 describe green
  - [ ] `typeof CSSLayerBlockRule` 가드 존재(jsdom·node 환경에서 ReferenceError 없음)

### Task 3: `noteClaim` specificity 편입 + verdict 반환 (TDD)

- **변경 대상**: 동일 파일들
- **작업 내용**: `ClaimState.candidates`에 `specificity` 추가, `noteClaim`에
  **마지막 optional** 파라미터 `specificity: number | null = null` + boolean 반환.
  design.md "판정 순서" 1–7 구현. 테스트 시나리오(스펙 요구 5종):
  - specificity 역전: `.a.b`(먼저, 2000) vs `.c`(나중, 1000) 다른 값 → 이전 승자
...
- **작업 내용**:
  1. `collectRulesForElement`: 규칙당 `spec = isRuleLayered(rule) ? null :
     matchedSpecificity(el, rule.selectorText)` 1회 계산, 세 호출에 전달. inline
     패스는 `null`.
  ...
  4. **수용 테스트**(스펙의 red→green 기준): decl 스텁(`length`/`item`/
     `getPropertyValue`/`getPropertyPriority` — 기존 테스트 파일의 claims 테스트
     패턴 참조)으로 `applyDeclarations`를 높은 spec 규칙 → 낮은 spec 규칙 순 호출:
...
- **e2e 시나리오** (`/e2e-write` 입력 후보 — 선택): "픽스처 페이지에 같은 속성을 다른
  ...
  "e2e 영향" 플래그로 판단.
- **수동 테스트**:
  - [ ] google.com 검색 버튼 선택 → `background-color`가 `#f8f9fa`(원문)로 표시
