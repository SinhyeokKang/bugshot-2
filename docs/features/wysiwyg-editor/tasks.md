...
- **작업 내용**: Tiptap 관련 패키지 설치
  ```
  pnpm add @tiptap/react @tiptap/starter-kit @tiptap/extension-link @tiptap/extension-image @tiptap/extension-placeholder tiptap-markdown
  ```
- **검증**:
  - [ ] `pnpm install` 정상 완료
  - [ ] `pnpm typecheck` 통과
...
- **작업 내용**:
  ...
  - CSS: ProseMirror 콘텐츠 영역 스타일 (outline 제거, 리스트 패딩, 이미지 max-width, placeholder)
  - 외관: shadcn/ui Textarea와 동일한 border/focus ring/text-sm 적용
- **검증**:
  - [ ] `pnpm typecheck` 통과
...
- **작업 내용**:
  ...
  - CRUD 함수: `saveInlineImage(refId, blob)`, `getInlineImage(refId)`, `deleteInlineImages(refIds)`, `getInlineImageKeys()`, `clearInlineImages()`
...
- **작업 내용**:
  ...
  - `markdown-it` 인스턴스 생성: `{ html: false, breaks: true, linkify: true }`
  - `dangerouslySetInnerHTML={{ __html: md.render(value) }}` 사용 (html:false로 XSS 방지)
...
- **작업 내용**:
  - `buildIssueHtml`: paragraph 분기의 `paragraphize(content)` → `markdownIt.render(content)` 교체
  - GitHub/Linear: paragraph 섹션 emit 시 `content`에 `inline:refId` 참조가 있으면 이미 해소된 URL로 치환된 상태여야 함 → `IssueCreateModal`에서 전처리 (Task 10)
  - `buildIssueMarkdown` (markdown 출력): paragraph 섹션은 마크다운 그대로 emit하므로 변경 불필요. 인라인 이미지 참조만 해소 필요 → Task 10에서 처리
- **검증**:
  - [ ] `buildIssueHtml`로 생성된 HTML에서 `**bold**`가 `<strong>bold</strong>`로 변환 확인
  - [ ] `pnpm typecheck` 통과
  - [ ] 마크다운 복사(클립보드) 시 서식 정상 반영

### Task 10: resolveInlineImages + 제출 흐름 통합

- **변경 대상**: `src/sidepanel/lib/resolveInlineImages.ts` (신규), `src/sidepanel/tabs/IssueCreateModal.tsx`, `src/sidepanel/tabs/DraftDetailDialog.tsx`
- **작업 내용**:
  - `resolveInlineImages(markdown)` 구현: `inline:refId` 패턴 추출 → blob-db 로드 → data URL 변환
  - `IssueCreateModal`: 제출 전 `draft.sections`의 각 paragraph 섹션에 대해 `resolveInlineImages()` 호출
  - 플랫폼별 이미지 처리:
...
- **검증**:
  - [ ] `resolveInlineImages.test.ts` 단위 테스트 작성 및 통과:
    - `inline:refId` 참조 없는 마크다운 → 그대로 반환
    - `inline:refId` 참조 1개 → data URL로 치환
    - 여러 참조 → 전부 치환
  - [ ] `pnpm test` 통과
...
| `resolveInlineImages.test.ts` | `resolveInlineImages` | no references, single ref, multiple refs, missing blob (graceful fallback) |
...
Task 4 (blob-db 확장) → Task 5 (이미지 드래그앤드롭)
  ↓                        ↓
Task 6 (DocSectionBody)   Task 7 (markdownToAdf) ← 병렬 가능
  ↓                        ↓
Task 8 (markdownToNotionBlocks) ← Task 7과 병렬 가능
  ↓
Task 9 (buildIssueHtml + GitHub/Linear)
  ↓
Task 10 (resolveInlineImages + 제출 흐름)
  ↓
Task 11 (번들 최적화)
```
...
- Task 6, 7, 8은 병렬 가능 (독립적인 변환기)
- Task 9는 Task 7, 8 완료 후
- Task 10은 Task 5, 9 완료 후
- Task 11은 마지막
