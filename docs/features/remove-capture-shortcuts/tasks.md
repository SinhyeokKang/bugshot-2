...
- **검증**:
  - [x] `pnpm typecheck` 통과(고아 import 0)
...
- **검증**:
  - [x] 진입 화면 **5버튼 전부** 정상 렌더·클릭(단축키 툴팁 없음, 형제 버튼 보존) — 자체 검증 확인
  - [x] ReplayButton 상태 툴팁 정상(보존 확인) — Tooltip* import·사용 보존
  - [x] `isCaptureEntryScreen` 게이트로 EmptyState 진입 정상(import·사용 보존)
  - [x] `pnpm typecheck` 통과
...
- **검증**:
  - [x] `pnpm typecheck` 통과(삭제 파일 참조 0)
  - [x] `startPicker`/`startAreaCapture`/`startVideoCapture` export 고아 아님(IssueTab에서 사용 grep)
...
- **검증**:
  - [x] `pnpm typecheck` 통과
  - [x] `pnpm test` — `isCaptureEntryScreen` 테스트 green(4 tests), 제거 함수 테스트 사라짐
  - [x] `capture-commands.ts`에 단축키 식별자 0건
...
- **검증**:
  - [ ] `pnpm build` 성공, manifest `__MSG__` dangling 경고 없음 — `/build`에서
  - [ ] `pnpm build:store`(manifest `key` 제거 경로)에서도 commands 정상 — `/build`에서
  - [x] manifest `commands`에 `_execute_action`만(소스 확인)
  - [x] `_locales` ko/en 키 대칭(`CMD_CAPTURE_*` 양쪽 제거)
...
- **검증**:
  - [x] 두 문서에 캡처 단축키 잔여 언급 0
  - [x] 마이그레이션 무손실 설명 존재(PERMISSION §8)
...
- **검증**:
  - [x] grep 0건(보존 식별자 제외)
  - [x] `pnpm typecheck` / `pnpm test` green (1936 tests)
  - [ ] `pnpm build:e2e && pnpm test:e2e` green (`capture-modes-layout.spec.ts` 그룹 구조 회귀 없음) — `/e2e-run`에서
