...
- Chrome `suggested_key` 4개 상한을 캡처 3개가 모두 소진해, 더 의미 있는 단축키를 추가할 여지를 막는다.
- OS·타 확장과 충돌이 잦은 키(Shift+S/F/X)라 best-effort로 미배정되는 경우가 많아 실제 작동 신뢰도가 낮다.
...
### 시나리오 1: 기존 사용자 업데이트 (마이그레이션 핵심)
1. 캡처 단축키가 있던 버전을 쓰던 사용자가 새 버전으로 업데이트
2. Chrome이 manifest에서 사라진 command 3개를 자동 정리 — 사용자가 `chrome://extensions/shortcuts`에서 커스텀 키를 재할당했더라도 **데이터 손실 없이** 해당 항목만 사라진다
3. `_execute_action`(Cmd/Ctrl+Shift+E, 또는 사용자가 재할당한 키)은 그대로 유지된다
4. 캡처는 진입 화면 버튼으로 동일하게 가능 — 기능 손실 없음
...
- `pnpm typecheck` / `pnpm test` / e2e green. (특히 `capture-modes-layout.spec.ts`의 버튼 testid 의존 회귀 없음.)
