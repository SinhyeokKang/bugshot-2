...
| `src/background/messages.ts` | background 메시지 핸들러 | `captureVisibleTab` 핸들러에 JPEG format 옵션 추가 |
...
이슈 설정 탭의 "본문 구성" 섹션 위에 "캡처" 섹션을 추가한다.
...
| `isEncoding=true` | disabled + progress 표시 | 없음 |
| `phase="recording"` (수동 비디오 녹화 중) | disabled | 없음 |

`replayEnabled=false`일 때만 안내 tooltip을 표시하여, "왜 비활성인지" + "어디서 켤 수 있는지"를 알려준다.
