...
현 구조에서 console/network/action 레코더(MAIN world)는 버퍼에 로그를 쌓고, **`sync`/`stop`/`pagehide` 시점에만 batch로 CustomEvent를 dispatch**한다. 브리지(현 `picker.ts` — 향후 분리 가능)가 이를 받아 `chrome.runtime.sendMessage`로 사이드패널에 전송한다.
...
- **꼬리 유실 축소 (binary 합격선)**: 측정 기준선(보강 전 빌드, 동일 페이지·동일 cross-origin 네비 타겟)을 5회 반복해 도착 개수 중앙값을 기록한 뒤, 보강 후 동일 절차에서 **"네비 직전 ~200ms 밖에 발생한 로그는 5회 모두 100% 도착"**을 확인한다. (~200ms 이내 꼬리는 갭1 특성상 유실 허용.)
- 로그 폭주 중에도 사이드패널 누적이 200ms 주기로 진행됨을 확인(디바운스 미채택 검증).
- 기존 `sync`/`stop`/`pagehide` flush·30s replay trim·세션 영속화에 회귀가 없다.
- 자동 flush 빈도 증가에도 IndexedDB write가 **초당 1회 수준 이하**로 제한된다(수신부 write 가드 확인). store set은 메모리라 매번 허용.
- manifest 권한 diff 0.
