...
값 조정 다섯 개지만 본론은 **"mono 표면"이 한 셀렉터로 안 잡힌다**는 구조다. CSS 뷰·DOM 트리·로그는 `.font-mono`로, Tiptap·프리뷰 코드블럭은 Tailwind preflight로 Geist를 받는다. 이 설계는 그 둘을 **`@layer base`의 단일 셀렉터 리스트로 묶어** 리거처를 한 곳에서 끄고, 크기는 표면별로 12px에 수렴시킨다.
...
- **`font-feature-settings`를 쓰지 않는다.** 가산이 아니라 **통째로 덮어쓰는** 속성이라 리거처만 끄려다 다른 feature를 날린다. `font-variant-ligatures`는 별도 속성이라 안전하다 — 정확한 도구가 이쪽이다.
  - **다만 "지금 실제로 잃는 게 있다"는 건 거짓이다.** `dist/assets/*.css:54442` 실측: preflight가 이미 `code,kbd,samp,pre { font-feature-settings: normal }`을 **직접** 건다. body(`globals.css:83`)의 `"rlig" 1, "calt" 1`은 *상속*으로만 도달하므로 preflight의 직접 선언에 항상 진다 → **`FrameBody`는 이 변경과 무관하게 이미 rlig/calt가 없다.** 리스트의 비-`pre` 표면(DomTree Card·CM 래퍼·LogSeekChip)은 전부 Geist Mono라 애초에 `rlig`/`calt`가 없다. 즉 이 선택은 **실손실 방어가 아니라 형태 예방**이다. sans 폰트를 바꾸거나 리스트가 넓어지면 그때 실제 손실이 생긴다.
- **`pre`/`code`가 `font-sans`를 명시한 경우**(`NetworkLogContent.tsx:733`의 `FrameBody`)도 이 리스트에 걸려 리거처 off가 적용된다. Pretendard에 `--` 리거처가 없어 무해하다. 이걸 피하려고 셀렉터를 정교하게 만드는 건 오버엔지니어링이다(대안 2 참조).
...
- **현재 역할**: `:1-2` 폰트 `@import`, `:4-6` `@tailwind`, `:8-67`·`:69-106` `@layer base`(토큰 표·body·스크롤바). `font-variant-ligatures` 지정 0건.
...
- **변경 내용**:
  ...
  4. `:106` `padding: 0.625em 0.75em` → **`1em`**(= 12px 사면 균등 — `font-size: 12px`가 같은 규칙에 있어 `em`이 12px로 풀린다). 사용자 판단. 짝인 `doc-section-body.css`도 함께.
...
- **변경 내용**: `:72`·`:86` `font-size: 0.875em` → `12px`, `:85` `padding` → `1em`. 에디터와 **동일 커밋**.
...
- **변경 내용**: **12px 전 표면 + 행간 두 그룹**(코드블럭 3개 18px / 리스트·칩 6개 `text-xs` 16px) + 리거처 off로 재작성. **두 진입 경로(`.font-mono` / preflight)**와 **두 클론 파일(`tiptap-editor.css` / `doc-section-body.css`)**을 짝으로 명시 — 이게 짝을 놓치게 만드는 구조라 남겨야 다음 사람이 안 밟는다. 14px 기각 사유(mono 자폭 1.2배)는 유지하되 13px 기각(너무 큼)을 추가. **`text-[12px]`가 아니라 `text-xs`인 이유**(행간 그룹)도 박는다.
...
1. **`.font-mono`에만 규칙을 건다** — 기각. **Tiptap·프리뷰가 빠진다.** 그게 v1.6.0이 밟은 함정이고 이 작업이 존재하는 이유다. 리거처만 놓고 보면 Tiptap이 자체 방어(`.ProseMirror`에 `font-variant-ligatures: none` + `font-feature-settings: "liga" 0` 주입)해서 우연히 통과하지만, **프리뷰는 ProseMirror가 아니라 그 방어도 못 받고** — 우연에 기대는 설계다.

2. **셀렉터를 정교하게 좁힌다** (`pre:not(.font-sans)` 등) — 기각. `FrameBody`의 `font-sans` `<pre>` 하나 때문에 셀렉터를 복잡하게 만드는 비용이 이득보다 크다. 그 요소에 리거처 off(Pretendard에 `--` 리거처 없음)가 적용돼도 관측 가능한 변화가 없다.
...
5. **`liga`를 `font-feature-settings: "liga" 0`으로 끈다** — 기각. 가산이 아니라 통째로 덮어쓰는 속성이라 리거처만 끄려다 다른 feature를 날린다(v1.6.0 design.md가 기록한 함정). `font-variant-ligatures`가 정확한 도구다.
   - **단, "지금 잃는 게 있다"는 논거는 실측으로 무너졌다** — preflight가 이미 `pre`/`code`에 `font-feature-settings: normal`을 직접 걸어 `FrameBody`는 이미 rlig/calt가 없고, 비-`pre` 표면은 전부 Geist Mono(rlig/calt 부재)다. **잃을 게 없다.** 기각 결론은 유지하되 근거는 "실손실"이 아니라 "잘못된 도구 + 형태 예방"이다. (이 정정 자체가 이 feature의 교훈 — 결론이 맞아도 근거가 미검증이면 다음 리뷰에 뒤집힌다.)
...
- **12px 판정이 다크모드에서 다르게 보인다.** v1.6.0이 뉴트럴 다크 테마를 출시했고, 다크 배경 위 밝은 글자는 halation으로 글리프가 번져 **라이트보다 굵고 조밀하게** 보인다 — Task 6은 **양 테마**에서 본다.
...
- **`globals.css`의 새 base 블록은 log-viewer에 안 나간다 — 그리고 log-viewer엔 mono 표면이 있다.** `globals.css`는 `src/sidepanel/main.tsx:7`만 import하고 `src/log-viewer/main.tsx:7`은 자체 `./styles.css`(별도 `@tailwind base` + 손복사 토큰 표)를 쓴다. **"log-viewer에 mono 표면 0건"은 거짓이다** — `src/log-viewer/App.tsx:4-7`이 `NetworkLogContent`·`ConsoleLogContent`·`IssuePreviewView`를 사이드패널에서 그대로 import한다. `dist-log-viewer/index.html` 실측: `.font-mono`·`.text-xs`·`.doc-section-body pre` 전부 있고 `font-variant-ligatures`만 **0건**. 갈라짐은 두 갈래다:
  - **12px는 함께 나간다** — `doc-section-body.css`는 번들되고 `.text-xs`는 공유 `tailwind.config.js` 산물이라 Task 3·4가 그대로 반영된다. 불변식은 log-viewer에서도 성립한다.
  - **리거처 off는 안 나간다** — 평소엔 무해하다(log-viewer는 `@font-face`가 없어 시스템 mono 폴백 — DESIGN.md:64의 불변식). 하지만 `.font-mono` 스택 1순위가 `Geist Mono Variable`이라 **Geist Mono를 시스템에 설치한 사용자**(개발자·QA — 정확히 우리 사용자층)에겐 로컬 해석돼 liga가 살아난다. 그러면 내보낸 `logs.html`의 네트워크 본문 `<pre>`에서 `------WebKitFormBoundary` 같은 연속 하이픈이 이어붙는다. **사이드패널은 늘 Geist를 받으므로 개발 중엔 안 보이고 export 산출물에만 남는 형태다.** 이 feature가 없애려는 버그가 정확히 그 사각지대에 존치된다.
  - 기존 결함이지 이번 변경의 회귀는 아니었다(전엔 사이드패널도 liga가 켜져 있었다). **이번에 닫았다** — `src/log-viewer/styles.css`에 같은 셀렉터 리스트를 손복사하고(토큰 표가 이미 쓰는 패턴) `tokens.test.ts`에 동등성 가드(`parseRule(LOG_VIEWER, ".font-mono")` === `parseRule(GLOBALS, ".font-mono")`)를 추가했다. 가드는 mutation으로 비공허성을 확인했다(log-viewer 값을 어긋내면 red).
- 곁들여, **`tokens.test.ts:112`의 주석**(*".font-mono 규칙은 사이드패널과 log-viewer 두 빌드에 똑같이 나가는데…"*)은 공유 `tailwind.config.js`가 주는 **유틸리티**엔 참이지만 **새 base 규칙엔 거짓**이 된다. `parseTokens` 동등성 가드(`:83-91`)는 `:root`/`.dark` 토큰 표만 대조해 이 구멍을 못 본다 — Task 1에서 주석을 정정한다.
- **`letter-spacing`을 0이 아닌 값으로 주면 Chrome이 리거처를 덤으로 끈다(CSS Text).** Task 6이 자간을 **0으로 확정**했으므로 지금은 `font-variant-ligatures`가 **단독 방어**다 — 그게 정확한 도구라 안전하고, 덕분에 e2e mutation 대조군도 단순하다(그 한 줄만 지우면 red. 튜닝 중 자간이 있던 동안엔 자간이 가려줘 mutation이 green이었다 — `e2e/GOTCHAS.md`에 기록). **나중에 자간을 도입하면 이 대조군이 모호해진다**는 것만 알아둔다.
- **DOM 트리 트렁케이션이 개선되지만 측정되지 않는다.** 13→12px로 v1.6.0이 만든 −10.3%가 일부 되돌아온다. 정량 확인은 스코프 밖 — 관찰만.
