...
**mono 표면에 `letter-spacing` 지정이 0건**이라 전부 브라우저 기본(0)이고, Geist Mono의 기본 트래킹이 넓게 읽힌다는 게 최초 관찰이었다. **실기기 검증에서 철회했다** — 12px에서 유의미한 차이를 만들려면 `-0.24px`(글자당 0.02em) 이상이 필요한데 그 구간은 고정폭 그리드를 흐트러뜨린다. **자간은 건드리지 않는다**(브라우저 기본 0 유지).

(앱 전체로 보면 `tracking-` 유틸이 5곳 있다 — `ConnectedBadge.tsx:7`의 `tracking-wider`, shadcn `card`/`alert`/`alert-dialog`/`dialog`의 `tracking-tight`. 전부 sans 표면이라 무관하다.)
...
- **자간을 건드리지 않는다.** 문제 4를 실기기 검증에서 철회했다(위). 리거처는 `font-variant-ligatures`가 단독으로 막는다.
...
- DESIGN.md의 mono 불변식이 **12px · 두 행간 그룹 · 리거처 off**를 담고, **두 진입 경로(`.font-mono` / preflight)**와 **두 클론 파일(`tiptap-editor.css` / `doc-section-body.css`)**을 짝으로 명시한다.
