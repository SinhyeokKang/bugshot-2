...
- **pin 형태**: 상단 원형 head(6~8px) + 하단 뾰족한 바늘(~4px). SVG 또는 CSS clip-path로 구현. head 부분에 variant 색상 적용, 바늘은 동일 색상 또는 약간 어둡게.
