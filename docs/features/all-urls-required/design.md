...
- 현재: BYOK/GitLab이 임의 origin을 `chrome.permissions.request`. `<all_urls>` 보유 시 즉시 true(프롬프트 없음).
- 변경: **없음**(외과적). 호출 유지해도 즉시 grant라 UX상 프롬프트만 사라짐. `LlmConnectDialog`·`GitlabConnectForm`의 거부 분기는 도달 불가 경로가 되지만 제거는 별도 정리 사항(이번 스코프 외).
...
- `CLAUDE.md`: 게이트웨이 섹션의 `optional_host_permissions: <all_urls>` 서술을 required로 수정.

### e2e
- `e2e/activetab-broad-permission.spec.ts`(및 광역 권한 전제 spec): 권한이 항상 보유 상태가 되므로, "권한 미보유 → 패널 닫힘/Replay OFF" 검증 시나리오는 더 이상 성립하지 않음 → 해당 단언 갱신/제거. dist-e2e는 이미 `<all_urls>`라 동작 자체는 그대로.
