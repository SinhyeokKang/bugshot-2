...
- **키 이름 유출(신규 노출의 본질) + 데이터성 키 방어**: body 마스킹(`maskJsonBody`)은 allowlist 키의 **값만** `***`로 바꾸고 **키 이름은 안 건드린다**. 그래서 다이제스트는 스키마 키명(`ssn`·`internalAccountId` 등)을 LLM에 싣는다 — 이건 shape의 일부라 **수용**. 그러나 **맵/딕셔너리형 응답**(`{"john@corp.com":{…}, "u_8471":{…}}`)에선 키가 곧 실제 데이터(PII·레코드ID)라 "값은 안 나감"이 깨진다. 그래서 `digestResponseShape`가 **코드 식별자 패턴(`^[A-Za-z_][A-Za-z0-9_]{0,39}$`)에 맞는 키만 인쇄하고, 벗어나는 키(이메일 `@`·UUID `-`·공백·점 포함·40자 초과)는 `<key>`로 redact**한다(`safeKey`). 값 타입은 유지. 한계: `u_8471`처럼 식별자 모양의 레코드ID는 못 거른다(패턴상 스키마 키와 구분 불가) — 이메일·UUID·비정형 키만 방어하는 수용된 경계다.
