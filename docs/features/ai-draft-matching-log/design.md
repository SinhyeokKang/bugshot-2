...
- **매칭 비용(PERF)**: `selectLogCandidates`는 `fitDraftContext`의 예산 측정과 `AiDraftDialog`의 `fitted.ctx` 재호출로 실행되는데, level≥1에서 `requests`가 삭제되므로 **실제 매칭 스캔이 도는 heavy 패스는 generate당 ~1-2회**(level-0 build + fitted 재호출)다. 매 heavy 호출이 `O(terms≤20 × requests × responseBody 길이)` substring 스캔(`requestMatchesQuery`가 원문 훑음). **주의: `requests`는 term 캡(20)만 bounded이고 요청 개수는 recorder cap(`NETWORK_MAX_ENTRIES`, ~5000)까지 슬라이스 없이 전달된다** — worst-case `20 × ~5000 × substring`이 사이드패널 메인 스레드 동기 실행. v1은 사용자 발동 액션(generate)이라 현행(재계산) 수용하되, 실측 jank 관찰 시 (a) 모집단 슬라이스 또는 (b) 매칭 결과를 트리밍 레벨 키로 메모이즈. (예산 측정 자체는 렌더된 프롬프트 문자열 길이 기반이라 full `requests` 배선이 level-0 측정을 부풀리진 않는다 — matched 인쇄분만 계산에 들어감.)
...
- **키 이름 유출(신규 노출의 본질) + 데이터성 키 방어**: body 마스킹(`maskJsonBody`)은 allowlist 키의 **값만** `***`로 바꾸고 **키 이름은 안 건드린다**. 그래서 다이제스트는 스키마 키명(`ssn`·`internalAccountId` 등)을 LLM에 싣는다 — 이건 shape의 일부라 **수용**. 그러나 **맵/딕셔너리형 응답**(`{"john@corp.com":{…}, "u_8471":{…}}`)에선 키가 곧 실제 데이터(PII·레코드ID)라 "값은 안 나감"이 깨진다. 그래서 `digestResponseShape`가 2단 방어한다: (1) **dictionary-shape collapse** — 최상위 객체가 `키 >8 && 값 baseKind 균일`이면 map으로 보고 개별 키를 인쇄하지 않고 `{<N> entries: <kind>}`로 축약(접두-토큰 레코드ID `cus_H3k9xY2z`류 대량 유출의 주경로 차단). (2) 그 외(작은 객체·값 타입 혼합)는 **코드 식별자 패턴(`^[A-Za-z_][A-Za-z0-9_]{0,39}$`)에 맞는 키만 인쇄하고 벗어나는 키(이메일 `@`·UUID `-`·공백·점·40자 초과)는 `<key>`로 redact**(`safeKey`). 값 타입은 유지. **잔여 경계(수용)**: ≤8키 map·값 타입 혼합 map의 식별자형 레코드ID(`u_8471`·`cus_x`)는 collapse 판정을 벗어나 safeKey로도 통과한다(패턴상 스키마 키와 구분 불가). 이메일·UUID·비정형 키는 어느 경로든 방어. 완전 차단은 shape 정보 손실(정당한 균일 스키마 객체까지 collapse)과 상충해 이 경계를 수용.
