...
- **검증**:
  ...
  - [x] **arm 창이 `chrome.tabs.update` 전에 닫힌다** — 열린 채 남은 3초 창이 타임아웃되면 `frameBlocked`가 뒤늦게 날아와 방금 성공한 재수립을 롤백시킨다. handoff 경로의 소유자는 background(`decideDeviceSignal`의 `armed: false` + `applyDeviceSignal`의 `clearArmTimer`)이고 패널은 관여하지 않는다
  - [x] **ACK 만료 handoff가 Full로 강등한다** — pending만 안 남기고 폭을 그대로 두면 같은 폭 재선택이 `noop`으로 죽고 페이지 전체 캡처가 영구 차단된다
...
- [ ] **`390`↔`768` 전환에서 페이지가 재로드되지 않는가**(경량 경로 — 스크롤 위치·입력값이 유지된다). `전체`를 거칠 때만 재로드된다
...
- [ ] 사이드패널 폭을 320px까지 좁힐 때 라벨이 **잘리는 게 아니라 한꺼번에 접혀 아이콘만 남는가**(다른 탭 바와 같은 거동), 그리고 넓히면 되돌아오는가
...
- 신규 페이지 "디바이스 뷰포트로 반응형 검증하기". **`element/` 하위에 두지 않는다** — `guide/ko/SUMMARY.md`의 IA가 element/screenshot/video/logs로 캡처 모드별 분기인데 이 기능은 그 넷을 횡단하고 컨트롤도 모드 공통 진입 화면에 있다. `element/` 안에 넣으면 스크린샷·영상 사용자가 못 찾는다. 상위 레벨이나 `quick-start` 인접에 둔다.
  ...
  - 모드 진입 시 페이지가 재로드된다는 것, 되돌릴 수 없는 화면에서 주의할 것. **폭을 바꾸는 것만으로는 재로드되지 않는다**(390→768은 그 자리에서 바뀐다)는 점도 함께 — 안 쓰면 폭을 고를 때마다 재로드된다고 오해한다
