...
- **검증**:
  - [x] `recordModeMeta("tab")` → `{ icon: "appWindow", labelKey: "issue.mode.video" }`
  - [x] `recordModeMeta("screen")` → `{ icon: "monitorPlay", labelKey: "issue.mode.screenRecord" }`
  - [x] `pnpm test` green
...
- **검증**:
  - [x] PostToolUse 훅(locales.test.ts) 통과 — ko/en 키 대칭·빈 값 없음
  - [x] 키가 플랫 문자열인지(중첩 아님) 확인
  - [x] `pnpm test` green
...
- **검증**:
  - [x] 설정 화면에 "녹화 모드" 행 노출, 현재 값이 활성 탭에 반영
  - [x] 탭 클릭 시 값 변경·영속화 (리로드 후 유지)
  - [x] `pnpm typecheck` 통과
...
- **검증**:
  - [x] 캡처 진입 화면이 3행으로 렌더 (element / [요소캡처·범위캡처] / [녹화(우측 ⚙) · 리플레이])
  - [x] `recordingMode`에 따라 녹화 버튼 아이콘·라벨 즉시 전환(리로드 불필요)
  - [x] 녹화 버튼 본체 클릭 → 해당 모드 녹화 시작
  - [x] ⚙ 클릭 → 설정 탭(이슈 sub-tab) 열림, **녹화는 시작되지 않음**(stopPropagation)
