...
- **현재**: navigation 행 전체를 `kindColor`=파랑(`:38`)으로 span에 적용(`:287`). nav URL은 이미 `InlineLink`(`:121`).
- **변경**:
...
- **토크나이저 과탐/누락**: `(url:line:col)` 괄호 종료, `errors/185.` 후행 점, 쿼리스트링, 멀티 URL 경계 → 단위 테스트로 고정.
...
- **다크모드 대비**: 중립 본문 + 연틴트 배경 가독성. 기존 토큰이라 보장되나 시각 확인 권장.
