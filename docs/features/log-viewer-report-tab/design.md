...
- **`src/sidepanel/tabs/IssueTab.tsx`** — `SubmitSuccessView`(`:346-377`, 제출 완료 화면)에서 `submitResult.logsDropped`가 true면 `useEffect`로 `toast.warning(...)`(sonner) 1회 노출. 문구는 **플랫폼 무관 공통 i18n 키 1개**(보간 없음, 예: "첨부 용량 한도로 `logs.html`이 누락되었습니다").
