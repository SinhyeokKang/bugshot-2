...
- **플랫폼 용량 초과로 logs 첨부 드랍**: 이슈는 정상 생성되되 `logs` 카테고리 첨부만 격리되어 빠진다(image/video 실패와 달리 전체 실패 아님). 이번 변경으로 이 경우 제출 완료 페이지에서 **"{platformName} 용량 한도로 logs.html이 누락되었습니다" 류 경고 토스트**(공통 키 + 플랫폼명 보간)가 뜬다(기존 silent drop 개선). 위험 큰 순: GitLab(10MB 하드캡) > Linear(가장 silent) > Notion(5MiB). Jira·Asana는 한도가 커 사실상 미발생.
