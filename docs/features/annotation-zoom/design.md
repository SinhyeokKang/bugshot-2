...
1. `scale` 단일 state → `fit`(fit-width 기준 배율) + `fitAll`(전체 조망 배율) + `zoom`(사용자 선택, `null`이면 fit 추종)으로 분리. 표시 배율 = `zoom ?? fit`. 둘 다 뷰포트 실측 크기에서 파생된다.
...
- 변경:
  ...
  - **`tool` 초기값을 `null` → `"select"`로.** 현행은 `handlePointerDown`(`:233`)이 `if (!tool) return;`으로 먼저 빠져나가 진입 직후 팬이 죽는다. `AnnotationToolbar`의 `showStyleRow`(`:74`)는 이미 `(tool !== null && tool !== "select") || hasSelection`이라 **select에서도 스타일 행이 숨겨진다** — 초기 화면은 현행과 동일하다(조건 재정의 불필요).
...
- 팬 진행 중(`panRef.current?.moved`)이면 enter/leave 커서 갱신을 스킵. `AnnotationOverlay`가 `cursorLocked: boolean` prop으로 넘긴다.
...
- **리사이즈로 `fit > zoom`이 되는 경우**(zoom=0.5 고정 후 패널을 넓혀 fit=0.6): 현재 배율이 `zoomStops`에 없는 값이 되어 Select value가 어떤 항목과도 안 맞는다. **`fit`이 `zoom`을 추월하면 `zoom`을 `null`로 되돌린다**(맞춤 상태로 복귀). 그 이하로 축소할 방법이 어차피 없으므로 사용자 의도와도 맞는다.
...
`ShapeNode`에는 `restCursor={toolCursor(tool, panEnabled)}` / `cursorLocked={panRef.current?.moved ?? false}`를 넘긴다 (위 "변경 범위" 참조).
