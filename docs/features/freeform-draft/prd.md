...
- Style diff, before/after 스냅샷, 캡처 결과 미디어 블록(스크린샷/영상) — freeform에서 제외. Tiptap 에디터 내 사용자 직접 인라인 이미지 첨부는 가능.
...
- **탭이 unsupported URL인 경우**: freeform도 기존 사이드패널 활성화 가드(`isSupportedUrl`)를 따르므로 supported URL에서만 진입 가능하다. 구현 범위를 최소화하기 위해 unsupported URL 특수 처리는 하지 않는다.
