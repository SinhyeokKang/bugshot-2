...
- 배너는 **닫기(dismiss) 가능**하며, 닫은 시점의 확장 버전을 `chrome.storage`에 영속한다. **minor+ 업데이트(기능 추가) 시 재노출**하고, patch(버그픽스)에는 재노출하지 않는다.
...
2. 탭 헤더 위에 "사용 방법이 궁금하다면? 가이드 ›" 배너가 보인다.
...
엣지 케이스:
- 설정 store가 아직 hydrate 안 된 초기 프레임: 플리커 방지를 위해 hydrate 완료 전에는 배너를 렌더하지 않는다.
