...
- 배너는 **닫기(dismiss) 가능**하며, 닫은 시점의 확장 버전을 `chrome.storage`에 영속한다. **minor+ 업데이트(기능 추가) 시 재노출**하고, patch(버그픽스)에는 재노출하지 않는다. (운영 규칙: minor bump는 "가이드도 함께 갱신된 릴리스"에 한정해야 재팝업이 의미 있다 — `/push` 신선도 검사가 이를 강제. 가이드 무관한 minor에서 재노출되면 나그가 되므로 비목표/위험에 명시.)
- **보조 진입점**: 설정 > 앱 설정 푸터의 [개인정보 처리방침] 버튼을 [유저 가이드]로 교체해, dismiss와 무관하게 항상 가이드에 접근 가능하게 한다. (앱 내 privacy 링크는 제거 — 스토어 등록 정보의 privacy URL에만 존재.)
- ko/en 양쪽 문구를 제공한다.
...
- 배너 재노출을 위한 수동 토글 UI(설정에서 "다시 보기" 스위치). 재노출은 버전 기반 자동만.
- 첫 실행 온보딩 투어·툴팁·코치마크.
- 가이드 검색·앱 내 임베드(iframe).
- 배너 클릭/진입 트래킹·애널리틱스. 이번 스코프는 계측 없음(진입점 동작만). 향후 계측 요구는 별도 스코프.
- 가이드 무관한 minor 릴리스에서의 재팝업 억제 로직. 재팝업은 `major.minor` 상승 기준 자동만이며, "그 minor에 가이드가 실제로 바뀌었는지"는 코드가 아니라 `/push` 신선도 검사(운영 규칙)로 보장한다.
...
2. 탭 헤더 위에 "유저 가이드 바로가기 ›" 배너가 보인다.
3. 배너 본문을 클릭 → 새 탭에서 GitBook 가이드가 열린다. 사이드패널은 그대로 유지(배너도 유지).
4. 배너 우측 X를 클릭 → 배너가 사라지고(닫은 버전 기록), 이후 사이드패널을 다시 열어도 나타나지 않는다.
5. 확장이 minor+ 버전으로 업데이트되면(기능 추가) 다음 패널 오픈 시 배너가 다시 노출된다. patch 업데이트면 계속 숨김.
6. 배너를 닫은 사용자도 **설정 > 앱 설정 푸터의 [유저 가이드]** 버튼으로 언제든 가이드를 연다.

엣지 케이스:
- 설정 store가 아직 hydrate 안 된 초기 프레임: 플리커 방지를 위해 hydrate 완료 전에는 배너를 렌더하지 않는다. **단 추적 대상은 `useSettingsUiStore`**(배너 상태가 사는 store)이며, App.tsx의 기존 `settingsHydrated`(=`useSettingsStore`/accounts)와는 별개 store이므로 GuideBanner가 자체 hydrate 가드를 가져야 한다(설계 상세 참조).
- 새 탭 열기 실패(드물게 `chrome.tabs` 거부): 조용히 무시(기존 외부 링크 버튼과 동일).
- 비정상 버전 문자열: `shouldShowGuideBanner`가 파싱 실패 시 재노출 안 함(나그 방지).

## 성공 기준

- **신규 설치 후 첫 패널 오픈 시 배너가 dismiss 없이 노출된다**(초기값 null → `shouldShowGuideBanner` true). 배경의 "첫 실행 시 진입점 부재" 문제가 닫힌다.
- 모든 최상위 탭(debug/issue-list/integrations/settings)에서 배너가 동일하게 보인다.
- 배너 본문 클릭 시 GitBook URL이 새 탭으로 열리고 사이드패널/배너는 유지된다.
- X 클릭 후 사이드패널을 닫았다 다시 열어도 배너가 안 보인다(영속 확인).
- 닫은 뒤 minor+ 버전 업데이트 시 재노출, patch 업데이트 시 미노출(`shouldShowGuideBanner` 단위 테스트로 보장).
- 설정 푸터 [유저 가이드] 버튼이 항상 노출되고 클릭 시 가이드가 열린다(privacy 버튼은 사라짐).
- ko/en 전환 시 배너·설정 버튼 문구가 각각 바뀐다.
