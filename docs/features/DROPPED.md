...
## 2026-08-20 — 어댑터 8벌의 로컬 `fetch` 목을 `@/test/fetch-mock`으로 이주시키기 (지금은 안 한다)

`coverage-ratchet-recovery` Task 1이 공용 `src/test/fetch-mock.ts`를 새로 깔았다. 기존 로컬 목은 **명명 헬퍼 8벌 + 무명 인라인 3벌 + `jira-media-id.test.ts`의 `stubFetch`** 까지 11~12곳인데, 이번 배치는 **신규 테스트만 공용 헬퍼를 쓰고 기존 목은 한 줄도 건드리지 않았다.**

**왜 지금 안 하나** — 실패 모드가 다르다. `jira-api.test.ts:265`의 `mockFetchByUrl`은 미매칭을 `404 + {}`로 흘리고 신규 `mockFetchRoutes`는 **throw**한다(무음 404가 "URL이 틀렸다"를 "응답 파싱이 틀렸다"로 오진시키기 때문에 의도한 차이다). 이주하면 jira의 기존 27개 라우트 테스트가 미매칭 하나마다 red로 뒤집히고, 그중 어느 것이 진짜 결함이고 어느 것이 픽스처 게으름인지 가리는 비용이 이주 자체보다 크다. `github-api.test.ts`는 `globalThis.fetch` 직접 대입이라 `vi.unstubAllGlobals()`와 복원 주체가 겹치는 문제도 따로 있다.

**무엇이 바뀌면 다시 보나** — 셋 중 하나:
- 어느 어댑터 테스트를 **다른 이유로 크게 손대게 될 때** 그 파일만 함께 이주한다(전면 이주가 아니라 편승).
- `mockFetchRoutes`에 "미매칭을 404로 흘리는" 옵트인 모드가 필요해질 때. **지금 넣지 않는 게 요점이다** — 옵션을 먼저 만들면 이주가 값싸 보이는 대신 무음 404가 공용 헬퍼로 승격된다.
- 로컬 목 사본이 12벌을 넘어설 때. `복제본` 계열은 회고 34건(3위)의 반복 함정이라 그때는 비용 계산이 뒤집힌다.

**다시 볼 때 확인할 것**: 공용 헬퍼가 `headers`를 안 준다는 점(어댑터 8파일에 `.headers.get(` 0건이라 뺐다). 이주 대상이 응답 헤더를 읽으면 그건 이주가 아니라 헬퍼 확장 요청이다.
