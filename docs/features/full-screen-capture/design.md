...
- 고정 요소 수집·숨김: `document.body.querySelectorAll("*")`를 순회하며 `getComputedStyle(el).position`이 `fixed`/`sticky`인 요소 수집(picker 오버레이 shadow host 제외). 숨김은 `el.style.visibility = "hidden"` 직접 대입이 아니라 **기존 picker 스타일 조작 패턴(picker.ts L489-503, L596-615)대로 prev 값·priority 저장 + `setProperty("visibility", "hidden", "important")`** — 페이지의 `visibility: visible !important`에 지지 않고 복원 시 인라인 원값을 정확히 되살린다. **`display:none`이 아니라 `visibility`** — 레이아웃이 바뀌면 스티치 타일이 어긋난다. `transition: visibility`가 걸린 요소는 잔상이 남을 수 있다(수동 테스트 항목).
...
// picker.scrollCaptureTo → scrollCaptureTo(scrollSession, y, hideFixed)  (세션 없으면 no-op 응답)
...
5. 스티칭: 첫 타일 이미지의 `naturalWidth / metrics.viewport.width`로 DPR 스케일 산출(`capture.ts:58-93`의 기존 패턴 재사용) → canvas(`vw*scale × totalHeight*scale`)에 `tileDrawRect` 좌표로 `drawImage` → `toDataURL("image/webp", 0.92)`. `destY × scale` 반올림은 누적 오차가 없는 정책(타일별 `Math.round`)으로 고정하고 테스트로 박는다.
...
| `issue.capturing.progress` | `{n} / {total}` | `{n} / {total}` |
