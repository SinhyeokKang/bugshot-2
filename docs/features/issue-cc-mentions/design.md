...
| `src/i18n/namespaces/settings.ts` | `field.cc.label` / `field.cc.select` / `field.cc.search` / `field.cc.empty` ko·en 동시 추가 (기존 `field.assignee.*` 옆) |
...
[IssueCreateModal | DraftDetailDialog] handleXSubmit
    ...
    Linear  : ccMarkdownLine(이름) + payload.subscriberIds
...
// GET /users?page_size=100 (+ start_cursor 페이지네이션, has_more 동안 반복)
// results에서 type === "person"만 매핑. 403(capability 부재)은 NotionError로 그대로 전파.
...
- **Linear `IssueCreateInput.subscriberIds` 존재 검증**: 구현 시 GraphQL introspection 또는 실호출로 확인. 없으면 cc 줄(시각)만 남기고 구독 생략 — 이 경우 Linear는 알림 미보장임을 보고에 명시.
- **Notion `/v1/users` capability**: 통합(integration)에 "사용자 정보 읽기" capability가 없으면 403. BugShot Notion 앱 설정 확인 필요. capability 변경 시 기존 연결 사용자의 재인증이 필요할 수 있음 → 403이면 콤보박스 에러 표시로 graceful 처리(제출 비차단).
- **Notion API 멘션 알림 여부**: API로 생성한 본문 mention이 알림을 발송하는지 실워크스페이스 검증 필요 (페이지 접근 권한이 있는 사용자만 알림됨).
