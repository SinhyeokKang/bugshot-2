...
| `src/log-viewer/buildLogsHtml.ts` | 빌드된 template HTML에 JSON 데이터를 주입하여 최종 HTML 문자열 반환 |
...
| `src/i18n/namespaces/logs.ts` | `logSummary.console.detail`의 `console-log.json` → `logs.html` |
...
| 각 플랫폼 submit 파일의 `guessMime()` | `.html` → `text/html` 분기 추가 |
...
```typescript
// src/log-viewer/buildLogsHtml.ts
...
기존 `src/i18n/index.ts`의 `useT()`는 `useSettingsUiStore(s => s.locale)`에 의존. log viewer용 `useT()`는 `navigator.language`로 locale을 정적 감지하고, logs 네임스페이스 키만 포함한 경량 사전 사용.
...
상단 바 구성:
- 좌: "BugShot Logs" 타이틀 + 페이지 URL 표시
- 중: Network / Console 탭 (Tabs 컴포넌트)
- 우: 다크/라이트 토글 + "Download HAR" + "Download JSON" 버튼
...
마찬가지로 `buildIssueMarkdown`의 `networkLogPath` import가 있다면 stub 처리:
```typescript
// log viewer에서 사용되지 않는 import를 위한 stub
"@/sidepanel/lib/buildIssueMarkdown": path.resolve(__dirname, "./src/log-viewer/stubs/buildIssueMarkdown.ts"),
```
...
3. **`</script>` 이스케이프**: 네트워크 응답 body에 `</script>`가 포함되면 HTML 파싱이 깨짐. `JSON.stringify()` 후 `<` → `<` 치환으로 방지.
4. **alias 충돌**: `@/i18n` redirect가 log viewer 빌드에서만 적용되어야 함. 메인 빌드에 영향 없음 (별도 Vite config).
5. **컴포넌트 import chain**: `NetworkLogContent`가 `networkLogPath` (from `buildIssueMarkdown`)을 import하는데, 이는 log viewer에서 불필요. alias stub으로 해결하거나, 해당 import를 컴포넌트에서 분리.
