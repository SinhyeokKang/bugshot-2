...
- **검증**:
  - [ ] `pnpm build:e2e` → `dist-e2e/manifest.json`의 `host_permissions`에 `<all_urls>` 포함, dev `key` 유지
...
- **검증**:
  ...
  - [ ] e2e 파일에 고의 타입 오류 삽입 시 `pnpm typecheck`가 잡음 (확인 후 원복)
...
- **검증**:
  ...
  - [ ] `pnpm test` green (vitest가 e2e spec 미수집)
...
- **검증**:
  - [ ] design.md 체크 목록 16개와 test가 1:1 대응
