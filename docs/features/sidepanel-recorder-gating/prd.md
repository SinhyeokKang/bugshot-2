...
이 상시 캡처는 cross-page 로그 누적을 위해 `recording` 기본값을 `false`→`true`로 바꾸며 도입됐고, 그 부작용으로 SigV4 등 본문 서명 요청을 보내는 사이트(AWS 콘솔)에서 장애가 발생했다. 본문 손상 자체는 핫픽스(`ba9f26e` + 후속)로 해결됐지만, "확장이 안 쓰일 때도 모든 페이지의 트래픽을 상시 래핑·버퍼링한다"는 구조적 오버헤드와 간섭 면적은 그대로 남아 있다.
...
- 핫픽스(본문 무손상)에 더해, 평소 브라우징의 노출 면적·오버헤드를 상시 캡처 도입 이전(레코더 미동작) 수준으로 되돌린다.
...
- **30s Replay와의 비대칭**: 30s Replay는 게이팅 대상이 아니라(비목표) 패널 미활성 탭에서도 직전 30초 영상을 잡는다. 따라서 30s Replay로 건진 영상에는 직전 구간의 network/console 로그가 동반되지 않을 수 있다(영상은 있으나 로그 섹션은 비어 미표시). 이 비대칭은 수용하며, 가이드 본문에서 명시한다.

## 성공 기준

- 사이드패널을 한 번도 열지 않은 탭에서 AWS 콘솔 로그인·리전 선택·권한 작업이 정상 동작한다(레코더 미동작 확인).
- 사이드패널을 연 탭에서는 network/console/action 로그가 정상 수집된다.
- 사이드패널을 닫거나 다른 탭으로 전환하면 이전 탭의 `recording`이 `false`로 돌아간다(패널 닫힘은 `port.onDisconnect`, 탭 전환은 `chrome.tabs.onActivated` 보완으로 각각 보장).
