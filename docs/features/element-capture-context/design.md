...
before 캡처가 확장에 성공하면 그 조상의 selector를 사이드패널이 보관하고, after 캡처 시 selector를 되돌려 보내 **같은 조상을 다시 측정**한다. rect 좌표가 아니라 대상만 고정하므로 스크롤·리플로우와 무관하게 항상 올바른 위치를 찍는다.
...
// 확장 rect가 뷰포트 면적에서 차지할 수 있는 상한. 일반적 모달(30~50%)은 통과하고
// 풀스크린 백드롭은 차단한다. 실물 튜닝 대상.
...
// 일반 페이지의 구조 단위 — 태그·role로 의미가 확정돼 오탐이 없다.
const STRUCTURE_SELECTOR =
  'tr,li,form,fieldset,article,section,nav,aside,header,footer,figure,' +
  '[role="row"],[role="listitem"],[role="group"],[role="tabpanel"],' +
...
`findContextAncestor` 판정 순서:

```
1. semantic  = el.parentElement?.closest(OVERLAY_SELECTOR + ',' + STRUCTURE_SELECTOR)
2. positioned = el부터 위로 올라가며 getComputedStyle(a).position이 fixed|absolute이고
                zIndex !== "auto" 인 최근접 조상 (요소 자신 제외)
3. 둘 다 있으면 더 가까운 쪽:
     positioned.contains(semantic) ? semantic : positioned
4. 하나만 있으면 그것, 없으면 null
```

3번이 최근접 우선을 보장한다. `positioned`가 `semantic`을 포함하면 `semantic`이 더 안쪽이므로 그쪽을 쓴다.
...
`captureContext`는 `reset`·`backToStyling`·요소 재선택 시 초기화한다. 버퍼 승격 경로(`editor-store.ts:580~`)에서는 `beforeImage`·`afterImage`와 함께 복원한다.

## 기존 패턴 준수

- **세션 영속화**: editor-store는 zustand persist가 아니라 `IssueRecord`로 수동 직렬화하고(`editor-store.ts:855~`) 이미지는 boolean 플래그만 남긴다. `captureContext`는 optional 필드 + `?? null` 폴백으로 추가하므로 **마이그레이션이 필요 없다** — `frameId`·`origin`이 이미 같은 방식으로 처리된다.
...
`<div class="card">` 같은 시맨틱 없는 컨테이너까지 커버할 수 있다. 채택하지 않았다. 임계값 튜닝이 필요하고 오탐 시 엉뚱한 영역이 찍혀 **리포트 정확도를 직접 해친다**. 시맨틱 태그·role은 의미가 구조적으로 확정돼 오탐이 불가능하다는 점에서 등급이 다르다.
...
- **`position:absolute` 오탐 범위**: 신호에 `absolute`가 포함돼 드롭다운·툴팁이 아닌 평범한 레이아웃 요소가 걸릴 수 있다. 면적 게이트가 1차 방어선이지만, 요소를 감싸는 작은 absolute 래퍼가 걸리면 확장 실익이 거의 없는 rect가 나올 수 있다(현행과 비슷한 크기라 무해).
- **`<section>`·`<nav>` 같은 큰 랜드마크**: 그 사이에 작은 구조 단위가 없으면 후보가 되고, 크기에 따라 게이트를 통과할 수 있다. 40% 상한이 방어하지만 뷰포트가 큰 화면에서는 예상보다 넓게 찍힐 여지가 있다 — 실물 튜닝 항목.
