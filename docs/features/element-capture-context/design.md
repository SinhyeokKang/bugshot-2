...
> **낮은 발동률은 위험 요소가 아니다.** 뷰포트보다 큰 컨테이너(세로로 긴 다이얼로그, 항목 많은 `<li>`, 긴 `<article>`, 작은 창)가 전부 폴백돼 확장이 걸리는 빈도가 낮아지는 것은 **P1이 의도한 결과**다. 폴백은 현행 동작이므로 정확도 손실이 없다. 발동률을 측정한다면 목적은 "게이트가 설계대로 동작하는지 확인"이지 "낮으니 완화하자"가 아니다 — 정확도와 기능 체감이 충돌하면 정확도가 이긴다.
