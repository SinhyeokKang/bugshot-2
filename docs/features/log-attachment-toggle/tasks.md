...
### Task 1: `selectAttachedLogs` / `logCardTypeCounts` 단위 테스트 먼저 (TDD)

- **변경 대상**: `src/store/__tests__/editor-store.test.ts`(또는 신규), `src/sidepanel/components/__tests__/logCardTypeCounts.test.ts`
- **작업 내용**:
  - `selectAttachedLogs`: `logsAttach=false`면 세 로그 모두 null, `true`면 captured>0인 타입만 반환하는 케이스.
  - `logCardTypeCounts`: 캡처된 타입만 세그먼트 포함, 에러 건수 반영, 전부 0이면 빈 문자열 등.
- **검증**:
...
- **작업 내용**:
  ...
  - 복원: 세 로그를 attach와 무관하게 pending IDB에서 항상 로드. `logsAttach`는 hydrate.
  - 레거시 스냅샷 폴백: 구 필드에서 `logsAttach` 파생(`?? true`).
- **검증**:
  - [ ] `pnpm typecheck`
...
- **작업 내용**:
  - 탭형(console/network/action, 캡처된 타입만) + 개수 Badge + forceMount + 뷰 모드 Content 재사용.
...
- **작업 내용**: `buildEditorMarkdownContext`·`buildEditorLogsCaptureInput`의 3 attach 게이트 → `logsAttach`(action은 `supportsActionLog` 유지).
- **검증**:
  - [ ] `pnpm typecheck`
...
- **작업 내용**: `logCard.title` + 타입별 count 키 추가. 미사용된 `logCard.description`/`actionLog.cardDescription`은 실제 미참조 확인 후 제거.
- **검증**:
  ...
  - [ ] log-viewer 사전(`src/log-viewer/i18n.ts`)에 해당 키 없음 재확인(불필요 갱신 방지)
...
- **e2e 시나리오** (`/e2e-write` 입력):
  ...
  - `e2e/action-log-scope.spec.ts` 갱신: `network/console/action-log-card` 3-testid 검증 → `log-attachment-card` 단일 + 탭 검증으로 재작성.
- **수동 테스트** (자동화 어려운 것):
...
- Task 4(카드)·Task 5(다이얼로그)는 Task 2 이후 **병렬 가능**.
