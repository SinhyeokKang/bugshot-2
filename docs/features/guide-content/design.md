...
| `settings/ai.md` AI LLM 연동 | AI 모델 서브탭: LLM 프로바이더 연결(BYOK), 연결 시 스타일 편집·초안 작성에 AI 배너 등장. 미연결 시 비노출. |
| `element/README.md` | 요소 모드 개요: 요소 클릭 → 스타일 라이브 수정 → before/after로 이슈. 4-1/4-2/4-3 바로가기. |
| `element/picker.md` 요소 선택 | "Element" 버튼 또는 `Shift+S` → picking 단계, 페이지에 crosshair. 요소 클릭 → 선택. DOM 트리 네비(부모/자식), Repick. iframe 내부 미지원 안내. |
| `element/styling.md` 스타일링 | 스타일 패널 섹션(Class/Layout/Container/Size/Overflow/Typography/Effects/Transition/Text). 라이브 반영. Reset Changes(되돌림), AI 스타일 배너(AI 연결 시). Next → 초안. |
| `element/issue.md` 이슈 작성(요소) | **자기완결**: 캡처 후 초안 화면 전체. 제목(접두어), 재현 환경(자동: OS/브라우저/URL/뷰포트/시각 + 사용자 추가 변수), **스타일 변경 before/after 표**, 본문 섹션, 로그 첨부(콘솔/네트워크 토글), 미리보기 → 제출(플랫폼 필드 입력) → 완료 URL. 마크다운 복사. |
| `screenshot/README.md` | 스크린샷 모드 개요 + 5-1/5-2/5-3 바로가기. |
| `screenshot/capture.md` 스크린샷 캡처 | "Screenshot" 버튼 → 영역 드래그 선택 → 캡처. 산출(원본/뷰포트/시각). |
| `screenshot/annotation.md` 어노테이션 | 캡처 이미지에 주석 추가/편집/초기화(Pencil). 어떤 도구가 있는지. |
| `screenshot/issue.md` 이슈 작성(스크린샷) | **자기완결**: 제목, 재현 환경, **주석 스크린샷 첨부**, 본문 섹션, 로그 첨부, 미리보기 → 제출 → 완료. (요소 모드와 흐름 동일, 미디어만 스크린샷) |
...
| `video/issue.md` 버그 리포트 작성(녹화) | **자기완결 + 녹화 전용 로그 정책 강조**: 영상 첨부 + **액션 로그**(녹화 모드에서만 수집) + 콘솔/네트워크 로그 토글. 본문 섹션, 미리보기 → 제출. 영상은 플랫폼별 처리 차이가 있으면 명시. |
...
| `logs/viewer.md` 로그 뷰어 | **개발자 관점**: 이슈에 첨부된 `logs.html`을 받아 여는 법 + 타임라인 마커(콘솔/네트워크/네비/액션) + 영상 플레이어 + 마커 클릭으로 영상 점프. 로그-영상 시간 동기. |
...
- **사실 정합성**: 단축키(`Shift+S` 등)·필드명·연결 방식이 코드와 어긋나면 사용자 혼란. 명세 표를 기준으로 작성하고, 불확실하면 해당 소스 재확인.
