...
- **작업 내용**:
  ...
  - 대체: **tier 계약 불변식 테스트**로. (Task 4 이후 채워지므로 여기선 제거만 하고 빈 자리를 남긴다.)
- **검증**:
...
- **작업 내용**:
  - `AiTier = "nano" | "full"` 타입, `AiContextOverflowError` 클래스 추가.
  - `AIProvider`에 `readonly tier`·`readonly supportsImages` 추가. Chrome → `{tier:"nano", supportsImages:false}`, OpenAI·Anthropic → `{tier:"full", supportsImages:true}`.
  ...
  - `useAI()` 반환에 `tier`·`supportsImages` 추가 — **provider에서 파생**한다(새 판정 로직을 만들지 않는다. `llm?.modelId` 유무 판정은 이미 `:53`에 있다).
- **검증** — 이 경로는 현재 테스트 0개다. 여기서 처음 박는다:
  - [ ] `ai-provider.test.ts`에 `createChromeAIProvider` 테스트 추가: `tier === "nano"`, `supportsImages === false`
  - [ ] `createOpenAICompatibleProvider`·`createAnthropicProvider`: `tier === "full"`, `supportsImages === true`
...
- **작업 내용**:
  - `AiDraftSessionContext`에 `tier`·`supportsImages` 추가.
  ...
  - 프롬프트의 스크린샷 지시(`buildAiDraftPrompt.ts:144`)를 `supportsImages`로 게이팅. (Task 4에서 tier별 파일로 이관되지만, 그 전에 이 게이트부터 세운다.)
- **검증**:
  - [ ] **회귀 재현 테스트(red 먼저)**: `buildAiDraftRequest.test.ts` — `supportsImages: false` + modeImages 있음 → `images === undefined`
  - [ ] `supportsImages: true` → 기존 concat 동작 유지 (기존 4개 테스트가 tier 필드 추가 후에도 green)
...
### Task 4: tier별 프롬프트 분리

Task 0~3이 끝난 뒤. **이게 본체다.**

- **변경 대상**: `src/sidepanel/lib/prompts/{caps,context,draft.nano,draft.full,styling.nano,styling.full}.ts` (신규), `buildAiDraftPrompt.ts`·`buildAiStylingPrompt.ts` (디스패처로 축소)
- **작업 내용**:
  - `caps.ts`: `PROMPT_CAPS: Record<AiTier, PromptCaps>` (design.md 표대로). `buildLogSummary.ts`의 `MAX_ERRORS`/`MAX_ACTIONS`는 **건드리지 않는다** — 요약 생성기의 상한이고 다른 소비자가 있다. tier 캡은 프롬프트에 싣는 시점에 추가 slice.
  - `context.ts`: `selectRelevantTokens`·`extractVarRefs`·`selectStyles`·`extractLayoutContext`·`buildStyleDeltaBlock`.
  - `draft.nano.ts`·`styling.nano.ts`: 나노 본문 **독립 작성**. `responseConstraint`가 강제하는 규칙 전부 삭제(JSON only / no fences / no extra fields / denied prop 목록 / "빈 문자열 사용"). 지시는 긍정형만. 이미지 언급 0. few-shot 1개.
  - `draft.full.ts`·`styling.full.ts`: 역할 격상, 분석 절차, few-shot, 확대된 캡, 레이아웃 컨텍스트. **거절방지 문구 제거**. **JSON 형식 규칙은 유지** (BYOK structured output 연결이 비목표라 유일한 방어선).
...
- **검증** — Task 0에서 비운 자리를 **불변식 테스트**로 되메운다:
  - [ ] 나노 초안 system prompt가 컨텍스트 없는 기본 상태에서 `NANO_SYSTEM_BUDGET_CHARS` 이하
  - [ ] 나노 프롬프트에 이미지·스크린샷 언급 없음 (전 캡처 모드)
  - [ ] 나노 프롬프트에 `"JSON"`·`"fences"`·denied prop 목록 없음
  - [ ] 풀 프롬프트에는 JSON 형식 규칙이 **있음**
  - [ ] 풀 스타일링 프롬프트에 `"You CAN and MUST"` 없음
  - [ ] 풀 스타일링 프롬프트에 레이아웃 컨텍스트 포함 (`display` 등)
  - [ ] tier별 캡이 실제로 적용됨 (나노 diffs 8 / 풀 diffs 50)
...
### Task 5: 나노 컨텍스트 예산 관리

- **변경 대상**: `src/sidepanel/lib/nanoBudget.ts` (신규), `src/sidepanel/tabs/AiDraftDialog.tsx`, `src/i18n/namespaces/ai.ts`
- **작업 내용**:
  - `trimDraftContext(ctx, level)`: level 0 캡만 → 1 로그 제거 → 2 기존 초안 제거 → 3 diff·토큰 제거.
  - `fitNanoDraftContext(ctx, build, budgetChars)`: 예산에 맞을 때까지 level을 올리며 재빌드. 최종 level에서도 초과면 **던지지 않고** 그대로 반환(2차 게이트가 판정).
  - `isSessionOverBudget(session)`: 세션 생성 후 `inputUsage` vs `inputQuota` 실측. 미지원이면 `false`(통과).
  - `AiDraftDialog`: 나노 tier면 `fitNanoDraftContext` 적용 → 세션 생성 → 2차 게이트 초과 시 `AiContextOverflowError` → 전용 토스트.
...
- **검증**:
  ...
  - [ ] `fitNanoDraftContext`: 거대 컨텍스트 → level이 올라가며 예산 내로 수렴
...
### Task 7: 풀 tier 이미지·레이아웃 컨텍스트 보강

- **변경 대상**: `src/sidepanel/tabs/AiDraftDialog.tsx` (`getModeImages`), `src/sidepanel/tabs/styleEditor/AiStylingDialog.tsx`
- **작업 내용**:
  - `getModeImages`: screenshot 모드에서 `screenshotAnnotated ?? screenshotRaw` → **둘 다** 반환(사용자가 표시한 곳과 원본을 대조 가능). 풀 tier 전용이므로 `supportsImages` 게이트 뒤에 있다.
...
- **검증**:
  ...
  - [ ] 풀 스타일링 프롬프트에 `display`·뷰포트 폭이 실림
...
| 대상 | 케이스 |
|---|---|
| `ai-provider.ts` (**신규 커버리지**) | Chrome/OpenAI/Anthropic의 `tier`·`supportsImages`. `createSession` 라이프사이클. Chrome 세션의 `measureInputUsage`/`inputQuota` 노출·부재 폴백 |
...
| `prompts/caps.ts` | tier별 캡이 프롬프트에 실제 적용 |
| `nanoBudget.ts` | `trimDraftContext` level별 / `fitNanoDraftContext` 수렴 / `isSessionOverBudget` 폴백 |
| `buildAiDraftPrompt.ts` | **tier 계약 불변식** (예산·이미지 언급 유무·JSON 규칙 유무) — Task 0에서 제거한 워딩 단정의 대체 |
| `buildAiStylingPrompt.ts` | 동상 + 거절방지 문구 부재(풀) / 레이아웃 컨텍스트 포함(풀) |
...
Task 0 (테스트 결합 해제)  ← 반드시 첫 번째. 안 하면 프롬프트를 못 고친다
   │
Task 1 (능력 표면 + 나노 세션 테스트)  ← tier의 단일 출처
   │
   ├─ Task 2 (선행 픽스 A: 이미지 모순)   ┐
   ├─ Task 3 (선행 픽스 B·C: 데이터 손실) ├─ 병렬 가능
   │                                      ┘
Task 4 (tier별 프롬프트 분리)  ← 본체. Task 0~3 완료 후
   │
   ├─ Task 5 (나노 예산 관리)      ┐
...
**Task 1이 두 번째인 이유**: `tier`가 없으면 Task 2~7의 모든 분기가 성립하지 않는다. 동시에 나노 경로 테스트가 0개인 상태를 여기서 해소해, 이후 태스크가 안전망 위에서 진행된다.

## 가이드 영향

**없음.**

사용자에게 노출되는 UI·플로우는 그대로다(같은 버튼, 같은 다이얼로그, 같은 배지). 프롬프트 품질과 내부 tier 분기만 바뀐다.
