...
- **스타일 cap이 사용자의 최신 편집을 먼저 자른다.** `{...specifiedStyles, ...styleEdits.inlineStyle}` spread 순서상 사용자가 방금 넣은 prop이 객체 tail인데, `slice(0, 30)`이 tail부터 버린다. 선언이 30개 넘는 요소(Tailwind 컴포넌트에서 흔하다)에선 **AI가 사용자 편집을 보지 못하고 되돌린다.** (신규 추가 prop에 한함 — 기존 prop 수정은 spread가 삽입 위치를 보존해 잘리지 않는다.)
...
2. **compact 프롬프트가 나노 예산 안에서 동작한다.** 정적 본문은 2,000자(≈영문 500토큰) 이하, 지시는 긍정형, `responseConstraint`가 강제하는 규칙은 삭제. 전송 전 문자 예산(10,000자)으로 절삭하고 user turn 직전 `measureContextUsage()` 실측 + 런타임 `QuotaExceededError` 매핑으로, 초과가 확정되면 전용 에러로 안내한다.
...
|---|---|---|---|
| BYOK 미설정 (Chrome 나노) | `compact` | `false` | 10,000 (절삭 예산 — 정적 본문 목표 2,000자는 별도 테스트 상수) |
...
- 나노의 출력 언어 동작 변경. Chrome Prompt API 문서상 한국어는 미지원 언어지만 **실측상 현재 한국어를 출력한다**(사용자 확인). `CHROME_AI_LANG_OPTIONS`의 `outputLanguage: "en"`과 "한국어로 써라" 지시를 **둘 다 그대로 둔다** — 지금 되는 것을 깨지 않는다. `outputLanguage: "ko"`로 바꾸는 시도는 `NotSupportedError` 위험이 있어 금지한다. 단 이 ko 출력은 문서상 미지원 언어의 우발적 동작이라 **Chrome 업데이트로 언제든 깨질 수 있다** — 수동 테스트의 "한국어 출력 유지" 항목이 감시 지점이다.
- buffered(멀티) 요소가 초안 프롬프트에 안 들어가는 문제
- **나노 미지원/모델 미다운로드 브라우저의 안내·다운로드 유도.** 현재 `useAI`는 `downloadable`/`downloading`을 전부 `unavailable`로 접어 AI 진입점을 조용히 숨긴다 — 이번에도 그대로 둔다(의도된 제외).
...
- **현재**: 스크린샷 모드는 이미 설명 입력이 필수다(`submitDisabled = captureMode !== "element" && !input.trim()` — 텍스트 없이 제출 가능한 건 element 모드뿐). 프롬프트는 "스크린샷을 분석하라"고 하는데 이미지는 전송조차 되지 않는다. 나노는 사용자 설명과 URL·페이지 제목만 보고, 봤다고 가정한 스크린샷 내용을 지어낸다.
- **변경 후**: 나노 프롬프트에 스크린샷 언급이 없다. 사용자 설명·URL·선택자만으로 초안을 쓴다. 이미지 resolve도 건너뛴다(불필요한 IndexedDB 작업 제거). 설명 입력 필수는 현행 유지. 다이얼로그의 disclaimer 자리에 "Chrome 내장 AI는 이미지를 볼 수 없어 입력한 설명을 기반으로 작성합니다" 한 줄을 조건부 표시해 기대치를 사전 조정한다(`aiDraft.nanoImageNotice`).

### S2. BYOK 사용자가 같은 스크린샷 버그를 리포트한다

- rich 프롬프트가 스크린샷(현행대로 annotated 1장 — 2장 전송안은 BYOK 이미지 비용 2배 대비 품질 근거가 없어 기각)을 받는다. 시니어 QA 엔지니어 역할과 분석 절차를 지시받아, 사용자 서술과 이미지·로그를 대조해 확증된 것만 기술하고 추론은 `notes`에 가설로 분리한다.
...
- **변경 후**: compact 캡(에러 3건 / 액션 5건)으로 먼저 줄이고, 초과하면 로그 → 기존 초안 → 스타일 diff 순으로 절삭한다(예산 10,000자). 그래도 못 맞추면 `prompt()` 직전 실측(`measureContextUsage`)과 런타임 `QuotaExceededError` 매핑이 잡아 전용 에러를 띄운다 — main "분석할 내용이 너무 많아 Chrome 내장 AI가 처리할 수 없습니다" + description "API 키를 연결하면 더 큰 용량의 모델을 사용할 수 있습니다"(재시도가 무의미함을 알린다. "로그를 줄이라"는 지시는 사용자가 실행할 수단이 없어 기각). 이때 사용자가 입력한 설명이 함께 사라지는 문제는 비목표(실패 시 입력 소실)로 남는다 — 별도 작업.
...
- **변경 후**: 첫 턴은 시스템 프롬프트만으로 충분하므로 중복 블록을 보내지 않는다. 이후 턴은 직전 턴 대비 **변경된 prop만** delta로 보낸다. delta는 promptStyle 무관 공통 적용 — compact는 창 보호, rich는 토큰 비용 절감. 스타일링에서 그래도 초과하면 초안과 같은 전용 에러를 받는다(게이트는 두 다이얼로그 공통).
...
- `measureContextUsage`(신명칭)가 없는 구버전 Chrome: 구명칭 폴백 → 둘 다 없으면 문자 예산 절삭 + 런타임 `QuotaExceededError` 매핑만으로 동작한다(측정 실패가 기능을 막지 않는다).

## 성공 기준

1. compact 초안 system prompt가 **컨텍스트 없는 기본 상태에서 `COMPACT_SYSTEM_TARGET_CHARS`(2,000자 ≈ 영문 500토큰) 이하**다 (불변식 테스트로 단언).
2. compact 프롬프트에 **이미지·스크린샷 언급이 없고**(본문 고정 — 게이트 없음), 이미지 미지원 좌표(나노)에서 `images`가 `undefined`로 전달되며 인라인 이미지 resolve가 호출되지 않는다. 다이얼로그에 나노 고지 한 줄이 조건부로 보인다.
3. AI 응답이 `notes` 섹션 키를 누락해도 **사용자가 쓴 `notes` 텍스트가 보존**된다. AI가 `""`를 반환한 경우에도 그 섹션이 **실제로 프롬프트에 실렸을 때만** 비워진다 — 절삭으로 AI가 못 본 섹션은 `""`여도 보존(절삭×비우기 충돌 차단).
...
8. 스타일링 멀티턴(promptStyle 공통)에서 첫 턴에 요소 상태가 중복 전송되지 않고, 이후 턴은 delta만 보낸다. 컨텍스트 초과 전용 에러는 초안·스타일링 두 다이얼로그에서 동일하게 동작한다.
