...
- **검증**:
  - [x] `pnpm typecheck` 통과 (union exhaustive 충족)
  - [x] `PLATFORM_TAB_KEYS`가 `Record<PlatformId, string>` 만족(컴파일)
...
- **검증**:
  - [x] `BG_REQUEST_TYPE_MAP: Record<BgRequest["type"], true>`가 컴파일 → 키 누락 시 타입 에러로 검출
  - [x] `pnpm typecheck` 통과
...
- **검증**:
  - [x] `isClickupOAuthConfigured()` env 유무 분기 단위 테스트
  - [x] `parseClickupCallbackParams()` code/error/state 파싱 단위 테스트
...
- **검증**:
  ...
  - [x] `clickupFetch` 헤더 구성(PAT vs OAuth) 단위 테스트
  - [x] `flattenLists` 순수 헬퍼 단위 테스트(folderless + folder list 병합)
  - [x] 401 → 재연결 에러(`oauthRevoked`) 분기 단위 테스트(refresh 없이 즉시 throw)
...
- **검증**:
  - [x] 각 `clickup.*` 메시지가 대응 API 함수로 라우팅(타입체크)
...
- **검증**:
  - [x] `buildClickupIssueBody` CC 줄 주입/미주입 단위 테스트
  - [ ] inline 이미지 markdown 치환 헬퍼 단위 테스트(있을 때/없을 때)
  - [x] `submitToClickup` 업로드 순서·`logsDropped`·`inlineImagesDropped` 처리 단위 테스트(sendBg mock)
...
- **검증**:
  - [x] `initialClickupFields` last/defaults 우선순위 + 3단계 prefill 단위 테스트
...
- **검증**:
  ...
  - [x] 7개 플랫폼 전부 연결 시 탭 격자 정상(`grid-cols-7`)
...
- **검증**:
  - [ ] `pnpm build` manifest에 host 반영 (config에 `api.clickup.com` 추가됨 — build 미실행)
