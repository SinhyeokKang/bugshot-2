...
- **검증**:
  - [x] 네트워크 로그에서 JSON 응답 펼침 시 키/값/괄호가 Geist Mono로 렌더.
  - [x] WebSocket JSON 프레임도 동일하게 mono.
  - [x] 행 크기 12px(래퍼 `text-xs` 상속), 색상 토큰·들여쓰기 불변.
  - [ ] `--` 포함 값이 리거처로 붕괴되지 않음(2셀 유지). *(렌더폭 — 빌드/수동)*
  - [x] `JsonTreeViewer.test.tsx` green(래퍼 `font-mono text-xs`, 행에 `text-[13px]` 부재).
...
- **검증**:
  - [x] 접힌 요약과 펼친 `<pre>` 상세가 같은 mono 서체.
  - [x] `LinkifiedText`의 URL 링크가 mono로 렌더되며 클릭 동작 유지.
  - [x] 아이콘·타임스탬프 칩·레벨 배경 틴트 불변.
  - [x] `ConsoleLogContent.test.tsx` 확장 green(접힌 행 메시지 span에 `font-mono`).
...
- **검증**:
  - [x] verb 문장 + chip(태그/셀렉터/입력값)이 전부 mono(콘솔과 통일).
  - [x] `InlineChip`·`ResolvedTargetChip` 색상·테두리 불변, 서체만 mono 상속.
  - [x] 네비게이션 URL(`InlineLink`) 링크 동작 유지.
  - [x] 행간이 콘솔 인라인과 동일(16px — relaxed 제거됨). *(다중 행 가독성은 수동)*
  - [x] `ActionLogContent.test.tsx` green(콘텐츠 span에 `font-mono`).
...
- **검증**:
  - [ ] 로그뷰어(`logs.html`) 영상 타임라인 마커 호버 시 툴팁 내용 mono. *(export 산출물 — 빌드/수동)*
  - [ ] 트리밍 다이얼로그(`ReplayTrimDialog` → `TrimTimeline`) 타임라인 마커 툴팁도 mono. *(수동)*
  - [x] `labelParts` 색상(레벨/메서드/URL 톤) 불변.
  - [x] `TimelineMarkers.test.tsx` green(호버 시 portal 툴팁에 `font-mono`).
...
- **검증**:
  - [x] Messages 탭에서 non-JSON WS 프레임 펼침 시 본문이 mono(12px).
  - [x] 같은 탭의 JSON 프레임(JsonTreeViewer)과 서체 통일.
  - [x] 형제 raw body `<pre>`(`BodyBlock` L588 `font-mono text-xs`)와 동일 스타일.
  - [ ] `------WebKitFormBoundary` 등 `--` 연속이 리거처로 붕괴되지 않음. *(렌더폭 — 빌드/수동)*
  - [x] 접힌 프레임 프리뷰가 mono·`truncate` 유지 — 접힘/펼침 간 서체 점프 없음. open/close 이벤트 행 라벨도 mono.
  - [x] `NetworkLogContent.test.tsx` 확장 green(FrameBody `<pre>`와 접힘 프리뷰 span에 `font-mono`).
...
- **검증**:
  - [x] 표/문구·카운트가 실제 코드 표면과 일치(로그 5곳→11곳, DESIGN L79 표 + L88 + tokens.test L146 갱신).
  - [x] `FrameBody`가 더는 font-sans 대표 사례로 남지 않음(코드베이스에 font-sans `<pre>` 0개 확인).
