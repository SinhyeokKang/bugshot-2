...
- **작업 내용**: `EntryAccordion` 접힌 행 메시지 `<span>`(L243)에 `font-mono text-xs` 추가.
- **검증**:
...
- **작업 내용**: `ActionRow` 콘텐츠 `<span>`(L322)에 `font-mono text-xs` 추가. `leading-relaxed` 유지.
- **검증**:
...
### Task 6: DESIGN.md mono 불변식 표 갱신 (문서)
- **변경 대상**: `docs/DESIGN.md` §4 "mono 표면 불변식"
- **작업 내용**: "리스트·칩" 그룹 표(L79)·§88 "로그 N곳" 문구에 신규 표면 추가 — JSON 트리 · 콘솔 인라인 요약 · 액션 로그 행 · 마커 툴팁. 액션 행의 `leading-relaxed`(다중 행 문장) 예외를 각주로 명시.
- **검증**:
  - [ ] 표/문구가 실제 코드 표면과 일치.
  - [ ] `/push` 또는 `/doc-check` 신선도 검사 통과.

> Task 5는 `docs(DESIGN): ...` 별도 커밋. Task 1~4 구현 후 `/guide` 흐름과 별개로 문서만.
...
Task 1~5는 서로 독립이라 **병렬 가능**(Task 1/5는 같은 `NetworkLogContent.tsx` 계열이나 함수가 달라 충돌 없음 — Task 1은 JsonTreeViewer, Task 5는 FrameBody). Task 6(문서)는 1~5 확정 후. 권장: 1 → 2 → 3 → 4 → 5 → (컴포넌트 테스트) → 6.
