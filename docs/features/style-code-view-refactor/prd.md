...
**포지셔닝**: 폼(편집) 탭이 디자이너·QA 등 비개발 사용자용 주 경로로 남고, CSS 탭은 CSS를 직접 다루는 파워유저용 보조 경로다. 이 리팩터는 핵심 잡(버그 캡처·등록)을 바꾸지 않고, 코드 편집 경로의 품질만 끌어올린다.

## 목표

- 탭을 **편집(폼) / CSS(코드)** 2탭으로 재구성하고, 각 탭에 아이콘을 부여한다(탭명 변경: 폼→편집, 코드→CSS).
- CSS 탭을 **CodeMirror 6 기반 CSS 에디터**로 교체한다 — 신택스 하이라이팅, 줄번호, CSS 자동완성(prop명 제안 + 값 제안), prop 추가·삭제. prop명 완성은 `@codemirror/lang-css` 기본, 값 완성은 커스텀 `completionSource`로 보강한다.
...
- **selector 라인 편집·pseudo-class(`:hover` 등) 상태 시뮬레이션**: selector는 요소 고정이라 편집 대상이 아니고(재직렬화가 정본으로 복원), DevTools식 상태 강제도 이번 스코프에서 제외한다.
...
3. 선언 값을 바꾸거나(`color: #333;` → `color: red;`) 새 prop을 타이핑하면 자동완성이 제안하고, 페이지에 **즉시 반영**된다. 바뀐 prop만 오버라이드로 잡혀 변경사항 다이얼로그·before/after에 나타난다. 상단 박스모델 그래픽도 apply 후 재측정된 computed로 갱신된다.
   - prefill된 specified 선언 라인을 **지우면 그 속성을 원복**한다 — `initial` 오버라이드로 잡혀 페이지에서 해당 속성이 초기값으로 되돌고 변경사항에 나타난다(삭제=원복).
...
- **specified 선언 라인 삭제**: 삭제=원복. `computeOverrides`가 edited에서 빠진 specified prop을 `initial` 오버라이드로 방출 → 해당 속성이 CSS 초기값으로 되돌고 변경사항에 잡힌다. (인라인 오버라이드 모델이라 cascade 원값으로의 완전 복원이 아닌 `initial` 리셋임 — DevTools의 규칙 제거와 다름.)
...
- 박스모델 그래픽이 computed의 px 값(margin/border/padding/size)을 표시한다. 비px(`auto` 등)는 `0`으로 그리되 `contentLabel`에 원문을 보존한다. 선언 편집으로 apply되면 재측정된 computed로 갱신된다.
- 선언 편집이 라이브 반영되고, specified와 다른 prop만 변경사항으로 잡힌다(무편집 시 오버라이드 0). specified 라인 삭제는 `initial` 원복으로 잡힌다.
- CSS 자동완성이 prop명을 제안하고(lang-css 기본), 값도 커스텀 completionSource로 제안한다.
