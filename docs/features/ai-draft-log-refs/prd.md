...
- **로그 블록 부피 축소 없음.** `logToCodeBlock.MAX_CHARS = 16384` 전문을 그대로 쓴다. 화면을 먹는 문제는 이미 구현된 코드블럭 접기(v1.6.2)가 담당한다(아래 "의존" 참조).

  > **`MAX_CHARS`는 블록당이 아니라 body당이다.** `truncate()`가 `requestBody`·`responseBody`에 **각각** 걸리므로(`serializeNetworkRequest`) 블록 하나의 실제 상한은 **~32.8KB**(16KB × 2)다. `serializeConsoleEntry`는 args + stack + pageUrl **3곳**에 각각 걸린다. 상한 3개면 최대 ~98KB — 400px 패널 실측으로 약 84화면이다.
  >
  > **수용 손실 — Slack 부피.** 접기는 패널 UI만 흡수한다. 이 본문이 Slack으로 나가면 `splitSlackText`(`SLACK_TEXT_LIMIT = 3800`)가 최대 ~26개 스레드 답글로 분할할 수 있다. 수동 삽입도 지금 똑같은 축을 가지므로 이 feature가 새로 만드는 문제가 아니라 **빈도만 늘리는** 것이고, 사용자는 전송 전 에디터에서 블록을 보고 지울 수 있다 — 완화 없이 수용한다(fence 짝수 보존은 기존 `submitToSlack` 테스트가 커버).
...
## 의존: 코드블럭 접기 (✅ 충족 — v1.6.2 구현 완료)

이 feature는 **선별**만 책임진다. **부피**는 코드블럭 접기가 담당한다 — 15줄 초과 코드블럭을 15줄로 접는 기능으로, **v1.6.2(`25ef65a`)에서 구현·머지 완료됐다**: `src/sidepanel/hooks/useCodeCollapse.ts` · `src/sidepanel/lib/codeCollapse.ts`(`CODE_COLLAPSE_LINE_THRESHOLD = 15`) · `TiptapEditor.tsx`의 `CodeCollapseNodeView` · `e2e/code-block-collapse.spec.ts`. (해당 feature 문서 `docs/features/code-block-collapse/`는 구현 완료로 삭제됨 — 이하 근거는 문서가 아니라 코드를 직접 가리킨다.)

> ### ✅ 하드 게이트: 충족됨
>
> 초안 시점엔 "collapse 구현 전 배포 금지"의 하드 게이트였다 — "전문 유지"(비목표) 결정 전체가 collapse가 부피를 흡수한다는 전제 위에 서 있기 때문이다. 그 전제는 현재 코드에서 성립하므로 게이트는 충족됐고, 시나리오 A-4("블록은 15줄로 접혀 있다")는 실제로 성립하는 서술이다. 기록으로 남긴다: 만약 collapse가 회귀·제거되면 이 feature의 전제도 같이 깨진다.

**collapse의 위임 범위는 세로축까지다.** `tiptap-editor.css`가 `white-space: pre` + `overflow-x: auto`(의도적 — 줄을 접으면 들여쓰기가 소실된다)라 개행 없는 body(minified JSON·HTML)는 **1줄 16384자 → 약 118,000px 가로 스크롤**이 되고, collapse는 `overflow-y: hidden`만 걸므로(`code-collapse.css` 확인) **접힘이 끝나도 가로축은 남는다.**

→ **가로 스크롤은 미해결이며, 이 feature가 새로 만드는 문제가 아니다** — 수동 삽입 블록도 지금 똑같은 상태다. 이 feature의 위임은 세로축에 한정하고, 가로축은 별건으로 남긴다.

코드가 이 feature의 설계 하나를 이미 확정해준다: **로그 코드블럭만 타게팅은 불가능하다** — `serializeConsoleEntry()`(`logToCodeBlock.ts`)는 language를 안 붙이므로 삽입된 콘솔 로그는 사용자가 손으로 친 코드블럭과 **마크다운상 완전히 동일**하다. 그래서 아래 "코드블럭 보존"이 출처를 가리지 않는다.
...
> **⚠ `3`은 임의값이다.** "단일 버그의 직접 증거가 3을 넘는 경우는 드물다"는 경험칙 외에 근거가 없다. 후보 풀 비율로 정의하려 했으나 rich(합계 최대 10)와 compact(합계 최대 6)에서 의미가 갈려 오히려 나빠진다. **`MAX_LOG_REFS` 상수 하나로 빼고, 임의값으로 못박는다.**
>
...
>
> **정직하게 기록하는 긴장: 조정 신호가 실사용에서 쌓이지 않는다.** 사용자는 시나리오 D(초과 폐기)와 B(관련 로그 없음)를 화면에서 구분할 수 없고(둘 다 코드블럭 0개, 시나리오 D의 수용 손실 참조) warn은 로컬 전용이므로, "이슈 리포트가 오면 조정한다"는 루프는 사실상 닫혀 있다. 상한 조정의 실질 트리거는 개발자 자신의 로컬 재현·dogfooding뿐이다. 침묵(무토스트)과 PostHog 스코프 미개방을 각각 유지하는 대가로 이 관측 불가를 수용한다.
...
> **BYOK에선 스키마가 구조적 강제력이 없다.** 경로별로 갈린다 — OpenAI-compat 경로(`ai-provider.ts:414/427`)는 `!!responseSchema → response_format: {type:"json_object"}`(`:385`)로만 번역하고 스키마 자체를 버린다. Anthropic 경로는 스키마를 버리진 않지만 `JSON.stringify(responseSchema)`를 system prompt에 **텍스트로 주입**할 뿐이라(`ai-provider.ts:509`) 역시 강제가 아니다(부수적으로 모델이 후보 ref 목록을 한 번 더 보게 된다). LOCAL_BYOK(loopback Ollama)는 **compact 스타일 + OpenAI-compat 경로** 조합이라 "compact = nano = `responseConstraint` 강제"라는 등식의 예외다. `enum`이 구조적으로 걸리는 건 **Chrome nano의 `responseConstraint`뿐** → BYOK 계열은 프롬프트 문장이 유일한 통제 채널이고, **앱단 ref 검증은 선택이 아니라 필수**다.
...
3. 발생 현상 섹션에 AI가 쓴 산문이 들어오고, **그 아래에 실제 로그 코드블럭**이 붙는다. 내용은 `LogInsertDialog`로 직접 골랐을 때와 동일하다(Tiptap 왕복 후 동일 — 목표 절 참조).
4. 블록은 15줄로 접혀 있다(코드블럭 접기 — v1.6.2 구현 완료). 펼쳐서 확인하거나, 필요 없으면 지운다.
...
**이 침묵의 지각 리스크도 수용한다.** D에서 AI **산문**은 에러를 언급하는데("`/api/pay`에서 500 발생") 블록만 없으면, 사용자가 이 불일치를 버그로 지각할 수 있다. 그리고 이 침묵이 결정 2의 "조정 신호가 실사용에서 쌓이지 않는다"와 합성된다 — 둘 다 인지한 채 수용한다. 침묵 자체는 기존 컨벤션과 정합한다(AI 초안 완료 성공 토스트 없음, 수동 삽입 무토스트, `[bugshot]` warn-only 선례 다수 — 특히 `generateReproPrefill`이 AI 산출물을 조용히 버리는 동일 계열 선례).
...
| AI가 `description` 키를 누락 | 블록 삽입 안 함. 기존 내용 보존(`mergeAiDraftSections.ts:41`)이 유지돼야 한다. **게이트는 `parsed.sections.description?.trim()` truthy** — 이게 없으면 merge가 살린 사용자 원문 위에 블록이 붙는다 |
| AI가 `description`을 **빈 문자열로** 반환 | **키 누락과 동일 취급 — 블록 삽입 안 함.** merge 경로는 다르지만(`mergeAiDraftSections.ts:41`의 `!aiText && !(id in aiSections)` 가드를 안 탄다) 삽입 게이트가 truthy 판정이라 같은 결과로 수렴한다. 빈 산문은 키 누락과 같은 "실패 신호"이고, 산문 없이 블록만 붙는 기형 출력을 차단한다 |
...
3. 프롬프트 어디에도 원본 `id`가 새지 않는다 — 유닛 테스트로 증명(`buildAiDraftPrompt.test.ts:773`의 인젝션 테스트와 같은 결). **UUID 정규식으로 검사하지 않는다** — `crypto.randomUUID()`가 없으면 `nr-${Date.now()}-…`(`network-recorder.ts:68`)/`cl-${Date.now()}-…`(`console-recorder.ts:47`)로 폴백하므로 정규식은 그 경로를 못 잡는다. `id` 값 자체를 `not.toContain`으로 검사한다.
