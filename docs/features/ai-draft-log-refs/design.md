...
editor-store: networkLog.requests / consoleLog.entries      ← 스냅샷(await 이전에 잡음)
        ...
        │                                          ↓
        │                   ★ 게이트: refs.length && "description" in parsed.sections
...
**`"description" in parsed.sections` 게이트가 필수인 이유**: AI가 그 키를 누락하면 merge는 prev(사용자 원문)를 살리는데(`mergeAiDraftSections.ts:41-44`), 게이트 없이 `appendLogBlocks`를 무조건 호출하면 **그 사용자 원문 위에 블록이 붙는다.** PRD 엣지 케이스 표("블록 삽입 안 함")가 기대 동작이고, 이 게이트가 그걸 구현한다.
...
```ts
// 상한이자 "나열해버림" 임계값. 근거 없는 임의값 — 이슈 리포트가 오면 조정한다(PRD 참조).
...
`stripInlineImageRefs`를 `stripPreservedContent`로 바꿔야 하는 곳은 **AI draft 경로 5곳**이다:
`draftRich.ts:145` · `draftCompact.ts:102` · `promptBudget.ts:41`(`selectionOf`) · `promptBudget.ts:52`(`contentfulSectionsOf`) · `mergeAiDraftSections.ts:32`
...
>
> 이 둘은 AI draft 경로가 아니라 **트래커 export 빌더**다. `sectionRefs.length > 0`일 때 이미지 ref를 지우고 그 자리에 image/placeholder 블록을 재삽입하는 경로이고, 여기를 `stripPreservedContent`로 바꾸면 **이미지가 있는 섹션의 코드블럭이 Notion·Jira 이슈 본문에서 통째로 삭제된다.** 사용자가 수동으로 넣은 로그가 트래커로 안 나가는 조용한 데이터 손실 — 이 feature의 목적과 정반대이고, 위가 경고하는 "기준이 갈리는 틈"을 export 쪽에서 여는 셈이다.
>
...
**더 단순한 대안(검토 대상)**: `selectDraftSections(existingDraft, ids, budget, strip)`의 4번째 `strip` 인자는 **3개 호출처(`promptBudget.ts:41`·`draftCompact.ts:102`·`draftRich.ts:145`)에서 항상 같은 값**이 들어간다. 이 인자를 제거하거나 내부 고정하면 교체 대상이 5곳 → 2곳으로 줄고, 위가 경고하는 "5곳 동기화" 위험이 **구조적으로** 사라진다. 외과적 범위를 넘는다면 채택하지 않되, 다음 변경 때도 같은 부담이 남는다는 걸 안 채로 넘긴다.
...
이 feature가 먼저 나가면 AI가 넣은 ~32.8KB 블록(body당 16KB × 2)이 접히지 않는다. "전문 유지" 결정의 전제가 collapse다. **PRD에서 하드 게이트로 확정** — collapse 구현 전에는 배포하지 않는다. 세로축만 collapse가 담당하며 가로 스크롤은 둘 다 미해결이라는 위임 범위도 PRD "의존" 절에 명시돼 있다.
...
주의할 축 어긋남: `canInsertLogs = sectionIds.includes("description")`는 **enabled** 기준이고 merge의 보호 판정은 `fitted.includedSections`(**prompted**) 기준이다. rich는 `existingDraftChars = UNLIMITED`라 이 어긋남은 compact(400자)에서만 드러난다.
...
`setDraft: (draft) => set({ draft })`(`editor-store.ts:703`)는 히스토리를 안 남기고, Tiptap history는 살아 있으나 `setContent(displayMd, { emitUpdate: false })`(`TiptapEditor.tsx:336-351`)가 doc을 교체하므로 Ctrl+Z로 AI 이전 상태 복원이 보장되지 않는다. 대조군으로 AI 스타일링은 전체/행 단위 리셋이 완비돼 있다(`StyleChangesDialog.tsx:132-186`).

상한 초과로 블록이 안 붙은 사용자의 유일한 대응이 재생성인데(PRD 시나리오 D), **재생성이 본문을 또 덮어쓴다.** 여기에 위험 2(누적 중복)가 겹친다. 이 feature에서 해결하지 않되, 증폭 사실을 기록한다.

### 9. 접근성 — 자동 삽입 블록의 스크린리더 경험

수동 경로는 사용자가 `LogInsertDialog` 안에서 그 로그를 **이미 봤다**(`max-h-[400px]`로 잘린 채). AI 경로는 무엇이 얼마나 큰지 모른 채 결과를 받는다. 수백 줄짜리 `codeBlock`이 예고 없이 생기면 스크린리더·키보드 사용자에게 그 블록을 건너뛸 랜드마크가 없다 — `codeBlock`은 NodeView 없는 StarterKit 기본 노드다(`TiptapEditor.tsx:94-95`). collapse 하드 게이트가 시각적 부피는 흡수하지만 a11y 축은 별개다. 이 feature에서 해결하지 않되, collapse 구현 시 함께 볼 항목으로 남긴다.
