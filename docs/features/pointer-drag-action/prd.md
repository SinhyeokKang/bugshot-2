...
- 드래그 앤 드롭 동작을 액션 로그에 **`drag` 종류 1건**으로 기록한다. 각 건은 **source 요소(드래그 시작)와 target 요소(드롭 지점)** 를 담는다.
- 두 캡처 경로를 모두 커버한다:
  1. **포인터 휴리스틱**: `pointerdown → pointermove(임계 초과) → pointerup` 시퀀스 (마우스·터치·펜 통합, 라이브러리 dnd 커버).
  2. **네이티브 HTML5 DnD**: `dragstart`(source) + `drop`(target) (`draggable=true` 커버).
- 드래그로 판정된 시퀀스 끝에 따라오는 `click`을 억제해 **drag 1건만** 남긴다(이중 기록 방지).
- 액션 로그 UI·로그 뷰어 마커·AI 재현 단계·JSON export 모두에서 drag를 일관되게 표현한다(ko·en).

## 비목표 (Non-goals)

- 드래그 **경로(중간 좌표 스트림)** 기록 — source→target 끝점만. (Jam식 rrweb 좌표 샘플링은 도입하지 않음.)
...
### 시나리오 A — 라이브러리 dnd (포인터 휴리스틱)

1. 사용자가 녹화 모드로 캡처를 시작한다(액션 레코더 활성).
2. 페이지에서 리스트 아이템 A를 마우스로 누르고(`pointerdown`), 끌어서(`pointermove`, 시작점에서 10px 초과 이동), 다른 영역 B에 놓는다(`pointerup`).
3. 레코더가 시퀀스를 drag로 판정 → `pointerup` 지점의 `elementFromPoint`로 target(B)을 식별 → `drag` 엔트리(source=A, target=B) 1건 적재.
4. 뒤따르는 `click`은 억제되어 안 남는다.
5. 액션 로그에 "**A** 을(를) **B** (으)로 드래그"로 표시된다.

### 시나리오 B — 네이티브 HTML5 드래그

1. (녹화 중) 사용자가 `draggable=true` 요소 A를 끌어 드롭 영역 B에 놓는다.
2. `dragstart`에서 source(A) 보류 → `drop`에서 target(B) 확정 → `drag` 엔트리 1건 적재.
...
- **임계 미달**: 10px 미만 이동 후 pointerup → 드래그 아님 → 기존 click 경로로 정상 기록(억제 안 함).
- **우클릭/보조 포인터 드래그**: primary 포인터(`isPrimary` + `button===0`)만 후보.

## 성공 기준

- 라이브러리 dnd 드래그 1회 → 액션 로그에 `drag` 1건(source·target 식별), click 0건.
- 네이티브 `draggable=true` 드래그 1회 → `drag` 1건.
