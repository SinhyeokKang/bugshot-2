...
3. **DOM 트리 탐색** — DOM Tree Dialog를 연다. 태그·id·class가 Geist Mono 13px로 렌더되며, CSS 코드 뷰와 같은 크기·같은 폰트다. 라벨이 폭에 안 맞아 CSS로 잘리면 hover로 라벨 문자열을 볼 수 있다(`title` 툴팁 — 이번에 추가). 단 `formatElementName`이 클래스를 3개로 캡하고 `+N`을 붙이는 건 **라벨 설계**라 툴팁도 동일하게 캡된 문자열이다 — 툴팁이 푸는 건 CSS 트렁케이션뿐이지 "전체 클래스 목록"이 아니다.
