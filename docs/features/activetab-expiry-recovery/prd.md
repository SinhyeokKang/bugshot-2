...
이번 과업의 목적: **30s Replay로 받는 광역 권한을 패널 전반(선제 닫기 판정·캡처 경로)으로 확대 적용해, 권한 만료 다이얼로그·패널 닫힘을 만나는 경우의 수를 최대한 줄인다.** 새 권한 요청 진입점은 만들지 않는다(옵트인은 30s Replay 설정 1곳 유지). 체감 대상은 30s Replay 옵트인을 승인한 사용자로 한정되며, 혜택 고지는 가이드에서 다룬다(새 UI는 비목표).

## 목표

- cross-origin 네비게이션 판정 시 광역 host 권한을 조회해, **보유 시 same-origin과 동일하게 취급**한다 — 패널 유지, deferred 다이얼로그 예약 없음, stale 세션 정리만.
- 광역 권한 보유자는 cross-origin 이동 후에도 캡처가 그대로 동작한다 — 두 겹의 근거: ① 진입 가드(`ensureSupportedTab` → `classifyTabSupport`)는 `tab.url` 가시성 기반인데 광역 보유 시 http/https URL이 가시라 supported로 통과, ② 캡처 시점 가드는 에러 후 분류 방식이라 captureVisibleTab·executeScript가 성공하면 발화하지 않음 (코드 변경 불필요, 자연 효과).
...
광역 권한 보유 = 30s Replay 옵트인을 승인한 사용자(`chrome.permissions.contains`로 판정, Replay 스위치 ON/OFF와 무관 — 권한은 사용자가 Chrome 설정에서 수동 철회하지 않는 한 유지). 수동 철회 시엔 `contains`가 매 판정 직전 조회되므로 다음 네비게이션부터 미보유 경로(S1)로 자연 폴백한다.
...
- cross-origin 이동 **후** 새 녹화 시작: tabCapture는 invoke 필수(Chrome 제약) → 현행 만료 다이얼로그 경로. 광역 권한이 줄여주지 못하는 다이얼로그 케이스(다른 하나는 S5의 보존+비커버 URL).

### S5. 광역 비커버 URL로 이동 (`chrome://`·웹스토어 = 미지원, `file:` = 지원이지만 광역 범위 밖)
- **변경 없음**: 광역 보유 여부와 무관하게 현행 분기를 탄다(비커버 URL은 same-origin 취급 조건 불충족).
  - 비보존 → 패널 닫힘(현행).
  - 보존(drafting 등) → deferred 유지 → idle 복귀 시 만료 다이얼로그(현행). 광역 보유자가 다이얼로그를 만나는 나머지 케이스.
...
- [ ] 광역 보유 시: http/https 간 cross-origin 이동 후 패널이 닫히지 않고, idle 복귀 시 만료 다이얼로그가 뜨지 않으며, 캡처가 다이얼로그 없이 동작한다.
- [ ] 광역 미보유 시: 비보존 cross-origin 패널 닫힘·보존 deferred 다이얼로그·`window.close()` 동작이 현행과 완전히 동일하다(회귀 없음).
- [ ] 광역 비커버 URL(`chrome://`·`file:`) 이동 시 보유 여부와 무관하게 현행 분기(비보존 닫힘·보존 deferred)가 동작한다.
- [ ] 광역 권한을 Chrome 설정에서 수동 철회하면 다음 네비게이션부터 미보유 경로로 폴백한다.
