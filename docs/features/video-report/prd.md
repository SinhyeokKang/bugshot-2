...
영상은 logs.html 안에 dataUrl로 임베드돼 외부 의존성 0인 단일 파일을 유지한다.

## 비목표 (Non-goals)

- **별도 `recording.mp4` 첨부 폐지.** 영상은 logs.html 안에만 있다.
- **타임라인 마커 / 재생바 위 로그 표식 UI** — 후속. (사용자: "추후 타임스탬프 UI를 재생바에 추가하는 식으로.")
- **키보드 단축키**(Space/←→/J·K), **자동 스크롤 + Resume pill** — 후속. 코어는 행 클릭 점프 + 현재 행 하이라이트뿐.
- **Preview 패널 다운로드 진입점** — 제출 자동 첨부만. Preview에 logs.html 다운로드 UI는 추가하지 않는다.
...
저장된 draft가 오래돼 영상 blob이 IndexedDB에서 만료됐거나(`getVideoBlob` null), 구버전 draft라 `videoStartedAt`/`videoEndedAt`이 없는 경우:
...
- 비디오 모드 이슈를 4개 플랫폼에 제출하면 영상이 임베드된 `logs.html`이 첨부되고, `recording.mp4`는 더 이상 첨부되지 않는다.
...
- 신규 Chrome 권한 0건. 새 외부 전송 0건(기존 영상이 logs.html로 합쳐질 뿐, 수집·전송 대상 동일).
...
| 질문 | 결론 |
|---|---|
| 영상을 logs.html에 어떻게 담나 | `__BUGSHOT_DATA__` JSON에 `video.dataUrl`로 임베드. `recording.mp4` 별도 첨부 폐지. |
| 임베드로 logs.html이 ~20MB+가 되면 (제출 첨부 한도) | 첨부 시도 후 실패는 격리 — 이슈 본문·`bugshot.md` 첨부는 정상 진행, logs.html 첨부만 best-effort 누락. (action-recorder/video-report-player와 동일 위험 인식.) |
