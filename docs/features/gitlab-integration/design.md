...
1. **base URL 가변**: gitlab.com과 self-managed를 같은 `gitlab` 플랫폼으로 다루기 위해 모든 GitLab API 호출은 `auth`에 실린 base URL을 prefix로 쓴다. Jira의 `apiKey` auth가 이미 `baseUrl`을 갖는 선례(`messages.ts:356`)와 동일.
...
| `src/sidepanel/tabs/connect/GitlabConnectForm.tsx` | 온보딩(OAuth/PAT 버튼) + PAT 다이얼로그(Instance URL + Token 2필드) + 연결 요약 + 기본값 설정 |
...
| `src/sidepanel/tabs/IntegrationsTab.tsx` | 플랫폼 서브탭 | `PlatformSubTab`·`PLATFORM_ORDER`·`PLATFORM_LABEL_KEYS`에 gitlab, `grid-cols-4`→`grid-cols-5`, TabsTrigger(`SiGitlab`) + TabsContent(`GitlabConnectForm`) |
| `src/sidepanel/tabs/SubmitFieldsDialog.tsx` | 제출 필드 다이얼로그 | configured/canSubmit 분기 + TabsList + 필드 렌더 분기에 gitlab |
...
- client ID: `import.meta.env.VITE_GITLAB_CLIENT_ID`. 누락 시 `isGitlabOAuthConfigured()===false` → OAuth 버튼 숨김.
...
- **테스트 우선**: `gitlab-api.ts`의 순수 함수(`normalizeProject`, `mapCreateIssueBody`, `messageForGitlabStatus`, `buildAuthHeader`, `normalizeIssueStatus`)와 `gitlab-oauth.ts`의 `parseGitlabCallbackParams`/`isGitlabOAuthConfigured` 테스트 선작성.
...
- **exhaustive switch 누락**: gitlab 분기를 빠뜨리면 런타임 fallback(예: PlatformChip이 GitHub로 표시)으로 조용히 틀릴 수 있음. `tsc`의 `never` 체크가 잡는 곳(messages 핸들러)과 안 잡는 곳(if/else 체인)을 구분해 수동 점검.
