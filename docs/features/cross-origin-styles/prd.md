...
> **⚠️ 권한 모델 변경 (supersede)**: 이 문서는 처음에 "`<all_urls>` optional 유지, grant된 사용자만 동작"을 전제로 작성됐다. 이후 `docs/features/all-urls-required/`에서 **`<all_urls>`를 required로 승격**하기로 결정했다. all-urls-required가 먼저 머지되면:
> - 본문의 "이미 grant된 사용자에 한해" 전제는 **모든 사용자가 보유**로 바뀐다.
> - design.md의 `chrome.permissions.contains(<all_urls>)` 게이트는 **불필요**해진다(항상 true) → 구현 시 게이트 생략·단순화.
> - 아래 비목표의 "required 승격 안 함"은 **무효**(all-urls-required가 대체).
>
> 구현 순서: **all-urls-required 선행** → 이 feature.
