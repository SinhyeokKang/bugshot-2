...
- **관련**: `src/content/scroll-capture.ts:isRepeatedPositionedElement`·`beginScrollCapture`·`hideRepeatedElements`, 그물 `src/content/__tests__/scroll-capture.test.ts`·`scroll-capture-dom.test.tsx`, `e2e/capture-methods.spec.ts`, fixture `e2e/fixtures/pages/scroll-capture.html`.
