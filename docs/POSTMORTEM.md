...
- **계열**: `fail-open`
- **그물**: `unit`
- **증상**: 사용자 도달 전(리뷰)에 잡았다. GitHub 레포 검색이 `owner/name`·붙여넣은 URL을 못 찾는 걸 고치려고 입력을 슬래시로 가르는 휴리스틱을 넣었는데, **GitHub 검색 문법을 직접 친 입력까지 그 휴리스틱이 삼켰다** — `repo:facebook/react`가 `react in:name user:repo:facebook`이라는 무의미한 쿼리로 재조립됐다. 이전 코드는 입력을 그대로 흘려 GitHub가 유효하게 파싱했으므로 **개선이 만든 회귀**다. 같은 이유로 Code 버튼이 주는 클론 URL(`react.git`)도 이름이 안 맞아 0건이었다.
- **근본 원인**: 둘이 겹쳐야 이 형태가 된다. ① **입력을 재해석하는 코드가 "이미 우리 문법이 아닌 입력"의 존재를 안 셌다.** 검색창의 입력 우주는 `<이름>`·`<owner>/<name>`·URL 셋이라고 전제하고 갈랐는데, 실제 우주에는 **목적지 서비스의 문법으로 쓰인 입력**이 하나 더 있었다. 이 축은 새 형식을 지원할 때마다 넓어지지 좁아지지 않는다. ② **그 회귀를 같은 커밋의 `fail-open`이 가렸다.** "존재하지 않는 owner를 타이핑하는 중이면 422가 오니 빈 배열로 삼킨다"를 넣었는데, 깨진 쿼리가 만드는 422도 상태 코드가 같아 구분 없이 삼켜졌다 — 즉 **휴리스틱이 만든 실패를 휴리스틱과 함께 들어온 침묵이 흡수**해서, 사용자에게는 영원히 "결과 없음"으로만 보인다. 각각은 방어적 선택이었고 둘 다 테스트로 고정돼 있었는데, **둘의 교집합을 잰 케이스가 없었다**.
- **재발 방지**: (1) **사용자 입력을 재해석하는 함수는 "손대지 않고 통과시킬 입력"의 케이스를 먼저 쓴다** — 목적지가 자체 쿼리 문법을 가진 서비스면(GitHub 검색 qualifier·Jira JQL·Linear 필터) 그 문법으로 쓰인 입력은 우리가 조립할 대상이 아니라 **통과 대상**이다. 판정은 문법 전수 열거가 아니라 한 가지 신호로 좁힌다(여기선 스킴을 뗀 뒤의 `:` 유무). (2) **에러를 상태 코드로 삼킬 땐 "그 코드를 우리 코드가 스스로 유발할 수 있는가"를 묻는다** — 답이 예면 삼킴이 외부 사정이 아니라 **자기 버그의 은폐 장치**가 된다. 삼키는 분기를 넣는 커밋에서 같은 코드를 유발하는 입력 케이스를 함께 테스트에 박는다. (3) **입력 정규화 함수의 테스트는 "바뀌어야 할 입력"과 "바뀌면 안 될 입력"을 같은 describe에 짝으로 둔다** — 전자만 있으면 사정거리가 넓어져도 전부 green이다(`buildRepoSearchQuery`가 정확히 그랬다: 9케이스 전부 "갈라져야 한다" 쪽이었다).
- **관련**: `src/background/github-api.ts:buildRepoSearchQuery`(통과 분기 + `.git` 제거)·`searchRepos`(422 흡수 지점), 그물 `src/background/__tests__/github-api.test.ts`("qualifier를 직접 쓴 입력은 손대지 않고 그대로 보낸다"·"클론 URL의 .git 접미사는 이름에서 뗀다"), 픽스 `4141801c`, 유입 `964a30c5`. 계열 선행: **2026-08-12**(fail-open — 에러 삼킴이 상위 증상을 가림).

---

## 2026-09-16 — CLAUDE.md가 "무방비"라고 스스로 적어둔 번들 경계를 실제로 넘었고, 7000개 테스트와 typecheck가 전부 green이었다

- **영역**: `lib`, `컴포넌트`
- **그물**: `unit`
- **증상**: 사용자 도달 전(리뷰)에 잡았다. 순수 술어 `canCreateIssue`를 `background/github-api.ts`에 두고 사이드패널 컴포넌트(`RepoCombobox`)에서 value import했다. CLAUDE.md가 명시로 금지하는 방향("사이드패널·store는 `@/background/*`를 value import하지 않는다" — 끌면 OAuth 런처·설정 저장소가 패널 번들 그래프에 딸려온다)인데 **`pnpm typecheck`도 `pnpm test` 7000여 건도 전부 green**이었다.
- **근본 원인**: 그 방향을 보는 그물이 **없다**. `src/store/__tests__/bundleBoundary.test.ts`는 이름이 경계 전반을 재는 것처럼 보이지만 실제로 잠그는 건 **store 방향**(store → `@/sidepanel/*`·`@/background/*`·`@dnd-kit/*`)과 `lib/session-keys.ts`의 런타임 import 0, 두 축뿐이다. 사이드패널→background는 스캔 대상이 아니고, **CLAUDE.md 본문이 그 사실을 "여전히 무방비"라고 이미 적어두고 있었다**. 즉 이 함정은 문서에 경고가 있고 규칙도 명문화돼 있는데 **자동 판정만 없는** 자리였고, 그래서 규칙을 아는 채로도 밟힌다 — 번들 크기는 테스트가 재는 값이 아니라서 증상이 실행 중 아무 데도 안 나타난다.
- **재발 방지**: (1) **"문서가 금지하는데 그물이 없다"고 명시된 축은 그물 부재 자체를 grep으로 주기 판정한다** — `grep -n "무방비\|잡히지 않\|그물이 없" CLAUDE.md docs/*.md`로 뽑아 각 축에 대응 테스트가 실재하는지 대조한다(2026-09-09 항목의 "문서의 그물 서술은 별도 전수 절차를 둔다"와 같은 처방, 이번엔 *부재*를 센다는 점만 다르다). (2) **순수 술어를 어느 디렉터리에 둘지는 "누가 쓰나"가 아니라 "누가 쓰게 될 때 무엇이 딸려오나"로 정한다** — 소비처가 현재 사이드패널 하나뿐이어도 판정이 어댑터 옆에 있으면 그 import가 유일한 자연스러운 선택지가 된다. `src/lib/` leaf가 기본값이고, 선례는 `lib/jira-sprint.ts:isActiveSprint`다. (3) **`bundleBoundary.test.ts`의 사정거리를 그 파일 주석에 명시한다** — 이름이 넓고 실제 범위가 좁으면 다음 사람도 "경계는 잠겨 있다"고 읽는다. (미대응: 사이드패널→background 방향 스캔 추가는 이번 범위 밖으로 남겼다.)
- **관련**: `src/lib/github-repo.ts`(승격된 leaf + 사유 주석), `src/sidepanel/tabs/githubFields/RepoCombobox.tsx`(소비처), 사정거리가 좁은 그물 `src/store/__tests__/bundleBoundary.test.ts`, 선례 `src/lib/jira-sprint.ts`, 등재 `docs/DIRECTORY.md`. 계열 선행: **2026-09-09**(문서의 그물 서술이 실제 테스트와 어긋남 — 그쪽은 *존재한다고 적힌 그물이 없었고*, 이쪽은 *없다고 적힌 그물이 정말 없어서 밟혔다*).

---

## 2026-09-09 — 그물 열둘이 공허했고, 그걸 잡은 건 이번에도 유닛이 아니라 뮤테이션이었다 (같은 처방을 세 번 읽고도)

- **영역**: `컴포넌트`, `lib`, `툴체인`
- **계열**: `미검증단언`, `드리프트`
- **그물**: `unit`
- **증상**: 사용자 노출 증상 없음. 사이드패널 `index.html`에 런타임 에이전트용 조작 매뉴얼(`__BUGSHOT_AGENT__`)을 정적 임베드하고 그 주장을 소스에 바인딩하는 스캔을 짰는데, **그 스캔이 자기가 잰다고 이름 붙인 축을 열두 번 안 쟀다.** 전 구간 `pnpm test`·`pnpm typecheck` green이었고, 드러난 건 매번 뮤테이션(구현을 되돌려 red 확인)뿐이다. 그중 하나는 **매뉴얼이 막겠다고 세운 무음 실패를 매뉴얼이 직접 지시하도록 뒤집어도 통과**하는 상태였다.
- **근본 원인**: 열둘이 네 형태로 접힌다. ① **문구 판정의 사정거리를 안 좁혔다(3회 반복)** — 정규식을 매뉴얼 전체에 걸어 순서 계약 문장을 통째로 지워도 다른 문단의 `before`가, URL 문단을 지워도 `chrome.windows.create({ tabId })`의 `tabId`가, 복구 지시를 지워도 pre-arm 문단의 `after a reload`가 대신 매치했다. ② **존재만 재고 방향·매핑을 안 쟀다** — `BEFORE`→`AFTER` 역전, 두 버튼 testid 맞바꾸기, 표 행↔셀렉터 대응이 전부 green. 특히 테스트 주석에 *"매핑을 직접 잰다"*고 써놓고 표 7행 중 3행만 쟀고, 고치면서도 **소스 배선만 조여 매뉴얼 표는 여전히 안 잡히는** 채로 한 번 더 통과시켰다. ③ **판정 대상이 위반을 표현할 수 없었다** — `</script` 가드가 이미 첫 `</script>`에서 잘린 문자열을 검사해 위반 상태가 정의상 불가능했고, 섹션 파서는 `m` 플래그 lookahead 탓에 **모든 섹션을 빈 문자열**로 돌려주면서도 testid 대조가 green이라 죽은 줄 몰랐다. ④ **소스 쪽 판정이 코드가 아닌 것을 셌다** — 이름만 매칭해 `activate*Recorder(` 호출 대신 import 문이, `addEventListener("visibilitychange")` 대신 `removeEventListener`가 매치했고, `codeOnly` 미적용 지점에선 주석 처리된 testid가 "실재한다"로 통과했으며, `<Button[\s\S]*?</Button>` 블록 렉싱은 수집한 5블록 중 **2개가 이미 `<ButtonGroup`에서 시작**하고 있었다(첫 `</Button>`이 우연히 자식을 닫아 green이었을 뿐). 그리고 ④를 고치며 만든 `openingTags`가 속성 문자열 안의 `>`를 태그 끝으로 읽어 **새 구멍을 냈다** — Tailwind 임의 셀렉터(`[&>svg]:size-4`)가 저장소 14곳에 있고 대상 파일에만 0건이라, 한 줄 들어오는 순간 testid 뒤 `onClick`이 통째로 안 보인다. 공통 뿌리는 선행 회고 셋이 이미 적어둔 것과 같다 — **2026-09-09**(1) *"새 그물은 반드시 뮤테이션으로 자기검증한다"*, **2026-09-06**(5) *"스캔은 만들자마자 그 스캔이 잡아야 할 형태를 손으로 먹여본다"*, **2026-08-27**①(소스 스캔이 저장소 자기 포맷에 false green). **셋 다 이번 세션에서 읽고 주석으로 인용까지 했는데, 인용문 아래에서 같은 걸 밟았다**(2026-09-06 (2)가 예고한 실패 모드 그대로).
- **재발 방지**: (1) **문구 축은 짜는 즉시 "그 문장만 지우는" 뮤테이션을 넣는다.** 지웠는데 green이면 사정거리가 넓은 것 — 섹션으로 좁히고, 그래도 green이면 같은 섹션 안에 같은 단어가 또 있는 것이다(실제로 `same window`·`one row per step`이 그랬다). 판정 문구는 **설명문이 아니라 지시구**를 고른다(`/reload the target page/` > `/reload/`). (2) **부정 단언을 짝으로 단다** — 방향이 있는 주장(먼저/나중, 하라/하지마라)은 긍정 매치만으론 못 잰다. `expect(/open the panel before/).toBe(true)` 옆에 `expect(/open (it|the panel) after/).toBe(false)`를 둔다. (3) **판정 대상이 위반을 표현할 수 있는지 먼저 묻는다** — 가공된 문자열(잘라낸 것·정규화한 것)에 가드를 걸면 원리적으로 red가 안 난다. `</script` 검사는 원본 html에서, 섹션 검사는 파서가 실제로 뭘 돌려주는지 `length > 0` 앵커로 함께 잰다. (4) **소스 판정 3종 세트를 기본값으로 쓴다** — `codeOnly()`로 주석 제거 + 호출은 `fn(`까지 + JSX는 **여는 태그만**(`openingTags`: `(?=[\s/>])` 태그 경계, 중괄호 깊이로 `=>`의 `>` 회피, **따옴표 추적으로 속성값 안 `>` 회피**). 자식까지 무는 블록 렉싱은 쓰지 않는다. 신규 스캔을 짤 때 `grep -rn 'codeOnly\|openingTags' src/**/__tests__/`로 기존 처방을 먼저 본다. (5) **하드랩 문서를 재는 정규식은 공백을 접고 판정한다**(`prose()` = `section().replace(/\s+/g, " ")`) — 안 접으면 문장을 안 건드린 리랩만으로 false red가 나고, 개별 `\s+` 땜질은 다음 구문에서 또 샌다. 검증은 **"리랩만 바꾼 뮤테이션이 green인가"**로 한다. (6) **뮤테이션 복구는 `cp` 백업으로 한다.** 대상이 아직 미커밋이면 `git checkout`은 복구가 아니라 **삭제**다 — 이번에 그걸로 뮤테이션 2회가 "적용조차 안 된 무효 측정"이 됐고, 실패 개수가 baseline과 같다는 데서 뒤늦게 알아챘다. 뮤테이션 결과는 항상 **baseline 대비 증분**으로 읽는다.
- **관련**: `src/sidepanel/__tests__/agentManual.test.ts`(`codeOnly`·`openingTags`·`buttonWiring`·`section`·`prose` — 이 파일의 헬퍼 다섯이 전부 위 처방의 산물), `src/sidepanel/index.html`(`__BUGSHOT_AGENT__` 블록), 형제 처방 `src/sidepanel/lib/__tests__/pageUrl-callsites.test.ts:codeOnly`. 선행: **2026-09-09**(그물 넷이 공허 — 뮤테이션 자기검증) · **2026-09-06**(회고를 인용하며 그 처방을 어김) · **2026-08-27**(소스 스캔이 자기 리포 포맷에 false green) · **2026-08-19**(대상 0건이라 정규식이 망가져도 green).

---

## 2026-09-09 — 에이전트가 authoritative하게 읽는 매뉴얼에 거짓 다섯을 실었고, 둘은 매뉴얼 자신의 헤드라인을 무효화했다

- **영역**: `컴포넌트`, `background`, `에디터`
- **계열**: `미검증단언`, `드리프트`
- **그물**: `unit`
- **증상**: 사용자 노출 증상 없음(미출시). `__BUGSHOT_AGENT__` 매뉴얼 초안의 사실 주장 다섯이 소스와 어긋났고, 검증 3벌이 전부 잡았다. 이 문서는 에이전트가 **authoritative하게** 읽으므로 틀린 문장 하나가 곧 무음 오작동이다. 둘은 특히 나빴다 — **캡처 선택표의 첫 행**(`mode-element`)은 `captureLogSupport.ts`가 로그 매트릭스에서 제외하는 모드라(`// element: 로그 없음`), 에이전트가 매뉴얼 헤드라인인 순서 계약을 완벽히 지켜도 그 행을 고르면 **로그 0건으로 정상 제출**된다. 그리고 캡처 섹션은 별도 윈도우 배치를 지시해놓고 *"포그라운드 패널이면 캡처 실패"*라고 적어 **자기 조언과 모순**됐다.
- **근본 원인**: 셋으로 갈린다. ① **매트릭스를 안 보고 표를 썼다** — 캡처 모드별 로그 지원은 `supportsConsoleNetworkLog`/`supportsActionLog`가 단일 출처인데(`screenshot|freeform|video`만 true), 표를 UI 진입 버튼 기준으로 나열하면서 그 축을 안 겹쳐봤다. 같은 형태로 `TiptapEditor.tsx:595`의 `heading: false`를 안 보고 *"headings … survive"*라고 썼다(`## Steps`가 평문 문단으로 뭉개진다). ② **하네스 사정을 제품 동작으로 옮겨 적었다** — *"포그라운드 패널이면 캡처 실패"*의 출처를 역추적하니 `e2e/GOTCHAS.md`와 **POSTMORTEM 2026-07-26**인데, 둘 다 *"e2e의 `openPanel`은 사이드패널을 **탭**으로 연다(실제 제품에선 탭이 아니다)"*라고 **명시**해둔 것이다. `tab.active`는 윈도우별이라 별도 윈도우 배치에선 통과한다. 같은 뿌리로 *"패널을 다시 보이게 하면 레코더가 돌아온다"*도 틀렸다 — 근거인 `visibilitychange` 핸들러는 바로 위 주석이 *"같은 탭으로 (네비게이션 없이) 복귀"* 전제임을 밝히고 있고, 별도 윈도우면 패널이 hidden이 안 돼 안 뛴다(정확한 복구는 대상 페이지 리로드). ③ **"핸들러가 없다"를 "동작이 없다"로 옮겼다** — `OrderedListEditor`에 `onPaste`가 없는 걸 보고 *"ignore pasted markdown"*이라 썼는데, 실제로는 단일행 `<Input>`이라 개행이 뭉개져 **한 행에 통째로** 들어간다. 세 형태의 공통점은 **소스를 읽되 그 값을 소비하는 쪽을 안 따라간 것**이다.
- **재발 방지**: (1) **모드·플랫폼별 표를 쓰면 그 축의 매트릭스 함수를 테스트에서 직접 호출해 고정한다** — `expect(supportsConsoleNetworkLog("element")).toBe(false)`처럼. 표의 행이 늘 때 게이트가 함께 늘었는지는 문서 리뷰로 안 잡히고 호출만이 잡는다. (2) **`e2e/GOTCHAS.md`·POSTMORTEM을 사실 출처로 인용할 때 "이게 제품 동작인가 하네스 사정인가"를 먼저 판정한다** — 두 문서는 **테스트 환경의 전제**를 다수 담고 있고(사이드패널=탭, eval-host 탭 상주 등), 그걸 사용자·에이전트용 산출물에 옮기면 거짓이 된다. 판정 신호: 그 항목이 `e2e/`·하네스 파일을 관련 항목으로 달고 있으면 하네스 사정이다. (3) **핸들러 부재를 동작 부재로 번역하지 않는다** — `onPaste`가 없으면 "무시된다"가 아니라 **브라우저 기본 동작이 일어난다**. 기본 동작을 확인하고 그걸 쓴다. (4) **"이 조언을 따른 상태에서 저 문장이 참인가"를 문서 안에서 교차 검사한다** — 배치·순서를 지시하는 문서는 지시 A와 경고 B가 서로 다른 전제 위에 설 수 있다. 이번엔 같은 문서의 두 섹션이 모순됐고, 그물은 `/active/i` 존재만 봐서 못 잡았다(지금은 `/per window/`+`/same window/`가 잡는다). (5) **전달 경로가 환경마다 같은지 확인한다** — `pnpm dev`에선 crxjs의 `crx:html-inline-scripts`가 인라인 script를 전부 제거해 **매뉴얼이 통째로 사라진다**(빌드 산출물은 원문 보존 실측). 그물이 **소스 파일**을 읽으므로 "산출물에서 소실 ↔ 테스트 green"이 구조적으로 분리돼 있고, dev가 그 상태의 실증이다. 미대응 — post-build 체커(`scripts/check-prearm-chunk.mjs` 패턴, CI `verify` job이 이미 `pnpm build`를 돈다)가 후보다.
- **관련**: `src/sidepanel/index.html`(매뉴얼 본문), `src/sidepanel/lib/captureLogSupport.ts`(`supportsConsoleNetworkLog`·`supportsActionLog` — element 제외), `src/sidepanel/components/TiptapEditor.tsx:595`(`heading: false`), `src/background/capture-throttle.ts:captureOwnedTab`(`if (!tab.active)` — 윈도우별), `src/sidepanel/hooks/useBackgroundRecorder.ts`(재주입 트리거 넷), `src/background/tab-bindings.ts:stopRecorders`, `src/sidepanel/components/OrderedListEditor.tsx`. 선행: **2026-07-26**(같은 `tab.active` 가드 — 그때는 하네스가 제품에 맞춰졌고 이번엔 반대로 옮겨 적었다) · **2026-08-27**②(문서 서술이 코드 변경 없이 조용히 거짓이 됨).
...
- **계열**: `복제본`, `미검증단언`
- **그물**: `unit`
- **증상**: freeform("이슈 작성") 드래프팅 중 탭이 이동해도 재현 환경 `Page` 행이 세션 시작 URL로 굳었다(사용자 제보 — 제출된 이슈의 Page가 `/projects`인데 버그는 `/invite/<token>`에서 났다). 픽스 자체는 리졸버 + 비영속 `livePageUrl`로 단순했다. **회고 가치는 그 픽스의 첫 판본이 생산 지점 여섯 중 하나(미리보기 복사 본문 4분기)를 빠뜨린 채 유닛 전부 green이었고, 그 사실을 리뷰 둘이 독립적으로 잡았다는 데 있다.** 화면엔 이동 후 URL이, 클립보드 복사본엔 세션 원점이 나갔다 — 고치려던 증상과 정확히 같은 형태다.
- **근본 원인**: 세 겹이다. ① **조립을 호출부에 흩었다.** 리졸버는 순수하게 잘 뺐는데 `{captureMode, targetUrl, livePageUrl}` 3필드 조립을 여섯 곳에서 각자 했다. ② **그 위험을 알고도 열거로 관리했다.** `pageUrl.ts` 주석에 *"생산 지점이 넷이라 삼항을 복붙하면 하나가 빠진다"*고 쓰고 **2026-09-04 항목을 인용**했는데, 그 항목의 재발방지 (2)가 바로 *"종착점을 열거로 관리하지 않는다 — 집합 대조 스캔이 다음 그물"*이었다. 인용하면서 처방은 안 따랐고, 열거 자체도 틀렸다(실제 여섯). ③ **모듈 내부만 촘촘하고 모듈 사이가 통째로 비었다.** 리졸버·store·훅은 뮤테이션 6종이 전부 red인데, 발행 배선(App 한 줄)·화면 Page 행·제출 시 레코드 정정·저장 억제 배선은 **지워도 6787건이 전부 green**이었다. 유닛이 각 모듈의 계약만 보고 그것들을 잇는 선을 아무도 안 봤다.
- **재발 방지**: (1) **순수 함수를 뺐으면 "누가 그 인자를 조립하는가"를 같은 커밋에서 하나로 모은다.** 리졸버 추출은 절반이고, state→인자 매핑이 N곳에 남으면 중복은 그대로다. 판정 신호: 새 순수 함수의 호출부가 2곳을 넘고 인자가 전부 같은 store에서 나오면 셀렉터를 함께 만든다. (2) **회고를 인용하는 주석을 쓰는 순간이 그 처방을 실제로 적용했는지 확인할 시점이다.** 인용은 알고 있다는 증거가 아니다 — 이번엔 인용문 바로 아래에서 처방을 어겼다. (3) **"모듈이 잘 테스트됐다"와 "기능이 테스트됐다"를 구분한다.** 모듈 N개짜리 변경은 **모듈 사이 링크마다** 뮤테이션을 넣어본다(`git stash` 없이 직접 지웠다 되돌리는 것으로 충분). 이번에 구멍 4개가 전부 링크에 있었다. (4) **뮤테이션 실험의 빌드 실패를 삼키지 않는다** — `pnpm build:e2e >/dev/null 2>&1`로 조용히 돌리면 typecheck 실패 시 dist-e2e가 옛 빌드로 남아 green이 나고, "뮤테이션이 통과했다 → 그물이 없다"는 **정반대 결론**이 나온다(실제로 한 번 냈다). 뮤테이션은 타입 유효하게 만들고(분기 삭제·반환값 변경), 산출물에서 마커를 직접 센다. `e2e/GOTCHAS.md`에 항목화. (5) **소스 스캔을 만들면 그 스캔이 자기 리포 코드 포맷에 false green인지 즉시 실측한다** — 첫 판본이 줄 시작 앵커라 한 줄 리터럴을 놓쳤고(`f({ …, pageUrl: target?.url })`), 두 번째 판본은 **주석 처리된 호출을 "존재한다"로 통과**시켰다(삭제 뮤테이션이 흔히 주석 처리다). 스캔은 만들자마자 그 스캔이 잡아야 할 형태를 손으로 먹여본다. (6) **병렬 리뷰와 뮤테이션 실험을 동시에 띄우지 않는다** — 정적 리뷰어가 뮤테이션 중인 워킹트리를 읽고 존재하지 않는 🔴을 올렸다. 파일을 쓰는 에이전트는 읽는 에이전트와 시간을 겹치지 않게 배치한다.
- **관련**: `src/sidepanel/lib/pageUrl.ts`(`resolvePageUrl`·`selectPageUrl` — 단일 출처), `src/store/editor-store.ts`(`livePageUrl` 비영속·탭 축·`confirmDraft`), `src/sidepanel/hooks/useTabSupport.ts:useBoundTabState`(판정+URL 한 재조회, store write 없음)·`useLivePageUrl.ts`(발행 배선을 잠그려고 뗀 훅)·`useEditorSessionSync.ts:isLivePageUrlOnlyChange`, `src/sidepanel/tabs/PreviewPanel.tsx`(빠뜨렸던 복사 본문 4분기)·`IssueCreateModal.tsx:handleSubmit`(레코드 정정), 그물 `src/sidepanel/lib/__tests__/pageUrl-callsites.test.ts`(소스 스캔 + `label: "Page"` 앵커 + 주석 제거)·`e2e/page-url-live.spec.ts`(실 탭 네비게이션 — 이 축의 유일한 실브라우저 검증). 선행: **2026-09-04**(같은 `복제본` 계열, 원복 책임 23곳 — 이번에 인용해놓고 재발) · **2026-08-16**(공허한 골든 — 생산자를 우회하는 테스트) · **2026-08-27**(소스 스캔이 자기 리포 포맷에 false green).
...
- **계열**: `복제본`, `미검증단언`
- **그물**: `unit`
- **증상**: `TabsList`가 `bg-muted text-muted-foreground`를 들어 **비활성 탭 라벨이 라이트 4.34:1**(AA 미달)로 렌더됐다. 상속이라 트리거마다 색을 안 줘도 따라오고, 마운트 지점이 18곳(직접 9 + `CollapsingTabsList` 경유 9)이라 **최상위 nav를 포함한 앱 전역**이 해당됐다. 픽스는 `text-foreground/60` 한 줄. 같은 날 앞 항목의 후속이고, 그때 "미해결"로 남겨둔 바로 그 건이다.
- **근본 원인**: 픽스가 아니라 **그물을 만드는 과정**이 두 번 틀렸다. ① 스코프를 `src/components/ui/`로 넓히자 `kbd.tsx`가 red가 됐고, "소비처 3곳이 `CHIP_CLS`로 덮으니 렌더는 통과"라는 **예외를 만들고 그 전제를 잠그는 테스트 3케이스(35줄)를 붙였다.** 리뷰 둘이 독립적으로 같은 답을 냈다 — `kbd.tsx`의 base를 같은 `text-foreground/60`으로 바꾸면 소비처가 이미 덮으므로 **렌더가 한 픽셀도 안 바뀌면서 예외 자체가 사라진다**(프로젝트의 실제 `cn`으로 두 base의 병합 결과 문자열이 동일함을 실측). 게다가 그 전제 테스트는 `tag.includes("text-foreground")` **부분문자열 매칭**이라 `text-foreground/40`(2.70:1)·`hover:text-foreground`를 통과시켰다 — **같은 파일 헤더가 "이름이 아니라 계산으로 판정한다"고 선언해놓고 그 테스트만 이름 매칭이었다.** ② 앞 항목의 재발 방지 (5)("그물 옆에 다른 파일의 현재 상태를 열거하지 않는다")가 **같은 배치에서 두 번 더 재발**했다: 스코프 주석에 "tabs.tsx가 4.34:1로 **렌더된다**"를 현재형으로 적어 바로 다음 커밋에 거짓이 됐고, 같은 사실을 앞 배치에 `docs/DIRECTORY.md`에도 적어둔 걸 잊어 `DESIGN.md`·`POSTMORTEM.md`만 갱신하고 빠뜨렸다.
- **재발 방지**: (1) **스코프를 넓혀 red가 뜨면 "예외를 어떻게 정당화하나"가 아니라 "이 파일을 규칙에 맞출 수 있나"를 먼저 묻는다.** 여기선 규칙을 만족시키는 비용이 **한 줄**이었고 예외를 지키는 비용이 35줄이었다. 판정 기준: 소비처가 이미 값을 덮고 있다면 base를 규칙에 맞춰도 **렌더가 안 바뀐다** — `cn()`으로 병합 결과 문자열을 실측해 확인한다(추론하지 말 것, twMerge의 그룹 판정은 눈으로 못 센다). (2) **예외의 비용은 그 예외를 지키는 코드가 아니라 그 코드가 원칙에서 이탈할 확률까지다.** 전제를 잠그는 장치를 짜게 되면 그 장치가 파일의 판정 원칙(여기선 "이름이 아니라 계산")을 따르는지 **그 자리에서** 대조한다. 값 하한이 본체인 규칙에 `includes()`가 등장하면 그게 신호다. (3) **같은 사실을 두 곳 이상에 적었으면 그 목록을 코드가 바뀔 때 함께 여는 습관이 아니라 grep으로 만든다** — `grep -rn "4.34\|muted-foreground" docs/ src/**/__tests__/`처럼 **수치·토큰명으로** 훑어야 산문 표현이 달라도 걸린다(파일명 기억에 의존하면 이번처럼 셋 중 하나가 빠진다). (4) **회고에 재발 방지를 쓰는 것만으로는 다음 배치를 못 막는다** — 이번이 그 실증이다. 문장으로 남길 게 아니라 **그물이 되는 형태**(테스트·grep)로 바꿀 수 있는지 항목마다 따진다. (5) **예외를 지우면 그 예외가 지키던 축도 함께 사라진다** — 삭제한 3케이스 중 `CHIP_CLS`의 **색 값**을 잠그던 것은 대체가 없었고 뮤테이션으로 드러났다(`CHIP_CLS`를 muted-foreground로 되돌려도 green). `Kbd`가 표면을, 소비처가 글자색을 각각 들어 **리터럴이 두 파일로 갈리는** 조합이라 스캐너가 원리적으로 못 본다. 한 쌍이 고정이라는 점을 이용해 소스에서 뽑아 `"bg-muted " + CHIP_CLS`로 이어붙여 같은 계산에 먹이는 케이스로 복구했다(값을 적어두는 게 아니라 소스에서 재는 것이라 장부가 아니다).
- **미해결(같은 규칙, 그물 밖)**: `src/components/ui/sonner.tsx:28`의 `group-[.toast]:bg-muted group-[.toast]:text-muted-foreground`(라이트 4.34:1)는 **모든 클래스가 variant prefix를 물고 있어** `themeClasses`가 통째로 버린다 — 스코프를 넓혀도 안 보인다. `cancel:` 옵션 사용처가 0건이라 현재는 렌더되지 않는 잠복 코드다.
- **관련**: `src/components/ui/tabs.tsx:15`(`TabsList` — 상속 원점), `src/components/ui/kbd.tsx:8`(예외 대신 규칙 적용), `src/styles/__tests__/muted-surface-contrast.test.ts`(스코프 무제외 + `Kbd`×`CHIP_CLS` 케이스), `src/sidepanel/components/ActionLogContent.tsx:82`(`CHIP_CLS`), `docs/DIRECTORY.md`·`docs/DESIGN.md`(갱신 누락이 드러난 두 곳). 선행: 같은 날 앞 항목(그물 세 판본이 새는 방향으로 틀린 건).

## 2026-08-30 — AA 미달 2줄을 고치는 건 쉬웠고, 그걸 지킬 그물이 세 번 연속 헐거웠다

- **영역**: `디자인`, `lib`
- **계열**: `미검증단언`, `라이브러리전제`
- **그물**: `unit`
- **증상**: 재현 환경 섹션의 읽기 전용 Input 둘이 `bg-muted` 위 `text-muted-foreground`라 라이트에서 4.34:1(AA 4.5 미달). DESIGN.md §2가 이미 금지한 관용구이고 `code-collapse.css`가 같은 함정을 밟은 선례까지 있었는데도 남아 있었다. **픽스 자체는 2줄이다** — 회고 가치는 그 규칙을 자동으로 잠그려 한 그물이 세 판본 연속 **새는 방향으로** 틀렸다는 데 있다.
- **근본 원인**: 셋 다 "판정의 본체가 무엇인가"를 잘못 잡았다. ① **이름으로 금지하면 대체값이 안 잠긴다** — 첫 판본은 "muted 표면 위 muted-foreground 금지"였고, 그러면 채택값 `text-foreground/60`을 `/50`으로 내리는 회귀(라이트 3.69:1)가 통과한다. 규칙의 본체는 토큰 이름이 아니라 **하한**이다. ② **상속받은 근사가 관대한 쪽으로 틀렸다** — `tokens.test.ts`가 알파 합성을 HSL 성분 선형 보간으로 하며 주석에 "base 팔레트라 이 근사가 유효하다"고 적어뒀는데, 라이트 `--foreground`(222.2°/84%)와 `--muted`(210°/40%)는 채도 차가 커서 성립하지 않는다. 같은 조합을 실제 3.69:1인데 **4.36:1로 읽어 AA 미달을 통과로** 판정했다. 브라우저는 sRGB에서 합성하고, 다크는 두 토큰이 무채색이라 두 방식이 같은 값을 내 **라이트에서만** 드러난다. ③ **"마지막 `bg-`가 승자"가 Tailwind와 어긋났다** — 표면을 값 기준으로 넓히며 `classes.filter(c => c.startsWith("bg-")).at(-1)`로 승자를 골랐는데 `bg-cover`·`bg-clip-text`·`bg-gradient-to-r`은 background-**color**가 아니라 서로를 덮지 않는다. `bg-muted … bg-cover`가 무음 통과했고, 이건 **리팩터 직전 코드(`parts.some(...)`)에선 red였다** — 판정을 정교하게 만드는 커밋이 구멍을 냈다.
- **재발 방지**: (1) **그물을 "금지 목록"으로 쓰고 싶어지면 규칙의 본체가 이름인지 값인지 먼저 묻는다.** 대비·크기·시간처럼 **하한/상한이 본체인 규칙**은 목록으로 잠그면 대체값이 통째로 미검증으로 남는다. 판정 함수가 상수 이름을 비교하고 있으면 그 자리에서 의심한다. (2) **근사를 물려받을 때 "유효하다"는 주석을 실측으로 갈음하지 않는다** — 이번엔 두 방식의 값을 같은 입력으로 나란히 출력해 3.69 vs 4.36을 보고서야 드러났다. 값이 **관대한 쪽으로** 어긋나는 근사는 통과를 만들어내므로 red가 안 뜬다. 색 계산은 `src/test/cssContrast.ts` 단일 출처를 쓰고, 알파가 끼면 sRGB 합성인지 확인한다(`grep -n "alpha" src/test/cssContrast.ts`). (3) **Tailwind 클래스를 접두사로 그룹핑하지 않는다** — `bg-`·`text-`는 프로퍼티가 아니라 네임스페이스라 색·크기·위치·클립이 한 접두사를 공유한다. 승자 선택을 넣을 땐 **후보를 색 토큰으로 좁히고**, 좁힌 결과가 놓치는 쪽(팔레트 색 `bg-white` 등)이 **오탐인지 누락인지** 따진다 — 누락은 조용하고 오탐은 red라, 애매하면 오탐 쪽으로 기운다. (4) **판정을 정교하게 만드는 커밋일수록 이전 판정이 잡던 것을 다시 실측한다**(2026-07-31 계열과 같은 형태, 이번엔 대상이 테스트 자신이었다) — 뮤테이션을 **리팩터 전/후 양쪽에서** 돌려 "전에는 red였는데 지금 green"이 없는지 본다. (5) **그물 옆에 다른 파일의 현재 상태를 열거하지 않는다** — 스코프 제외 근거로 14줄 주석에 4개 파일 사정을 적었는데 리뷰가 닿기도 전에 두 군데가 이미 틀렸다(TabsList 소비처 집계가 `CollapsingTabsList` 경유를 빠뜨림, sonner description은 `bg-background` 위라 애초에 통과였다). 아무 테스트도 강제하지 않는 사실은 장부가 되고, 장부는 다음 사람을 오도한다.
- **미해결(같은 규칙, 그물 밖)**: `src/sidepanel/lib/highlightJson.ts:15`의 `JSON_TOKEN_CLASS.null`이 muted 표면 `pre`(`tiptap-editor.css`·`doc-section-body.css`) 안에서 같은 4.34:1인데, **표면은 CSS·글자색은 TS 상수라 리터럴이 갈려 두 그물 다 못 본다** — 리터럴 단위 스캐너의 구조적 사각이다.
- **관련**: `src/styles/__tests__/muted-surface-contrast.test.ts`(`offendingLiterals`·`themeClasses`·`bgColor`·`mutedSurfaceToken`), `src/test/cssContrast.ts`(`contrastOnSurface` — sRGB 합성), `src/styles/__tests__/tokens.test.ts`(알파 합성 소비처), `src/sidepanel/tabs/DraftingPanel.tsx:723,728`(픽스 대상), `src/sidepanel/components/code-collapse.css:37`(같은 함정의 선례·채택값 출처), `src/components/ui/tabs.tsx:15`·`src/components/ui/kbd.tsx:8`(같은 관용구를 적용해 해소한 shadcn 두 곳 — 후속 배치).
...
- **계열**: `fail-open`, `복제본`
- **그물**: `jsdom`
- **증상**: `chrome.storage.session.get(editor:${tabId})`이 reject하면 사이드패널이 **영구 빈 화면**으로 굳는다. `useEditorSessionSync`가 `setHydratedTabId`를 `.then` 안에서만 부르는데 `.catch`가 없어 훅이 `false`를 고정하고, `App.tsx`의 `if (!editorHydrated || !settingsHydrated) return null`이 렌더를 통째로 막는다. 복구 수단은 패널 재오픈뿐이고, 실패가 조용해서(unhandled rejection 외 신호 없음) 사용자에겐 "확장이 안 뜬다"로만 보인다.
- **근본 원인**: **이 실패 모드는 이미 회고에 있었다** — 2026-07-26 "persist storage 실패를 '전파하면 안전'으로 일반화하면 사이드패널이 통째로 빈 화면으로 굳는다"가 `App.tsx`의 그 `if` 한 줄을 정확히 지목했다. 그런데 그때 막은 건 **settings 절반**(`chromeLocalStorage.getItem`의 에러 삼킴)뿐이고 editor 절반은 그대로 남았다. 왜 안 보였나가 핵심이다 — 그 회고의 재발방지가 **`grep -rn 'chromeLocalStorage\|createJSONStorage' src/store`**였다. 즉 전수 대상을 *"zustand persist 어댑터를 쓰는 스토어"*라는 **메커니즘**으로 정의했는데, `useEditorSessionSync`는 persist 미들웨어를 **안 쓴다**(tabId-scoped 키가 persist의 "one store, one key"에 안 맞아 손으로 구현했다 — ARCHITECTURE.md에 그렇게 적혀 있다). 같은 게이트에 물린 두 hydration 중 하나가 다른 메커니즘이라는 이유로 처방의 사정거리 밖이었다. 불변식은 "persist 어댑터가 에러를 삼켜야 한다"가 아니라 **"렌더 게이트를 여는 boolean은 실패로 닫힌 채 남을 수 없다"**인데, 처방이 불변식이 아니라 그 불변식을 어긴 *구현*을 겨눴다.
- **재발 방지**: (1) **재발방지의 전수 대상은 메커니즘이 아니라 불변식으로 정의한다.** 이 축의 옳은 grep은 어댑터 이름이 아니라 **게이트 자체**다 — `grep -n "return null" src/sidepanel/App.tsx`로 렌더를 막는 조건을 전부 뽑고, 그 조건에 들어가는 각 boolean의 **모든 세팅 경로**를 역추적해 실패 분기에서도 반드시 세팅되는지 본다(현재 둘: `settingsHydrated`←`persist.onFinishHydration`, `editorHydrated`←`useEditorSessionSync`). 새 게이트를 그 `if`에 추가하는 커밋이 이 검사의 트리거다. (2) **`void promise.then(...)`에 `.catch`가 없고 그 `.then` 안에서 게이트를 여는 형태는 전부 같은 함정이다** — `grep -rn "void chrome.storage" src/sidepanel`로 뽑아 각 체인의 종단을 확인한다. (3) **삼키되 남긴다** — `chromeLocalStorage.getItem`이 `console.error`를 함께 드는 게 정책의 절반인데 그 절반이 복제될 때 자주 빠진다. 삼키는 catch를 새로 쓰면 로깅을 짝으로 붙인다. (4) catch 스코프를 조회에만 좁히지 **않은 것은 의도**다 — hydrate·마이그레이션이 터져도 빈 화면보다 강등이 낫다. 대신 잔여 위험(조회만 일시 실패하고 스냅샷은 실재했다면 이후 debounce save가 그 키를 덮어쓴다)을 주석과 ARCHITECTURE.md에 남겼다. (5) **이 축은 jsdom으로 잡힌다** — `renderHook` + `get.mockRejectedValue`로 훅 반환값이 `true`가 되는지만 재면 된다. e2e로는 storage reject를 밖에서 강제할 수 없다.
- **관련**: `src/sidepanel/hooks/useEditorSessionSync.ts`(`.catch` → 강등 + `console.error`), 렌더 게이트 `src/sidepanel/App.tsx`, 정책 선례 `src/store/chrome-storage.ts:chromeLocalStorage.getItem`, 그물 `src/sidepanel/hooks/__tests__/useEditorSessionSyncHydration.test.tsx`(신규), 문서 `docs/ARCHITECTURE.md`(§편집 세션 영속화에 불변식 추가). 선행 회고: **2026-07-26**(같은 게이트의 settings 절반 — 이번 항목이 그 나머지 반쪽이다).

## 2026-08-26 — 정적 감사 3건 중 2건은 처방대로 고치면 회귀였고, 남은 1건을 막으려 짠 그물 셋은 각자 다른 방식으로 공허했다

- **영역**: `컴포넌트`, `에디터`
- **계열**: `미검증단언`
- **그물**: `unit`
- **증상**: 사용자 관측 증상 없음 — **진단과 그물의 판정이 틀린** 형태다. `/audit`이 🔴 2건·🟡 1건을 올렸는데 코드와 대조하니 🔴 하나는 기각, 🟡 하나는 절반만 사실이었다. 처방을 그대로 적용했으면 ① cross-page 로그 누적 기능이 죽고 ② DESIGN §14 규정을 위반했을 것이다. 그리고 확정분을 막으려 처음 짠 그물 3개가 전부 통과했는데, 셋 다 자기가 주장하는 것보다 약한 것만 재고 있었다.
- **근본 원인**: 오판 2건이 같은 형태다 — **이름·형태가 같으면 정책도 같다고 전제했다.** ① 감사는 `usePickerMessages`의 `*Recorder.data` 세 핸들러에 `isStalePickerDocument` 게이트가 없다고 🔴을 올렸다. 그 게이트는 이름 그대로 **picker lifecycle 전용**이고(background가 `frameCommitted`를 쏘는 주석에도 그렇게 적혀 있다), 로그는 **정반대 정책**이다(ARCHITECTURE.md "Cross-page 누적" — `onBeforeNavigate`가 네비게이션 직전 로그 꼬리를 *일부러* sync한다). 게이트를 걸었으면 그 꼬리가 통째로 드롭된다. 버려야 하는 경우는 **이미 다른 축**(`shouldClearLogs`→`logClear`→`lastLogClearAt`→`shouldDropPreArmEntry`)이 처리하고 있었다. ② statusBadges 7종에 "잠금 클래스가 없다"고 했지만 `opacity-50`과 스피너는 이미 있었고 실제 갭은 `cursor-not-allowed` 하나였다 — 표준 관용구를 통째로 복사했으면 §14의 "스피너를 든 버튼은 `opacity-50`을 뺀다"를 어긴다. **그물 3건의 공허도 각자 원인이 다르다**: (a) 소스 스캔을 `/<button\b[^>]*>/`로 짰더니 `onClick={(e) => …}`의 화살표가 태그 끝으로 오인돼 전부 `null`이었다(red이긴 했으나 *다른 이유로*였다), (b) 스캔 대상을 파일명 `*StatusBadge.tsx`로 고르고 **그 필터의 출력을 앵커로 다시 대조**해 항등식이 됐다 — 다른 이름의 8번째 배지는 무음으로 빠진다, (c) hydration 회귀 테스트의 "스토어를 건드리지 않는다"가 기대값을 **스토어 기본값**(`idle`/`null`)으로 둬서 "catch가 아무것도 안 한다"와 "catch가 강제 리셋한다"를 구분하지 못했다.
- **재발 방지**: (1) **감사·리뷰 리포트의 발견은 "고칠 수 있다"이지 "고쳐야 안전하다"가 아니다** — 특히 "여기에 저기 있는 가드가 없다"류는 **그 가드의 이름이 무엇을 위한 것인지**와 **이 자리에 이미 다른 축이 있는지**를 먼저 센다. 값싼 판정법: 그 가드의 기존 적용처를 전부 뽑아(`grep -n "isStalePickerDocument" src/`) **공통점이 무엇인지** 본다. 여기선 7곳 전부 picker 메시지였다. (2) **그물의 대상 선정을 파일명·이름 관례에 묶지 않는다** — 필터가 곧 사각이 되고, 그 필터 출력을 앵커로 재확인하면 항등식이다. 내용(속성·import·호출)으로 고르면 새 파일이 자동으로 대상이 된다. 이 저장소가 반복해 밟은 "열거하면 목록 밖에서 샌다"의 **파일명 판**이다. (3) **JSX를 정규식으로 자르지 않는다** — 태그 안엔 `=>`·문자열 속성의 `>`·`{`가 다 들어온다. 중괄호 깊이 + 따옴표 구간 skip이 최소 요건이고, 두 함정을 **자기검증 앵커 케이스로 박아** 스캐너 회귀를 red로 만든다. (4) **기대값이 기본값과 같은 케이스는 단독으로 아무것도 증명하지 못한다**(2026-08-06 재확인) — 비기본값을 심고 그게 **살아남는지**로 잰다. (5) **그물은 작성 직후 뮤테이션으로 비공허를 증명한다** — 이번엔 3건 전부 실측했고(비관례 이름 배지 추가 / 따옴표 skip 제거 / catch가 스토어 리셋), 안 돌렸으면 (b)·(c)는 green인 채로 머지됐다.
- **관련**: 기각 대상 `src/sidepanel/hooks/usePickerMessages.ts`(`isStalePickerDocument` 적용처 7곳은 전부 picker 메시지 — 변경 없음), 정책 `docs/ARCHITECTURE.md`(Cross-page 누적)·`src/sidepanel/lib/log-prearm-filter.ts`, 픽스 `src/sidepanel/tabs/statusBadges/*StatusBadge.tsx`(7벌 — `aria-disabled:cursor-not-allowed`만), 그물 `src/sidepanel/tabs/statusBadges/__tests__/lockAffordance.test.ts`(내용 기반 선정 + 스캐너 앵커 2종 + opacity 금지 반대방향 단언)·`useEditorSessionSyncHydration.test.tsx`, 문서 `docs/DESIGN.md`(§14 잠금 클래스 census 갱신). 계열 선행: **2026-08-06**(기대값을 SUT가 계산해 항진명제) · **2026-08-06**(그물을 같은 시점에 써서 맹점 상속) · **2026-08-20**(지정된 뮤테이션만 돌리면 못 보는 축은 정의상 그 목록에 없다).

## 2026-08-20 — 8명이 병렬로 "에러 전파"를 각자 채우자 공용 헬퍼는 6번 재확인되고 어댑터 고유 배선은 0번 재졌다

- **영역**: `어댑터`, `background`
- **계열**: `복제본`, `미검증단언`
- **그물**: `unit`
- **증상**: 사용자 관측 증상 없음 — **그물의 분포가 틀렸다**는 형태다. 8 어댑터 테스트를 서브에이전트 8명이 병렬로 채운 뒤 교차 뮤테이션을 돌리자 12건 중 **7건이 생존**했다. github는 `Authorization`·`Accept`·`X-GitHub-Api-Version`·`User-Agent`·`cache`를 **전부 지워도 저장소 6291 test가 green**이었고(실패 모드가 401 = 전 기능 정지), jira는 JSON 요청의 `Content-Type`을 지워도 green(Jira는 415를 준다), slack은 `messageForSlackError`의 `invalid_auth`(실제 revoke 시 오는 코드)를 폴스루에서 떼어내도 green이었다. 반대로 공용 `readErrorBody`의 JSON 파싱 실패 폴백은 뮤테이션 **한 방에 5파일이 동시에 red** + 전용 유닛까지 6중이었다.
- **근본 원인**: **"검증 축 3개(인코딩·필수 파라미터·에러 전파)를 각자 채워라"는 지시가 8명에게 같은 문장으로 갔고, 그 축을 채우는 가장 값싼 길이 8명 모두에게 같았다.** 에러 전파는 어댑터마다 공용 `readErrorBody`를 지나므로 아무나 케이스를 쓰면 축이 채워진 것처럼 보이고, 실제로는 8명이 같은 헬퍼를 다섯 번 다시 잰 것이다. 반면 **요청 헤더 배선은 축 목록에 없었다** — 헤더는 "인코딩"도 "쿼리 파라미터"도 아니라 세 축 어디에도 안 걸리고, 그래서 8명 중 아무도 안 쟀다(asana 한 명만 우연히 쟀다). 두 번째 층위도 있다: **목이 관측하지 못하게 만들어진 지점을 목을 고쳐도 못 연다** — asana·github·gitlab의 i18n 목이 *키 문자열*에서 placeholder를 찾는 형태(`k.replace(/\{(\w+)\}/…)`)였는데 `*.error.generic` 키에는 `{status}`가 없어 `t(key, {status})`의 두 번째 인자가 원리적으로 관측 불가였고, 목을 고친 뒤에도 `toContain(키)` 단언이 **2차 그물로 남아** 여전히 통과했다. clickup만 params를 노출하는 목을 쓰면서 *그 함정을 정확히 경고하는 주석*까지 달아뒀는데 나머지 3파일로 전파되지 않았다 — 병렬 작업에서 한 명이 얻은 교훈은 자동으로 퍼지지 않는다.
- **재발 방지**: (1) **N명에게 같은 축 목록을 주면 축의 *교집합*이 과잉 검증되고 *차집합*이 빈다.** 병렬 분배 시 축 목록에 **"이 어댑터에만 있는 것"** 칸을 강제로 넣는다(헤더·인증 배선·재시도·페이지네이션 종료 조건 같은 어댑터 고유 표면). 값싼 판정법: **한 뮤테이션이 여러 파일에서 동시에 red면 그건 중복 그물이고, 그 파일들이 고유 축을 안 쟀다는 신호다** — 배치 끝에 공용 헬퍼 한 곳을 망가뜨려 몇 파일이 우는지 세본다. (2) **"순수 함수 유닛이 있다"는 그 함수가 호출된다는 뜻이 아니다.** `buildAuthHeader`는 유닛이 있었지만 결과가 와이어에 실리는지는 아무도 안 봤다. 전수 대상: `grep -rn "buildAuthHeader\|Authorization" src/background/*-api.ts` 대비 `__tests__`에서 그 헤더를 **요청 기록으로** 단언하는 파일 수. (3) **목의 관측 가능성을 먼저 실측한다** — 목을 새로 쓰거나 복제할 때 "이 목으로 그 인자를 볼 수 있나"를 뮤테이션 1건으로 확인하고 시작한다. 그리고 목을 고쳐도 `toContain`·`toMatch` 같은 **느슨한 단언이 2차 그물로 남아** 같은 축을 계속 가린다 — 목과 단언을 **같이** 조여야 열린다. (4) **픽스처가 두 분기를 동시에 구분하는지 센다** — clickup `setTaskCompleted`의 `statuses` 2원소 배열로는 "조건을 만족하는 첫 원소"와 "그냥 첫 원소"를 한 배열에서 동시에 가를 수 없다(어느 한쪽은 늘 `[0]`과 일치한다). 순서를 뒤집는 게 아니라 **픽스처를 두 벌로 나누는** 게 정답이다. (5) **계획서가 지정한 뮤테이션은 가설이다** — 이 배치에서 지정 뮤테이션 4건이 대상 자체가 없었다(linear는 variables 키가 `teamId`가 아니라 `id`, gitlab은 인코딩이 0건, notion `searchDatabases`는 페이지네이션이 없고, asana의 구조분해는 등가 뮤턴트). 서브태스크마다 **지정 목록 밖에서 최소 1건**을 고르는 규칙이 이 4건을 전부 드러냈다.
- **관련**: 그물 `src/background/__tests__/{github,jira,slack,clickup,asana,gitlab,linear,notion}-api.test.ts`(8벌 — 헤더·`Content-Type`·alias 코드·URL·i18n 목·픽스처 분리)·`notion-expand-block.test.ts`(신규), 공용 `src/test/fetch-mock.ts`(이 배치용으로 신설), 계획 `docs/features/coverage-ratchet-recovery/tasks.md`(Task 5 머리말에 "지정 mutation은 가설" 정정 등재). 계열 선행: **2026-08-20**(지정된 뮤테이션만 돌리면 테스트가 못 보는 축은 정의상 그 목록에 없다 — 같은 배치의 앞 항목, 이번엔 *병렬 분배*가 그 구멍을 8배로 키웠다) · **2026-08-14**(중복을 이름으로 세면 매번 모자란다) · **2026-08-06**(그물을 지킬 코드와 같은 시점에 써서 맹점 상속).
## 2026-08-20 — Radix ScrollArea의 `display:table` 안에서 `break-words`가 min-content를 안 줄여 긴 URL이 가로로 잘렸고, 그 그물은 넓은 뷰포트에서 공허했다

- **영역**: `컴포넌트`, `e2e`
- **계열**: `라이브러리전제`, `미검증단언`
- **그물**: `시각`
- **증상**: 내보낸 `logs.html`의 Action 탭에서 navigation 행의 긴 URL이 접히지 않고 가로로 넘쳐 **잘린 채** 보였다. 가로 스크롤바가 없어 넘친 부분에 닿을 방법이 없다 — Radix가 네이티브 가로 스크롤바를 숨기고 `ScrollBar`도 vertical만 렌더하기 때문이다. 사용자가 URL 전체를 볼 수 없으니 이슈 재현 경로가 통째로 유실된다. `pnpm test`·typecheck 전부 green이었다.
- **근본 원인**: 두 겹이다. ① **프리미티브가 조용히 얹는 box 모델**: `ScrollArea.Viewport`는 자식을 `<div style="min-width:100%; display:table">`로 감싼다(shadcn 래퍼엔 안 보이고 Radix 내부에 있다). table은 shrink-to-fit이라 폭이 콘텐츠를 따라간다. ② **`overflow-wrap:break-word`는 스펙상 min-content 폭에 기여하지 않는다** — 줄은 접히지만 요소의 *자연 폭*은 URL 전체 길이 그대로다. 그래서 shrink-to-fit 컨텍스트에 놓이는 순간 table이 뷰포트 밖으로 밀린다. `overflow-wrap:anywhere`(와 `word-break:break-all`)만 min-content를 실제로 줄인다. 형제 로그 컴포넌트 4곳(`ConsoleLogContent`·`NetworkLogContent` 2곳·`TimelineRow`)은 전부 `break-all`이라 무사했고 `ActionLogContent`만 `break-words`로 혼자 샜다 — **같은 ScrollArea 안 형제인데 wrap 전략이 갈렸다는 게 유일한 신호였다.** ③ 그리고 그 그물을 처음 짰을 때 **넓은 기본 뷰포트에서 vacuous green**이었다: 픽스를 되돌려도 21/21 통과. 실측하니 단일 패널(1256px)에선 URL 자연 폭이 2129px인데도 overflow가 **0**이고, `screenshot`을 주입해 분할(495px)로 열어야 174px가 나온다. 즉 이 버그는 **패널이 좁을 때만** 재현되는데, 사용자가 본 화면이 영상 분할 모드였다는 사실을 spec에 옮기지 않고 "긴 URL이면 재현된다"로 전제했다.
- **재발 방지**: (1) **`ScrollArea` 안에서 긴 불가분 토큰(URL·selector·해시·base64)을 그리는 표면은 `break-words`를 쓰지 않는다** — `grep -rn "break-words" src/` 결과 중 조상에 `ScrollArea`가 있으면 냄새다. 지금 남은 `break-words` 2곳(`DocSectionBody`·`IssuePreviewView`)은 산문 표면이라 무관하지만, 로그·코드 표면에 새로 얹으면 같은 함정이다. 판정 질문은 "이 문자열에 공백이 있나"이지 "길어 보이나"가 아니다. (2) **shrink-to-fit 컨테이너(`display:table`·`inline-block`·`float`·grid `auto` 트랙) 안에서는 `break-word`와 `anywhere`가 다르게 동작한다** — 두 값을 동의어로 취급하지 않는다. 컨테이너가 available width를 주면 둘이 같아 보여서, 넓은 화면 육안 확인으로는 절대 안 갈린다. (3) **"넘치지 않는다"를 재는 e2e는 넘칠 수 있는 폭에서 재야 한다** — `overflow > 0`을 전제로 쓰는 축(2026-xx `/e2e-bigjson` 행 번호)의 **반대 방향 판**이라 공허해지는 방향이 뒤집힌다. 재현 조건 자체(`링크 자연 폭 > 패널 폭 × 2`)를 전제로 단언해 픽스처가 짧아지거나 레이아웃이 넓어지면 **판정보다 먼저** red가 나게 한다. 자연 폭은 요소를 제자리에 복제해 `white-space:nowrap`으로 펴서 잰다(서체·크기를 물려받아야 정확). (4) **revert-후-red를 실제로 돌린다** — 이번 vacuous green은 그 절차 하나로 잡혔고, 안 돌렸으면 "회귀 가드"라는 이름의 빈 테스트가 머지됐다. (5) **e2e 셀렉터를 그 테스트가 검증하지 않는 축에 묶지 않는다** — mono 실측 테스트가 행을 `span.break-words`로 잡고 있어서 서체와 무관한 wrap 클래스 변경에 함께 죽었다. 레이아웃 슬롯(`span.flex-1`)처럼 검증 축과 독립된 앵커를 쓴다. `grep -rn 'span\.\(break-\|truncate\|overflow-\)' e2e/`가 이 부류다. (6) **이 축은 유닛으로 원리상 못 잡는다** — jsdom엔 레이아웃 엔진이 없어 `scrollWidth`/`clientWidth`가 항상 0이다(프로브로 실측 확인). `toHaveClass` 단언도 구현 복사라 그물이 아니다 — ScrollArea를 교체하거나 Radix가 `display:table`을 걷어내면 무음 통과한다.
- **관련**: `src/sidepanel/components/ActionLogContent.tsx:ActionRow`(텍스트 span — `break-words` → `[overflow-wrap:anywhere]`), 형제 대조군 `ConsoleLogContent.tsx:227`·`NetworkLogContent.tsx:509,581`·`log-viewer/components/TimelineRow.tsx:66`(전부 `break-all`), 프리미티브 `src/components/ui/scroll-area.tsx`(Radix Viewport의 `min-width:100%; display:table`은 이 래퍼 밖에 있다), 그물 `e2e/logview/log-viewer.spec.ts`("긴 URL 액션 행")·`e2e/logview/fixtures.ts:makeLongUrlActionLog`/`LONG_NAV_URL`/`TINY_PNG`, 함정 기록 `e2e/GOTCHAS.md`. 계열 선행: **2026-07-20**(shadcn `Kbd`의 `inline-flex`가 `truncate`의 ellipsis를 무력화 — 프리미티브가 조용히 얹은 box 모델이 CSS 속성의 전제를 깬 같은 축. 그때는 `display:inline-flex`, 이번엔 Radix의 `display:table`) · **2026-08-16**(`text-mono`가 `cn()`의 twMerge에서 조용히 삭제 — 같은 컴포넌트, 같은 "className은 맞는데 렌더가 다르다" 계층).
...
- **계열**: `미검증단언`
- **그물**: `unit`
- **증상**: 사용자 관측 증상은 없다 — 둘 다 **계획과 그물이 거짓이었다**는 형태다. (a) `docs/features/coverage-ratchet-recovery/`의 prd→design→tasks가 *"jira만 2차 본문 갱신 실패 격리가 없다"* 를 전제로 **Task 8-2(픽스)** 와 의존 간선(`8-2 → Task 7의 jira 행`)을 세웠는데, `background/messages.ts:850-868`엔 `try { … } catch { console.warn }` + `{key,url,logsDropped}` 반환이 `b4b98df9`(2026-04-24) 이래 있다. 그대로 진행했으면 "재현 테스트가 예상외 green" → 없는 결함을 만들거나 no-op 커밋으로 끝난다. (b) 래칫 회복용으로 판 신규 그물에서 4축이 공허했다 — `update*Account` 8행 표가 병합 **방향**도 **교체 의미론**도 안 잠갔고(스프레드를 뒤집어도, `defaults`를 딥머지해도 green), persist `migrate` 콜백은 never-called이라 `migrateV1ToV2` 호출을 통째로 지워도 green이며, `"4xx/5xx"` 라는 이름의 대조군 픽스처엔 4xx가 없어 임계값을 `>= 500`으로 옮겨도 green이었다.
- **근본 원인**: 둘이 같은 형태다 — **"확인했다"의 단위가 문장이지 실측이 아니었다.** ① 기획 문서는 어댑터 5벌을 훑으며 "격리 관용구가 4갈림(try/catch · `.catch(()=>null)` · 블록 감싸기 · **격리 없음**)"이라는 표를 만들었는데, 앞의 3갈림은 `submitTo*.ts` 실물에서 읽었고 4번째 칸만 *"jira는 사이드패널이 아니라 background 안"* 이라는 **위치 정보**를 격리 부재로 옮겨 적었다. 위치가 다르면 확인 비용이 올라가고, 그 순간 표의 한 칸이 관찰에서 추론으로 바뀐다 — 그리고 그 칸이 픽스 태스크 하나와 태스크 간 간선의 유일한 근거였다. ② 그물 쪽은 tasks.md가 **태스크마다 뮤테이션을 1~2건 지정**해뒀고 그게 전부 red였다. 지정된 뮤테이션은 테스트를 쓰면서 머릿속에 있던 축이라 항상 red다 — **테스트가 못 보는 축은 정의상 그 목록에 없다.** 실제로 공허를 드러낸 건 지정 목록 밖에서 고른 뮤테이션(스프레드 뒤집기·딥머지·호출 제거·임계값 이동)이었다. 그리고 그 공허의 구체적 형태는 전부 **픽스처가 구분을 만들지 않은 것**이다: stub의 `defaults`가 `{}`라 딥머지와 교체가 같은 결과를 내고, jira stub엔 patch가 건드리는 키가 없어 병합 방향 두 개가 같은 결과를 내며, 픽스처에 4xx가 없어 두 임계값이 같은 결과를 낸다. 커버리지는 **줄이 실행됐는지**만 세므로 이 넷을 하나도 못 본다 — 네 경우 다 해당 줄은 실행된다.
- **재발 방지**: (1) **설계 문서의 표에서 "없다/안 한다" 칸은 있는 칸보다 비싸다** — 부재는 파일 하나를 열어선 확인이 안 되고 호출 경로 끝까지 따라가야 한다. 표를 옮겨 적을 때 부재 주장에는 근거 커밋이나 `file:line`을 함께 박고, 그 칸 위에 태스크를 세울 거면 **착수 첫 단계가 그 칸의 재확인**이어야 한다(`git log -S "<주변 문자열>"`이 "언제부터 있었나"를 1분에 답한다). (2) **tasks.md가 지정한 뮤테이션은 그물의 하한이지 검증이 아니다** — 각 그물마다 **지정 목록 밖에서** 최소 1건을 골라 실측한다. 고르는 법이 있다: 그 SUT의 분기·연산자·호출 중 *테스트를 쓰면서 생각하지 않았던 것*을 고른다(스프레드 순서, `??`/`||`, 비교 임계값, 함수 호출 자체의 존재). (3) **픽스처가 구분을 만드는지 먼저 묻는다** — "이 두 구현이 다르면 이 픽스처에서 출력이 다른가"에 답할 수 없으면 그 케이스는 커버리지만 올린다. 전수 대상: 기대값이 빈 컬렉션이거나(`defaults: {}`·`errors: []`) 임계값 근처 값이 빠진 픽스처. `grep -rn "defaults: {}" src/**/__tests__/`가 이번 케이스의 형태다. (4) **테스트 이름이 약속한 입력이 픽스처에 실제로 있는지 센다** — `"4xx/5xx"`·`"A와 B"`처럼 이름이 집합을 열거하면 그 원소가 픽스처에 하나씩 있어야 한다(2026-08-19의 "테스트 제목이 버그를 계약으로 굳혔다"와 같은 축, 이번엔 제목이 *커버 범위*를 과장한 쪽). (5) **zustand persist `migrate` 같은 콜백은 단계 함수를 export해 테스트해도 배선이 안 잠긴다** — 단계를 직접 부르는 테스트와 별개로 `useSettingsStore.persist.getOptions().migrate!(payload, version)`을 태우는 케이스를 둔다. 전수 대상: `grep -rn "migrate:" src/store/`의 콜백들.
- **관련**: `src/background/messages.ts:850-868`(전제가 틀렸던 jira 2차 write 격리 — 무변경), 기획 `docs/features/coverage-ratchet-recovery/prd.md`·`tasks.md`(8-2 취소·간선 3개로 정정), 그물 `src/store/__tests__/settings-store.test.ts`(8행 표에 stale 키 + `persist.getOptions().migrate` 5케이스)·`src/store/__tests__/blob-db-logs.test.ts`(값 팩토리를 `save`로 접어 `never` 캐스트 8개 제거 — 팩토리 오배선이 이제 `TS2345`)·`src/sidepanel/lib/__tests__/buildEditorCapture.test.ts`(404 추가)·`bodyOutputEdgeAxes.test.ts`(죽은 `mode` 인자 제거·축 B 사정거리 명시)·`submitToClickup.test.ts`(전수 표 주석 정정). 계열 선행: **2026-08-19**(전수 스캔의 대상 집합이 비면 항진명제 — 같은 *픽스처가 구분을 안 만든다* 축) · **2026-08-19**(design.md 스니펫의 동일성 비교를 실측 없이 옮겨 무효 픽스 — 같은 문서 계열, 이번엔 같은 기획 디렉터리) · **2026-08-15**(`toEqual`이 무엇을 배제하는지 안 물었다).
...
- **계열**: `복제본`, `라이브러리전제`
- **그물**: `e2e`
- **증상**: 이슈 Description에 인라인 이미지 3장을 넣고 마크다운 복사 → **Notion·Slack·Jira 세 곳 모두 붙여넣기가 완전히 비었다.** 이미지만 빠진 게 아니라 제목·Environment·재현 단계·로그 요약까지 본문이 전부 유실됐다. 복사 버튼은 "복사됨"으로 정상 전환돼 실패가 무음이었다. 짝으로, 복사본의 Media 섹션은 `(첨부 이미지 참조)`, 로그 요약은 `logs.html에서…`를 말했지만 **클립보드에 그런 파일은 없다.**
- **근본 원인**: 두 개가 겹쳤다. ① **복사 경로가 제출 경로 빌더를 재사용하는데 복사엔 첨부가 없다** — `MarkdownContext`에 복사/제출 구분 축이 없어 `md.imageAttached`·`logSummary.logs.lead`가 존재하지 않는 파일을 가리켰다(제출에선 넷 다 참이라 여태 안 걸렸다). ② **리치 에디터는 클립보드 `text/html`에 `data:` 이미지가 하나라도 있으면 paste를 통째로 버린다** — 실패 모드가 "일부 손실"이 아니라 "전부 손실"이다. 클라이언트 온리라 대신 쓸 호스팅 URL이 없어 **복사로 이미지를 전달할 방법이 원리적으로 없다**(제출은 플랫폼 API 업로드라 무관). **진단에서 그럴듯한 가설이 두 번 틀렸다**: (a) "3앱이 같이 실패하니 우리 write 버그" → `paste` 이벤트 + `clipboardData`로 실측하니 `text/plain` 213,702 + `text/html` 214,070 chars가 **둘 다 정상 얹혀** `clipboard.write`는 무죄였다(`navigator.clipboard.read()`는 DevTools 포커스 때문에 `NotAllowedError`라 못 쓴다). (b) "메타 주석이 `<html>` 밖으로 밀렸고 `<!--StartFragment-->`가 없다" → **이미지 없는 리포트가 같은 구조로 정상 붙어** 반증됐다. 가른 건 대조 실험 둘이다 — 이미지 0장은 정상, **수 KB짜리 거의 빈 이미지 1장으로도 재실패**(크기 무관, `data:` 자체가 원인).
- **재발 방지**: (1) **축은 optional로 두고 부재 = 현행 동작으로 만든다** — `forClipboard`를 `MarkdownContext`/`LogSummaryContext`에 optional로 넣었고, `emitMarkdownLogSummary`를 공유하는 제출 빌더 4개(linear·clickup·markdown·asana)가 축을 안 실으므로 새 호출부가 잊으면 회귀가 아니라 현행으로 떨어진다(fail-safe 방향). 생산지는 `PreviewPanel` 하나뿐이고 `buildReportData`(logs.html)는 **의도적으로 안 켠다** — 단일 파일이라 `data:`가 유일한 렌더 수단이라 두 호출부의 요구가 상반된다. (2) **첨부 문구는 화이트리스트로 잠근다** — `attachmentPhraseScan.test.ts`가 문구 키를 쓰는 **파일**이 축 경유/제출 전용 면제 중 하나로 분류되지 않으면 red, 죽은 면제 항목도 red. 열거는 다음 문구에서 구멍이 난다. **줄 단위로 좁히지 않은 게 의도다** — 로그 리드의 3항이 두 줄로 갈려서, 줄 단위 판정은 구현 형태를 강제한다. (3) **축 켜짐/꺼짐을 쌍으로 단언하고 "뒤집으면 출력이 달라진다"를 별 케이스로 둔다** — 축을 무시하는 구현이 공허하게 통과하는 걸 막는다(픽스 전 실측: 그 케이스만 fail). (4) **e2e 스텁의 "읽는 범위"는 타입 축과 내용 축 둘이다** — 2026-08-19 항목이 `text/plain`만 읽어 html flavor 삭제가 green이던 걸 고쳤는데, 이번엔 html을 읽으면서도 `</p>` 존재만 봐서 `data:` 이미지를 못 봤다. `freeform-draft.spec.ts`가 인라인 이미지를 삽입하고 **편집 화면의 `img` 가시성과 복사본의 `data:image` 부재를 함께** 단언한다(한쪽만 보면 어느 절반이든 조용히 회귀). 뮤턴트로 red 실측: 복사 경로에 `resolveSectionImages`를 되돌리면 그 단언에서 빨개진다. (5) **정규식 치환값은 콜백으로 넘긴다** — `placeholderInlineImages`가 문구를 그대로 주면 안내 문구의 `$&`·`$1`이 특수 치환으로 해석된다. (6) **문구 생성은 `withLocale` 안에서** 한다 — 호출부의 훅 기반 번역기로 만들면 화면 언어가 박혀 `bodyLocale`과 갈린다(치환을 빌더 진입점 `forClipboardSections`에 둔 이유). (7) **소스 스캔 그물이 있는 파일에선 주석에 호출 표기를 쓰지 않는다** — `builderLocaleWrap.test.ts`가 export 세그먼트를 "다음 최상위 선언까지"로 자르므로, 새 헬퍼 위 주석이 **앞 세그먼트에 속해** 그 안의 `t()` 문자열이 래퍼 밖 누출로 읽혀 false red가 났다. (8) **픽스가 기존 주석을 거짓으로 만들었는지 본다** — IDB 왕복을 없애자 "본문 조립의 첫 await가 IndexedDB 왕복이라…"가 사실이 아니게 됐고, 그대로 두면 다음 사람이 "이 구조는 불필요하다"고 판단한다(구조는 여전히 load-bearing — 조립에 await가 다시 생기면 무음 실패가 돌아온다). `/code-review`가 잡았다.
- **관련**: `src/sidepanel/lib/buildIssueMarkdown.ts`(`MarkdownContext.forClipboard`·`forClipboardSections`·media 4지점·`emitLogSummaryHtml`), `issueBodyShared.ts`(`LogSummaryContext.forClipboard`·`emitMarkdownLogSummary`), `resolveInlineImages.ts`(`placeholderInlineImages`·`placeholderSectionImages` — `resolveSectionImages`는 표시·logs.html용으로 잔존), `src/sidepanel/tabs/PreviewPanel.tsx:buildForCopy`, `src/i18n/namespaces/logs.ts`(키 4개), 그물 `src/sidepanel/lib/__tests__/attachmentPhraseScan.test.ts`(신설)·`e2e/freeform-draft.spec.ts`·`e2e/GOTCHAS.md`. 선행 회고: **2026-08-19**(스텁이 읽는 범위가 곧 계약 — 타입 축), **2026-07-14**(단일 출처 우회 1곳이 남아 침묵 첨부), **2026-06-25**(본문이 첨부를 참조 못 해 고아 첨부).
