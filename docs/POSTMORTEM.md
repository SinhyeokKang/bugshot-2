...
- **관련**: `.github/workflows/ci.yml`(`e2e` job의 shard 스텝 — `--` 제거 + 사유 주석), `e2e/README.md`(실행 섹션 — `--`는 이름 필터에만), `e2e/playwright.config.ts`(샤딩 대상 config), `docs/features/ci-e2e-gate/`(설계·태스크 문서 동일 수정). 계열: **2026-07-28**(vacuous green — 검증이 실제로 무는지), **2026-07-16**(미검증 단언이 설계에 박힘).
