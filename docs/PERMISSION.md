...
`<all_urls>`가 required라 광역 host 권한은 **항상 보유**한다. 새 URL이 커버 범위(http/https 지원 URL)면 cross-origin도 same-origin처럼 패널 유지 — `<all_urls>`가 captureVisibleTab 캡처 권한을 실제로 주므로 만료 자체가 발생하지 않는다. 아래 경로는 **`file:` 등 비커버 URL로의 이동에만** 적용된다(미지원 URL은 이제 패널을 유지하고 안내를 그린다 — § 패널 종료/유지 정책)(`<all_urls>`는 file:을 포함하지만 캡처는 별도 "파일 URL 액세스" 토글을 요구 — `isBroadCoveredUrl`이 `chrome.extension.isAllowedFileSchemeAccess()` 실측값을 받아 **토글 ON일 때만** file:을 커버로 친다. 즉 토글 OFF인 file:만 이 경로를 탄다).
...
| cross-origin + 커버 URL(http/https 지원 URL, 또는 파일 접근 토글 ON인 `file:`) | 보존/비보존 무관 | same-origin과 동일 취급 — 패널 유지, 비보존이면 stale 세션만 제거. deferred 미발생 |
| cross-origin + 비커버 + **판독된 미지원 URL**(`chrome://`·웹스토어) | 비보존 | **패널 유지** + 세션만 제거 — 패널이 "캡처할 수 없습니다" 안내를 그린다 |
| cross-origin + 비커버 + **판독된 지원 URL**(`file:`, 파일 접근 토글 OFF) | 비보존 (idle 포함) | 패널 닫기 + `setActivated(false)` + 세션 제거 |
...
`chrome.permissions.contains` 조회는 제거됐다(`broadGranted=true` 고정). `file:`은 지원 URL이고 광역 커버 여부는 Chrome '파일 URL 액세스' 토글에 달렸다 — `deactivatePanelIfCrossOrigin`이 `chrome.extension.isAllowedFileSchemeAccess()`를 실측(실패 시 false 폴백)해 넘긴다(토글 ON이면 커버·유지, OFF면 닫힘/만료 분기). 미보유 분기는 프로덕션 도달 불가 — `resolveNavigationAction` 순수함수 테스트의 회귀 자산으로만 남는다.
