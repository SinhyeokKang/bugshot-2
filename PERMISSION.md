...
```
<all_urls>                         — 모든 페이지 (picker·로그 레코더 주입 + captureVisibleTab(30s Replay) + BYOK LLM·GitLab self-managed 임의 origin fetch)
...
|---|---|---|
| `<all_urls>` | 모든 페이지 picker·로그 레코더 주입 + `captureVisibleTab`(30s Replay) + BYOK LLM·GitLab self-managed 임의 origin fetch | `picker.ts`, `recorders-entry.ts`, `background/messages.ts`(captureVisibleTab), `ai-provider.ts` |
