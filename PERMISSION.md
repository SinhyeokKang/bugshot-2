...
| 트리거 | 코드 위치 |
|---|---|
| 툴바 아이콘 클릭 | `tab-bindings.ts:155` — `chrome.action.onClicked → activateTab()` |
| `Cmd+Shift+E` 단축키 | `_execute_action` → 아이콘 클릭과 동일하게 `action.onClicked` 발화 |
| 컨텍스트 메뉴 "BugShot" 클릭 | `background/index.ts:127` — `contextMenus.onClicked → activateTab()` |

### 의존 API

| API | 용도 | 사용 위치 |
|---|---|---|
| `chrome.tabs.captureVisibleTab()` | 요소·영역·30s Replay 스크린샷 | `background/messages.ts:101` (bg handler) |
...
| `chrome.scripting.executeScript()` | content script 재주입·뷰포트 측정 | `picker-control.ts:38,63,381` |
...
```
background/index.ts:83 — disableGlobalSidePanel()
...
`shouldPreserveSession()` (`tab-bindings.ts:54`): 네비게이션 중에도 패널을 유지할 captureMode/phase 조합.
...
| 함수 | 위치 | 필요 env |
|---|---|---|
| `isOAuthConfigured()` | `oauth.ts:209` | `VITE_ATLASSIAN_CLIENT_ID` + `VITE_OAUTH_PROXY_URL` |
...
env 누락 시 해당 플랫폼의 OAuth 버튼이 UI에서 자동 비활성화.
...
API Key/PAT 모드는 OAuth 인프라(refresh, proxy, identity API)를 일절 거치지 않는다.

---

## 8. commands

4개 단축키 등록 (Chrome `suggested_key` 상한).

| 커맨드 | 기본 키 (Mac) | 동작 |
|---|---|---|
| `_execute_action` | `Cmd+Shift+E` | 사이드 패널 토글 (`action.onClicked` 발화) |
| `capture-element` | `Cmd+Shift+S` | 요소 선택 캡처 |
| `capture-screenshot` | `Cmd+Shift+F` | 화면 영역 캡처 |
| `capture-video` | `Cmd+Shift+X` | 영상 녹화 |

### 캡처 단축키 흐름

```
chrome.commands.onCommand (background/index.ts:142)
  → runtime.sendMessage({ type: "shortcut.capture", command, tabId })
  → useCaptureShortcuts.ts:15 (sidepanel)
      → resolveCaptureShortcut(command, state) — 게이트 조건:
          phase === "idle" 또는 (element 모드 + 미선택)
      → startPicker() / startAreaCapture() / startVideoCapture()
```

### 단축키 미할당 대응

`suggested_key`는 best-effort. OS·타 확장과 충돌 시 Chrome이 자동으로 미할당:
- `useCommandShortcuts.ts`: `chrome.commands.getAll()` 결과에서 빈 shortcut 필터링 → 호버 툴팁 미표시
- `chrome.commands.onCommand`은 미할당 커맨드에 대해 발화하지 않음 → 자연 no-op
...
```
background/index.ts:100-114 — setupContextMenu()
...
```
background/index.ts:126 — contextMenus.onClicked
...
| `${PROXY_URL origin}/*` | OAuth proxy (client_secret 은닉) | `oauth.ts`, `github-oauth.ts`, `notion-oauth.ts` |
...
Linear는 PKCE 지원으로 proxy 불필요 — `api.linear.app/oauth/token`으로 직접 교환.
...
```
SettingsTab.tsx:89-103 — handleReplayToggle()
├── chrome.permissions.contains({ origins: REPLAY_ORIGINS })
├── 미부여 시 chrome.permissions.request({ origins: REPLAY_ORIGINS })
├── 부여 → setReplayEnabled(true)
└── 거부 → toast.error("settings.replay.permissionDenied")
```

`REPLAY_ORIGINS` = `["https://*/*", "http://*/*"]` (`use-30s-replay.ts:15`)
...
### chrome.permissions.contains() 호출 2곳

| 위치 | 용도 |
|---|---|
| `SettingsTab.tsx:95` | Replay 토글 시 이미 부여됐는지 확인 (불필요한 프롬프트 방지) |
| `use-30s-replay.ts:86` | Replay 활성화 시작 시 — 사용자가 설정에서 권한 철회했으면 자동 비활성화 + 토스트 |
...
- `use-30s-replay.ts:86-93`: 다음 Replay 시작 시 `contains()` 체크 → 미부여 → `setReplayEnabled(false)` + 토스트 "permissionRevoked"
