...
| 컨텍스트 메뉴 "BugShot" 클릭 | `background/index.ts:76` — `contextMenus.onClicked → activateTab()` |

### 의존 API

| API | 용도 | 사용 위치 |
|---|---|---|
| `chrome.tabs.captureVisibleTab()` | 요소·영역·30s Replay 스크린샷 | `background/messages.ts:155` (bg handler) |
...
```
chrome.commands.onCommand (background/index.ts:105)
...
```
background/index.ts:50-65 — setupContextMenu()
...
```
background/index.ts:76 — contextMenus.onClicked
...
```
background/index.ts:123 — webNavigation.onBeforeNavigate
...
```
background/index.ts:144 — webNavigation.onCommitted
