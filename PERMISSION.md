...
### chrome.permissions.request() 호출 트리거 3곳 (실제 호출 지점 2곳 — GitLab self-managed는 LLM과 동일 헬퍼 공유)

#### 1. 30s Replay 토글

```
RecordingSettingsCard.tsx — handleReplayToggle() (SettingsTab이 렌더)
...
```
ai-provider.ts:394 — requestHostPermission(baseUrl)
...
| 위치 | 용도 |
|---|---|
| `RecordingSettingsCard.tsx` | Replay 토글 시 이미 부여됐는지 확인 (불필요한 프롬프트 방지) |
