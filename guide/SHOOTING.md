...
| 화면 | 높이 |
|---|---|
| 설정 > 이슈 설정 | **1384**(ko) / **1444**(en) — `공통` 섹션에 이슈 본문 언어 행이 붙으면서 옆 1263에서 늘었다. **ko/en이 60px 다르니 한쪽 값을 돌려쓰지 말고 로케일별로 다시 재다** |
...
**2026-08-13 재촬영: `settings-issue-1`(ko/en).** 이슈 설정 최상단 섹션이 `제목 설정` → `공통`(en `Common issue settings` → `Common`)으로 개명되고 이슈 본문 언어 Select 행이 생겨 placeholder로 되돌려둔 자리를 채운다. **상단 밴드**(`top = 60`, 상단 2개만 공률) — 공통 카드가 CSS y 138~407이라 가시 범위 470 안에 제목 접두어 Input·본문 언어 Select 두 행이 모두 들어간다(본문 언어는 기본값 `자동 (한국어)`/`Auto (English)` 그대로). 마스킹 대상 문자열 없음.

> **낡은 `dist`를 찍을 뻔했다.** i18n 개명 커밋보다 9분 먼저 빌드된 `dist`가 남아 있어 옵 라벨이 그대로 박힐 뻔했다 — 산출물 문자열을 직접 grep해 잡았다(`grep -rl "<새 문구>" dist/`). **시각 비교만으로는 모른다** — §7 재개 절차의 "dist가 마지막 커밋보다 오래됐는가"를 mtime이 아니라 **바뀐 문자열이 산출물에 있는지**로 확인할 것. 또 `pnpm build` 뒤에 **확장을 리로드**해야 한다(`chrome.developerPrivate.reload(<id>)`) — 언팩 확장은 자동으로 안 바뀜다.
