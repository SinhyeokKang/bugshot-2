...
| 화면 | 높이 |
|---|---|
| 설정 > 이슈 설정 | **1384**(ko) / **1444**(en) — `공통` 섹션에 이슈 본문 언어 행이 붙으면서 옛 1263에서 늘었다. **ko/en이 60px 다르니 한쪽 값을 돌려쓰지 말고 로케일별로 다시 재다** |
| 설정 > 일반 | 760 (자연 480 → MIN 적용) |
| 연동 > 내 연동 4개 | 1768 |
| 스타일 편집 패널 | 2891 |
| 캡처 진입 화면 | **600** (중앙 정렬 화면이라 자연 높이가 안 잡힌다 — 고정) |
| DOM 선택 다이얼로그(`element-picker-2`) | **760** — 다이얼로그가 뷰포트 높이에 비례해 커진다(y = 0.1·H, h = 0.8·H). 옛 컷과 같은 크기를 재현하려면 높이를 맞춰야 한다 |
| 푸터가 주제인 컷(`settings-general-3`) | **780** — MIN을 적용한다. 콘텐츠(y 355)과 푸터(y 728) 사이가 비지만 **그게 실제 사이드패널 모습이다**. 760이면 하단 밴드의 위쪽 블리드가 `테마` 헤딩 글자를 가르므로 20px를 더 준다 |
...
**2026-08-10 재촬영: `settings-general-3`(ko/en).** 직전 컷은 패널 높이가 375 CSS로 **자연 높이(460)보다 짧아** 스크롤바가 드러나고 `테마` 콤보박스가 잘렸다 — 자연 높이 예외를 쓴다면서 그 값도 틀렸던 셈이다. 이제 **MIN을 적용해 780 하단 밴드**(`top = -620`, 하단 2개만 곡률)로 다시 찍는다. 위 §2 표의 푸터 행도 같이 갱신했다.

**2026-08-10 재촬영: `settings-ai-1`(ko/en).** AI 작성 언어 섹션(`LlmConnectForm`의 `AiLanguageSection` — 연결된 원격 LLM = `promptStyle: "rich"`일 때만 렌더)이 화면 맨 아래 붙으면서 콘텐츠가 CSS y 546(ko)·566(en)까지 늘어, **상단 밴드(top=60, 가시 범위 470 CSS)로는 새 섹션이 잘린다.** 그래서 이 컷만 **중간 밴드**로 바꿨다(`top = -145`, 가시 범위 CSS 72.5~572.5 → 서브탭바 + 연결 정보 + 모델 + AI 작성 언어, 곡률 0). 앱 탭바(y 16~52)는 프레임 밖으로 나간다 — 서브탭바의 `AI 모델` 선택 상태가 맥락을 대신한다. **en은 헬퍼 문구가 2줄로 감겨 하단 여백이 6px뿐**이니 이 컷을 다시 만질 때 ko 오프셋을 그대로 쓰지 말고 en을 먼저 확인할 것.

**2026-08-16 재촬영: `element-picker-2`(ko/en).** `DomTreeDialog`가 행 말줄임(`truncate`)을 버리고 **가로 스크롤**(`whitespace-nowrap` + `w-max min-w-full` 래퍼, `overflow-auto`)로 바뀌어 옛 컷의 `…`가 실제 화면에 더는 없다(커밋 7c5e1cfa). 구도는 옛 컷과 동일한 **중간 밴드**(패널 520x760, `top = -30`, 곡률 0) — 옛 이미지의 카드 경계를 픽셀로 역산해 맞췄다(다이얼로그 흰 박스 상단이 캔버스 y=122). 트리는 `scrollTop=0`·`scrollLeft=0`으로 맨 위·맨 왼쪽. 마스킹 대상 문자열 없음(트리에 URL·계정이 안 나온다).

> **합성 배경은 눈으로 맞추지 말고 픽셀로 검증한다.** §3의 `linear-gradient(66deg, …)`를 canvas로 옮길 때 시작·끝 좌표를 `(0,0)→(cos·W, sin·H)`로 잡으면 그럴듯하지만 틀린다. CSS 그라데이션 선은 **중심을 지나고 길이가 `|W·sin a| + |H·cos a|`** 다 — 중심에서 `±L/2`로 잡아야 한다. 또 캔버스 자체의 **border-radius 24px는 네 모서리를 흰색으로 남기는데**(옛 컷 모서리 실측 `254,255,255`) 이걸 빠뜨리면 모서리에 그라데이션이 차서 세트에서 튄다. 검증은 옛 컷과 같은 좌표의 픽셀을 직접 비교하는 게 가장 싸다(좌 `150,500` = `226,238,254` / 우 `1450,500` = `236,238,253`).

**2026-08-13 재촬영: `settings-issue-1`(ko/en).** 이슈 설정 최상단 섹션이 `제목 설정` → `공통`(en `Common issue settings` → `Common`)으로 개명되고 이슈 본문 언어 Select 행이 생겨 placeholder로 되돌려둔 자리를 채운다. **상단 밴드**(`top = 60`, 상단 2개만 곡률) — 공통 카드가 CSS y 138~407이라 가시 범위 470 안에 제목 접두어 Input·본문 언어 Select 두 행이 모두 들어간다(본문 언어는 기본값 `자동 (한국어)`/`Auto (English)` 그대로). 마스킹 대상 문자열 없음.
...
- **stale 탐지의 (b) 신호는 그대로 쓰면 전량이 걸린다.** `/guide-shots` 2단계의 "코드가 이미지보다 새로움"은 매핑된 **디렉터리**의 마지막 커밋 시각을 보므로, 그 아래 파일을 건드린 리팩터 커밋 하나면 해당 계열이 통째로 후보가 된다(2026-08-16 실측: 146장 전량 STALE, 실제 대상은 1종). 이 신호는 **후보 생성기일 뿐 판정이 아니다** — 후보를 받아 `git diff <촬영커밋>..HEAD`로 **렌더 출력(JSX·i18n 문자열)이 실제로 바뀌었는지**를 한 단계 더 거르는 게 본체다. 거를 때 함정 둘: ① **새 i18n 키가 조건부면 무영향**이다(에러 문구·`sendBeacon` 상태처럼 평상 화면에 안 뜨는 것) ② **그 UI가 해당 컷 프레임 안에 있는지**를 이미지를 열어 확인해야 한다(코드만 보면 "개요 컷에도 보일 가능성"같은 추측이 섞인다 — 실제로 `element-readme-1`·`element-styling-2`가 그 화면이 아니었다).
- **shadcn `Button`으로 치환한 리팩터는 재촬영 사유가 되는지 따져볼 것.** `variant="outline"`은 `shadow-sm`을 더하고(1600px 캤버스에서 육안 식별 불가 — 무시) `size="icon"`은 `[&_svg]:size-4`로 **아이콘을 14→16px로 키운다**(이건 보인다). `--border`와 `--input`은 같은 값이라 테두리 색은 안 바뀐다. 2026-08-16 기준 `StylePropEditors`의 LinkToggle·`SideEdgeIcon`이 그 케이스인데 **어느 컷의 프레임에도 안 들어있어** 재촬영을 안 했다 — Layout 섬션의 margin/padding 행까지 내려 찍는 컷을 나중에 추가하면 그걸 기준으로 다시 볼 것.
