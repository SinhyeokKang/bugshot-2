# Integrations

A report you worked on only truly shines once it reaches the issue tracker your team uses. In the **Integrations** tab, connect whichever platform you're on — Jira, GitHub, Linear, Notion, GitLab, Asana, or ClickUp (Slack for a quick channel share, or a Custom Webhook pointed at your own server, live in the same place) — and you can file captured bugs straight into it as issues. Want to give your team a quick heads-up before filing a formal issue? You can send to **Slack** too.

No platform connected yet? No worries. You can still capture and draft as usual, and when you actually need a connection, an amber **"Add a platform to start filing issues."** banner appears at the bottom of the capture and preview screens. Click **Add platform** on the right and you'll land right here on the **Integrations** tab. No pressure — just start by connecting one.

![Integrations tab](../assets/integrations-readme-1.jpg)

## Jump to

- [Connecting Platforms](platforms.md) — How to connect all eight platforms and what each needs.
- [Issue Tracking](issue-tracking.md) — Browse and manage your drafts and submitted issues.
