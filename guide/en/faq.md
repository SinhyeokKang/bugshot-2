...
Seven trackers — Jira, GitHub, Linear, Notion, GitLab, Asana, and ClickUp — plus **Slack** now too, for a quick share to a channel or DM. If your tool isn't on that list, a **[Custom Webhook](integrations/custom-webhook.md)** sends the report as-is to a server you built (you bring the server). Whatever your team uses, there's no need to switch: file issues straight into your usual tracker, or give your team a heads-up on Slack before filing. You only have to connect it once, over in [Integrations](integrations/README.md).
