...
Logs from several origins (including iframes embedded in the page) all land on one timeline. In the Console and Network lists, an **origin filter** above the list lets you narrow down by origin, so you're never unsure which origin a log came from.
