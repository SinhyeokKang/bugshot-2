...
![본문 구성 설정](../assets/settings-issue-2.jpg)
...
녹화 버튼이 **탭만 담을지, 화면 전체나 다른 창까지 담을지**를 여기서 미리 골라 둡니다. 캡처 진입 화면의 녹화 버튼은 여기서 고른 모드대로 동작해요.

- **탭 녹화** — 지금 보고 있는 탭만 녹화합니다. 빠르고, 공유 선택 창도 거치지 않습니다.
- **화면 녹화** — 화면 전체나 특정 창을 골라 녹화합니다. 탭 바깥(다른 앱 창, 새 창으로 뜨는 결제·로그인 화면)까지 담아야 할 때 쓰세요.

한 번 골라 두면 다음에 바꾸기 전까지 그대로 유지됩니다. 캡처 화면에서 녹화 버튼 오른쪽 **톱니바퀴(⚙)**를 누르면 이 설정으로 바로 올 수 있어, 매번 설정을 헤맬 필요가 없습니다.

![녹화 모드 설정](../assets/settings-issue-4.jpg)
...
![30초 리플레이 토글](../assets/settings-issue-5.jpg)
