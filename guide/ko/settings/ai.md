...
여기에 버그를 한 줄로 가볍게만 적고(요소 모드에서는 비워 두셔도 됩니다) **초안 작성**을 누르면, AI가 제목과 켜 둔 본문 섹션을 한 번에 채워 줍니다. 캡처·로그뿐 아니라 이미 적어 두신 제목·본문도 참고하니, 다듬고 싶을 때 다시 눌러도 됩니다. 버그와 직접 관련된 에러 로그가 있으면 **실제 로그 원문을 발생 현상에 코드 블록으로** 함께 넣어 주는데, 로그 내용은 AI가 쓰는 게 아니라 캡처된 원문 그대로입니다. 모드마다 근거가 조금씩 다른데, 자세한 흐름은 [이슈 작성(요소)](../element/issue.md)에서 다룹니다.
