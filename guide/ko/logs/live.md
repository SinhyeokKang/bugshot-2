...
캡처(요소·스크린샷·영상) 없이 **로그만 담아 이슈를 만들 수도 있습니다**. 콘솔·네트워크를 보다가 **이슈 작성** 버튼을 누르면, 캡처 단계를 건너뛰고 바로 이슈 초안으로 들어갑니다. 콘솔·네트워크에 더해 **액션 로그**(클릭·입력 등 사용자 동작)까지 세 종류가 기본으로 첨부되니, 그대로 제출하면 됩니다.

여기서 보던 로그 중 **문제된 한 건은 이슈 본문에 그대로 넣을 수도 있습니다** — 초안의 발생 현상 같은 문단 섹션 헤더에서 **로그 추가**를 누르면 됩니다. 첨부 파일을 열지 않아도 이슈에서 바로 보이니, "이 응답이 문제입니다"를 말로 풀어 쓰지 않아도 됩니다. 자세한 사용법은 [이슈 작성(스크린샷)](../screenshot/issue.md)의 본문 섹션 안내에 있습니다.
