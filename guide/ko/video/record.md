...
"여기 이 버튼이요", "이 영역이 깨져요" — 말로만 설명하기엔 애매한 부분, 녹화 화면 위에 직접 그려서 짚어 드릴 수 있습니다. 녹화가 시작되면 녹화 컨트롤에 **화면에 그리기** 버튼이 나타납니다.

- **화면에 그리기** 버튼을 누르면 그리기가 켜지고, 마우스 커서가 십자 모양으로 바뀝니다. 이 상태에서 페이지를 **누른 채 끌면** 지나간 자리에 선이 그려집니다.
- 그린 선은 **몇 초 뒤 스르륵 사라지므로** 따로 지울 필요가 없습니다. 강조하고 싶을 때마다 그때그때 그으시면 됩니다.
- 그리는 동안 페이지 클릭은 잠시 막히지만 **스크롤은 그대로 됩니다**. 다 그리셨으면 **화면에 그리기** 버튼을 다시 누르거나 페이지에서 **Esc** 키를 눌러 그리기를 끄면, 페이지를 평소처럼 다룰 수 있습니다.
