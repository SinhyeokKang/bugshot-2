...
**재현 과정은 알아서 채워집니다.** 녹화를 마치고 이 화면에 들어오면, AI가 방금 기록된 액션 로그를 바탕으로 **재현 과정을 자동으로 정리해** 넣어 줍니다(AI를 연결해 둔 경우에만 — 연결된 AI가 없으면 비어 있는 채로 둡니다). 정리하는 동안엔 화면 위에 보라색 오버레이가 잠깐 덮이고, 끝나면 채워진 단계가 나타납니다. 마음에 들지 않으면 재현 과정 섹션 오른쪽 위의 **휴지통(전체 초기화)** 버튼으로 한 번에 비우고 직접 쓰시면 됩니다.

> 이 자동 채움이 부담스러우시면 **설정 > 이슈 설정 > AI 설정 > 재현 과정 채우기**에서 꺼 두실 수 있습니다.
