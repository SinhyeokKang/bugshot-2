...
로그를 함께 첨부하는 캡처라면 여기에 **`API Hosts`** 행이 하나 더 붙기도 합니다. 수집된 네트워크 로그에서 페이지와 같은 도메인을 쓰는 API 서버 주소를 찾아 알아서 채워 드리는 행입니다(화면에는 영문 `API Hosts`로 표시됩니다). "그래서 어느 서버를 봐야 하나요?"를 다시 묻지 않아도 되도록 준비해 둔 자리라고 보시면 됩니다. 이 행이 채워지면 재현 환경 섹션이 **펼쳐진 채로** 열리니 값을 한 번 확인해 보세요. 자동으로 채운 값이라 마음에 안 드시면 **직접 고치거나 행째로 지우셔도** 됩니다 — 한 번 고치시면 그 값이 그대로 유지됩니다.
...
스크린샷 모드에서도 세 종류의 로그가 함께 첨부됩니다. 로그 섹션에는 **`logs.html` 카드 하나**가 뜨고 **스위치는 기본 켜짐**입니다 — 스위치 하나로 세 로그를 **통째로** 담거나 뺍니다(로그별로 따로 끄지는 않습니다). 카드에는 어떤 종류가 몇 건 잡혔는지가 요약돼 보입니다.
...
카드를 누르면 **콘솔·네트워크·액션 탭**으로 나뉜 창이 열립니다. 각 탭에서 실제로 어떤 로그가 담기는지 확인할 수 있고, 창 아래 **첨부 해제**로 끌 수도 있습니다.
