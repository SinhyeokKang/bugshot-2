...
>
> Jira를 **OAuth(브라우저 로그인)로 연결해 두셨다면** 스프린트 목록이 비어 보일 수 있습니다. 스프린트를 읽으려면 Jira 권한이 하나 더 필요한데, 이미 연결해 둔 계정은 연결하던 시점의 권한만 가지고 있기 때문입니다. 연동 설정에서 Jira를 한 번 다시 연결해 주시면 목록이 제대로 뜹니다. API 토큰으로 연결하셨다면 해당되지 않습니다.
