...
- **트래커로 등록**: 제출 창이 열립니다(이때 Slack 탭은 빠집니다). 원하는 트래커를 골라 정식 이슈로 옮기면, 그 카드는 일반 이슈로 바뀌고 Slack 이력은 정리됩니다.

![Slack 카드의 자세히·트래커로 등록 버튼](../assets/integrations-platforms-3.jpg)
