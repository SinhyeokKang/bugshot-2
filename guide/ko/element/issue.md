...
표 안의 before·후 이미지에는 **주석을 달 수 있습니다.** 캡처 범위가 고른 요소를 감싼 영역까지 넓어지다 보니 이미지 하나에 여러 요소가 함께 담기는데, "이 버튼의 여백이 문제"처럼 정확히 어디를 봐야 하는지 짚어 주고 싶을 때가 있죠. 이미지에 마우스를 올리면 우상단에 **주석 추가** 버튼이 나타납니다. 누르면 [스크린샷 어노테이션](../screenshot/annotation.md)과 같은 편집기가 열리니, 화살표나 도형으로 표시한 뒤 **주석 완료**를 누르세요. before와 after를 각각 따로 표시할 수 있고, 주석을 단 뒤에는 버튼이 **주석 제거**·**주석 수정** 두 개로 바뀝니다. **주석 제거**를 누르면 처음 캡처한 원본으로 돌아가니 마음 편히 그려 보셔도 됩니다.

> 주석을 단 이미지가 그대로 이슈에 첨부되고 미리보기·AI 초안에도 쓰입니다. 원본은 확장 안에만 남고 밖으로 나가지 않습니다.
...
오른쪽 **AI 초안 작성**을 누르면 작은 입력창이 열립니다. 버그를 한 줄로 가볍게만 적어 주면(요소 모드에서는 비워 두셔도 됩니다) AI가 **제목과 본문 섹션을 한 번에** 채워 줍니다. 채워지는 건 켜 둔 섹션뿐이고, 제목 접두어도 그대로 유지됩니다. 이미 적어 두신 제목·본문이 있다면 그 내용까지 참고하고, 본문에 붙여 둔 이미지는 지우지 않고 그대로 둔 채 글만 다듬어 채웁니다.
...
제출 전 본문을 미리보기로 확인합니다. **복사**를 누르면 본문이 마크다운으로 클립보드에 담겨, 다른 문서나 메신저에 그대로 붙여넣을 수 있습니다.
