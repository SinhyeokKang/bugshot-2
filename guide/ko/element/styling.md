...
- 사이트가 이미 걸어 둔 스타일 **자체를 끄고 싶을 땐**, 드롭다운 항목의 **`unset`**을 고르세요(값을 입력하는 동안에는 `unset`·되돌리기 항목이 잠시 숨습니다 — 입력을 비우면 다시 나옵니다). 되돌리기가 "내가 바꾼 값만 취소"라면, `unset`은 "이 스타일은 적용되면 안 된다"를 표현합니다. 예컨대 말줄임(`text-overflow`)이 걸리면 안 되는 자리에 걸려 있을 때 `unset`으로 꺼서 고친 모습을 보여줄 수 있습니다.
