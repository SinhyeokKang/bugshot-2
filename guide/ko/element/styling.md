...
- 항목 우측 **이 변경 초기화** — 그 항목 하나만 원래 값으로 되돌립니다. 페이지와 스타일 패널이 바로 갱신됩니다.
- 요소 카드 우상단 **이 요소의 변경 초기화** — 그 요소의 모든 수정을 한 번에 되돌립니다.
- 좌측 하단 **전체 초기화** — 모든 요소의 변경을 되돌립니다(이건 한 번 더 확인을 거칩니다).

항목·요소 초기화는 따로 묻지 않고 바로 실행되니, 부담 없이 정리하셔도 됩니다. 되돌려서 변경이 하나도 안 남으면 다이얼로그는 저절로 닫힙니다.
...
> 담은 요소는 **변경사항 보기**에서 요소별로 확인하고, 항목·요소 단위로 따로 빼낼 수 있습니다. 한 번에 비우려면 **전체 초기화**를 누르거나, 이슈 작성을 취소하거나 제출을 마치면 그동안 담은 요소가 모두 비워집니다.
