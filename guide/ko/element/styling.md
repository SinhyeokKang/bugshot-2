...
![폼/코드 편집 토글](../assets/element-styling-4.jpg)

패널 위쪽 요소 이름 아래에 **폼 / 코드** 토글이 있습니다. 편한 쪽으로 오가며 편집하면 됩니다.

- **폼** — 방금 본 섹션들처럼 입력칸·드롭다운으로 다룹니다. 기본값이라 대부분 이걸로 충분합니다.
- **코드** — CSS를 **직접 타이핑**합니다(`padding: 3rem;`처럼). 브라우저 개발자 도구의 `element.style`을 손보는 것과 같습니다.

두 모드는 **같은 편집 내용을 공유**합니다. 폼에서 바꾼 값은 코드에도, 코드에 친 값은 폼에도(폼이 지원하는 속성이라면) 그대로 나타나니, 왔다 갔다 해도 작업이 사라지지 않습니다.

코드 모드가 특히 요긴한 두 경우가 있습니다.

- 폼에 칸이 없는 속성(예: `cursor: pointer;`)까지 자유롭게 넣고 싶을 때.
- 사이트가 `!important`로 걸어 둔 스타일을 이겨야 할 때 — 값 끝에 `!important`를 붙여 주면 됩니다.

> 고른 모드는 **기억됩니다**. 코드 모드를 쓰다 패널을 닫아도, 다음에 요소를 선택하면 코드 모드로 다시 열립니다. class·Text·변경사항 보기·AI 스타일링은 두 모드에서 똑같이 쓸 수 있습니다.
