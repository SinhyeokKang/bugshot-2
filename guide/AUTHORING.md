...
| 스타일 편집/CSS 뷰·왕복 | `src/sidepanel/tabs/StyleEditorPanel.tsx`(탭·아이콘 Paintbrush/Code2) · `styleEditor/StyleCssView.tsx`(CSS 뷰)·`CssCodeMirror.tsx`(CodeMirror·자동완성) · `styleEditor/cssBlock.ts`(serialize/parse/computeOverrides)·`boxModel.ts`·`BoxModelDiagram.tsx` · `inlineCssText.ts`(재사용) · `settings-ui-store.ts`(`styleEditorView` 영속) · 탭 라벨 `editor.ts` `editor.view.*`(편집/CSS) |
