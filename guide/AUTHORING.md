...
faq.md                          # 8. 자주 묻는 질문 (강점 간접 안내 — 단일 페이지)
...
- **H1(페이지 제목) 섹션**은 예외: `H1 → 🌐 언어 전환 링크 → 스크린샷 → 도입 문단` 순(🌐는 항상 3번째 줄 고정, §7).
...
- **언어 전환 링크**: 본문 페이지(SUMMARY 제외) **맨 위 — H1 제목 바로 아래**에 한 줄(H1 / 빈 줄 / 🌐 링크 / 빈 줄 / 본문). 상대 페이지의 **대응 URL**로 연결(같은 문서의 다른 언어). 페이지당 정확히 1개. SUMMARY.md에는 넣지 않는다. (과거엔 페이지 **맨 아래 footer**였으나 스크롤 없이 바로 보이도록 상단으로 이동함 — 하단엔 두지 않는다.)
  - GitBook URL은 **파일 경로 기반**이고 ko/en이 **동일 경로**를 쓴다(제목이 달라도 slug는 경로 그대로 — `element/issue.md` → ko `…/ko/element/issue`, en `…/en/element/issue`). 그래서 현재 페이지의 경로를 상대 언어 사이트에 그대로 붙인다.
  - 경로 매핑 규칙:
  ...
  - ko `element/issue.md` 상단:
    ...
    스타일 수정을 마치고 …
    ```
  - en `element/issue.md` 상단:
    ```
    # Filing an issue (element mode)

    🌐 [한국어](https://bugshot.gitbook.io/ko/element/issue)

    Once you're done tweaking styles …
    ```
  - 새 페이지 추가 시 H1 바로 아래에 그 페이지 경로에 맞는 대응 URL을 직접 넣는다. 경로·SUMMARY 중첩을 바꾸면 GitBook slug도 바뀌므로 이 URL을 함께 갱신한다. **`---` 구분선은 붙이지 않는다**(상단이라 불필요).
...
# 6. 언어 전환 링크 페이지당 1개 (SUMMARY 제외) + H1 바로 아래(3번째 줄)
for f in $(find ko en -name '*.md' ! -name 'SUMMARY.md'); do
  c=$(grep -c '🌐' "$f"); [ "$c" != "1" ] && echo "ABNORMAL $f: $c"
  ln=$(grep -n '🌐' "$f" | head -1 | cut -d: -f1); [ "$ln" != "3" ] && echo "POS $f: line $ln"
done
```
