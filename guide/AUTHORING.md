...
| 2. 재현 환경 | 자동 메타(OS/브라우저/URL/뷰포트/시각 + 요소를 고른 캡처면 DOM selector) readonly + 사용자 추가 변수 row. 로그를 첨부하는 모드는 여기에 `API Hosts` 자동 행이 더 붙는다(아래 사실 스냅샷 "재현 환경 `API Hosts` 자동 행" 참조) |
| 3. 미디어 | **모드별 고유** — 요소=before/after 스타일 표(표 안 before·after 이미지에 **각각 주석 가능** — 아래 사실 스냅샷 "diff table 이미지 주석") / 스크린샷=주석 이미지 / 녹화=영상 |
| 4. 본문 섹션 | 발생 현상·재현 과정·기대 결과·비고(설정 토글대로). **문단 섹션 헤더 우측 `[로그 추가][영역 캡처][이미지 추가]`** — 로그 추가는 로그 1건을 본문에 코드블럭으로 넣는다(요소 모드는 로그가 없어 비활성, 재현 과정은 번호 목록이라 버튼 없음). 본문에 넣은 **인라인 이미지는 hover 시 `[주석 달기][삭제]`(+기록 있으면 `[원본 복원]`)로 그 자리에서 편집**한다(아래 "본문 이미지 어노테이션"). 아래 사실 스냅샷 "로그 본문 추가" 참조 |
| ✨ AI 초안 작성 | 번호 없는 강조 섹션. 배너(AI 연결 시) → 입력창에 버그 한 줄(요소 모드는 비워도 됨) → **제목+본문 한 번에** 채움(켜 둔 섹션만, 접두어 유지). 사용자가 **이미 적어 둔 제목·본문도 컨텍스트로 참고**하고, 본문에 붙인 **inline 이미지·코드블럭은 보존(텍스트만 교체)**. **모드별 근거**: 요소=before/after 스타일·전후 이미지 / 스크린샷=주석 이미지 + 콘솔·네트워크·액션 로그 요약 / 녹화·이슈 작성(freeform)=콘솔·네트워크·액션 로그 요약. **AI 근거의 로그 스코프는 로그 첨부 스코프와 동일**하다(요소 모드만 로그 없음 — `supportsConsoleNetworkLog`/`supportsActionLog` 단일 출처. 과거 "스크린샷 AI 근거는 주석 이미지뿐, 액션 로그를 본다고 쓰지 말 것" 서술은 **stale** — 쓰지 않는다). 관련 로그(에러뿐 아니라 유저·캡처 맥락과 매칭되는 정상 200 응답 포함)가 있으면 AI가 지목해 **실제 로그 원문이 발생 현상에 코드블럭으로 자동 삽입**된다(아래 사실 스냅샷 "AI 로그 코드블럭 자동 삽입" 참조) |
| 5. 로그 첨부 | 요소=없음 / **스크린샷·녹화·이슈 작성 모두 `logs.html` 단일 카드 1개**(스위치 하나로 콘솔·네트워크·액션 3종을 **통째로** on/off, 기본 on — 로그별 개별 해제 없음). 카드 클릭 시 console/network/action 3탭 미리보기 다이얼로그로 내용 확인·[첨부 해제] |
...
| AI 작성 언어(초안·재현 과정 출력 언어) | `src/sidepanel/lib/aiLanguage.ts`(프리셋·auto 해석) · `settings-ui-store.ts`(`aiLanguage`) · 노출 게이트 `src/sidepanel/tabs/settings/LlmConnectForm.tsx`(`capabilities.promptStyle === "rich"`) · 라벨 `src/i18n/namespaces/settings.ts`(`llm.section.outputLanguage`/`llm.outputLanguage.*`) |
| 이슈 본문 언어(섹션 제목·표 헤더 언어) | `src/i18n/locales.ts`(`BodyLocale`·`normalizeBodyLocale`·`resolveBodyLocale`) · `src/i18n/index.ts`(`withLocale` 전역 스왑) · `settings-ui-store.ts`(`bodyLocale`) · UI `src/sidepanel/tabs/SettingsTab.tsx`(`IssueSettingsContent`) · 라벨 `src/i18n/namespaces/settings.ts`(`settings.issueCommon`/`settings.titlePrefix.label`/`settings.bodyLocale{,.auto,.help}`) |
| 캡처 방식 3축(영역·화면·페이지 전체)·스크롤 캡처 | `src/sidepanel/scroll-capture.ts`(오케스트레이터·truncated 판정) · `src/content/scroll-capture.ts`(자동 스크롤·fixed 및 붙은 sticky 반복 요소 은폐/복원·동적 후보 추적) · `src/sidepanel/lib/scroll-capture-plan.ts`(타일 계획·상한·스티치 좌표) · 라벨 `src/i18n/namespaces/issue.ts`(`issue.capturing.method.*`/`.scrolling`/`.canceling`/`.progress`/`.truncated`) · UI `src/sidepanel/tabs/IssueTab.tsx`(CapturingState 하단 툴바) |
...
| 녹화 중 그리기(펜·사각형·형광펜) | `src/content/annotation.ts`(SVG 오버레이·EMA 스무딩·트레일 페이드 / 사각형은 `annotation-draw.ts:rectPoints`로 네 꼭짓점을 같은 시각에 찍어 통째로 만료 — 펜은 꼬리부터, 박스는 한 번에 사라진다) + `src/sidepanel/annotation-control.ts`(`setAnnotationTool` — `annotation.setTool`은 메시지 타입) + `src/sidepanel/components/annotation/{ToolbarGroups.tsx,recording-pen.ts}`(konva와 공유 툴바·스타일) + editor-store `annotationTool/Color/Thickness` + i18n `annotation.pen`/`.rect`/`.highlight`/`.color.*`/`.thickness.*`(`issue.recording.penHint`는 Esc 툴팁) — **스크린샷 주석(konva)과 벡터 스타일 동일·별개 기능** |
| 영상 구간 자르기(트리밍 — 리플레이·탭/화면 녹화 공용) | `src/sidepanel/tabs/ReplayTrimDialog.tsx`·`TrimTimeline.tsx`, 백엔드 `src/sidepanel/30s-replay/trim-math.ts`·`apply-trim.ts`·`encode-range.ts`(녹화 구간 재인코딩)·`mp4-encoder.ts`(인코더 배선)·`trim-markers.ts`(타임라인 에러·페이지 이동 마커), 진입 `src/sidepanel/video-recorder.ts`(녹화 정지)·`30s-replay/use-30s-replay.ts`(리플레이 캡처), 라벨 `src/i18n/namespaces/issue.ts` (`issue.replay.trim.*`, `issue.replay.trimFailed`) |
| 재현 환경 `API Hosts` 자동 행 | `src/sidepanel/lib/apiHostRow.ts`(파생·게이트·동기화·제출 ctx strip) · `src/sidepanel/lib/environmentRows.ts`(`filterEnvironmentRows`가 `source` 표식 제거) · `src/types/environment.ts`(`EnvironmentRow.source`) · 게이트 `src/sidepanel/lib/captureLogSupport.ts` · 배선 `src/sidepanel/tabs/DraftingPanel.tsx`(`ReproEnvironmentSection`) |
...
| AI 로그 코드블럭 자동 삽입(logRefs) | `src/sidepanel/lib/prompts/logCandidates.ts`(후보 단일 출처·description 게이트·200 매칭 `selectMatchedLogCandidates`) · `src/sidepanel/lib/prompts/queryTokens.ts`(매칭 term 토크나이저) · `src/sidepanel/lib/prompts/responseDigest.ts`(shape 다이제스트·값 제외·맵키 redact) · `src/sidepanel/lib/renderLogRefs.ts`(상한 3·에러 우선 slice·역참조·삽입) · `src/sidepanel/lib/mergeAiDraftSections.ts`(코드블럭 보존) · 배선 `src/sidepanel/tabs/AiDraftDialog.tsx` |
...
- **본문 섹션**: 발생 현상(켜짐·문단) / 재현 과정(켜짐·번호 목록) / **미디어 · 로그**(항상·스위치 없음) / 기대 결과(켜짐·문단) / 비고(꺼짐·문단) — `DEFAULT_ISSUE_SECTIONS` 순. 라벨·플레이스홀더 override 가능(미디어 제외). **순서는 사용자가 바꾼다** — 설정 > 이슈 설정 > 본문 구성에서 각 행 좌측 손잡이를 드래그하거나 키보드(손잡이 포커스 → Space → ↑/↓ → Space, Esc 취소)로 재정렬하고, 놓는 즉시 영속된다. **미디어 · 로그**(`settings.section.media` = `미디어 · 로그`/en `Media & Logs`)는 요소 스타일 변경·캡처 미디어·로그 요약이 들어가는 자리로 **사용 여부 스위치가 없고 위치만 정한다**(본문 emit이 데이터 기반이라 캡처가 없으면 자동 생략되고, 로그 첨부 on/off는 작성 화면 로그 카드가 담당). **본문 구성** 제목 우측의 되돌리기 버튼(`settings.reorder.reset` = `기본 순서 복원`/en `Reset order`)은 기본 순서면 비활성이고, **순서만** 되돌린다(켜고 끈 상태는 보존). 대상: `settings/issue.md`.
...
- **미지원 URL에서의 패널 동작**: 툴바 아이콘·단축키·컨텍스트 메뉴는 **URL과 무관하게 항상 패널을 연다** — Chrome 웹스토어·`chrome://` 등에서 클릭이 무음으로 삼켜지던 과거 동작은 제거됐으니 **"이런 페이지에서는 패널이 활성화되지 않는다"고 쓰지 않는다**(stale). 미지원 페이지에서는 디버그 > 이슈 서브탭의 캡처 진입 화면이 안내로 바뀐다 — 제목 ko `이 페이지에서는 캡처할 수 없습니다` / en `Can't capture on this page`, 본문은 "브라우저가 확장 접근을 막아 둔 화면 + 웹 페이지(http, https, file)에서 실행 + **연동·설정은 그대로 사용 가능**". 캡처 버튼 5개(요소 스타일 편집·요소 캡처·스크린샷·녹화·30초 리플레이)는 안내로 **교체되고**, `이슈 작성` 버튼은 **자리를 지킨 채 비활성**(버튼 위치를 미리 익히게 하려는 의도 — 가이드에 "사라진다"고 쓰지 않는다), 콘솔·네트워크 서브탭은 비활성, `BugShot 가이드` 버튼과 연동 0개 CTA 배너는 **그대로 남는다**. 메인 탭 4개 중 **디버그만** 제약을 받고 연동·설정·이슈 목록은 완전히 동작하므로, 설치 직후 웹스토어 페이지에서 플랫폼 연결을 완주할 수 있다. 지원 페이지로 이동하면 **조작 없이** 캡처 화면으로 복구된다(새로고침 버튼 없음 — 있다고 쓰지 말 것). 대상: `faq.md`(작동하지 않는 페이지도 있나요?)·`quick-start.md`(2. 사이드패널 열기).
...
- **파일 첨부 토글**: 설정 > 이슈 설정 > **기타**에 `파일 첨부`(en `File attachments`) 토글, **기본 꺼짐**(`attachmentsEnabled` in `settings-ui-store.ts`). 켜면 이슈 작성(drafting) 화면에 `첨부 파일`(en `Attachments`, `section.attachments`) 영역 노출 → 임의 로컬 파일 선택. **최대 10개·합계 50MB** 하드캡(`src/sidepanel/lib/attachmentLimits.ts`), 플랫폼별 단건 경고(Notion 5MB·GitLab 10MB, 초과 시 "용량 초과" 표시·차단 아님). 제출 시 8개 플랫폼에 함께 업로드(Slack은 메시지 스레드 첨부). 기능 위치상 settings/issue.md에만 두고 중립적 기능 설명 톤으로 기술한다(3개 mode issue.md 공통 흐름엔 미편입).
- **재현 단계 자동 채움**: 설정 > 이슈 설정 **최하단**에 **기타** 섹션(`settings.otherSection` = `기타`/en `Other`, 본문 구성 아래)이 있고, 그 안에 `재현 과정 채우기` → `파일 첨부` 순으로 **재현 과정 채우기** 토글(`settings.autoReproPrefill.label` = `재현 과정 채우기`/en `Fill steps to reproduce`, `autoReproPrefill` in `settings-ui-store.ts`, **기본 켜짐**). 켜져 있으면 **녹화(video) 모드**로 이슈 작성(drafting)에 진입할 때 재현 과정(stepsToReproduce)이 비어 있고 AI(BYOK/Chrome 내장)가 가용하면, **액션 로그를 근거로 재현 과정을 자동 작성**한다(`useReproPrefill`). 본문 설정에서 **재현 과정 섹션이 꺼져 있으면** 자동 채움은 발화하지 않고(`isReproSectionEnabled` 단일 출처 — `sidepanel/lib/reproSectionEnabled.ts`), 설정의 이 토글도 **ON/OFF 값을 유지한 채 비활성**이 된다(설명 문구는 비활성 여부와 무관하게 `settings.autoReproPrefill.help` 고정). AI 미가용·빈응답이면 **채우지 않고** 비워 둔다(토스트로 재시도 안내). 채우는 동안 AI 초안·스타일링과 **같은 패널 전역 오버레이**(App.tsx `aiLoadingSurface` — 서피스별 tint: styling=teal/draft=purple/repro=amber)가 덮인다. 오버레이 **하단 중앙엔 `중단`(en `Stop`, `ai.stop`) 버튼**이 있어 진행 중 AI 작업을 **취소**(오버레이 닫힘 + 결과 폐기)한다 — BYOK 요청은 AbortSignal로 진행 중 호출과 재시도 대기까지 실제로 끊기고, Chrome 내장 세션은 진행 중 호출을 끊을 수 없어 결과만 폐기된다한다 — repro는 취소 시 비운 채로 두고 재발화 안 함(`userCancelled`). 3개 서피스(styling/draft/repro) 공통. 동작 시 액션 로그가 연결된 AI로 전송된다(프라이버시 고지 — settings/issue.md·privacy). 재현 과정 섹션 헤더 우측엔 **전체 초기화** 휴지통 버튼(`draft.stepsReset` = `전체 초기화`/en `Clear all`, 값 있을 때만 활성)이 있어 한 번에 비운다. **video 전용**(element/screenshot엔 자동 채움 없음 — captureMode 게이트). 대상: `settings/issue.md`(기타 절)·`video/issue.md`(본문 섹션 재현 과정 자동 채움). manual 전체 `✨ AI 초안 작성`(제목+본문 일괄)과는 별개 기능이다.
...
- **영상 구간 자르기(트리밍)**: **리플레이 전용이 아니다** — 30초 리플레이 캡처 직후뿐 아니라 **탭 녹화·화면 녹화를 정지했을 때도 길이와 무관하게 항상** 같은 오버레이가 이슈 초안 위로 자동으로 **한 번만** 뜬다(설정값 아님 — 순수 캡처 후 단계. 설정의 30초 리플레이 토글과 **무관**하게 녹화 자르기는 동작한다). 과거 "리플레이 캡처 직후에만"·대상 문서 `video/replay.md`만이라는 서술은 **stale**이다. 오버레이 상단은 **아이콘 4탭**(`영상`/`콘솔`/`네트워크`/`액션` — en `Video`/`Console`/`Network`/`Action`, 라벨 키 `issue.replay.trim.tab.video`·`issue.replay.trim.log.*`)이고 각 로그 탭엔 캡처 개수 Badge가 붙는다(0건도 표시·999 초과 `999+`). 영상 탭은 가운데에서 `<video>`로 재생, 로그 탭으로 전환하면 같은 자리에 로그 뷰어와 동일한 로그 목록이 인라인으로 펼쳐진다(0건이어도 empty로 조회 가능, 네트워크 탭은 상대 timestamp 표시·2-pane 유지). **타임라인 손잡이·재생/일시정지·되돌리기/다시 실행/확정/녹화 폐기는 전 탭 전역**. 영상 양쪽 끝의 **시작 지점·끝 지점**(en `Start`/`End`) 손잡이를 끌어 버그 구간만 남긴다. 손잡이를 끌면 **잘려나갈 로그가 로그 탭에서 실시간으로 흐림(opacity-40) 처리**돼 무엇이 빠질지 미리 확인된다(레벨/status 색상과 병렬일 때도 구분됨, 네트워크는 요청 리스트 row만 흐림·상세 pane 영향 없음). **재생은 영상 탭에서** — 로그 탭에선 자동 일시정지되고, play 버튼을 누르면 영상 탭으로 돌아가 재생된다. 타임라인의 **콘솔·네트워크 에러 마커 + 페이지 이동(action navigate) 마커**를 누르면 해당 로그 탭으로 전환된다(과거 마커 클릭→로그 다이얼로그 방식은 제거). **확정**(en `Apply`)하면 그 구간만 영상이 재인코딩되고 첨부 로그도 같은 구간으로 좁혀진다(흐림으로 표시됐던 로그가 그대로 빠짐 — 미리보기=실제 일치, **손잡이를 안 건드리고 확정하면 재인코딩 없이 즉시** 전체가 유지된다). 재인코딩이 시작되면 **화면을 덮는 대기 다이얼로그**가 뜬다 — 제목 `영상을 자르는 중`(en `Trimming the video`, `trim.encoding`), 안내 `이 패널을 열어둔 채 기다려 주세요. 패널을 닫거나 다른 탭으로 이동하면 진행이 멈춥니다.`(en `Keep this panel open while we trim. Closing it or switching tabs pauses the progress.`, `trim.keepTab`), 그 아래 진행률 바와 퍼센트. **닫는 컨트롤이 없어** 작업이 끝날 때만 사라진다(**녹화 경로 한정** — 30초 리플레이 트림은 프레임이 메모리에 있어 즉시 끝나므로 이 화면이 없다). 안내가 필요한 이유는 패널이 hidden이면 인코딩이 멈췄다 복귀 시 이어지기 때문이고, 대기 내내 읽혀야 해서 토스트가 아니라 이 화면에 있다(과거 `trim.keepTab` **토스트** + 액션바 진행률 서술은 stale). **인코딩 중 취소는 없다**. 재인코딩이 실패해도 **원본 영상이 그대로 첨부**된 채 작성 화면으로 넘어간다(`trimFailed` — 녹화를 잃지 않는다). **녹화 폐기**(en `Discard recording`, `trim.cancel`)는 확인 다이얼로그(`trim.cancelConfirm.*` — ko `녹화를 폐기할까요?` / `방금 찍은 영상과 함께 수집된 로그가 모두 삭제됩니다.`, en `Discard this recording?` / `The video you just captured and the logs collected with it will be deleted.`)를 거쳐 캡처를 폐기하고 진입 화면으로. **과거 라벨 `작성 취소`/en `Discard`는 stale이니 쓰지 않는다.** 사용자 노출 토스트 3종도 있다 — 진입 안내 `trim.toast`(`양쪽 손잡이를 끌어 버그 구간만 남겨보세요.` / en `Drag the handles to keep just the bug moment.`), 탭 전환 후 확정·폐기 시 `trim.wrongTab`·`trim.wrongTabDiscard`(원본 유지한 채 건너뜀), 영상은 잘렸으나 로그 저장에 실패한 `trim.logsPersistFailed`(재오픈 시 로그가 자르기 전으로 돌아올 수 있음). 확정하면 원본은 정리돼 재편집 불가. 선택 길이 readout(`{sel}s / {total}s`)은 **하단 액션바 가운데**에 있다(과거 "위쪽" 서술은 stale). UI 라벨은 `issue.replay.trim.*`(§6 영문 식별자 금지 대상 — 확정/녹화 폐기 등 현지화 라벨 사용). **대상**: `video/record.md`(ko "구간 자르기" / en "Trimming the clip" — **상세 설명의 단일 출처**. SUMMARY상 replay보다 먼저 오고 모든 녹화가 이 단계를 거치므로 여기에 둔다) + `video/replay.md`(같은 화면임을 밝히고 record.md로 위임 + 리플레이 고유분 "전체 확정 시 30초 유지"만). (과거 로그별 미리보기 다이얼로그 방식 → 인라인 탭 + muted 미리보기로 전환됨 — replay-trim-refactor. 이후 recording-trim에서 탭/화면 녹화까지 확장.)
- **캡처 방식 3축(스크린샷 안)**: `스크린샷`(en `Screenshot`)을 누르면 십자선이 뜨고 **사이드패널 하단에 캡처 방식 아이콘 버튼 3개**(ButtonGroup, 호버 시 툴팁)가 노출된다 — `영역 캡처`/en `Area capture`(기본 활성, 드래그) · `화면 캡처`/en `Screen capture`(보이는 뷰포트 전체, 드래그 없이 1클릭) · `페이지 캡처`/en `Page capture`(스크롤하며 타일 캡처 → 세로 스티칭). 라벨 키 `issue.capturing.method.area`/`.viewport`/`.fullPage`(`src/i18n/namespaces/issue.ts`). 페이지 캡처 진행 중엔 EmptyShell 문구가 `페이지를 캡처하는 중…`(en `Capturing the page…`)으로 바뀌고 **% 진행률 + 진행 바**가 뜨며, `취소`를 누르면 `캡처를 취소하는 중…` 후 페이지 스크롤·고정 요소가 원복되고 idle로 돌아간다. 제약(가이드에 명시): ① `position: fixed`와 이미 한 번 전부 노출된 뒤 붙은 `position: sticky` 요소는 **첫 노출에만** 남기고 후속 타일에선 숨김(아직 처음 등장하지 않았거나 화면보다 긴 sticky는 보존) ② **세로만** 스티칭(가로 오버플로는 보이는 폭까지) ③ 아주 긴 페이지는 상한까지만 담고 `페이지가 길어 일부만 캡처했습니다` toast ④ 캡처 중 페이지 **클릭·스크롤 모두 차단**(blocker가 wheel/touchmove를 `preventDefault` — 휠 조작이 먹지 않는다. 페이지가 밀리면 타일이 어긋나게 스티칭되기 때문). 코드: `src/sidepanel/scroll-capture.ts`(오케스트레이터)·`src/content/scroll-capture.ts`(스크롤·반복 positioned 요소)·`src/sidepanel/lib/scroll-capture-plan.ts`(타일 계획·스티치 좌표). 대상: `screenshot/capture.md`(ko/en), `quick-start.md`(스크린샷 한 줄).
- **어노테이션 도구**: 스크린샷 주석 편집기는 BugShot **자체 toolbar(konva 기반**, `AnnotationOverlay.tsx` + `annotation/AnnotationToolbar.tsx`)이고 **라벨이 ko/en 현지화**돼 있다(`editor.ts` `annotation.*`) — **과거 "markerjs2 영문 툴바" 안내는 stale이니 쓰지 않는다.** 각 아이콘 버튼은 **호버 시 툴팁**을 띄운다(캡처 방식 툴바와 같은 공용 `TooltipIconButton` — 이미지·녹화 어노테이션 툴바 공통). 도구: 선택/펜/화살표/사각형/원/텍스트/형광펜(`ANNOTATION_TOOLS` 순서 — 펜이 화살표 앞), 색상 5종(빨강·노랑·초록·파랑·검정), 선 두께 3단계(얇게·보통·굵게 — 선·도형·펜·형광펜에 적용. 텍스트는 두께 대신 글자 크기 3단계(작게·보통·크게)), 실행 취소·다시 실행, 삭제, 취소·주석 완료. **선택 도구와 그리기 도구는 상호 배타**다 — 2단 스타일 행(색상 + 두께/글자 크기)은 **항상 렌더되지만 선택 도구(=그리기 도구 미선택)에선 disabled**이고, 잠긴 동안에도 직전 그리기 도구의 형태(두께 vs 글자 크기)를 유지한다. 선택한 도형의 색·두께를 나중에 바꾸는 재스타일 기능은 **없다**(과거엔 있었으나 제거 — 가이드에 쓰지 말 것). 추가·수정·제거 진입 버튼은 drafting 화면(`draft.addAnnotation`/`.editAnnotation`/`.removeAnnotation`) — 스크린샷 미디어 카드와 **요소 모드 diff table의 before·after 이미지**가 같은 라벨 계열을 공유한다(아래 "diff table 이미지 주석"). **캔버스 줌·팬**: 편집기는 **선택 도구가 켜진 채로** 열리고, 진입 배율은 **너비 맞춤**(fit-width — 이전의 전체 맞춤에서 바뀜. 페이지 전체 캡처가 15%로 열려 식별 불가하던 문제). 캔버스 **하단**에 플로팅 배율 컨트롤 `[−][n% ▾][+]`(`annotation.zoomOut`/`zoomLevel`/`zoomIn`)와, 배율을 조작했을 때만 나타나는 **너비 맞춤** 버튼(`annotation.zoomFit`)이 뜬다. 콤보박스 항목 순서는 `이미지 전체(n%)`(`annotation.zoomFitAll` — fit-all, 세로로 긴 이미지에서만 노출) → `너비 맞춤(n%)` → fit보다 큰 프리셋(최대 400%) — 오름차순이라 전체가 맨 위. 확대·축소 앵커는 **뷰포트 중앙**. **팬**은 선택 도구로 빈 캔버스를 드래그(커서 `grab`), 도형 위 드래그는 도형 이동. 캔버스 뷰포트는 포커스 가능해 **화살표 키 스크롤** 지원. 배율 컨트롤은 도구와 무관하게 **항상 활성**이고, 배율을 바꾸면 `applyScale`이 **그리기 도구를 `select`로 강제 해제**한다(컨트롤이 캔버스 하단을 가리므로 그 위를 눌렀다면 그리려던 게 아니라는 전제). 컨트롤이 없는 영역은 `pointer-events-none`으로 통과시켜 그리기·팬을 막지 않는다. **완료 결과물은 배율과 무관하게 원본 해상도**. 가이드엔 내부 라이브러리명 노출 없이 "한국어로 표시되는 자체 툴바" 수준으로 기술. 대상: `screenshot/annotation.md`.
...
- **로그 정책**: 요소=로그 없음 / **스크린샷·이슈 작성(freeform)·녹화 = 콘솔·네트워크·액션 3종을 `logs.html` 하나로 묶는 단일 토글, 기본 on**(로그별 개별 토글 아님 — 카드 1개[`LogAttachmentCards.tsx`] + 스위치 1개, 클릭 시 console/network/action 3탭 미리보기 다이얼로그[`LogPreviewDialog.tsx`, 0건 탭도 활성·EmptyCase·0부터 배지]). **저장된 이슈 상세도 편집 가능(draft·Slack 보존) 상태면 같은 단일 토글로 첨부 on/off**(`IssueRecord.logsAttached`, blob 불변·가역). 액션 로그가 녹화 전용이라는 과거 서술은 **stale이니 쓰지 않는다**(v1.5.8에서 세 로그가 동일 스코프가 됨 — 요소 모드만 로그 없음). 로그는 **사이드패널이 열려 있는 동안 캡처 시작 전부터** 수집되고, 캡처에 진입하면 그 버퍼가 그대로 이월된다("캡처를 켜야 모이기 시작한다"고 쓰지 말 것). 자동 수집은 trailing throttle로 실시간 스트리밍(레코더 ~200ms flush, 사이드패널 IDB 저장은 ~1s로 묶음) — 과거 "~1.5초" 표기는 stale이니 가이드엔 구체 숫자 대신 "실시간"으로.
- **재현 환경 `API Hosts` 자동 행**: 로그를 첨부하는 캡처(스크린샷·녹화·이슈 작성)에서 재현 환경에 readonly 메타 행(통상 5행 — 요소를 고른 캡처면 `DOM`이 붙어 6행) 외에 **`API Hosts` 커스텀 행 1개**가 자동으로 주입된다. 값은 수집된 네트워크 로그 중 **페이지와 같은 registrable domain을 쓰는 다른 hostname**을 요청 수 내림차순으로 `, ` 연결한 것(`src/sidepanel/lib/apiHostRow.ts` — PSL 판정은 `tldts`). **라벨은 하드코딩 영문 `API Hosts`**로 기존 env 라벨 6종(OS/Browser/Page/DOM/Viewport/Captured)과 같은 규칙이라 ko 가이드에서도 영문 그대로 인용하고 "화면에는 영문 표시"를 한 줄 붙인다(§6 예외 조항). **일반 커스텀 행이라 값 수정·행 삭제가 가능**하고, 한 번 수정하면 이후 로그가 더 도착해도 덮어쓰지 않는다. 삭제하면 같은 draft에서 되살아나지 않는다(로그 첨부를 다시 켜도 마찬가지 — 명시 삭제 래치가 우선). 주입 시점에 **재현 환경 섹션이 펼쳐진 채로** 열린다(못 본 값은 고칠 수 없다는 전제). **행이 안 생기는 조건 3가지를 반드시 함께 안내**한다 — ① 페이지가 자기 hostname으로만 요청(BFF·같은 호스트 API) ② 로그 첨부 off ③ 요소 스타일 편집 모드. 셋 다 관측이 "행 없음"으로 동일해 사용자가 정상/실패를 구분할 수 없으므로 **가이드가 그 구분을 대신한다**. 서드파티(Sentry·GA 등 비동족)는 애초에 포함되지 않는다. 내부 메커니즘(registrable domain·PSL·정렬 축)은 노출하지 않고 "페이지와 같은 도메인을 쓰는 API 서버 주소" 수준으로 쓴다. 대상: `screenshot/issue.md`·`video/issue.md`(2. 재현 환경 절 — 헤딩 추가 없이 문단 + `>` 인용구). **`element/issue.md`는 대상 아님**(모드 게이트로 제외 — 요소 모드는 로그 자체가 없다).
...
- **AI 로그 코드블럭 자동 삽입**: AI 초안 생성 시 AI가 후보 목록([n1]/[c1]/[m1] 태그)에서 **관련 로그 id만 지목**하면, 앱이 store 원본을 `logToCodeBlock` 직렬화로 **발생 현상(description) 섹션 끝에 코드블럭으로 삽입**한다 — 로그 본문은 모델을 거치지 않아 **내용 환각이 구조적으로 불가능**(수동 [로그 추가]와 같은 직렬화·같은 결과). 후보는 **에러**(network 4xx/5xx·error / console error, `n*`/`c*`)에 더해 **유저·캡처 컨텍스트(콘솔 에러·액션·selector·유저 입력)와 exact-match되는 성공 200 응답**(`m*` — rich 스타일 전용)까지다. 요소 모드·description 섹션 비활성이면 미동작. 유효 지목이 **3개를 넘으면 에러 로그(n*/c*)를 우선 살리고 매칭(m*)부터 잘라 상위 3개만 삽입**한다(과거엔 초과 시 전부 버렸으나, 기존 에러 로그 삽입까지 사라지는 회귀라 에러 우선 slice로 바뀜). 200 후보에 대해서는 응답의 shape 다이제스트(키·타입, **값 제외**·데이터성 키 redact)만 프롬프트에 실려 모델이 "정상으로 보이는 200"을 원인으로 가려내고, 지목 시 삽입되는 건 기존 경로 그대로 원문이다. 사용자가 넣어 둔 코드블럭은 AI 재생성에도 보존되고(이미지처럼 섹션 하단으로 hoist), 같은 로그를 다시 지목해도 중복 삽입되지 않는다. 삽입 블록도 일반 텍스트라 수정·삭제 자유, 16줄 이상이면 접힘(코드블럭 접기). 가이드엔 내부 메커니즘(태그·상한 숫자·다이제스트) 노출 없이 "AI가 고른 실제 로그 원문이 붙는다(에러뿐 아니라 상황과 맞는 정상 200도) + 지어내지 않는다 + 없으면 안 붙는 게 정상 + 직접 넣은 블록은 유지" 수준으로 기술(faq "AI가 로그를 지어내지는 않나요?" 문항 참조). 대상: `screenshot/issue.md`·`video/issue.md`(✨ 절)·`settings/ai.md`·`faq.md`.
- **마크다운 복사본은 제출본과 갈린다 (이미지·로그 축)**: 미리보기의 `복사`/en `Copy`는 두 flavor(`text/plain`+`text/html`)를 클립보드에 얹지만 **이미지와 로그 파일은 담지 않는다.** 본문에 삽입한 인라인 이미지는 안내 문구로 치환되고(ko `(이 이미지는 복사되지 않습니다 — 이슈로 제출하면 첨부됩니다)` / en `(Image not copied — attached when you submit the issue)`, `md.inlineImageNotCopied`), Media 섹션은 `md.imageNotCopied`/`md.videoNotCopied`, 로그 요약 리드는 `logSummary.logs.notCopied`로 바뀐다(`src/i18n/namespaces/logs.ts`). **로그 요약의 통계 줄**(`Network: 17 requests`·`User actions: 3`)**은 그대로 남는다** — 복사본에서도 참이라서다. 이유는 클립보드 HTML에 `data:` 이미지가 하나라도 있으면 Notion·Slack·Jira가 붙여넣기를 **통째로** 거부해 제목·재현 단계까지 전부 유실되기 때문이고(실측, 크기 무관), 클라이언트 온리라 대신 쓸 호스팅 URL이 없다. **편집·미리보기 화면에는 이미지가 그대로 보이고, 제출 경로(8개 플랫폼)는 무영향**이다 — 축이 `MarkdownContext.forClipboard`이고 생산지는 `PreviewPanel`뿐이다. 가이드엔 "복사는 텍스트 공유용, 이미지가 필요하면 제출"을 안심 톤으로 쓴다. 대상: `screenshot/issue.md`·`video/issue.md`·`element/issue.md`(6. 미리보기 절 — 공통 흐름이라 세 페이지 동일 문구). `logs/viewer.md`의 복사 버튼은 log-viewer 자체 구현(`copyReport`)이라 이 축과 무관하다.
- **코드블럭 접기**: **16줄 이상**(임계값 15 — `CODE_COLLAPSE_LINE_THRESHOLD` in `src/sidepanel/lib/codeCollapse.ts`) 코드블럭은 15줄 높이로 **접힌 채** 렌더되고 하단이 페이드 처리된다. 코드블럭에 hover하면 하단 중앙에 pill이 뜬다 — 접힘 `펼치기 ({count}줄)`/en `Expand ({count} lines)`, 펼침 `접기`/en `Collapse`(`codeBlock.expand`/`.collapse` in `src/i18n/namespaces/editor.ts`). 괄호 안 숫자는 **블럭의 전체 줄 수**다. **로그 코드블럭만이 아니라 모든 코드블럭**에 같은 규칙이 적용된다(삽입된 로그와 손으로 친 코드블럭은 마크다운상 구분 불가). 적용 표면은 이슈 작성 에디터·미리보기·저장된 초안 상세·`logs.html` 리포트 탭. 에디터에서 **접힌 블럭 안에 커서를 넣으면 자동으로 펼쳐지고**, 커서가 나가도 펼침이 유지된다. **접힘은 순수 표시 상태다** — 영속되지 않고(화면을 옮기면 다시 접힌 상태로 시작), 마크다운 복사·8개 트래커로 나가는 이슈 본문·`logs.html`엔 **전문이 그대로** 들어간다. **모든 코드블럭 왼쪽에는 행 번호가 붙는다**(줄 수와 무관하게 항상, 인라인 `code`는 제외). 번호는 표시 전용이라 복사·이슈 본문엔 안 들어가고, 접힌 블럭에선 보이는 줄까지만 보인다. 가이드엔 "긴 로그는 접혀서 들어가고 pill로 펼친다 + 등록되는 본문엔 전문이 그대로(행 번호는 화면에서만)"를 안심 톤으로 쓰고, 임계값 숫자(15줄)는 노출해도 무방. 대상: `screenshot/issue.md`·`video/issue.md`(로그 본문 넣기 절 — `logs/live.md`는 그 두 페이지로 상세를 위임하므로 미기술).
- **diff table 이미지 주석**: 요소 모드 작성 화면의 before/after 비교 표에서 **두 이미지에 각각 주석**을 달 수 있다. 이미지에 hover(또는 Tab으로 카드에 포커스)하면 우상단에 액션 그룹이 뜬다 — 주석이 없으면 `[주석 추가]`(`draft.addAnnotation`) 하나, 있으면 `[주석 제거][주석 수정]`(`draft.removeAnnotation`/`.editAnnotation`) 둘. 편집기는 스크린샷 주석과 **같은 konva 오버레이**(`AnnotationOverlay`) 재사용이고, 재주석은 주석본 위에서 이어 그린다. `[주석 제거]`는 확인 없이 **최초 캡처 원본**으로 되돌린다(직전 주석본이 아니다). 표시·미리보기·AI 초안·제출 첨부가 전부 주석본으로 수렴하고 원본은 확장 안에만 남는다. after는 스타일을 더 고쳐 **다시 찍히면 주석이 폐기**된다(옛 좌표가 새 픽셀 위에서 엉뚱한 곳을 가리키는 게 주석이 없는 것보다 나쁘다) — before는 재캡처 경로가 없어 유지. 저장된 초안 상세·미리보기에서는 **표시만 주석본**이고 편집 버튼은 뜨지 않는다. 코드: `src/sidepanel/components/StyleChangesTable.tsx`(SnapshotCell·조건부 렌더)·`ImageActions.tsx`(액션 그룹)·`src/sidepanel/lib/diffAnnotation.ts`(슬롯·라우팅·배경 이미지)·`src/store/editor-store.ts`(`beforeAnnotated`/`afterAnnotated`·폐기 규율). 대상: `element/issue.md`(3. 미디어 절). `element/picker.md`의 "스크린샷에 주석을 달아 전달하는 편이 빠릅니다" 우회 안내는 이 기능이 이유를 없애 **삭제됨**(stale — 되살리지 말 것).
- **본문 이미지 어노테이션**: 본문(Tiptap 에디터) 문단 섹션에 붙여넣기·드래그·`이미지 추가`(`draft.addImage`)로 넣은 인라인 이미지에 **hover하면 우상단 액션 그룹**(코드블럭 collapse와 동일한 결·`block-actions` 스타일)이 뜬다 — `[주석 달기][삭제]`, 어노테이션 기록이 있으면 앞에 `[원본 복원]`이 더 붙는다(라벨 키 `editor.image.annotate`/`.reset`/`.delete` = `주석 달기`/`원본 복원`/`삭제`, en `Annotate`/`Restore`/`Delete`). **주석 달기**는 스크린샷 주석과 같은 konva 오버레이(`AnnotationOverlay`)를 재사용해 그 이미지를 편집하고 결과를 그 자리에 반영(래스터 webp), **원본 복원**은 주석 전 원본으로 되돌리고(주석 기록이 있을 때만 노출), **삭제**는 확인 없이 즉시 이미지 노드를 제거한다. 원본 백업·주석 결과는 **IndexedDB 로컬**에만 있고 이슈로 나가는 건 늘 현재 표시 중(주석됐으면 주석된) 이미지다(원본 백업은 외부로 안 나감 — privacy 코어밸류 정합). 작성 중(DraftingPanel)·제출 후 편집(DraftEditDialog) 양쪽 동작. 캔버스 그리기 자체는 스크린샷 어노테이션과 동일하니 상세는 그쪽으로 위임하고, 여기선 "넣은 이미지를 그 자리에서 주석·복원·삭제"만 안내. 코드: `src/sidepanel/components/TiptapEditor.tsx`(NodeView·오버레이 배선)·`src/sidepanel/lib/inlineImageAnnotation.ts`(blob 스왑)·`src/store/blob-db.ts`(`inlineImageOrigins` 백업 store). 대상: `screenshot/issue.md`·`video/issue.md`(본문 섹션 절 — `logs/live.md`·`element/issue.md`은 위임/미기술).
- **액션 로그 프라이버시**(가이드에 반드시 반영): 입력값(최대 500자)·드롭다운 선택값은 **마스킹에 걸리지 않으면 원문 그대로 기록·첨부**된다. 클릭·드래그한 **요소의 화면 표시 텍스트(접근가능 이름, 최대 80자)도 함께 기록**된다. 마스킹은 2층 — ① 필드 종류·라벨(비밀번호 타입, autocomplete, 이름/id/aria-label/연결 라벨/placeholder의 민감 키워드, 한국어 포함) ② 값 형태(이메일, 9자리 이상 숫자열=전화·카드·주민·계좌). 마스킹의 값 형태 기준은 **입력값뿐 아니라 요소 이름·필드 라벨에도 적용**된다. 리치 텍스트 편집기(메일 본문·문서)는 **값으로도 요소 이름으로도 안 남기고** 입력 사실만 기록. 다만 **라벨·값 어느 쪽에도 단서가 없는 값(검색어·일반 텍스트)은 원문이 남으므로**, 민감 화면에선 제출 전 로그 첨부를 끄라고 안내한다. 로그 첨부는 기본 on이고 작성 화면의 로그 카드 스위치로 통째로 해제 가능(로그별 개별 해제 아님). 대상: `logs/viewer.md`·각 `issue.md`의 로그 첨부 단계.
- **로그 URL 프라이버시**: console/network/action의 page URL, network 요청 URL, action navigation from/to는 모두 민감 query·fragment 키(`token`·`access_token`·`id_token`·`refresh_token`·`api_key`·`key`·`secret`·`password`·`auth` 등)의 값만 `***`로 마스킹한다. 일반 query와 경로는 재현을 위해 남는다. “액션 종류에 따라 raw URL이 남는다”는 과거 설명은 stale이다. 대상: `logs/live.md`·`logs/viewer.md` 및 각 `issue.md`의 로그 프라이버시 안내.
...
- **picker가 닿지 않는 것 (Shadow DOM 내부·`::before`/`::after`)**: picker는 `document.elementFromPoint`로 hit-test하고 `composedPath`/`shadowRoot`를 쓰지 않으므로 **Shadow DOM 내부 요소는 잡히지 않고 호스트 요소가 선택**된다. `::before`/`::after`는 DOM 노드가 아니라 애초에 hit-test 대상이 아니고, 스타일 편집도 요소의 인라인 스타일 오버라이드라 가상 요소엔 도달하지 못한다 — 둘 다 브라우저 동작이라 회피책이 없다. **이 설명은 가이드에만 있다**: 과거 사이드패널 캡처 진입 화면(`issue.mode.elementLimit`)에 캡션으로 노출했으나 "첫 화면에 과한 정보"로 판단해 UI에서 제거했고(i18n 키도 삭제), 설명 책임을 가이드로 옮겼다. 따라서 **이 소절을 지우면 제품 어디에도 안내가 없어진다**. 폴백 권고는 iframe 항목과 같은 **스크린샷 + 주석**. 대상: `element/picker.md`(ko "닿지 않는 요소" / en "Elements you can't reach" 소절).
...
- **element 편집 전후 이미지의 캡처 범위**: 선택한 요소만 잘라 담지 **않는다**. 요소를 감싼 가장 가까운 의미 단위 컨테이너(대화상자·표의 행·목록 항목·`article` 등)가 ① 화면에 온전히 들어오고 ② 요소를 완전히 포함하며 ③ 화면 면적의 40% 이하일 때 그 컨테이너까지 넓혀 찍는다(`src/content/capture-context.ts`·`src/sidepanel/lib/capture-basis.ts`). 셋 중 하나라도 어긋나면 요소 bbox+24px로 폴백한다. `form`·`fieldset`은 개인정보 노출 면적 때문에 후보에서 제외하고, iframe 내부 요소에는 확장을 적용하지 않는다. 가이드에 "선택한 요소만 담는다"고 쓰지 말 것.
- **설정 기본값**: 30초 리플레이 **꺼짐**, 테마 **라이트**(system 아님), 언어는 브라우저 언어 자동 판정(`detectLocale()`), 파일 첨부 **꺼짐**, 재현 단계 자동 채움 **켜짐**, AI 작성 언어 **자동**(= 화면 언어를 따름), 이슈 본문 언어 **자동**(= 화면 언어를 따름).
- **이슈 본문 언어**: 설정 > 이슈 설정 **최상단** 섹션이 `제목 설정`에서 **`공통`**(en `Common`, `settings.issueCommon`)으로 바뀌었고, 그 안에 `제목 접두어`(en `Title prefix`, `settings.titlePrefix.label` — 섹션 제목이 바뀌면서 신설된 행 라벨)와 **`이슈 본문 언어`**(en `Issue body language`, `settings.bodyLocale`) 셀렉터가 두 행으로 놓인다. **화면 언어·AI 작성 언어와 독립된 세 번째 축**(`bodyLocale` in `settings-ui-store.ts`, 기본 `auto`). auto는 저장 시점 스냅샷이 아니라 **호출 시점에 화면 언어로 해석**되고 셀렉터에 괄호로 표기된다(`자동 (한국어)`/en `Auto (English)`, `settings.bodyLocale.auto`). **AI 작성 언어와 달리 임의 언어를 못 고른다** — 사전 조회라 옵션이 등록 로케일(`LOCALES` = ko·en·fr)뿐이다. **바뀌는 범위를 반드시 함께 안내한다**: 복사·제출되는 본문의 **섹션 제목·표 헤더·로그 요약 문장만** 바뀌고, **사용자가 쓴 본문·AI가 쓴 글·사이드패널 화면(탭·버튼·미리보기)은 화면 언어 그대로**다. 섹션 `labelOverride`(본문 구성에서 직접 바꾼 라벨)는 본문 언어보다 **우선**한다. 노출 게이트가 없어 AI 작성 언어와 달리 **항상 보인다**(그쪽은 rich BYOK 전용). 두 축의 역할 분담(**AI 작성 언어 = 내용 / 이슈 본문 언어 = 스캐폴딩**)과 "둘 다 바꿔야 이슈가 통째로 그 언어"를 양쪽 페이지에 교차로 쓴다. 대상: `settings/issue.md`(ko "공통" / en "Common")·`settings/ai.md`(ko "AI 작성 언어" / en "AI writing language" — 역할 분담 표). **과거 ai.md의 "화면은 한국어로 두고 이슈 본문만 영어로 받는 식"·en "get the issue body in French"는 stale이다** — 그때 바뀐 건 AI가 쓴 *내용*뿐이고 섹션 제목은 화면 언어였다. 이 기능이 스캐폴딩 쪽을 사실로 만들었으므로 두 설정을 구분해 쓴다.
- **AI 작성 언어**: 설정 > AI 모델 탭 하단에 `AI 작성 언어`(en `AI writing language`, `llm.section.outputLanguage`) 셀렉터. **화면 언어(`locale`)와 독립된 축**(`aiLanguage` in `settings-ui-store.ts`, 기본 `auto`). auto는 저장 시점 스냅샷이 아니라 **매 생성 때 화면 언어로 해석**하므로 나중에 화면 언어를 바꿔도 AI만 옛 언어로 남지 않는다 — 셀렉터엔 해석 결과가 괄호로 붙는다(`자동 (한국어)`/en `Auto (English)`, `llm.outputLanguage.auto`). 프리셋 9종은 **자기 언어 표기**로 노출(English · 한국어 · 日本語 · 中文(简体) · Español · Français · Deutsch · Português (Brasil) · Русский — `AI_LANGUAGE_OPTIONS` in `src/sidepanel/lib/aiLanguage.ts`). 적용 범위는 **AI 초안 작성 + 재현 과정 채우기**이고 **AI 스타일링은 대상 아님**. **노출 조건이 "LLM 연결됨"이 아니라는 게 함정** — 원격 BYOK LLM일 때만 뜨고 **Chrome 내장 AI·localhost 로컬 엔드포인트(Ollama 등)에서는 숨는다**(그 경로는 화면 언어를 따른다. 내부적으론 `capabilities.promptStyle === "rich"` 게이트 — 가이드엔 이 용어를 노출하지 않는다). "왜 나에겐 안 보이지?"가 바로 나오는 지점이라 **가이드는 이 조건을 반드시 함께 안내**한다. 대상: `settings/ai.md`(ko "AI 작성 언어" / en "AI writing language").
...
- **BYOK 프로바이더 프리셋**(단일 출처 `PROVIDER_PRESETS` in `src/sidepanel/lib/ai-provider.ts`): OpenAI · Anthropic · Gemini · **Mistral** · Groq · Together · OpenRouter · Ollama 8종 + 목록에 없으면 `직접 입력`/en `Custom`(`llm.providerCustom`)으로 OpenAI 호환 엔드포인트 직접 입력. **프리셋 label은 고유명사라 ko 가이드에서도 영문 그대로 인용**하고 "화면에는 영문 표시"를 한 줄 붙인다(§6 예외 조항). 프리셋이 추가·제거되면 `settings/ai.md`(ko/en)의 목록을 즉시 갱신한다 — 사용자가 "지원 안 함"으로 오해해 이슈를 여는 경로다(#198 Mistral). 대상: `settings/ai.md`(ko "연결하기" / en "Connect").
- **로그 뷰어 마커**: 콘솔/네트워크/액션 3종. 페이지 이동은 액션 마커의 variant(별도 타입 아님). `logs.html`은 빌드 산출물 → 일반 사용자는 "이슈 첨부로 받은 리포트를 여는" 개발자 관점으로만 기술.
- **로그 뷰어 통합 타임라인 패널**: 영상 리포트 한정(영상 없는 스크린샷 모드엔 미표시 — `video && !videoError` 게이트, `src/log-viewer/App.tsx`·`components/TimelinePanel.tsx`·`TimelineRow.tsx`·`timeline-merge.ts`·`timeline.ts`(`findActiveIndex`)). 영상 **아래**에 콘솔·네트워크·액션 로그를 absTs 시간순으로 **한 줄씩 병합**한 리스트(좌측 스크러버 마커와 별개 UI). 상단에 **단일선택 타입 필터 탭**(`전체·콘솔·네트워크·액션` — `timeline.filter.*`, 각 탭 개수 Badge) + **검색창**(`타임라인 검색…`/en `Search timeline…` — `timeline.searchPlaceholder`, console args·network URL/응답 본문까지 매칭). **행 클릭 = 영상 seek + 오른쪽 해당 로그 탭 전환 + 스크롤**(왼쪽 시간 칩만 눌러도 seek). 네트워크 행은 액션 로그처럼 method를 자연어 동사 문장으로 조립(`timeline.net.verb.*` — ko `{path} 가져옴/전송/…`, en `Fetched/Posted/… {path}`, 경로 슬롯은 파랑 밑줄이나 링크 이동 아님). **재생(playhead) 진행 시 현재 시각 이하 마지막 행 자동 하이라이트**(`findActiveIndex`). 가이드엔 내부 용어(absTs·verb 키) 노출 없이 "영상 아래 3종 통합 목록 + 필터·검색 + 눌러서 영상 이동 + 재생 따라 현재 줄 강조" 수준. 대상: `logs/viewer.md`(ko "통합 타임라인" / en "The merged timeline").
...
- **연결 후 기본값**(Connect 탭에서 지정 — `connect/*ConnectForm.tsx`): **위치**(Jira·GitLab=프로젝트 / GitHub=저장소 / Linear=팀→프로젝트 / Notion=DB / Asana=워크스페이스→프로젝트 / ClickUp=워크스페이스→스페이스→리스트 / Slack=채널) + **이슈에 채워질 값**(담당자 6개[Jira·GitHub·GitLab·Linear·Asana·ClickUp — **Notion·Slack은 담당자 개념 없음**] / 라벨 3개[GitHub·GitLab·Linear] / 기본 이슈 타입[Jira] / 상태·select 속성값[Notion]). 라벨은 실제 UI 문구로: `담당자`/`Assignee`·`라벨`/`Label`·`기본 이슈 타입`/`Default issue type`. **3가지 규칙을 반드시 함께 안내**: ① **직전 제출값이 기본값보다 우선**(기본 담당자를 정해도 마지막에 다른 사람을 골랐으면 그 사람이 이어진다 — 기본값이 직전값을 가리지 않는다) ② **후보 조회에 상위 값이 선행 필요**(GitHub=저장소, GitLab=프로젝트, Linear=팀, Asana·ClickUp=워크스페이스. 미선택 시 콤보박스 비활성 + "먼저 선택하세요" 안내. **Jira만 예외** — 유저 검색이 사이트 전역이라 프로젝트 없이도 고를 수 있다) ③ **상위 값을 바꾸면 하위 담당자·라벨 기본값이 비워진다**(다른 저장소·프로젝트의 멤버라 무효 — 조용히 엉뚱한 사람이 배정되는 것보다 낫다). **GitHub 저장소 검색만 사정거리가 넓다** — 본인 소유를 넘어 공개 저장소까지 닿고 입력은 이름·`owner/name`·저장소 URL·클론 URL·GitHub 검색 문법을 받는다(`buildRepoSearchQuery`). 이슈를 못 여는 저장소(`archived`·`has_issues:false`)는 `이슈 꺼짐`/`Issues off`·`보관됨`/`Archived` 배지 + 선택 불가. 출처: `integrations/platforms.md`("연결 후 기본값" 절).
- **재연동(다시 연결)은 해제 없이 가능하다**: "플랫폼 추가" 목록에서 **이미 연결된 플랫폼도 버튼이 활성**이고 문구가 `{플랫폼} 다시 연결`/en `Reconnect {platform}`이다(과거 타일의 초록 체크 표시는 제거됨 — 쓰지 않는다). **과거의 "연결 완료라 비활성 → 먼저 해제해야 재연동" 서술은 stale이니 쓰지 않는다.** 인증 만료 안내에는 `닫기`/en `Close` + `다시 연결`/en `Reconnect` 두 버튼이 있고(과거 `확인` 단일 버튼은 stale), 후자를 누르면 연동 탭으로 이동하며 그 플랫폼의 연결 방식 창이 자동으로 열린다. **재연동 시 초기화 범위를 반드시 함께 안내**: 그 플랫폼의 **기본값은 초기화**되고, **다른 계정으로** 연결한 경우에만 직전 제출값도 함께 초기화된다(같은 계정이면 보전). 문구 정본은 `src/i18n/namespaces/app.ts`의 `platform.reconnect.note`이고, OAuth가 가능한 플랫폼은 연결 방식 창 안에, 토큰으로 직행하는 경우와 Slack은 별도 확인 창에 뜬다. 소스: `src/sidepanel/tabs/connect/{PlatformConnectFlow,JiraConnectForm,SlackConnectForm,ReconnectConfirmDialog}.tsx` · `src/sidepanel/components/OAuthExpiredDialog.tsx` · `src/store/settings-store.ts`(`setAccount`). 대상: `integrations/platforms.md`("다시 연결하기" 절).
- **Jira는 제출 시점에도 두 가지를 고른다**(연동 탭 기본값과 별개): **프로젝트**(맨 위 — 옮기면 이슈 타입·담당자·상위 에픽·연결 이슈가 비고 우선순위·참조는 남는다)와 **스프린트**(이슈 타입 바로 아래). 스프린트 칸은 **createmeta가 "이 프로젝트·이슈 타입에 스프린트 필드가 있다"고 답할 때만** 그려진다 — 칸반 전용·보드 미연결 프로젝트에는 아예 안 나오므로 "항상 있는 필드"로 쓰면 거짓이다. 보드가 여럿이면 항목에 보드명이 함께 보이고, 직전 제출값이 복원되되 그 사이 종료된 스프린트면 자동으로 비워진다. **OAuth 연동 사용자는 재연동 전까지 목록이 빌 수 있다**(scope가 늘었고 기존 토큰은 동의 시점 권한에 고정 — API 토큰 연동은 무관. 해법은 위 "재연동" 항목의 다시 연결이고, **해제가 선행돼야 한다고 쓰지 않는다**). 참고로 그 상태에서 뜨던 가짜 "인증이 만료되었습니다" 안내는 제거됐다 — 목록이 비는 것만 남는다. 소스: `src/sidepanel/tabs/jiraFields/{SprintField.tsx,useSprintFieldMeta.ts,sprint-sticky.ts,project-change.ts}` · `src/background/jira-api.ts`(`pickSprintField`/`listSprints`) · `src/background/oauth.ts`(`SCOPES`). 출처: `integrations/platforms.md`("연결 후 기본값" 절 말미 인용구 2개).
- **플랫폼 표** (현 시점 9개 스냅샷 — 인증 축이 있는 8개 + 축 밖의 Custom Webhook):
  ...
  | GitLab | OAuth / PAT (**OAuth는 gitlab.com 전용** — self-managed 인스턴스는 PAT만 된다. 가이드에서 self-managed 안내에 OAuth 버튼을 섞지 말 것) | instanceUrl(기본 gitlab.com prefill — self-managed면 변경. **https 강제**, loopback만 예외 — 평문 http는 `gitlab.instanceUrl.insecure` 안내로 거부), pat | gitlab.com PAT |
  | Asana | OAuth / PAT | pat | app.asana.com my-apps |
  | ClickUp | OAuth / API Token | pat(`pk_...`) | app.clickup.com 설정 > Apps |
  | Slack | OAuth 전용 | — (토큰 입력 없음) | — |
  | Custom Webhook | 주소 직접 입력(연결 수단 선택 없음) | url(**https 강제**, 사설망 http만 예외 — 그 경우 평문 경고 배지), secret(선택), 고급: 요청 헤더·format·JSON 템플릿 | — (사용자가 직접 만든 수신 서버) |
...
- **이미지**: 모든 스크린샷 자리에 placeholder + 의미 있는 alt/캡션. **실제 촬영·재촬영은 [SHOOTING.md](./SHOOTING.md)가 단일 출처**이고 `/guide-shots` 스킬이 그걸 로드해 실행한다 — 이 문서는 "어느 자리에 무슨 이미지가 오는가"까지만 정한다.
  ...
  - **화면이 바뀌어 기존 실제 스크린샷이 stale해지면 `dummy.jpg`로 되돌린다**(ko·en 양쪽). 틀린 화면을 그대로 두는 것보다 비어 있는 편이 낫고, 되돌려 둬야 재촬영 대상이 눈에 띈다. 섹션이 사라져 자리가 없어지면 해당 파일은 삭제하고, 순번이 밀리면 뒤 번호를 당겨 **페이지 내 순번이 연속**이 되게 맞춘다. 어느 화면을 다시 찍어야 하는지는 캡션이 말해 주므로 캡션을 먼저 정확히 고친다.
  - **재촬영 대기**(placeholder로 되돌려 둔 자리): `video/record.md`의 `video-record-5`(녹화 정지 후 자르기 화면 — 4탭·타임라인 손잡이·하단 액션바가 보이게). 촬영 후 이 목록에서 지운다. **이 목록은 `dummy.jpg`와 바이트가 같은 자리만** 담는다 — "코드가 이미지보다 새로워 다시 찍어야 하는" 자리는 여기 쌓지 말고 `/guide-shots check`가 매번 판정한다(목록에 남겨두면 촬영 후 지우는 걸 잊어 곧 거짓말이 된다. 실제로 2026-08-09에 채운 5장이 여기 그대로 남아 있었다).
...
# 3b. ko/en 페이지별 heading level·순서 대칭
for p in $(cd ko && find . -name '*.md'|sort); do
  diff <(grep '^#' "ko/$p" | sed 's/[^#].*//') <(grep '^#' "en/$p" | sed 's/[^#].*//') || echo "HEADING ORDER MISMATCH $p"
done

# 3c. ko/en SUMMARY 링크 경로·순서·indent 대칭 + SUMMARY가 실제 md 트리를 전부 포함하는지 확인
diff <(grep -oE '^ *[-*] \[[^]]+\]\([^)]+\.md\)' ko/SUMMARY.md | sed -E 's/\[[^]]+\]/[]/') <(grep -oE '^ *[-*] \[[^]]+\]\([^)]+\.md\)' en/SUMMARY.md | sed -E 's/\[[^]]+\]/[]/')
for lang in ko en; do
  diff <(cd "$lang" && find . -name '*.md' ! -name SUMMARY.md | sed 's#^./##' | sort) <(grep -oE '\([^)]+\.md\)' "$lang/SUMMARY.md" | tr -d '()' | sort)
done
