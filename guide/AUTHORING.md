...
| AI 로그 코드블럭 자동 삽입(logRefs) | `src/sidepanel/lib/prompts/logCandidates.ts`(후보 단일 출처·description 게이트) · `src/sidepanel/lib/renderLogRefs.ts`(상한 3·역참조·삽입) · `src/sidepanel/lib/mergeAiDraftSections.ts`(코드블럭 보존) · 배선 `src/sidepanel/tabs/AiDraftDialog.tsx` |
...
- **AI 로그 코드블럭 자동 삽입**: AI 초안 생성 시 AI가 후보 목록([n1]/[c1] 태그)에서 **관련 로그 id만 지목**하면, 앱이 store 원본을 `logToCodeBlock` 직렬화로 **발생 현상(description) 섹션 끝에 코드블럭으로 삽입**한다 — 로그 본문은 모델을 거치지 않아 **내용 환각이 구조적으로 불가능**(수동 [로그 추가]와 같은 직렬화·같은 결과). 후보는 **에러만**(network 4xx/5xx·error / console error), 요소 모드·description 섹션 비활성이면 미동작. 유효 지목이 **4개 이상이면 전부 버린다**(화면 안내 없음 — "관련 로그 없음"과 구분 안 됨, 의도된 침묵). 사용자가 넣어 둔 코드블럭은 AI 재생성에도 보존되고(이미지처럼 섹션 하단으로 hoist), 같은 로그를 다시 지목해도 중복 삽입되지 않는다. 삽입 블록도 일반 텍스트라 수정·삭제 자유, 16줄 이상이면 접힘(코드블럭 접기). 가이드엔 내부 메커니즘(태그·상한 숫자) 노출 없이 "AI가 고른 실제 로그 원문이 붙는다 + 지어내지 않는다 + 없으면 안 붙는 게 정상 + 직접 넣은 블록은 유지" 수준으로 기술(faq "AI가 로그를 지어내지는 않나요?" 문항 참조). 대상: `screenshot/issue.md`·`video/issue.md`(✨ 절)·`settings/ai.md`·`faq.md`.
