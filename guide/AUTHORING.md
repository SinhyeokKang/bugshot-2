...
- **디바이스 뷰포트**: 디버그 > **이슈 작성** 서브탭의 서브탭 바 **바로 아래 독립 행**(로그 탭 필터 행과 같은 컨테이너)에 4세그먼트 컨트롤 `[전체][390][768][1024]`(en `Full`). 기본 `전체`. `전체`만 사전 키(`issue.device.full`)이고 **폭 라벨은 사전에 없다** — `DEVICE_PRESETS.width`를 그대로 찍는다(번역 대상이 아니라 이중 저장을 없앴다). 접근명은 `.aria.full`/`.aria.width`. **`430`은 없다**(390과 브레이크포인트 구간이 겹쳐 제외). 각 세그먼트는 기기 아이콘(Monitor/Smartphone/Tablet/Laptop) + **폭 숫자 병기**이고, 패널이 좁으면 다른 탭 바처럼 라벨이 접혀 아이콘만 남되 접근명은 폭을 그대로 말한다. 구현은 top 문서 안에 같은 URL을 싣는 iframe 래퍼(`#__bugshot_device_frame__`)이고 원본 body 자식은 스타일시트 한 장으로 감춘다 — **가이드엔 이 메커니즘을 노출하지 않는다**("그 폭짜리 화면 안에서 다시 그려집니다" 수준). **에뮬레이트 범위를 반드시 표로 명시**: 폭(`innerWidth`·`vw`)·미디어쿼리·`position: fixed`는 O / **DPR(2x·3x srcset)·터치 이벤트·User-Agent는 X**(`debugger` 권한 미취득이 확정 제약 — 없는 것을 있다고 쓰면 리포트가 틀린다). 기기 이름(`iPhone 14`) 대신 숫자를 쓰는 이유도 같다. **재로드 규칙 2가지를 함께**: 모드 진입·`전체` 복귀는 재로드하지만 **폭에서 폭으로의 전환은 재로드하지 않는다**(스크롤·입력값 유지). 최초 ON 1회 확인 다이얼로그(`issue.device.modeWarning.*` — ko 제목 `페이지를 다시 불러옵니다`/en `The page will reload`, 버튼 `계속`·`취소`)는 재로드 + **원본·래퍼 동시 실행에 따른 중복 요청·자동저장**을 함께 고지하고, `[계속]` 이후 재노출 없음(`deviceModeWarned` in `settings-ui-store.ts`). **cross-origin 이동은 탭 전체가 그 사이트로 다시 열린다**(`issue.device.handoffToast`) — 이유는 프레임에 가두면 third-party storage partitioning으로 로그인이 풀린 화면이 떠 재현 정보가 틀리기 때문이고, 가이드엔 "로그인 상태를 지키려고"까지만 쓴다. **폭 승계는 보장이 아니다** — 패널이 500ms `expiresAt` 안에 준비하지 못하면 폭을 나르지 않고 `전체`로 강등하며, 통보(`issue.device.handoffToast` 또는 세션 만료 안내)는 승계 여부와 무관하게 뜬다. 가이드엔 "드물게 폭이 안 이어지면 한 번 더 고르면 된다" 수준으로 쓰고 시간 상한은 노출하지 않는다. 리다이렉트 루프면 안내 후 `전체` 복귀(`issue.device.loop.*`). **모드 ON 제약 3가지**: (1) **페이지 캡처만 잠김**(`issue.capturing.method.fullPageDeviceLocked` — 나머지 캡처·녹화는 정상) (2) 페이지 안 iframe 요소 선택 불가(`app.iframeUnsupported.bodyDeviceMode`) (3) 프레임 삽입을 막는 페이지는 `전체`로 롤백(`issue.device.blocked`). 가용 폭을 넘는 세그먼트는 `aria-disabled` + 툴팁(`issue.device.tooltip.tooNarrow`)이고 창을 넓히면 조작 없이 풀린다. 캡처·녹화 중에는 행 전체 잠김(`.tooltip.locked`), **작성 화면에서는 행이 숨고 재현 환경에 `뷰포트 390px` 표시**(`issue.device.indicator`). 리포트 재현 환경의 뷰포트 값·캡처 이미지·영상이 전부 고른 폭으로 수렴한다. 소스: `src/sidepanel/components/DeviceViewportBar.tsx`·`device-viewport-controller.ts`·`src/content/device-frame.ts`·`src/background/device-frame-coordinator.ts`, 라벨 `src/i18n/namespaces/issue.ts`(`issue.device.*`)·`app.ts`(`app.iframeUnsupported.bodyDeviceMode`). 대상: `device-viewport.md`(ko/en — 상세의 단일 출처)·`screenshot/capture.md`(페이지 캡처 잠금 1줄 + 위임 링크).
...
- **설정 기본값**: 30초 리플레이 **꺼짐**, 테마 **라이트**(system 아님), 언어는 브라우저 언어 자동 판정(`detectLocale()`), 파일 첨부 **꺼짐**, 재현 단계 자동 채움 **켜짐**.
