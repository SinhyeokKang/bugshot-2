...
| 스타일 폼/코드 뷰·왕복 | `src/sidepanel/tabs/StyleEditorPanel.tsx`(토글) · `styleEditor/StyleCodeEditor.tsx`(코드 textarea) · `styleEditor/inlineCssText.ts`(serialize/parse 왕복) · `settings-ui-store.ts`(`styleEditorView` 영속) · 토글 라벨 `editor.ts` `editor.view.*`·`editor.codePlaceholder` |
...
- **스타일 편집 폼/코드 뷰**: 요소 스타일 편집 패널 상단(요소 이름 밴드 아래, sticky)에 **폼/코드 토글**(`editor.view.form`/`.code` = `폼`/`코드`, en `Form`/`Code`, shadcn Tabs). 기본 **폼**. **코드** 모드는 raw CSS textarea 하나(`editor.codePlaceholder` = `padding: 3rem;` placeholder)로 `element.style` 인라인을 직접 타이핑 편집 — 폼에 칸 없는 임의 속성(`cursor` 등)·`!important`(값 끝에 붙이면 CSSOM priority로 적용) 입력 채널. 두 모드는 같은 `styleEdits.inlineStyle` 단일 출처를 공유해 **폼↔코드 양방향 동기화**(왕복 무손실). 고른 모드는 **영속**(`styleEditorView` in `settings-ui-store.ts`, 기본 `form` — 다음 요소 선택 시 유지). class·Text 섹션·변경사항 보기·AI 배너·푸터는 두 모드 **공통**(모드 전환은 스타일 편집 영역만 스왑, 비활성 모드는 hidden으로 마운트 유지). 대상: `element/styling.md`(ko "폼으로, 또는 코드로" / en "By form, or by code").
