...
- `guide/ko`·`guide/en`은 사용자 가이드의 in-repo 소스다(코드가 진실, 원격에서 직접 편집하지 않는다). **bugshot-web docs-portal이 빌드타임에 `guide/{ko,en}`를 fetch해 `bug-shot.com/{locale}/docs`로 서빙**한다(guide 변경 시 `.github/workflows/trigger-web-deploy.yml`이 Vercel 재배포 트리거). (레거시 GitBook `bugshot.gitbook.io/{ko,en}`는 **확장 신버전(가이드 링크 교체)의 웹스토어 리뷰 통과까지 유지**하고 이후 내림 — 현 배포본 사용자는 아직 gitbook 링크를 쓰므로. 신규 규칙은 docs-portal 기준으로 쓴다.)
- **언어별 독립**이다:
...
| 어노테이션 | `src/sidepanel/components/AnnotationOverlay.tsx` + `annotation/AnnotationToolbar.tsx`·`presets.ts` (BugShot 자체 konva 툴바 — 도구/색상/두께, 라벨은 `editor.ts` `annotation.*`로 ko/en 현지화) |
...
- **언어 전환 링크 없음**: 본문 페이지 상단의 per-page `🌐` 언어 전환 링크는 **전부 제거됐다**(과거엔 H1 바로 아래 1줄). 언어 전환은 **bug-shot.com/docs의 글로벌 로케일 스위처**가 담당하므로 문서 본문엔 언어 링크를 넣지 않는다. 새 페이지는 `H1 → 빈 줄 → (스크린샷) → 본문`으로 시작한다. (docs-portal 이전 전까지 gitbook엔 글로벌 스위처가 없지만, 이관이 임박해 소스에서 선제 제거함.)
  - **slug 매핑은 여전히 유효**(문서 상호참조 상대 `.md` 링크 + docs-portal 라우팅용). GitBook/docs slug는 파일 경로 기반, ko/en 동일 경로:
    ...
    - 루트 `quick-start.md`·`faq.md` → `/quick-start`·`/faq` (docs-portal은 **파일 경로 기반 flat slug** — SUMMARY 소개 하위 중첩은 사이드바 그룹만 바꾸고 slug엔 영향 없음. gitbook 시절 `/readme/quick-start`와 다르다.)
