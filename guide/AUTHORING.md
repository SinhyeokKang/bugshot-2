...
- **녹화 모드 설정**: 캡처 진입 화면(디버그 탭)의 **녹화 버튼은 1개**(`mode-record`)이고, 탭/화면 중 무엇을 녹화할지는 **설정 > 이슈 설정 > 녹화 설정 > 녹화 모드** Tabs에서 고른다(`recordingMode` in `settings-ui-store.ts`, 기본 `tab`). 설정 섹션 제목은 `녹화 설정`/en `Recording settings`(`settings.recording` 키, 30초 리플레이도 같은 섹션). 설정 Tabs 라벨은 짧게 `탭`/`화면`(en `Tab`/`Screen`, `settings.recordingMode.tab`/`.screen`, **아이콘 없음**) — 캡처 화면 녹화 버튼 라벨 `탭 녹화`/`화면 녹화`(`issue.mode.video`/`.screenRecord`)와 **키·문구 분리**. 캡처 진입 화면 Row3은 `[녹화 버튼][30초 리플레이][⚙ 정방형]` 3-segment ButtonGroup. **톱니바퀴 ⚙**(`mode-record-settings`)와 **비활성 30초 리플레이 버튼** 둘 다 설정 탭으로 이동하지 않고 **"녹화 설정" 다이얼로그**(`RecordingSettingsDialog` → 녹화 모드 Tabs + 30초 리플레이 토글)를 연다. 설정 탭 섹션과 다이얼로그는 같은 공용 카드(`RecordingSettingsCard`, store 단일 출처)라 **항상 sync**. 고른 모드는 캡처 화면 녹화 버튼 아이콘·라벨에 라이브 반영. 진입 화면 레이아웃: Row1 `[요소 편집][요소 캡처]`(ButtonGroup) / Row2 `[범위 캡처]` 단독 / Row3 위 3-segment. (과거 `탭 녹화`/`화면 녹화` 분리 버튼 2개 → 단일 버튼 + 설정/다이얼로그로 전환 — `video/record.md`·`settings/issue.md`·`video/replay.md` 대상.)
