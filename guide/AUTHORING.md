...
- **이미지**: 모든 스크린샷 자리에 placeholder + 의미 있는 alt/캡션.
  ...
  - **재촬영 대기**(placeholder로 되돌려 둔 자리): `settings/issue.md`의 `settings-issue-4`(본문 구성 — 드래그 손잡이·미디어 · 로그 행·복원 버튼이 보이게)·`settings-issue-5`(기타 — 재현 과정 채우기·파일 첨부 2행), `logs/viewer.md`의 `logs-viewer-4`(통합 타임라인 패널 — 영상 아래 3종 병합 목록 + 필터 탭·검색창)·`logs-viewer-5`(리포트 탭). 촬영 후 이 목록에서 지운다.
