...
├── log-viewer/          # Self-contained HTML 로그 뷰어 (별도 Vite 빌드 → dist-log-viewer/index.html 단일 파일). Light-only, Console/Network/Action Log 탭(아이콘+배지), PageFooter(이슈 바로가기+탭별 다운로드). 영상 또는 스크린샷 임베드 시 ResizablePanelGroup으로 좌(미디어)/우(3탭) 분할 + 영상↔로그 타임라인 동기화(스크린샷은 정적이라 동기화 없음). markers.ts(buildMarkers — 활성 탭별 console/network/action 마커 생성, variant별 색 분류). components/VideoPlayer.tsx(커스텀 비디오 플레이어 — 재생/일시정지/다운로드/이슈 오버레이/마커 연동), components/ImageViewer.tsx(screenshot 모드 좌측 정적 이미지 뷰어 — VideoPlayer 컨테이너·배경 공유, 하단 컨트롤 없음), components/IssueTitleOverlay.tsx(VideoPlayer·ImageViewer 공용 상단 이슈 제목 오버레이 — 호버 시 dim+텍스트 노출), components/ProgressBar.tsx(타임라인 프로그레스 바 + variant별 마커 핀 + 툴팁). timeline.ts(findActiveIndex/toVideoSeconds/formatPlayerTime 순수 헬퍼). 경량 i18n(navigator.language 기반)
...
├── lib/                 # 공용 유틸 (session-keys, adf-sentinels, url-support, settings-storage, notion-page-id, key-obfuscation, pending-log-prune, capture-commands, element-label, inject-issue-url, external-links(USER_GUIDE_URLS — locale별 GitBook 가이드 URL, 설정·이슈 idle 푸터 버튼이 사용), utils)
