...
│   ├── 30s-replay/      # 30s Replay 캡처 — frame-buffer(직전 30초 순환 버퍼), mp4-encoder(WebCodecs VideoEncoder→mp4-muxer H.264 인코딩), use-30s-replay(captureVisibleTab 폴링 훅 + capture()), replay-context(ReplayProvider로 isReady/isEncoding/capture 공유)
