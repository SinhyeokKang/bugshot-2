...
│   └── messages.ts      # 메시지 핸들러 디스패치 (jira.* / github.* / linear.* / notion.* / gitlab.* namespace)
├── content/
...
│   │   ├── connect/       # 플랫폼별 연결 폼 (JiraConnectForm, GithubConnectForm, LinearConnectForm, NotionConnectForm, GitlabConnectForm) + ConnectMethodDialog(OAuth/토큰 연결 수단 선택 다이얼로그) + gitlabInstanceUrl(self-managed 인스턴스 URL 정규화) — IntegrationsTab 'add' 서브탭 content
...
│                        # settings v7: accounts: { jira?, github?, linear?, notion?, gitlab? } + lastSubmitFields per platform + global titlePrefix
...
├── styles/
└── types/               # platform.ts (PlatformId/Accounts/LastSubmitFieldsByPlatform), github.ts, jira.ts, linear.ts, notion.ts, gitlab.ts (GitlabAuth/GitlabPatAuth/GitlabOAuthAuth + 이슈 페이로드·상태 타입), environment.ts (EnvironmentRow), console.ts, network.ts, action.ts (ActionEntry/ActionLog/ActionLogSummary), messages.ts, picker.ts, log-viewer.ts (LogViewerData — 로그 뷰어 데이터 인터페이스, video 동기화 임베드 포함), user-agent-data.d.ts (NavigatorUAData 타입 보강) 등
