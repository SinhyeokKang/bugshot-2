...
├── log-viewer/          # Self-contained HTML 로그 뷰어 (별도 Vite 빌드 → dist-log-viewer/index.html 단일 파일). Light-only, Console Log/Network Log 탭(아이콘+배지), PageFooter(이슈 바로가기+탭별 다운로드). 경량 i18n(navigator.language 기반)
...
├── styles/
└── types/               # platform.ts (PlatformId/Accounts/LastSubmitFieldsByPlatform), github.ts, jira.ts, linear.ts, notion.ts, environment.ts (EnvironmentRow), console.ts, network.ts, messages.ts, picker.ts, log-viewer.ts (LogViewerData — 로그 뷰어 데이터 인터페이스), user-agent-data.d.ts (NavigatorUAData 타입 보강) 등
