...
│   ├── hooks/           # useBoundTabId, useAI, useBackgroundRecorder, useCaptureShortcuts, useCommandShortcuts, useEditorSessionSync, useIssueImages, usePickerMessages, useBufferThenSwitch(복수 element 버퍼 — 현재 요소의 after 스냅샷 캡처 후 버퍼에 push하고 다음 요소로 picker 재시작, busy 가드로 중복 클릭 차단), usePlatformFields(SubmitFieldsDialog 공용 platform fields state + open/draft 전환 시 idempotent reset), useThemeEffect
