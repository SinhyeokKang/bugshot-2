...
├── log-viewer/          # Self-contained HTML 로그 뷰어 (별도 Vite 빌드 → dist-log-viewer/index.html 단일 파일). Light-only, Console/Network/Action Log 탭(아이콘+배지), PageFooter(이슈 바로가기+탭별 다운로드). 영상 임베드 시 ResizablePanelGroup으로 좌(영상 플레이어)/우(3탭) 분할 + 영상↔로그 타임라인 동기화. timeline.ts(findActiveIndex/toVideoSeconds 순수 헬퍼). 경량 i18n(navigator.language 기반)
