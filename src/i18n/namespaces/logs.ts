const ko = {
  "networkLog.clear": "로그 지우기",
  "networkLog.search": "URL·본문 검색…",
  "logViewer.seekTo": "{time} 지점으로 이동",
  "networkLog.dialog.selectRequest": "요청을 선택하세요",
  "networkLog.detail.general": "일반",
  "networkLog.detail.url": "URL",
  "networkLog.detail.method": "Method",
  "networkLog.detail.status": "Status",
  "networkLog.detail.time": "Time",
  "networkLog.detail.contentType": "Content-Type",
  "networkLog.detail.requestHeaders": "요청 헤더",
  "networkLog.detail.responseHeaders": "응답 헤더",
  "networkLog.detail.copyCurl": "cURL 복사",
  "networkLog.detail.noBody": "본문 없음",
  "networkLog.tab.headers": "헤더",
  "networkLog.tab.request": "요청 본문",
  "networkLog.tab.response": "응답 본문",
  "networkLog.display.binary": "바이너리 응답 ({type} · {size}) · 본문 미저장",
  "networkLog.display.stream": "스트리밍 응답 ({type}) · 본문 캡처 안 됨",
  "networkLog.display.bodyTruncated": "본문 잘림 ({size} · 한도 {limit})",
  "networkLog.display.bodyOmitted": "본문 생략 (메모리 한도)",
  "networkLog.display.pending": "응답 대기 중",
  "networkLog.display.queued": "큐에 등록됨",
  "networkLog.display.queueFull": "실패 · 큐 가득 참",
  "networkLog.display.aborted": "취소됨",
  "networkLog.display.timeout": "시간 초과",
  "networkLog.display.blocked": "실패 · 상태 가려짐",
  "networkLog.display.blockedHint": "CORS 또는 연결 실패로 실제 서버 상태 코드를 읽을 수 없습니다.",
  "networkLog.filter.all": "전체",
  "networkLog.filter.json": "JSON",
  "networkLog.filter.js": "JS",
  "networkLog.filter.css": "CSS",
  "networkLog.filter.img": "Img",
  "networkLog.filter.font": "Font",
  "networkLog.filter.doc": "Doc",
  "networkLog.filter.other": "기타",
  "networkLog.filter.ws": "WS",
  "networkLog.tab.messages": "메시지",
  "networkLog.ws.framesCount": "{n} 프레임",
  "networkLog.ws.dropped": "{n} 누락",
  "networkLog.ws.all": "전체",
  "networkLog.ws.send": "송신",
  "networkLog.ws.receive": "수신",
  "networkLog.ws.opened": "연결 열림",
  "networkLog.ws.closed": "연결 닫힘 ({code})",
  "networkLog.ws.empty": "아직 프레임이 없습니다",

  "consoleLog.clear": "로그 지우기",
  "consoleLog.search": "메시지 검색…",
  "consoleLog.detail.stackTrace": "스택 트레이스",
  "consoleLog.filter.all": "전체",
  "consoleLog.filter.error": "Error",
  "consoleLog.filter.warn": "Warn",
  "consoleLog.filter.info": "Info",
  "consoleLog.filter.debug": "Debug",
  "consoleLog.filter.log": "Log",

  "log.originFilter.unknown": "(알 수 없음)",

  "actionLog.search": "액션 검색…",
  "actionLog.filter.all": "전체",
  "actionLog.filter.click": "클릭",
  "actionLog.filter.navigation": "이동",
  "actionLog.filter.input": "입력",
  "actionLog.filter.keypress": "키",
  "actionLog.filter.toggle": "토글",
  "actionLog.filter.select": "선택",
  "actionLog.filter.drag": "드래그",
  "actionLog.empty": "캡처된 액션이 없습니다",
  "actionLog.verb.click": "{target} 클릭",
  "actionLog.verb.input": "{field}에 {value} 입력",
  "actionLog.verb.navigate": "{target}(으)로 이동",
  "actionLog.verb.navigateBack": "{target}(으)로 뒤로가기",
  "actionLog.verb.navigateForward": "{target}(으)로 앞으로가기",
  "actionLog.verb.navigateReload": "{target} 새로고침",
  "actionLog.verb.navigateTraverse": "{target}(으)로 히스토리 이동",
  "actionLog.verb.keypress": "{keys} 키",
  "actionLog.verb.toggle.check": "{field} 체크",
  "actionLog.verb.toggle.uncheck": "{field} 해제",
  "actionLog.verb.select": "{field}에서 {value} 선택",
  "actionLog.verb.drag": "{source} 드래그",
  "actionLog.verb.dragTo": "{source}을(를) {target}(으)로 드래그",
  "actionLog.maskedValue": "가려진 값",
  // actionLog.role.*: 메인 dict 소비처는 없다(실소비는 log-viewer/markers.ts의 복제 사전).
  // drift 테스트(log-viewer/__tests__/i18n.test.ts)의 대조 원본이므로 지우지 말 것.
  "actionLog.role.button": "버튼",
  "actionLog.role.link": "링크",
  "actionLog.role.checkbox": "체크박스",
  "actionLog.role.radio": "라디오 버튼",
  "actionLog.role.tab": "탭",
  "actionLog.role.menuitem": "메뉴 항목",
  "actionLog.role.textbox": "입력란",

  "debug.tab.issue": "이슈 작성",
  "debug.tab.console": "콘솔",
  "debug.tab.network": "네트워크",
  "debug.tab.action": "액션",
  "debug.console.empty": "콘솔 로그가 없습니다",
  "debug.network.empty": "네트워크 요청이 없습니다",

  "md.section.env": "재현 환경",
  "md.section.description": "발생 현상",
  "md.section.stepsToReproduce": "재현 과정",
  "md.section.media": "미디어",
  "md.section.attachments": "첨부",
  "md.section.styleChanges": "스타일 변경사항",
  "md.section.before": "변경 전",
  "md.section.after": "변경 후",
  "md.section.expectedResult": "기대 결과",
  "md.section.notes": "비고",
  "md.videoAttached": "(첨부 녹화 파일 참조)",
  "md.imageAttached": "(첨부 이미지 참조)",
  // 클립보드 복사본에는 첨부가 없다 — 왜 없는지와 어디서 얻는지를 함께 적어 무음 유실을 막는다.
  "md.videoNotCopied": "(녹화 파일은 복사되지 않습니다 — 이슈로 제출하면 첨부됩니다)",
  "md.imageNotCopied": "(이미지는 복사되지 않습니다 — 이슈로 제출하면 첨부됩니다)",
  "md.inlineImageNotCopied": "(이 이미지는 복사되지 않습니다 — 이슈로 제출하면 첨부됩니다)",
  "md.column.property": "속성",
  "md.noValue": "(없음)",

  "logSummary.title": "로그 요약",
  "logSummary.network.line": "네트워크: {n}건 (에러 {errors}건)",
  "logSummary.network.lineNoError": "네트워크: {n}건 (에러 없음)",
  "logSummary.console.line": "콘솔: {n}건 (에러 {errors}건, 경고 {warns}건)",
  "logSummary.console.lineNoError": "콘솔: {n}건 (에러·경고 없음)",
  "logSummary.action.line": "사용자 동작: {n}건",
  "logSummary.logs.lead": "BugShot 리포트가 첨부되어 있습니다.",
  "logSummary.logs.notCopied": "로그 파일은 복사되지 않습니다 — 이슈로 제출하면 첨부됩니다.",
  "logSummary.logs.detail": "{file}에서 사용자 상호작용, 콘솔 로그, 네트워크 로그, 캡처된 미디어 등 추가적인 디버깅 정보를 확인할 수 있습니다.",

  "logCard.networkCount": "네트워크 {captured}(에러 {errors})",
  "logCard.networkCountNoError": "네트워크 {captured}",
  "logCard.consoleCount": "콘솔 {captured}(에러 {errors})",
  "logCard.consoleCountNoError": "콘솔 {captured}",
  "logCard.actionCount": "액션 {captured}",

  "json.showAll": "전체 보기",
  "json.moreItems": "… {n}개 더",
} as const;

type Bundle = Record<keyof typeof ko, string>;

const en = {
  "networkLog.clear": "Clear Log",
  "networkLog.search": "Search URL & body…",
  "logViewer.seekTo": "Jump to {time}",
  "networkLog.dialog.selectRequest": "Select a request",
  "networkLog.detail.general": "General",
  "networkLog.detail.url": "URL",
  "networkLog.detail.method": "Method",
  "networkLog.detail.status": "Status",
  "networkLog.detail.time": "Time",
  "networkLog.detail.contentType": "Content-Type",
  "networkLog.detail.requestHeaders": "Request Headers",
  "networkLog.detail.responseHeaders": "Response Headers",
  "networkLog.detail.copyCurl": "Copy as cURL",
  "networkLog.detail.noBody": "No body",
  "networkLog.tab.headers": "Headers",
  "networkLog.tab.request": "Request",
  "networkLog.tab.response": "Response",
  "networkLog.display.binary": "Binary response ({type} · {size}) · Body not saved",
  "networkLog.display.stream": "Streaming response ({type}) · Body not captured",
  "networkLog.display.bodyTruncated": "Body truncated ({size} · cap {limit})",
  "networkLog.display.bodyOmitted": "Body omitted (memory cap)",
  "networkLog.display.pending": "Waiting for response",
  "networkLog.display.queued": "Queued",
  "networkLog.display.queueFull": "Failed · queue full",
  "networkLog.display.aborted": "Aborted",
  "networkLog.display.timeout": "Timed out",
  "networkLog.display.blocked": "Failed · status hidden",
  "networkLog.display.blockedHint": "The real server status code can't be read due to a CORS or connection failure.",
  "networkLog.filter.all": "All",
  "networkLog.filter.json": "JSON",
  "networkLog.filter.js": "JS",
  "networkLog.filter.css": "CSS",
  "networkLog.filter.img": "Img",
  "networkLog.filter.font": "Font",
  "networkLog.filter.doc": "Doc",
  "networkLog.filter.other": "Other",
  "networkLog.filter.ws": "WS",
  "networkLog.tab.messages": "Messages",
  "networkLog.ws.framesCount": "{n} frames",
  "networkLog.ws.dropped": "{n} dropped",
  "networkLog.ws.all": "All",
  "networkLog.ws.send": "Sent",
  "networkLog.ws.receive": "Received",
  "networkLog.ws.opened": "Connection opened",
  "networkLog.ws.closed": "Connection closed ({code})",
  "networkLog.ws.empty": "No frames yet",

  "consoleLog.clear": "Clear Log",
  "consoleLog.search": "Search messages…",
  "consoleLog.detail.stackTrace": "Stack Trace",
  "consoleLog.filter.all": "All",
  "consoleLog.filter.error": "Error",
  "consoleLog.filter.warn": "Warn",
  "consoleLog.filter.info": "Info",
  "consoleLog.filter.debug": "Debug",
  "consoleLog.filter.log": "Log",

  "log.originFilter.unknown": "(unknown)",

  "actionLog.search": "Search actions…",
  "actionLog.filter.all": "All",
  "actionLog.filter.click": "Click",
  "actionLog.filter.navigation": "Navigation",
  "actionLog.filter.input": "Input",
  "actionLog.filter.keypress": "Keys",
  "actionLog.filter.toggle": "Toggle",
  "actionLog.filter.select": "Select",
  "actionLog.filter.drag": "Drag",
  "actionLog.empty": "No actions captured",
  "actionLog.verb.click": "Clicked {target}",
  "actionLog.verb.input": "Entered {value} in {field}",
  "actionLog.verb.navigate": "Navigated to {target}",
  "actionLog.verb.navigateBack": "Went back to {target}",
  "actionLog.verb.navigateForward": "Went forward to {target}",
  "actionLog.verb.navigateReload": "Reloaded {target}",
  "actionLog.verb.navigateTraverse": "Navigated via history to {target}",
  "actionLog.verb.keypress": "Pressed {keys}",
  "actionLog.verb.toggle.check": "Checked {field}",
  "actionLog.verb.toggle.uncheck": "Unchecked {field}",
  "actionLog.verb.select": "Selected {value} in {field}",
  "actionLog.verb.drag": "Dragged {source}",
  "actionLog.verb.dragTo": "Dragged {source} to {target}",
  "actionLog.maskedValue": "masked value",
  // actionLog.role.*: 메인 dict 소비처는 없다(실소비는 log-viewer/markers.ts의 복제 사전).
  // drift 테스트(log-viewer/__tests__/i18n.test.ts)의 대조 원본이므로 지우지 말 것.
  "actionLog.role.button": "button",
  "actionLog.role.link": "link",
  "actionLog.role.checkbox": "checkbox",
  "actionLog.role.radio": "radio button",
  "actionLog.role.tab": "tab",
  "actionLog.role.menuitem": "menu item",
  "actionLog.role.textbox": "text field",

  "debug.tab.issue": "Issue",
  "debug.tab.console": "Console",
  "debug.tab.network": "Network",
  "debug.tab.action": "Action",
  "debug.console.empty": "No console logs captured",
  "debug.network.empty": "No network requests captured",

  "md.section.env": "Environment",
  "md.section.description": "Description",
  "md.section.stepsToReproduce": "Steps to reproduce",
  "md.section.media": "Media",
  "md.section.attachments": "Attachments",
  "md.section.styleChanges": "Style changes",
  "md.section.before": "Before",
  "md.section.after": "After",
  "md.section.expectedResult": "Expected result",
  "md.section.notes": "Notes",
  "md.videoAttached": "(See attached recording)",
  "md.imageAttached": "(See attached image)",
  "md.videoNotCopied": "(Recordings are not copied — they are attached when you submit the issue)",
  "md.imageNotCopied": "(Images are not copied — they are attached when you submit the issue)",
  "md.inlineImageNotCopied": "(Image not copied — attached when you submit the issue)",
  "md.column.property": "Property",
  "md.noValue": "(none)",

  "logSummary.title": "Log Summary",
  "logSummary.network.line": "Network: {n} requests ({errors} errors)",
  "logSummary.network.lineNoError": "Network: {n} requests (no errors)",
  "logSummary.console.line": "Console: {n} logs ({errors} errors, {warns} warnings)",
  "logSummary.console.lineNoError": "Console: {n} logs (no errors or warnings)",
  "logSummary.action.line": "User actions: {n}",
  "logSummary.logs.lead": "BugShot report attached.",
  "logSummary.logs.notCopied": "Logs are not copied — they are attached when you submit the issue.",
  "logSummary.logs.detail": "{file} contains additional debugging context, including user interactions, console logs, network logs, and captured media.",

  "logCard.networkCount": "Network {captured} ({errors} err)",
  "logCard.networkCountNoError": "Network {captured}",
  "logCard.consoleCount": "Console {captured} ({errors} err)",
  "logCard.consoleCountNoError": "Console {captured}",
  "logCard.actionCount": "Actions {captured}",

  "json.showAll": "show all",
  "json.moreItems": "… {n} more items",
} satisfies Bundle;

const fr = {
  "networkLog.clear": "Effacer les logs",
  "networkLog.search": "Rechercher URL et corps…",
  "logViewer.seekTo": "Aller à {time}",
  "networkLog.dialog.selectRequest": "Sélectionnez une requête",
  "networkLog.detail.general": "Général",
  "networkLog.detail.url": "URL",
  "networkLog.detail.method": "Méthode",
  "networkLog.detail.status": "Statut",
  "networkLog.detail.time": "Durée",
  "networkLog.detail.contentType": "Content-Type",
  "networkLog.detail.requestHeaders": "En-têtes de requête",
  "networkLog.detail.responseHeaders": "En-têtes de réponse",
  "networkLog.detail.copyCurl": "Copier en cURL",
  "networkLog.detail.noBody": "Aucun corps",
  "networkLog.tab.headers": "En-têtes",
  "networkLog.tab.request": "Requête",
  "networkLog.tab.response": "Réponse",
  "networkLog.display.binary": "Réponse binaire ({type} · {size}) · Corps non enregistré",
  "networkLog.display.stream": "Réponse en streaming ({type}) · Corps non capturé",
  "networkLog.display.bodyTruncated": "Corps tronqué ({size} · plafond {limit})",
  "networkLog.display.bodyOmitted": "Corps omis (plafond mémoire)",
  "networkLog.display.pending": "En attente de réponse",
  "networkLog.display.queued": "En file d’attente",
  "networkLog.display.queueFull": "Échec · file pleine",
  "networkLog.display.aborted": "Interrompu",
  "networkLog.display.timeout": "Délai dépassé",
  "networkLog.display.blocked": "Échec · statut masqué",
  "networkLog.display.blockedHint": "Le code de statut réel du serveur ne peut pas être lu à cause d’un échec CORS ou de connexion.",
  "networkLog.filter.all": "Toutes",
  "networkLog.filter.json": "JSON",
  "networkLog.filter.js": "JS",
  "networkLog.filter.css": "CSS",
  "networkLog.filter.img": "Img",
  "networkLog.filter.font": "Police",
  "networkLog.filter.doc": "Doc",
  "networkLog.filter.other": "Autres",
  "networkLog.filter.ws": "WS",
  "networkLog.tab.messages": "Messages",
  "networkLog.ws.framesCount": "{n} trames",
  "networkLog.ws.dropped": "{n} non capturées",
  "networkLog.ws.all": "Toutes",
  "networkLog.ws.send": "Envoyées",
  "networkLog.ws.receive": "Reçues",
  "networkLog.ws.opened": "Connexion ouverte",
  "networkLog.ws.closed": "Connexion fermée ({code})",
  "networkLog.ws.empty": "Aucune trame pour l’instant",

  "consoleLog.clear": "Effacer les logs",
  "consoleLog.search": "Rechercher des messages…",
  "consoleLog.detail.stackTrace": "Trace de la pile",
  "consoleLog.filter.all": "Tous",
  "consoleLog.filter.error": "Erreur",
  "consoleLog.filter.warn": "Avert.",
  "consoleLog.filter.info": "Info",
  "consoleLog.filter.debug": "Debug",
  "consoleLog.filter.log": "Log",

  "log.originFilter.unknown": "(inconnu)",

  "actionLog.search": "Rechercher des actions…",
  "actionLog.filter.all": "Toutes",
  "actionLog.filter.click": "Clic",
  "actionLog.filter.navigation": "Navigation",
  "actionLog.filter.input": "Saisie",
  "actionLog.filter.keypress": "Touches",
  "actionLog.filter.toggle": "Bascule",
  "actionLog.filter.select": "Sélection",
  "actionLog.filter.drag": "Glissement",
  "actionLog.empty": "Aucune action capturée",
  "actionLog.verb.click": "Clic sur {target}",
  "actionLog.verb.input": "Saisie de {value} dans {field}",
  "actionLog.verb.navigate": "Navigation vers {target}",
  "actionLog.verb.navigateBack": "Retour à {target}",
  "actionLog.verb.navigateForward": "Navigation avant vers {target}",
  "actionLog.verb.navigateReload": "Rechargement de {target}",
  "actionLog.verb.navigateTraverse": "Navigation via l’historique vers {target}",
  "actionLog.verb.keypress": "Appui sur {keys}",
  "actionLog.verb.toggle.check": "Cochage de {field}",
  "actionLog.verb.toggle.uncheck": "Décochage de {field}",
  "actionLog.verb.select": "Sélection de {value} dans {field}",
  "actionLog.verb.drag": "Glissement de {source}",
  "actionLog.verb.dragTo": "Glissement de {source} vers {target}",
  "actionLog.maskedValue": "valeur masquée",
  // actionLog.role.*: 메인 dict 소비처는 없다(실소비는 log-viewer/markers.ts의 복제 사전).
  // drift 테스트(log-viewer/__tests__/i18n.test.ts)의 대조 원본이므로 지우지 말 것.
  "actionLog.role.button": "bouton",
  "actionLog.role.link": "lien",
  "actionLog.role.checkbox": "case à cocher",
  "actionLog.role.radio": "bouton radio",
  "actionLog.role.tab": "onglet",
  "actionLog.role.menuitem": "élément de menu",
  "actionLog.role.textbox": "champ de texte",

  "debug.tab.issue": "Ticket",
  "debug.tab.console": "Console",
  "debug.tab.network": "Réseau",
  "debug.tab.action": "Actions",
  "debug.console.empty": "Aucun log console capturé",
  "debug.network.empty": "Aucune requête réseau capturée",

  "md.section.env": "Environnement",
  "md.section.description": "Description",
  "md.section.stepsToReproduce": "Étapes de reproduction",
  "md.section.media": "Médias",
  "md.section.attachments": "Pièces jointes",
  "md.section.styleChanges": "Modifications de style",
  "md.section.before": "Avant",
  "md.section.after": "Après",
  "md.section.expectedResult": "Résultat attendu",
  "md.section.notes": "Notes",
  "md.videoAttached": "(Voir l’enregistrement joint)",
  "md.imageAttached": "(Voir l’image jointe)",
  "md.videoNotCopied": "(Les enregistrements ne sont pas copiés — ils sont joints à l’envoi du ticket)",
  "md.imageNotCopied": "(Les images ne sont pas copiées — elles sont jointes à l’envoi du ticket)",
  "md.inlineImageNotCopied": "(Image non copiée — jointe à l’envoi du ticket)",
  "md.column.property": "Propriété",
  "md.noValue": "(aucun)",

  "logSummary.title": "Résumé des logs",
  "logSummary.network.line": "Réseau : {n} requêtes ({errors} erreurs)",
  "logSummary.network.lineNoError": "Réseau : {n} requêtes (aucune erreur)",
  "logSummary.console.line": "Console : {n} logs ({errors} erreurs, {warns} avertissements)",
  "logSummary.console.lineNoError": "Console : {n} logs (aucune erreur ni avertissement)",
  "logSummary.action.line": "Actions utilisateur : {n}",
  "logSummary.logs.lead": "Rapport BugShot joint.",
  "logSummary.logs.notCopied": "Les logs ne sont pas copiés — ils sont joints à l’envoi du ticket.",
  "logSummary.logs.detail": "{file} contient du contexte de débogage supplémentaire : interactions utilisateur, logs console, logs réseau et médias capturés.",

  "logCard.networkCount": "Réseau {captured} ({errors} err)",
  "logCard.networkCountNoError": "Réseau {captured}",
  "logCard.consoleCount": "Console {captured} ({errors} err)",
  "logCard.consoleCountNoError": "Console {captured}",
  "logCard.actionCount": "Actions {captured}",

  "json.showAll": "tout afficher",
  "json.moreItems": "… {n} éléments de plus",
} satisfies Bundle;

export const logs = { ko, en, fr };
