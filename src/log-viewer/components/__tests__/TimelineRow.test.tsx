...
describe("TimelineRow — 카테고리별 렌더", () => {
  ...
  it("network 행: method 동사 문장(좌) + method/status/time 메타(우), 상세 버튼 없음", () => {
    ...
    // 좌측 문장: method가 동사로 경로 앞에 붙는다(경로는 별도 링크 span).
    expect(screen.getByText("/orders").parentElement!.textContent).toContain("POST");
    // 우측 메타: method·status·time (본문과 중복 표기 OK).
...
describe("TimelineRow — 타이포: console/action처럼 mono 본문, 요청 메타는 sans", () => {
  ...
  it("네트워크 문장 동사(method)는 foreground, method 색은 우측 메타에만", () => {
    render(<TimelineRow item={networkItem({ method: "POST" })} isActive={false} videoStartedAt={0} onActivate={noop} />);
    const posts = screen.getAllByText("POST");
    const verb = posts.find((el) => !el.closest('[data-testid="timeline-net-meta"]'))!;
    const metaMethod = posts.find((el) => el.closest('[data-testid="timeline-net-meta"]'))!;
    expect(verb.className).toContain("text-foreground");
    expect(metaMethod.className).toContain("text-green"); // methodColor(POST)
  });
});
