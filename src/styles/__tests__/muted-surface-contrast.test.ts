...
describe("muted 표면 위 글자 대비", () => {
  ...
  it("스캔 대상이 비어 있지 않다 (앵커)", () => {
    expect(walkSources(SRC).length).toBeGreaterThan(100);
  });

  // `Kbd`는 `bg-muted`를 스스로 깔고, 소비처는 그 위에 `CHIP_CLS`로 색을 덮는다 — 표면과 글자색이
  // 두 파일로 갈려 리터럴 스캔이 원리적으로 못 보는 조합이다(위 판정 한계). 한 쌍이 고정이므로
  // 여기서 직접 이어붙여 같은 계산에 먹인다. 값을 적어두는 게 아니라 소스에서 뽑아 재는 것이라
  // 장부가 아니다(tokens.test.ts가 code-collapse.css를 파일명으로 지목하는 것과 같은 형태).
  it.each(THEMES)("%s에서 Kbd 칩의 CHIP_CLS 색이 AA를 넘는다", (_label, selector) => {
    ...
    const literal = `"bg-muted ${m![1]}"`;
    expect(offendingLiterals(literal, parseTokens(GLOBALS, selector), selector === ".dark")).toEqual(
      [],
    );
  });
  ...
  it.each(THEMES)("%s에서 muted 표면 위 글자가 AA(4.5:1)를 넘는다", (_label, selector) => {
    ...
    const offenders = walkSources(SRC).flatMap((p) =>
