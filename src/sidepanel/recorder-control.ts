...
export async function clearNetworkRecorder(tabId: number): Promise<void> {
  await sendToTab(tabId, { type: "networkRecorder.clear" });
}

export async function clearConsoleRecorder(tabId: number): Promise<void> {
  await sendToTab(tabId, { type: "consoleRecorder.clear" });
}
...
export async function clearActionRecorder(
  ...
  await sendToTab(tabId, { type: "actionRecorder.clear", resupplyEntryNav: opts?.resupplyEntryNav });
}
