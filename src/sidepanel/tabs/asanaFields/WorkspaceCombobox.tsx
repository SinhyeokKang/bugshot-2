...
export function WorkspaceCombobox({ value, onChange, disabled }: Props) {
  ...
  const [items, setItems] = useState<AsanaWorkspace[]>([]);
  ...
  const label = value ? value.workspaceName : t("asana.field.workspace.select");
    ...
    <Popover open={open} onOpenChange={(v) => !disabled && setOpen(v)}>
      ...
      >
        <Command shouldFilter={false}>
          <CommandInput
            placeholder={t("asana.field.workspace.search")}
            value={query}
          ...
          <CommandList>
            {loading ? (
              ...
              <>
                <CommandEmpty>{t("asana.field.workspace.empty")}</CommandEmpty>
                <CommandGroup>
                  {filtered.map((w) => {
                    const selected = value?.workspaceGid === w.gid;
                    return (
                      <CommandItem
                        key={w.gid}
                        value={w.name}
                        onSelect={() => {
                          onChange(
                            selected
                              ? null
                              : { workspaceGid: w.gid, workspaceName: w.name },
                          );
                          setOpen(false);
                        }}
                      >
