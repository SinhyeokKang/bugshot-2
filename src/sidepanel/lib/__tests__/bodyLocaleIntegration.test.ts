...
// 10개 진입점 — 8개 빌더 + 위임 어댑터 2개(github·gitlab). 위임 대상이 감싸지면 따라온다.
...
describe("래핑 구간 안 store write 금지 (불변식)", () => {
  ...
  // 위 단언이 공허하지 않음을 실증한다 — 구독자가 실제로 로케일을 덮어쓸 수 있는 구조다.
  it("실증: 구독자가 있는 상태에서 store write가 일어나면 로케일이 덮인다", () => {
    setLocale("ko");
    const unsubscribe = useSettingsUiStore.subscribe((s) => setLocale(s.locale));
    try {
